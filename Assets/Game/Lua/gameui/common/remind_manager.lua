...
function RemindManager:__init()
	...
	self.pass_day_remind = GlobalEventSystem:Bind(OtherEventType.PASS_DAY, BindTool.Bind(self.OnDayChangeCallBack, self))

	Runner.Instance:AddRunObj(self, 8)
end

function RemindManager:OnDayChangeCallBack()
...
function RemindManager:__delete()
	GlobalEventSystem:UnBind(self.pass_day_remind)
...
function RemindManager:StepCheckRemind(now_time)
	...
	if not self.not_remind_list[remind_name] then
		if (self.has_remind_list[remind_name] and OnceTimeRemindList[remind_name] and num > 0) --在只提醒一次列表中并且已经提醒一次的就加到不提醒列表中
			or (OnceADayRemindList[remind_name] and self:RemindToday(remind_name) and OnceADayNormalRemindList[remind_name] == nil) then --每天提醒一次并今日已提醒过
...
function RemindManager:GetOnceADayRemindList(remind_name)
	if (OnceADayRemindList[remind_name] and self:RemindToday(remind_name)) then
		return true
	end
	return false
end
...
function RemindManager:SetImmdiateRemind(remind_name)
	DelayOnceRemindList[remind_name] = true
	if DelayRemindList[remind_name] then
...
function RemindManager:IsDelayRemind(remind_name)
	if DelayRemindList[remind_name] and DelayRemindList[remind_name].delay_time then
		if DelayOnceRemindList[remind_name] then
			DelayRemindList[remind_name].delay_timer = nil
			return false
		end
...
function RemindManager:RemindToday(remind_name)
	...
	local remind_day = PlayerPrefsUtil.GetInt(main_role_id .. remind_name)
	return remind_day == cur_day
end

--今日是否提醒过（一般打开某界面时设置）
function RemindManager:SetRemindToday(remind_name)
	if self.record_num_t[remind_name] == nil or (self.record_num_t[remind_name] == 0 and not OnceADayRemindList[remind_name]) then
		return
	end
...
function RemindManager:SetTodayDoFlag(remind_name)
	local main_role_id = GameVoManager.Instance:GetMainRoleVo().role_id
	local cur_day = TimeCtrl.Instance:GetCurOpenServerDay()
	PlayerPrefsUtil.SetInt(main_role_id .. remind_name, cur_day)
	RemindManager.Instance:Fire(remind_name)
end