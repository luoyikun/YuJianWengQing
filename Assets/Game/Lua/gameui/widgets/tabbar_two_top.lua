--通用二级底板顶边标签
------------------------------------------------------
TabBarTwoTop = TabBarTwoTop or BaseClass()

function TabBarTwoTop:__init()
...
function TabBarTwoTop:__delete()
...
function TabBarTwoTop:Init(parent_view, parent, tab_cfg)
...
function TabBarTwoTop:FlushTabbar()
	self.tabindex_to_index = {}
	for i, cfg in ipairs(self.tab_cfg) do
		...
		if fun_is_open then
			...
			if nil == tab_button then
				tab_button = TabButtonTwoTop.New(ResPoolMgr:TryGetGameObject("uis/views/commonwidgets_prefab", "BaseSecondPanelTopTab"))
...
function TabBarTwoTop:OnTabClick(index)
	if nil ~= self.select_index_callback then
		self.select_index_callback(index)
	end
end

function TabBarTwoTop:ChangeToIndex(tab_index)
	...
	if index ~= nil and nil ~= self.tab_button_list[index] then 
		for i,v in ipairs(self.tab_button_list) do
			v:SetHighLight(index == i)
		end
	end
end

function TabBarTwoTop:GetFirstOpenTabIndex()
	for i, cfg in ipairs(self.tab_cfg) do
...
function TabBarTwoTop:GetTabButton(tab_index)
	local index = self.tabindex_to_index[tab_index]
	return self.tab_button_list[index]
end

function TabBarTwoTop:RemindChangeCallBack(remind_name, num)
...
function TabBarTwoTop:SetSelectCallback(select_index_callback)
...
TabButtonTwoTop = TabButtonTwoTop or BaseClass(BaseRender)

function TabButtonTwoTop:__init()
end

function TabButtonTwoTop:__delete()
end

function TabButtonTwoTop:ShowRemind(is_show)
	self.node_list["RedPointImg"]:SetActive(is_show)
end

function TabButtonTwoTop:InitTab(cfg)
	self.node_list["HighLightText"].text.text = cfg.name
	self.node_list["HideText"].text.text = cfg.name
end

function TabButtonTwoTop:SetHighLight(is_on)
