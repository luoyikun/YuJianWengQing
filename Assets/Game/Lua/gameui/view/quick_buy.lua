...
function QuickBuy:LoadCallBack()
	...
	local close_btn = self:FindObjByPath("UnPropTips/Button_Close")
	close_btn.button:AddClickListener(
		BindTool.Bind(self.HandleClickClose, self))

	local buy_btn = self:FindObjByPath("BuyBtn")
	buy_btn.button:AddClickListener(
		BindTool.Bind(self.HandleClickBuy, self))
	local input_number_btn = self:FindObjByPath("UnPropTips/Many/ManyBtn")
	input_number_btn.button:AddClickListener(
		BindTool.Bind(self.HandleClickInput, self))
	local input_number_btn2 = self:FindObjByPath("UnPropTips/Many/PenBtn")
	input_number_btn2.button:AddClickListener(
		BindTool.Bind(self.HandleClickInput, self))

	self.desc_txt = self:FindObjByPath("UnPropTips/Text1")
	self.desc_txt2 = self:FindObjByPath("UnPropTips/Text2")
	self.item = ItemCell.New()
	self.item:SetInstanceParent(self:FindObjByPath("UnPropTips/item"))
	self.input_txt = self:FindObjByPath("UnPropTips/Many/ManyBtn/ManyTx")
	self.currency_txt = self:FindObjByPath("UnPropTips/Gold/Need_prop/Text_Number")
	self.currency_icon = self:FindObjByPath("UnPropTips/Gold/Need_prop/Icon")
	self.auto_buy = self:FindObjByPath("Toggle")
...
function QuickBuy:FindObjByPath(name_path)
