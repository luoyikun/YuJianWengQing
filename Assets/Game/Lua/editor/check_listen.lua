...
function CheckListen:CalcListenTotalNum()
	...
	-- 物品变化
	if ItemData and nil ~= ItemData.Instance then
		t["item_change"] = ItemData.Instance:GetNotifyCallBackNum()
	end

	-- 人物数据
	if PlayerData and nil ~= PlayerData.Instance then
		t["player_data_change"] = PlayerData.Instance:GetTotalEventNum()
	end

	-- 红点提醒
	if RemindManager and nil ~= RemindManager.Instance then
	...
	-- 功能引导
	if FunctionGuide and nil ~= FunctionGuide.Instance then
