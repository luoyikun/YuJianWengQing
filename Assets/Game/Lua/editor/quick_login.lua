...
function QuickLogin:IsOpenQuick()
	return "1" == PlayerPrefsUtil.GetString("is_quick_login")
end
...
function QuickLogin:Start()
	...
	self:Preload()
	PlayerPrefsUtil.DeleteKey("is_quick_login")
...
function QuickLogin:InitModule()
	...
	table.insert(module_list, UserDefalut.New())
...
function QuickLogin:Preload()
	...
	for _, v in ipairs(preload_list_cfg) do
		if v[3] == TypeUnityMaterial then
			ResPoolMgr:GetMaterial(v[1], v[2], load_complete, false)
		else
			ResPoolMgr:GetPrefab(v[1], v[2], load_complete, false)
		end
	end
end
...
function QuickLogin:ReqConnectLoginServer()
	local server_id = PlayerPrefsUtil.GetString("quick_login_server_id")
	if nil ~= server_id then
		server_id = tonumber(server_id)
	end

	local plat_name = PlayerPrefsUtil.GetString("quick_login_user_name")
...
function QuickLogin.ReqCreateRole()
	...
	protocol.plat_name = user_vo.plat_name
	protocol.role_name = LoginCtrl.RandomName()
	protocol.login_time = os.time()
	protocol.key = user_vo.plat_session_key
	protocol.plat_server_id = user_vo.plat_server_id
	protocol.plat_fcm = user_vo.plat_fcm
	protocol.avatar = 1
	protocol.prof =  math.floor(math.random(1, 4))
	protocol.sex = ROLE_PROF_SEX[protocol.prof]
...
function QuickLogin:ReqConnectGameServer()
	...
	GlobalEventSystem:Bind(LoginEventType.GAME_SERVER_DISCONNECTED, function(reason, disconnect_notice_type)
