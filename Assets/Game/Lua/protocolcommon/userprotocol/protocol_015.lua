...
function SCKnapsackMaxGridNum:Decode()
	...
	self.is_auto_extend = MsgAdapter.ReadChar()
	MsgAdapter.ReadChar()
end
...
--一键使用物品
CSUseItemMaxNum = CSUseItemMaxNum or BaseClass(BaseProtocolStruct)
function CSUseItemMaxNum:__init( )
	self.msg_type = 1564
	self.item_id = 0
	self.num = 0
end

function CSUseItemMaxNum:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteUShort(self.item_id)
	MsgAdapter.WriteShort(self.num)
end

CSBatchDiscardItem = CSBatchDiscardItem or BaseClass(BaseProtocolStruct)
function CSBatchDiscardItem:__init()
	self.msg_type = 1565
	self.index_list = {}
end

function CSBatchDiscardItem:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteInt(self.count)
	for i = 1, self.count do
		if self.index_list[i] then
			-- MsgAdapter.WriteShort(self.index_list[i])
			local item_info = self.index_list[i]
			MsgAdapter.WriteShort(item_info.index)
			MsgAdapter.WriteUShort(item_info.item_id)
			MsgAdapter.WriteInt(item_info.num or 1)
			MsgAdapter.WriteShort(0)
			MsgAdapter.WriteShort(item_info.num or 1)
