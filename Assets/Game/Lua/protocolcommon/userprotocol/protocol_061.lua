...
function SCRASpecialAppearancePassiveInfo:Decode()
	...
	for i = 1, self.rank_count do
		...
		rank_info.avatar = MsgAdapter.ReadChar()
		rank_info.sex = MsgAdapter.ReadChar()
		rank_info.prof = MsgAdapter.ReadChar()
		rank_info.reserve_1 = MsgAdapter.ReadChar()
...
function SCMultiMountAllInfo:Decode()
	...
	local temp_flag = bit:d2b(MsgAdapter.ReadUInt())						-- 特殊坐骑激活标志，0未激活，1激活
	for i = 1, 10 do
		self.special_img_active_flag[i] = temp_flag[32-i]
	end

	self.special_img_lv_list = {}
	for i = 0, GameEnum.COUPLEMOUNT_SPECIAL_IMG_MAX_ID_LIMIT do
		self.special_img_lv_list[i] = MsgAdapter.ReadChar()
	end
