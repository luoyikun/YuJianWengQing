...
SCGuildFBRewardRecordInfo = SCGuildFBRewardRecordInfo or BaseClass(BaseProtocolStruct)
function SCGuildFBRewardRecordInfo:__init()
	self.msg_type = 2701
	self.item_count = 0
	self.info_type = 0
end

function SCGuildFBRewardRecordInfo:Decode()
	...
	for i = 1, self.item_count do
		local ItemInfo = {}
		ItemInfo.item_id = MsgAdapter.ReadUShort()
		ItemInfo.reward_item_num = MsgAdapter.ReadShort()
		ItemInfo.item_color = MsgAdapter.ReadShort()
		ItemInfo.reserve_sh = MsgAdapter.ReadShort()
...
function SCWelfareInfo:Decode()
	...
	self.exp_buff_effect_second = MsgAdapter.ReadInt()					-- 经验BUFF生效时间
	self.add_double_exp = MsgAdapter.ReadInt()							-- 追加双倍可获得经验
	self.exp_buff_effect_rate = MsgAdapter.ReadInt()					-- 经验BUFF倍数
	self.role_level_after_fetch = MsgAdapter.ReadInt()					-- 领取离线经验升
	self.green_item_count = MsgAdapter.ReadInt()						-- 离线奖励绿色物品数量
	self.green_item_resolve_count = MsgAdapter.ReadInt()				-- 离线奖励绿色物品分解数量
	self.blue_item_count = MsgAdapter.ReadInt()							-- 离线奖励蓝色物品数量
	self.blue_item_resolve_count = MsgAdapter.ReadInt()					-- 离线奖励蓝色物品分解数量
	self.purple_item_count = MsgAdapter.ReadInt()						-- 离线紫色装备数量
	self.purple_item_resolve_count = MsgAdapter.ReadInt()				-- 离线分解紫色装备数量
	self.orange_item_count = MsgAdapter.ReadInt()						-- 离线金色装备数量
	self.orange_item_resolve_count = MsgAdapter.ReadInt()				-- 离线分解金色装备数量
	self.red_item_count = MsgAdapter.ReadInt()							-- 离线红色装备数量
	self.red_item_resolve_count = MsgAdapter.ReadInt()					-- 离线分解红色装备数量
	self.pink_item_count = MsgAdapter.ReadInt()							-- 离线奖励粉色物品数量
	self.pink_item_resolve_count = MsgAdapter.ReadInt()					-- 离线奖励粉色物品分解数量
	self.collect_item_count = MsgAdapter.ReadInt()						-- 集字物品数量
	...
	self.role_login_level =  MsgAdapter.ReadInt()
	self.exp_activityfind = {}
	for i = 0, COMMON_CONSTS.WELFARE_FIND_ACTIVITY_MAX_NUM do
		self.exp_activityfind[i] = MsgAdapter.ReadLL() 					-- 活动经验找回
	end
	self.is_first_send = MsgAdapter.ReadShort()							-- 第一次领取离线奖励
	MsgAdapter.ReadShort()
...
function SCActiveDegreeInfo:Decode()
	...
	self.activedegree_fetch_flag = MsgAdapter.ReadLL()
...
----------------------------------------use down
	...
	self.open_act_vo.oga_guild_battle_reward_type = MsgAdapter.ReadShort() 	--帮派争霸奖励类型
	self.open_act_vo.oga_guild_battle_reward_flag = MsgAdapter.ReadShort()	--帮派争霸奖励标志
end
...
function SCTouZiJiHuaInfo:Decode()
	...
	self.active_plan_3 = MsgAdapter.ReadChar()
	self.plan_3_reward_flag = MsgAdapter.ReadShort()
...
CSTouzijihuaFbBossOperate = CSTouzijihuaFbBossOperate or BaseClass(BaseProtocolStruct)
function CSTouzijihuaFbBossOperate:__init()
	self.msg_type = 2731
	self.operate_type = 0
	self.param = 0
end

function CSTouzijihuaFbBossOperate:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteShort(self.operate_type)
	MsgAdapter.WriteShort(self.param)
end

SCTouzijihuaFbBossInfo = SCTouzijihuaFbBossInfo or BaseClass(BaseProtocolStruct)
function SCTouzijihuaFbBossInfo:__init()
	self.msg_type = 2732
...
function SCTouzijihuaFbBossInfo:Decode()
	self.active_fb_plan = MsgAdapter.ReadShort()
	self.fb_plan_reward_flag = MsgAdapter.ReadShort()
	self.fb_pass_level = MsgAdapter.ReadShort()

	self.active_boss_plan = MsgAdapter.ReadShort()
	self.boss_plan_reward_flag = MsgAdapter.ReadShort()
	self.kill_boss_num = MsgAdapter.ReadShort()

	self.active_shenyu_boss_plan = MsgAdapter.ReadShort()
	self.shenyu_boss_plan_reward_flag = MsgAdapter.ReadShort()
	self.kill_shenyu_boss_num = MsgAdapter.ReadShort()
	MsgAdapter.ReadShort()
end
...
function SCQingYuanAllInfo:Decode()
	...
	self.param_ch5 = MsgAdapter.ReadChar()
	self.param_ch6 = MsgAdapter.ReadChar()
	...
	self.param6_1 = MsgAdapter.ReadInt()
	self.param6_2 = MsgAdapter.ReadInt()
...
function SCSupplyInfo:Decode()
	...
	for i = 1, self.supply_type_max do
		self.supply_list[i] = {}
		self.supply_list[i].supply_left_value = MsgAdapter.ReadLL()
