...
function SCJingLingInfo:Decode()
	...
	self.special_img_active_flag = {}							-- 形象激活标记
	for i = 0, GameEnum.UPGRADE_MAX_IMAGE_BYTE - 1 do
		self.special_img_active_flag[i] = MsgAdapter.ReadUChar()
	end
...
function SCJingLingAdvantageCount:__init()
	self.msg_type = 5607
	self.today_gather_blue_jingling_count = 0      --今天获得的蓝色仙宠数量
end
...
-- 仙宠奇遇仙宠蛋位置信息
SCJingLingEggPosInfo = SCJingLingEggPosInfo or BaseClass(BaseProtocolStruct)
function SCJingLingEggPosInfo:__init()
	self.msg_type = 5610
end

function SCJingLingEggPosInfo:Decode()
