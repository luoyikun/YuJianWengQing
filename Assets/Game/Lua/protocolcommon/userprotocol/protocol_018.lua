...
function SCTuMoTaskInfo:__init()
	...
	self.daily_task_exp_ball_valid_time = 0 							-- 经验球失效时间
	self.daily_task_exp_ball_exp = 0 									-- 经验球当前经验

	self.daily_task_can_fetch_flag = 0 									-- 日常任务可领取标记
	self.daily_task_fetch_flag = 0 										-- 日常任务领取标记
...
function SCTuMoTaskInfo:Decode()
	...
	self.daily_task_can_fetch_flag = MsgAdapter.ReadShort()
	self.daily_task_fetch_flag = MsgAdapter.ReadShort()
	self.daily_task_exp_ball_valid_time = MsgAdapter.ReadUInt()
	self.daily_task_exp_ball_exp = MsgAdapter.ReadLL()
end
...
--请求屠魔任务信息
CSTuMoTaskOpera = CSTuMoTaskOpera or BaseClass(BaseProtocolStruct)
function CSTuMoTaskOpera:__init()
	self.msg_type = 1856
	self.opera_type = 0
	self.param_1 = 0
	self.param_2 = 0
...
function CSTuMoTaskOpera:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
...
function SCShenzhaungInfo:Decode()
	--self.act_suit_id = MsgAdapter.ReadInt()
	self.part_list = {}
...
function SCCSAEquipInfo:Decode()
	...
	self.is_active_sys = MsgAdapter.ReadChar()	-- 是否合服激活
	MsgAdapter.ReadChar()
end
