...
function SCMountInfo:Decode()
	...
	self.active_special_image = {}						-- 特殊形象激活标记
	for i = 0, GameEnum.UPGRADE_MAX_IMAGE_BYTE - 1 do 					
		self.active_special_image[i] = MsgAdapter.ReadUChar()
	end
...
function SCWingInfo:Decode()
	...
	for i = 0, GameEnum.UPGRADE_MAX_IMAGE_BYTE - 1 do 					-- 形象激活标记
		self.active_image_flag[i] = MsgAdapter.ReadUChar()
	end

	self.active_special_image = {}
...
function SCShizhuangInfo:Decode()
	...
	for i = 0, SHIZHUANG_TYPE.MAX - 1 do
		...
		vo.use_special_img = MsgAdapter.ReadChar()
		vo.grade = MsgAdapter.ReadChar()
		vo.is_used_special_img = MsgAdapter.ReadChar()
		vo.grade_bless = MsgAdapter.ReadInt()

		vo.active_flag = {}
		for i = 0, GameEnum.UPGRADE_MAX_IMAGE_BYTE - 1 do 					-- 形象激活标记
			vo.active_flag[i] = MsgAdapter.ReadUChar()
		end

		vo.special_img_active_flag = {}
		for i = 0, GameEnum.UPGRADE_MAX_IMAGE_BYTE - 1 do 					-- 特殊形象激活标记
			vo.special_img_active_flag[i] = MsgAdapter.ReadUChar()
		end

		vo.special_img_grade_list = {}
		for i = 0, GameEnum.MAX_MOUNT_SPECIAL_IMAGE_ID_TWO - 1 do
			vo.special_img_grade_list[i] = MsgAdapter.ReadShort()
		end

		vo.valid_timestamp_list = {}
		for j = 0, GameEnum.MAX_MOUNT_SPECIAL_IMAGE_ID_TWO - 1 do
			vo.valid_timestamp_list[j] = MsgAdapter.ReadUInt()
		end
		vo.shuxingdan_count = MsgAdapter.ReadShort()
		MsgAdapter.ReadShort()

		vo.skill_level_list = {}
		for k = 0, SHIZHUANG.FASHION_SKILL_COUNT - 1 do 	--4个技能
			vo.skill_level_list[k] = MsgAdapter.ReadShort()
		end

		vo.equip_level_list = {}
		for k = 0, SHIZHUANG.FASHION_EQUIP_COUNT - 1 do 	--4个装备
			vo.equip_level_list[k] = MsgAdapter.ReadShort()
		end

		vo.equip_skill_level = MsgAdapter.ReadInt()
		vo.clear_bless_value_time = MsgAdapter.ReadUInt()
		self.item_list[i] = vo
	end
end
...
function CSShizhuangUseReq:__init()
	self.msg_type = 4771
	self.shizhuang_type = 0 	-- 1-武器，0-时装
	self.img_type = 0 			-- 0-普通形象，1-特殊形象
	self.img_id = 0 			-- 形象ID
end

function CSShizhuangUseReq:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteChar(self.shizhuang_type)
	MsgAdapter.WriteChar(self.img_type)
	MsgAdapter.WriteShort(self.img_id)
end

--请求升级时装特殊形象  4809
CSShizhuangSpecialImgUpgradeReq = CSShizhuangSpecialImgUpgradeReq or BaseClass(BaseProtocolStruct)
function CSShizhuangSpecialImgUpgradeReq:__init()
	self.msg_type = 4809
...
function CSShizhuangSpecialImgUpgradeReq:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteShort(self.shizhuang_type)
	MsgAdapter.WriteChar(self.use_special_img)
...
function CSShizhuangUpgradeReq:Encode()
	...
	MsgAdapter.WriteChar(self.repeat_times)
end
...
function SCQingyuanCoupleHaloInfo:Decode()
	self.equiped_couple_halo_type = MsgAdapter.ReadChar()				--已装备的夫妻光环类型
	self.is_today_buy_tejia_halo = MsgAdapter.ReadChar() 				-- 当前特价光环类型
	local couple_halo_max_type_count = MsgAdapter.ReadShort()
	self.tejie_halo_invalid_time = MsgAdapter.ReadUInt() 				-- 当前光环特价失效时间
...
function CSQingyuanFetchLoveContract:Encode()
	...
	MsgAdapter.WriteShort(self.opera_type)
	MsgAdapter.WriteShort(self.day_num)
...
function SCQingyuanLoveContractInfo:Decode()
	self.info_type = MsgAdapter.ReadInt()									-- 消息类型
...
function SCAdvanceNoticeInfo:Decode()
	...
	for i=1,16 do
		self.notice_day_fectch_flag_list[i] = MsgAdapter.ReadUChar()
	end
end
