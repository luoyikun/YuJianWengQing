...
function SCQunXianLuanDouUserInfo:Decode()
	...
	for i = 1, 3 do
		...
		self.first_rank_num[i] = data
	end
	
	self.first_rank_uid = {}
	self.cur_count = 0
	for i = 1, 3 do
		self.first_rank_uid[i] = MsgAdapter.ReadInt()
		if self.first_rank_uid[i] > 0 then
...
function SCQunXianLuanDouRankInfo:__init()
	...
	self.data.self_score = 0
	self.data.self_rank_pos = 0
end

function SCQunXianLuanDouRankInfo:Decode()
	self.data.self_score = MsgAdapter.ReadInt()
	self.data.self_rank_pos = MsgAdapter.ReadInt()
...
function SCChallengeFieldStatus:Decode()
	...
	for i = 1, 2 do
		local obj = {}
		obj.role_id = MsgAdapter.ReadUInt()
		obj.plat_type = MsgAdapter.ReadUInt()
...
function SCChallengeFieldOpponentRankPosChange:Decode()
	self.user_id = MsgAdapter.ReadUInt()
...
function CSChallengeFieldBestRankBreakReq:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteInt(self.op_type)  --0 请求信息 1 请求突破
end
...
-- 1V1竞技场请求准备正式开始战斗倒计时
CSChallengeFieldReadyStartFightReq = CSChallengeFieldReadyStartFightReq or BaseClass(BaseProtocolStruct)
function CSChallengeFieldReadyStartFightReq:__init()
	self.msg_type = 4944
end

function CSChallengeFieldReadyStartFightReq:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
end
...
function SCSendGuildBattleDailyRewardFlag:Decode()
	self.had_fetch = MsgAdapter.ReadShort()
