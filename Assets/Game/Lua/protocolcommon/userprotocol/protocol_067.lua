...
function SCGCZGlobalInfo:Decode()
	...
	self.cheng_zhu_prof = MsgAdapter.ReadChar()
	...
	self.rank_list = {}
	self.my_guild_rank_pos = MsgAdapter.ReadInt()
...
function SCGCZRewardInfo:Decode()
	...
	self.chengzhu_uid = MsgAdapter.ReadInt()
...
SCGongChengZhanFlagInfo = SCGongChengZhanFlagInfo or BaseClass(BaseProtocolStruct)
function SCGongChengZhanFlagInfo:__init()
	self.msg_type = 6706
end

function SCGongChengZhanFlagInfo:Decode()
...
function SCGCZWorshipActivityInfo:__init()
	self.msg_type = 6755
	self.worship_is_open = 0
	self.reserve_ch = 0
	self.reserve_ch = 0
	self.worship_end_timestamp = 0
end

function SCGCZWorshipActivityInfo:Decode()
	self.worship_is_open = MsgAdapter.ReadChar()
	self.reserve_ch = MsgAdapter.ReadChar()
	self.reserve_ch = MsgAdapter.ReadShort()
	self.worship_end_timestamp = MsgAdapter.ReadUInt()
end
...
-- 下发个人膜拜信息
SCRoleWorshipInfo = SCRoleWorshipInfo or BaseClass(BaseProtocolStruct)
function SCRoleWorshipInfo:__init()
	self.msg_type = 6757
	self.activity_type = 0
	self.worship_times = 0	-- 玩家膜拜次数
	self.next_worship_timestamp = 0 -- 玩家下次膜拜时间戳
end

function SCRoleWorshipInfo:Decode()
...
-- 广播角色开始拜谒
SCRoleWorshipStart = SCRoleWorshipStart or BaseClass(BaseProtocolStruct)
function SCRoleWorshipStart:__init()
	self.msg_type = 6758
	self.obj_id = 0
end

function SCRoleWorshipStart:Decode()
	self.obj_id = MsgAdapter.ReadUShort()
end

-- 拜谒请求
CSRoleWorshipReq =  CSRoleWorshipReq or BaseClass(BaseProtocolStruct)
function CSRoleWorshipReq:__init()
	self.msg_type = 6759
end

function CSRoleWorshipReq:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
end