...
function TeamStruct.TeamMemberInfoRead()
	local stu = {}
	stu.plat_role_id = MsgAdapter.ReadInt()
...
function TeamStruct.TeamItemRead()
	...
	stu.member_uuid_list = {}
	for i = 1, GameEnum.TEAM_MAX_COUNT do
		stu.member_uuid_list[i] = {}
		stu.member_uuid_list[i].plat_role_id = MsgAdapter.ReadInt()
		stu.member_uuid_list[i].plat_type = MsgAdapter.ReadInt()
	end
	return stu
end
...
--跨服邀请入队 9166
CSInviteUniqueUser = CSInviteUniqueUser or BaseClass(BaseProtocolStruct)
function CSInviteUniqueUser:__init()
	self.msg_type = 9166
...
function CSInviteUniqueUser:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
    MsgAdapter.WriteInt(self.role_id)
