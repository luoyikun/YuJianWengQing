...
-- 仙盟仓库日志事件列表
 SCGuildStorageLogList = SCGuildStorageLogList or BaseClass(BaseProtocolStruct)
 function SCGuildStorageLogList:__init()
	self.msg_type = 4216
	self.count = 0
	self.log_list = {}
end

function SCGuildStorageLogList:Decode()
	self.count = MsgAdapter.ReadInt()
	self.log_list = {}
	for i = 1, self.count do
		self.log_list[i] = {}
		self.log_list[i].log_owner_name = MsgAdapter.ReadStrN(32)
		self.log_list[i].log_time = MsgAdapter.ReadUInt()
		self.log_list[i].opt_type = MsgAdapter.ReadInt()
		self.log_list[i].item_id = MsgAdapter.ReadInt()
		self.log_list[i].xianpin_type_list = {}
		for j = 1, 6 do
			self.log_list[i].xianpin_type_list[j] = MsgAdapter.ReadUShort()
		end
	end
end
...
function SCGuildBoxInfo:Decode()
	...
	self.be_assist_count = MsgAdapter.ReadShort()
	MsgAdapter.ReadShort()
	...
	for i = 1, GameEnum.MAX_GUILD_BOX_COUNT do
		...
		self.info_list[i].is_thank_assist_uid = {}
		for j = 1, 4 do
			self.info_list[i].is_thank_assist_uid[j] = MsgAdapter.ReadChar()
		end
...
function SCGuildBossActivityInfo:__init()
	...
	self.totem_exp = 0
	self.cur_star_level = 0
	self.gather_num = 0
	self.next_change_star_time = 0
	self.rank_count = 0
	self.notify_type = 0 
	self.reserve_sh = 0
	self.rank_list = {}
...
function SCGuildBossActivityInfo:Decode()
	...
	self.cur_star_level = MsgAdapter.ReadShort()
	self.gather_num = MsgAdapter.ReadShort()
	self.next_change_star_time = MsgAdapter.ReadUInt()
	self.rank_count = MsgAdapter.ReadInt()
	for i = 1, self.rank_count do
		self.rank_list[i] = {}
		self.rank_list[i].user_id = MsgAdapter.ReadInt()
		self.rank_list[i].user_name = MsgAdapter.ReadStrN(32)
		self.rank_list[i].hurt_val = MsgAdapter.ReadLL()
	end
end
...
function CSGuildSkillUplevel:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteShort(self.up_type)
