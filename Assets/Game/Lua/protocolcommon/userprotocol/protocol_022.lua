...
function SCYiZhanDaoDiUserInfo:__init()
	...
	self.kill_num_reward_flag = 0
	self.show_jiesuan = 0
	self.reward_count = 0
	self.kill_num_reward_list = {}
end
...
function SCYiZhanDaoDiUserInfo:Decode()
	...
	self.kill_num_reward_flag = MsgAdapter.ReadInt()
	self.show_jiesuan = MsgAdapter.ReadShort()
	self.reward_count = MsgAdapter.ReadShort()
	self.kill_num_reward_list = {}
	for i = 1, self.reward_count do
		self.kill_num_reward_list[i] = {}
		self.kill_num_reward_list[i].item_id = MsgAdapter.ReadInt()
		self.kill_num_reward_list[i].num = MsgAdapter.ReadShort()
		self.kill_num_reward_list[i].is_bind = MsgAdapter.ReadShort()
	end
end
...
function SCYiZhanDaoDiRankInfo:__init()
	...
	self.count = 0
	self.self_score = 0
	self.self_rand_pos = 0
end

function SCYiZhanDaoDiRankInfo:Decode()
	self.self_score = MsgAdapter.ReadShort()
	self.self_rand_pos = MsgAdapter.ReadShort()
...
function SCCSAActivityInfo:Decode()
	...
	for i = 0, GameEnum.CSA_RANK_TYPE_MAX - 1 do
		...
		for j = 1, GameEnum.COMBINE_SERVER_ACTIVITY_RANK_REWARD_ROLE_NUM do
			...
			user.prof = MsgAdapter.ReadShort()
			-- user.reserve_ch = MsgAdapter.ReadChar()
			-- user.reserve_sh = MsgAdapter.ReadShort()
...
function SCCSARoleInfo:Decode()
	...
	self.rank_combine_consume_num = MsgAdapter.ReadShort()		--消费排名
	self.rank_combine_chongzhi_num = MsgAdapter.ReadShort()		--充值排名
end
...
function SCAllXiannvInfo:Decode()
	...
	MsgAdapter.ReadShort()
	self.shengwu_essence = MsgAdapter.ReadInt()
end
...
function SCXiannvShengwuChangeInfo:__init()
	...
	-- 		ESSENCE,									// 圣物精华信息, param4 圣物精华数量, param3 增量
	-- 	};
end
...
SCRAWeekendHappyInfo = SCRAWeekendHappyInfo or BaseClass(BaseProtocolStruct)
function SCRAWeekendHappyInfo:__init()
	self.msg_type = 2279
end

function SCRAWeekendHappyInfo:Decode()
	self.nex_free_time = MsgAdapter.ReadUInt()
	self.person_total_chou_times = MsgAdapter.ReadUInt()
	self.server_total_chou_times = MsgAdapter.ReadUInt()
	self.fetch_person_reward_flag = MsgAdapter.ReadUShort()

	self.item_count = MsgAdapter.ReadShort()
	self.chou_item_list = {}
	if self.item_count > 0 then
		for i = 1, self.item_count do
			self.chou_item_list[i] = {}
			self.chou_item_list[i].item_id = MsgAdapter.ReadUShort()
			self.chou_item_list[i].num = MsgAdapter.ReadShort()
			self.chou_item_list[i].is_bind = MsgAdapter.ReadChar()
			MsgAdapter.ReadChar()
			self.chou_item_list[i].xianpin_type_list = {}
			for j = 1, 6 do
				self.chou_item_list[i].xianpin_type_list[j] = MsgAdapter.ReadChar()
			end
		end
	end
end

SCRAZeroBuyReturnInfo = SCRAZeroBuyReturnInfo or BaseClass(BaseProtocolStruct)
function SCRAZeroBuyReturnInfo:__init()
	self.msg_type = 2280
end

function SCRAZeroBuyReturnInfo:Decode()
	self.zero_buy_return_buy_timestamp = {}
	for i = 0, 15 do
		self.zero_buy_return_buy_timestamp[i] = MsgAdapter.ReadUInt()
	end
	self.zero_buy_return_day_fetch_flag = {}
	for i = 0, 15 do
		self.zero_buy_return_day_fetch_flag[i] = MsgAdapter.ReadUInt()
	end
end

--狂嗨庆典
SCRAKuangHaiInfo = SCRAKuangHaiInfo or BaseClass(BaseProtocolStruct)
function SCRAKuangHaiInfo:__init()
	self.msg_type = 2281
end

function SCRAKuangHaiInfo:Decode()
	self.reward_flag = MsgAdapter.ReadUShort()
	local task_item_num = MsgAdapter.ReadShort()
	self.current_score = MsgAdapter.ReadShort()
	self.reserve_sh = MsgAdapter.ReadShort()
	self.task_list = {}
	for i = 1, task_item_num do
		local info = {}
		info.item_idx = MsgAdapter.ReadShort()								
		info.get_score = MsgAdapter.ReadShort()					        				    
		self.task_list[i] = info
	end
end

-------------------------------礼物收割-------------------------------------------------
SCRAGiftHarvestPlayerInfo = SCRAGiftHarvestPlayerInfo or BaseClass(BaseProtocolStruct)
function SCRAGiftHarvestPlayerInfo:__init()
	self.msg_type = 2282
end

function SCRAGiftHarvestPlayerInfo:Decode()
	self.round = MsgAdapter.ReadInt()
	self.get_score = MsgAdapter.ReadInt()
	self.kill_num = MsgAdapter.ReadInt()
end

SCRAGiftHarvestRankInfo = SCRAGiftHarvestRankInfo or BaseClass(BaseProtocolStruct)
function SCRAGiftHarvestRankInfo:__init()
	self.msg_type = 2283
end

function SCRAGiftHarvestRankInfo:Decode()
	self.rank_num = MsgAdapter.ReadInt()
	self.rank_lsit = {}
	for i=1,self.rank_num do
		data = {}
		data.role_id = MsgAdapter.ReadInt()
		data.cur_get_score = MsgAdapter.ReadInt()
		data.sex = MsgAdapter.ReadChar()
		data.prof = MsgAdapter.ReadChar()
		MsgAdapter.ReadShort()
		self.svatar_timestamp = MsgAdapter.ReadLL()
		data.name = MsgAdapter.ReadStrN(32)
		table.insert(self.rank_lsit, data)
	end
end

SCGiftHarvestSkill = SCGiftHarvestSkill or BaseClass(BaseProtocolStruct)
function SCGiftHarvestSkill:__init()
	self.msg_type = 2284
end

function SCGiftHarvestSkill:Decode()
	self.skill_index = MsgAdapter.ReadUInt()
	self.next_perform_timestamp = MsgAdapter.ReadInt()
end

SCGiftHarvestRoundNotice = SCGiftHarvestRoundNotice or BaseClass(BaseProtocolStruct)
function SCGiftHarvestRoundNotice:__init()
	self.msg_type = 2285
end

function SCGiftHarvestRoundNotice:Decode()
	self.round = MsgAdapter.ReadShort()
	self.round_state = MsgAdapter.ReadShort()
...
-------------------------------累计充值-------------------------------------------------
SCRATotalChargeMultiInfo = SCRATotalChargeMultiInfo or BaseClass(BaseProtocolStruct)
function SCRATotalChargeMultiInfo:__init()
	self.msg_type = 2286
end

function SCRATotalChargeMultiInfo:Decode()
...
SCRADoubleGetInfo = SCRADoubleGetInfo or BaseClass(BaseProtocolStruct)
function SCRADoubleGetInfo:__init()
	self.msg_type = 2287
end

function SCRADoubleGetInfo:Decode()
	self.double_get_reward_fetch_flag = MsgAdapter.ReadInt()
end

SCSingleChargeInfoMulti = SCSingleChargeInfoMulti or BaseClass(BaseProtocolStruct)
function SCSingleChargeInfoMulti:__init()
	self.msg_type = 2288
end

function SCSingleChargeInfoMulti:Decode()
	self.activity_type = MsgAdapter.ReadInt()
	self.single_charge_reward_fetch_flag = MsgAdapter.ReadInt()
end