...
-- 返回飞仙装装备列表
SCFeixianEquipInfo = SCFeixianEquipInfo or BaseClass(BaseProtocolStruct)
function SCFeixianEquipInfo:__init()
	self.msg_type = 1796
end

function SCFeixianEquipInfo:Decode()
	self.equipment_list = {}
...
--角色武器颜色变化
SCNoticeWuqiColor = SCNoticeWuqiColor or BaseClass(BaseProtocolStruct)
function SCNoticeWuqiColor:__init()
	self.msg_type = 1786
	self.obj_id = 0
	self.wuqi_color = 0
end

function SCNoticeWuqiColor:Decode()
...
function SCStoneInfo:Decode()
	self.reason = MsgAdapter.ReadShort()
	self.total_stone_level = MsgAdapter.ReadShort()
	self.stone_limit_flag = {}
	for i = 0, COMMON_CONSTS.MAX_STONE_EQUIP_PART - 1 do
		self.stone_limit_flag[i] = MsgAdapter.ReadShort()
	end
...
function SCDuanzaoSuitInfo:Decode()
	...
	for i = 0, FORGE.MAX_SUIT_EQUIP_PART do
		local t = {}
...
function CSEquipUpQuality:Encode()
	...
	MsgAdapter.WriteChar(self.select_bind_first)
	MsgAdapter.WriteShort(self.equip_index)
...
function CSEquipUpLevel:Encode()
	...
	MsgAdapter.WriteChar(self.use_lucky_item)
--	MsgAdapter.WriteShort(self.use_lucky_num)
end
...
-----------------飞仙装请求--------------------------------------------
CSFeixianEquipOpe = CSFeixianEquipOpe or BaseClass(BaseProtocolStruct)
function CSFeixianEquipOpe:__init()
	self.msg_type = 1795
end

function CSFeixianEquipOpe:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteShort(self.operate_type)
