...
function SCHotspringPlayerInfo:Decode()
	...
	self.partner_uid = MsgAdapter.ReadLL()					-- long long partner_uuid 
	self.give_free_times = MsgAdapter.ReadShort()			-- short give_free_present_time
	self.swinsuit = MsgAdapter.ReadShort()					-- short swinsuit_image
	self.uuid = MsgAdapter.ReadLL()							-- long long self_uuid

	self.partner_obj_id = MsgAdapter.ReadUShort()			-- 	ObjID partner_obj_id
	self.question_right_count = MsgAdapter.ReadChar()		-- 正确数量
	self.question_wrong_count = MsgAdapter.ReadChar()		-- 错误数量
	self.curr_score = MsgAdapter.ReadInt()					-- 获得积分
	self.total_exp = MsgAdapter.ReadLL()					-- 获得经验
	...
	self.start_fresh_gather_time = MsgAdapter.ReadUInt()	-- 采集物开始刷新时间
end
...
function SCHSAddExpInfo:Decode()
	self.add_exp_total = MsgAdapter.ReadLL()
...
function SCCrossGuildBattleSceneInfo:Decode()
	...
	for i = 1, CROSS_GUILDBATTLE.CROSS_GUILDBATTLE_MAX_FLAG_IN_SCENE do 		--CROSS_GUILDBATTLE_MAX_FLAG_IN_SCENE = 3-- 最大旗子数在场景中
		...
		flag_vo.cur_hp = MsgAdapter.ReadLL()
		flag_vo.max_hp = MsgAdapter.ReadLL()
...
function SCCrossGuildBattleInfo:Decode()
	...
	for i = 1, CROSS_GUILDBATTLE.CROSS_GUILDBATTLE_MAX_SCENE_NUM do 				--CROSS_GUILDBATTLE_MAX_SCENE_NUM = 6帮派场景个数	
		...
		vo.wuqi_id = MsgAdapter.ReadShort()							-- 武器ID
		vo.shizhuang_wuqi = MsgAdapter.ReadChar()					-- 时装武器
		vo.shizhuang_wuqi_is_special = MsgAdapter.ReadChar()		-- 时装武器特殊
		vo.shizhuang_body = MsgAdapter.ReadChar();					-- 时装身体
		vo.shizhuang_body_is_special = MsgAdapter.ReadChar()		-- 时装身体
		vo.halo_used_imageid = MsgAdapter.ReadShort()				-- 光环形象
		vo.toushi_used_imageid = MsgAdapter.ReadShort()				-- 头饰形象id
		vo.qilinbi_used_imageid = MsgAdapter.ReadShort()			-- 麒麟臂形象id
		vo.mask_used_imageid = MsgAdapter.ReadShort()				-- 面具形象id
...
function SCCrossGuildBattleGetRankInfoResp:Decode()
	...
	for i = 1, 6 do
		...
		for j = 1 ,10 do
			...
			guild_info.uid = MsgAdapter.ReadInt()
			guild_info.plat_type  = MsgAdapter.ReadInt()
			guild_info.rank = j
		...
		if self.rank_list[1] and self.rank_list[1].uid and self.rank_list[1].uid > 0 then
			local is_add = true
			for k,v in pairs(self.cg_list) do
				if v.uid == self.rank_list[1].uid then
					is_add = false
				end
			end
			if is_add and nil == self.cg_list[i] then
				self.cg_list[i] = self.rank_list[1]
...
function SCCrossGuildBattleBossInfo:Decode()
	self.scene_id = MsgAdapter.ReadInt()
	self.is_refresh = MsgAdapter.ReadChar()
	...
	for i = 1, self.count do
		...
		v.replace_boss_id = MsgAdapter.ReadInt()
...
SCCrossGuildBattleFlagInfo = SCCrossGuildBattleFlagInfo or BaseClass(BaseProtocolStruct)
function SCCrossGuildBattleFlagInfo:__init()
	self.msg_type = 5378
end

function SCCrossGuildBattleFlagInfo:Decode()
	self.scene_list = {}
	for i = 1, CROSS_GUILDBATTLE.CROSS_GUILDBATTLE_MAX_SCENE_NUM do
		local scene_temp_list = {}
		scene_temp_list.scene_id = MsgAdapter.ReadInt()
		scene_temp_list.flag_list = {}
		for k = 1,CROSS_GUILDBATTLE.CROSS_GUILDBATTLE_MAX_FLAG_IN_SCENE do
			local temp_list = {}
			temp_list.monster_id = MsgAdapter.ReadInt()
			temp_list.plat_type = MsgAdapter.ReadInt()
			temp_list.server_id = MsgAdapter.ReadInt()
			temp_list.guild_name = MsgAdapter.ReadStrN(32)
			temp_list.cur_hp = MsgAdapter.ReadLL()
			temp_list.max_hp = MsgAdapter.ReadLL()
			table.insert(scene_temp_list.flag_list, temp_list)
		end
		table.insert(self.scene_list, scene_temp_list)
	end
end
...
----跨服帮派战 场景盟友数---
SCCrossGuildBattleSceneGuilderNum = SCCrossGuildBattleSceneGuilderNum or BaseClass(BaseProtocolStruct)
function SCCrossGuildBattleSceneGuilderNum:__init()
	self.msg_type = 5368
end

function SCCrossGuildBattleSceneGuilderNum:Decode()
	self.guild_id = MsgAdapter.ReadLL()
	self.scene_info_list = {}
	for i = 1, CROSS_GUILDBATTLE.CROSS_GUILDBATTLE_MAX_SCENE_NUM do
		local scene_list = {}
		scene_list.scene_id = MsgAdapter.ReadInt()
		scene_list.guilder_num = MsgAdapter.ReadInt()
		table.insert(self.scene_info_list, scene_list)
	end
end


SCCrossGuildBattleBossHurtInfo = SCCrossGuildBattleBossHurtInfo or BaseClass(BaseProtocolStruct)
function SCCrossGuildBattleBossHurtInfo:__init()
	self.msg_type = 5369
	self.guildhurt_rank_list = {}
	self.boss_id = 0
	self.own_guild_rank = 0
end

function SCCrossGuildBattleBossHurtInfo:Decode()
	self.guildhurt_rank_list = {}
	...
	for i = 1, count do
		self.guildhurt_rank_list[i] = {}
		self.guildhurt_rank_list[i].guild = MsgAdapter.ReadLL()
		self.guildhurt_rank_list[i].guild_name = MsgAdapter.ReadStrN(32)
		self.guildhurt_rank_list[i].hurt = MsgAdapter.ReadLL()
	end
end