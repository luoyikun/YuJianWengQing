...
function SCShenzhouWeapondAllInfo:Decode()
	...
	self.hunqi_count = MsgAdapter.ReadInt()

	self.all_weapon_level_list = {}
	for i = 1, self.hunqi_count do
		...
		for j = 1, HunQiData.SHENZHOU_HUNYIN_MAX_SLOT do
			...
			self.all_weapon_level_list[i].hunyin_slot_list[j].is_lock = MsgAdapter.ReadChar()
		end
	end
...
SCRequestCreateBaby = SCRequestCreateBaby or BaseClass(BaseProtocolStruct)
function SCRequestCreateBaby:__init()
	self.msg_type = 2414
	self.type = 0
	self.param_1 = 0
...
function SCRequestCreateBaby:Decode()
	self.type = MsgAdapter.ReadShort()
	self.param_1 = MsgAdapter.ReadShort()

	self.param_2_1 = MsgAdapter.ReadUInt()
	self.param_2_2 = MsgAdapter.ReadUInt()
end
...
function CSBabyOperaReq:Encode()
	...
	--本来是LongLong类型分开两个Uint发送
	MsgAdapter.WriteUInt(self.param_4_1)
	MsgAdapter.WriteUInt(self.param_4_2)
end
...
function SCRATotalCharge4Info:__init()
	self.msg_type = 2425
	self.cur_total_charge = 0
	self.cur_consume_reward = 0
	self.ra_total_charge_4_cur_has_fetch_flag = 0

end

function SCRATotalCharge4Info:Decode()
	self.cur_total_charge = MsgAdapter.ReadInt()
	self.cur_consume_reward = MsgAdapter.ReadInt()
	self.ra_total_charge_4_cur_has_fetch_flag = MsgAdapter.ReadInt()
end
...
function SCRATianMingDivinationInfo:Decode()
	self.total_add_lot = MsgAdapter.ReadInt()
...
function SCWorldBossWearyInfo:Decode()
	...
	self.worldboss_weary_last_relive_time = MsgAdapter.ReadUInt()
end
...
----------------------------------------------------
-- 限时连充
SCRAContinueChongzhiInfo2 = SCRAContinueChongzhiInfo2 or BaseClass(BaseProtocolStruct)
function SCRAContinueChongzhiInfo2:__init()
	self.msg_type = 2466
	self.today_chongzhi = 0
	self.can_fetch_reward_flag = 0
	self.has_fetch_reward_falg = 0
	self.continue_chongzhi_days = 0
end

function SCRAContinueChongzhiInfo2:Decode()
	self.today_chongzhi = MsgAdapter.ReadUInt()								-- 今日充值数
	self.can_fetch_reward_flag = MsgAdapter.ReadShort()					-- 奖励激活标记 位0标记特殊奖励,其他位标记达标奖励，值0未激活，值1已激活
	self.has_fetch_reward_falg = MsgAdapter.ReadShort()					-- 奖励领取标记 位0标记特殊奖励,其他位标记达标奖励，值0未领取，值1已领取
	self.continue_chongzhi_days = MsgAdapter.ReadChar()						-- 连续充值天数
...
function SCRAMagicShopAllInfo:Decode()
	...
	self.res_ch = MsgAdapter.ReadChar()
	self.activity_day = MsgAdapter.ReadShort()
end

SCRAProfessRankInfo = SCRAProfessRankInfo or BaseClass(BaseProtocolStruct)
function SCRAProfessRankInfo:__init()
	self.msg_type = 2464
end

function SCRAProfessRankInfo:Decode()
	self.rank = MsgAdapter.ReadInt()
	self.ra_profess_score = MsgAdapter.ReadInt()
	self.ra_profess_to_num = MsgAdapter.ReadInt()
	self.ra_profess_from_num = MsgAdapter.ReadInt()
end