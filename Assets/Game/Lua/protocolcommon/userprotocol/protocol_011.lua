...
function SCVisibleObjEnterRole:Decode()
	...
	self.obj_id = MsgAdapter.ReadUShort()
	self.merge_server_id = MsgAdapter.ReadInt() -- (uuserverid)
	self.plat_type = MsgAdapter.ReadInt()-- (uuserverid)
	self.plat_role_id = MsgAdapter.ReadInt() -- (uuid)
	self.plat_type = MsgAdapter.ReadInt() -- (uuid)
	self.role_id = MsgAdapter.ReadInt()
	self.role_name = MsgAdapter.ReadStrN(32)
	self.hp = MsgAdapter.ReadLL()
	self.max_hp = MsgAdapter.ReadLL()
	self.mp = MsgAdapter.ReadLL()
	self.max_mp = MsgAdapter.ReadLL()
	...
	self.move_speed = MsgAdapter.ReadLL()
	self.pos_x = MsgAdapter.ReadShort()
	self.pos_y = MsgAdapter.ReadShort()
	...
	self.user_pet_special_img = MsgAdapter.ReadShort()          -- 精灵特殊形象(服务端字段jingling_phantom_img)

	self.top_dps_flag = MsgAdapter.ReadChar()					-- 最高dps标记
	self.first_hurt_flag = MsgAdapter.ReadChar()				-- boss第一刀标记
	...
	self.total_jingjie = MsgAdapter.ReadInt()					-- 境界
	self.is_in_xunyou = MsgAdapter.ReadChar()					-- 是否巡游
	self.is_fightback_obj = MsgAdapter.ReadChar()				-- 是否反击对象
	MsgAdapter.ReadShort()
	self.imp_guard_id_list = {}
	for i=1, ImpGuardData.IMP_GUARD_GRID_INDEX_MAX do
		self.imp_guard_id_list[i] = {}
		self.imp_guard_id_list[i] = MsgAdapter.ReadShort()		-- 小鬼守护形象
	end
	...
	if 0 ~= mask_t[32 - 3] then
		for i = 0, GameEnum.SPECIAL_BUFF_MAX_BYTE -1 do 							-- buff效果标记
			self.buff_mark[i] = MsgAdapter.ReadUChar()
		end
	else
		for i = 0, GameEnum.SPECIAL_BUFF_MAX_BYTE - 1 do 							-- buff效果标记
			self.buff_mark[i] = 0
	...
	-- 飞行高度
	if 0 ~= mask_t[32 - 18] then
		self.fly_height = MsgAdapter.ReadShort()
	else
		self.fly_height = 0
	end
end
...
function SCVisibleObjEnterFalling:Decode()
	...
	self.obj_pos_x = MsgAdapter.ReadShort()
	self.obj_pos_y = MsgAdapter.ReadShort()
	self.pos_x = MsgAdapter.ReadShort()
	self.pos_y = MsgAdapter.ReadShort()
...
function SCVisibleObjEnterMonster:Decode()
	...
	self.buff_mark = {}
	for i = 0, GameEnum.SPECIAL_BUFF_MAX_BYTE - 1 do
		self.buff_mark[i] = MsgAdapter.ReadUChar()
	end
...
function SCVisibleObjEnterGather:Decode()
	...
	self.param4_low = MsgAdapter.ReadUInt()
	self.param4_high = MsgAdapter.ReadUInt()
end
...
--结婚巡游进入视野
SCVisibleObjEnterMarryObj = SCVisibleObjEnterMarryObj  or BaseClass(BaseProtocolStruct)
function SCVisibleObjEnterMarryObj:__init()
	self.msg_type = 1140
end

function SCVisibleObjEnterMarryObj:Decode()
	self.obj_id = MsgAdapter.ReadUShort()
	self.marry_seq = MsgAdapter.ReadShort()
	self.pos_x = MsgAdapter.ReadShort()
	self.pos_y = MsgAdapter.ReadShort()
	self.dir = MsgAdapter.ReadFloat()
	self.distance = MsgAdapter.ReadFloat()
...
function SCBossDpsFlag:Decode()
	...
	self.owner_user_name = MsgAdapter.ReadStrN(32)
	self.guild_name = MsgAdapter.ReadStrN(32)
end
...
function SCGatherBeGather:Decode()
	...
	self.gather_obj_id = MsgAdapter.ReadUShort()
	self.gather_id = MsgAdapter.ReadInt()
	self.left_gather_times = 0
	-- self.left_gather_times = MsgAdapter.ReadShort()			-- 还没好先写死0剩余采集次数
	-- MsgAdapter.ReadShort()
end
...
-- 飞行高度改变
CSObjFlyHeightChange = CSObjFlyHeightChange or BaseClass(BaseProtocolStruct)
function CSObjFlyHeightChange:__init()
	self.msg_type = 1171
	self.fly_height = 0
end

function CSObjFlyHeightChange:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
	MsgAdapter.WriteShort(self.fly_height)
	MsgAdapter.WriteShort(0)
end

-- 飞行高度改变广播
SCObjFlyHeightChange = SCObjFlyHeightChange or BaseClass(BaseProtocolStruct)
function SCObjFlyHeightChange:__init()
	self.msg_type = 1172
	self.obj_id = 0
	self.height = 0
end

function SCObjFlyHeightChange:Decode()
	self.obj_id = MsgAdapter.ReadUShort()
	self.height = MsgAdapter.ReadShort()
end
...
SCBossDpsFlagInfo = SCBossDpsFlagInfo or BaseClass(BaseProtocolStruct)
function SCBossDpsFlagInfo:__init()
	self.msg_type = 1175
	self.obj_id = 0
	self.dps_flag = 0
end

function SCBossDpsFlagInfo:Decode()
	self.obj_id = MsgAdapter.ReadUShort()
	self.dps_flag = MsgAdapter.ReadShort()
end

-- 客户端脱离卡死
CSRoleReturnReAlivePosi = CSRoleReturnReAlivePosi or BaseClass(BaseProtocolStruct)
function CSRoleReturnReAlivePosi:__init()
	self.msg_type = 1176
end

function CSRoleReturnReAlivePosi:Encode()
	MsgAdapter.WriteBegin(self.msg_type)
end
