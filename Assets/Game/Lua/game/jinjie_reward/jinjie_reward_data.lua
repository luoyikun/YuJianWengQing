...
--形象系统进阶类型
ADVANCE_SYSTEM_TYPE = {
	JINJIE_TYPE.JINJIE_TYPE_FIGHT_MOUNT,
	JINJIE_TYPE.JINJIE_TYPE_FOOTPRINT,
	JINJIE_TYPE.JINJIE_TYPE_HALO,
	JINJIE_TYPE.JINJIE_TYPE_MOUNT,
	JINJIE_TYPE.JINJIE_TYPE_WING,
	JINJIE_TYPE.JINJIE_TYPE_LINGCHONG,
	JINJIE_TYPE.JINJIE_TYPE_FABAO,
	JINJIE_TYPE.JINJIE_TYPE_FASHION,
	JINJIE_TYPE.JINJIE_TYPE_SHENBING,
}

--伙伴系统进阶类型
GODDESS_SYSTEM_TYPE = {
	JINJIE_TYPE.JINJIE_TYPE_SHENGONG,
	JINJIE_TYPE.JINJIE_TYPE_SHENYI,
}

--外观系统进阶类型
APPEARANCE_SYSTEM_TYPE = {
	JINJIE_TYPE.JINJIE_TYPE_MASK,
	JINJIE_TYPE.JINJIE_TYPE_TOUSHI,
	JINJIE_TYPE.JINJIE_TYPE_YAOSHI,
	JINJIE_TYPE.JINJIE_TYPE_QILINBI,
	JINJIE_TYPE.JINJIE_TYPE_LINGZHU,
	JINJIE_TYPE.JINJIE_TYPE_XIANBAO,
	JINJIE_TYPE.JINJIE_TYPE_LINGGONG,
	JINJIE_TYPE.JINJIE_TYPE_LINGQI,
	JINJIE_TYPE.JINJIE_TYPE_WEIYAN,
}
...
function JinJieRewardData:GetSystemFreeIsOpen(system_type, target_type)
	...
	if open_server_day == 0 then 			--开服天数为0代表按系统开启时间计算
		return true
	end
...
function JinJieRewardData:GetSystemIsActiveSpecialImage(system_type)
	...
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_MASK then					-- 面饰
		is_active = MaskData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_YAOSHI then				-- 腰饰
		is_active = WaistData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TOUSHI then				-- 头饰
		is_active = TouShiData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_QILINBI then				-- 麒麟臂
		is_active = QilinBiData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGZHU then				-- 灵珠
		is_active = LingZhuData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_XIANBAO then				-- 仙宝
		is_active = XianBaoData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_WEIYAN then				-- 尾焰
		is_active = WeiYanData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGCHONG then			-- 灵宠
		is_active = LingChongData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGGONG then				-- 灵弓
		is_active = LingGongData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGQI then				-- 灵骑
		is_active = LingQiData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN then				-- 手环
		is_active = ShouHuanData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TALT then					-- 尾巴
		is_active = TailData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FLYPET then				-- 飞宠
		is_active = FlyPetData.Instance:GetHuanHuaIsActiveByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FABAO then				-- 法宝
...
function JinJieRewardData:GetSystemSpecialImageGrade(system_type, img_id)
	...
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_MASK then					-- 面饰
		grade = MaskData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_YAOSHI then				-- 腰饰
		grade = WaistData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TOUSHI then				-- 头饰
		grade = TouShiData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_QILINBI then				-- 麒麟臂
		grade = QilinBiData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGZHU then				-- 灵珠
		grade = LingZhuData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_XIANBAO then				-- 仙宝
		grade = XianBaoData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_WEIYAN then				-- 尾焰
		grade = WeiYanData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGCHONG then			-- 灵宠
		grade = LingChongData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGGONG then				-- 灵弓
		grade = LingGongData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGQI then				-- 灵骑
		grade = LingQiData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN then				-- 手环
		grade = ShouHuanData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TALT then					-- 尾巴
		grade = TailData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FLYPET then				-- 飞宠
		grade = FlyPetData.Instance:GetHuanHuaGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FABAO then				-- 法宝
		grade = FaBaoData.Instance:GetSingleSpecialImageGrade(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FASHION then				-- 时装
		grade = FashionData.Instance:GetSingleSpecialImageGrade(img_id, SHIZHUANG_TYPE.BODY)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHENBING then				-- 神兵
		grade = FashionData.Instance:GetSingleSpecialImageGrade(img_id, SHIZHUANG_TYPE.WUQI)
	end
...
function JinJieRewardData:GetSystemSpecialImageLevelCfg(system_type, img_id)
	...
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_MASK then					-- 面饰
		cfg = MaskData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_YAOSHI then				-- 腰饰
		cfg = WaistData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TOUSHI then				-- 头饰
		cfg = TouShiData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_QILINBI then				-- 麒麟臂
		cfg = QilinBiData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGZHU then				-- 灵珠
		cfg = LingZhuData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_XIANBAO then				-- 仙宝
		cfg = XianBaoData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_WEIYAN then				-- 尾焰
		cfg = WeiYanData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGCHONG then			-- 灵宠
		cfg = LingChongData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGGONG then				-- 灵弓
		cfg = LingGongData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGQI then				-- 灵骑
		cfg = LingQiData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN then				-- 手环
		cfg = ShouHuanData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TALT then					-- 尾巴
		cfg = TailData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FLYPET then				-- 飞宠
		cfg = FlyPetData.Instance:GetHuanHuaCfgInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FABAO then				-- 法宝
		cfg = FaBaoData.Instance:GetSpecialImageUpgradeInfo(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FASHION then				-- 时装
		cfg = FashionData.Instance:GetShizhuangSpecialImgUpgradeById(img_id, grade)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHENBING then				-- 神兵
		cfg = FashionData.Instance:GetSpecialImageUpgradeInfo(img_id, grade)
	end
...
function JinJieRewardData:GetSystemJinJieLevelBasePowerAndExtraAddPer(system_type)
	...
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_MASK then													-- 面饰
		base_power, extra_add_per = MaskData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_YAOSHI then												-- 腰饰
		base_power, extra_add_per = WaistData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TOUSHI then												-- 头饰
		base_power, extra_add_per = TouShiData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_QILINBI then												-- 麒麟臂
		base_power, extra_add_per = QilinBiData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGZHU then												-- 灵珠
		base_power, extra_add_per = LingZhuData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_XIANBAO then												-- 仙宝
		base_power, extra_add_per = XianBaoData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_WEIYAN then												-- 尾焰
		base_power, extra_add_per = WeiYanData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGCHONG then											-- 灵宠
		base_power, extra_add_per = LingChongData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGGONG then												-- 灵弓
		base_power, extra_add_per = LingGongData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGQI then												-- 灵骑
		base_power, extra_add_per = LingQiData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN then												-- 手环
		base_power, extra_add_per = ShouHuanData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TALT then													-- 尾巴
		base_power, extra_add_per = TailData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FLYPET then												-- 飞宠
		base_power, extra_add_per = FlyPetData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FABAO then												-- 法宝
		base_power, extra_add_per = FaBaoData.Instance:GetCurGradeBaseFightPowerAndAddPer()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FASHION then												-- 时装
		base_power, extra_add_per = FashionData.Instance:GetCurGradeBaseFightPowerAndAddPer(SHIZHUANG_TYPE.BODY)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHENBING then												-- 神兵
		base_power, extra_add_per = FashionData.Instance:GetCurGradeBaseFightPowerAndAddPer(SHIZHUANG_TYPE.WUQI)
	end
...
function JinJieRewardData:GetSystemSpecialImageCfg(system_type, img_id)
	...
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_MASK then							-- 面饰
		cfg = MaskData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_YAOSHI then						-- 腰饰
		cfg = WaistData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TOUSHI then						-- 头饰
		cfg = TouShiData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_QILINBI then						-- 麒麟臂
		cfg = QilinBiData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGZHU then						-- 灵珠
		cfg = LingZhuData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_XIANBAO then						-- 仙宝
		cfg = XianBaoData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_WEIYAN then						-- 尾焰
		cfg = WeiYanData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGCHONG then					-- 灵宠
		cfg = LingChongData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGGONG then						-- 灵弓
		cfg = LingGongData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGQI then						-- 灵骑
		cfg = LingQiData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN then						-- 手环
		cfg = ShouHuanData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TALT then							-- 尾巴
		cfg = TailData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FLYPET then						-- 飞宠
		cfg = FlyPetData.Instance:GetSpecialImageCfgInfoByImageId(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FABAO then						-- 法宝
		cfg = FaBaoData.Instance:GetSpecialImageCfg(img_id)
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FASHION then						-- 时装
		local list = FashionData.Instance:GetShizhuangSpecialImage()
		cfg = list[img_id]
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHENBING then						-- 神兵
		local list = FashionData.Instance:GetWuQiImageCfg()
		cfg = list[img_id]
	end
...
function JinJieRewardData:GetSystemIsUseCurSpecialImage(system_type)
	...
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_MASK then					-- 面饰
		use_img_id = MaskData.Instance:GetUsedImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_YAOSHI then				-- 腰饰
		use_img_id = WaistData.Instance:GetUsedImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TOUSHI then				-- 头饰
		use_img_id = TouShiData.Instance:GetUsedImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_QILINBI then				-- 麒麟臂
		use_img_id = QilinBiData.Instance:GetUsedImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGZHU then				-- 灵珠
		use_img_id = LingZhuData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_XIANBAO then				-- 仙宝
		use_img_id = XianBaoData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_WEIYAN then				-- 尾焰
		use_img_id = WeiYanData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGCHONG then			-- 灵宠
		use_img_id = LingChongData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGGONG then				-- 灵弓
		use_img_id = LingGongData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGQI then				-- 灵骑
		use_img_id = LingQiData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN then				-- 手环
		use_img_id = ShouHuanData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TALT then					-- 尾巴
		use_img_id = TailData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FLYPET then				-- 飞宠
		use_img_id = FlyPetData.Instance:GetUsedImageId()
		use_img_id = use_img_id + GameEnum.MOUNT_SPECIAL_IMA_ID
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FABAO then				-- 法宝
		local fabao_info = FaBaoData.Instance:GetFaBaoInfo()
		use_img_id = fabao_info.is_used_special_img ~= 0 and fabao_info.used_special_id or 0
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FASHION then				-- 时装
		local fashion_info = FashionData.Instance:GetFashionInfo()
		if fashion_info.is_used_special_img == 1 then
			use_img_id = fashion_info.use_special_img + GameEnum.MOUNT_SPECIAL_IMA_ID
		else
			use_img_id = 0
		end
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHENBING then				-- 神兵
		...
		if wuqi_info.is_used_special_img == 1 then
			use_img_id = wuqi_info.use_special_img + GameEnum.MOUNT_SPECIAL_IMA_ID
		else
			use_img_id = 0
		end
	end
...
function JinJieRewardData:GetSystemCurJinJieGradeImageId(system_type)
	...
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_MASK then					-- 面饰
		image_id = MaskData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_YAOSHI then				-- 腰饰
		image_id = WaistData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TOUSHI then				-- 头饰
		image_id = TouShiData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_QILINBI then				-- 麒麟臂
		image_id = QilinBiData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGZHU then				-- 灵珠
		image_id = LingZhuData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_XIANBAO then				-- 仙宝
		image_id = XianBaoData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_WEIYAN then				-- 尾焰
		image_id = WeiYanData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGCHONG then			-- 灵宠
		image_id = LingChongData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGGONG then				-- 灵弓
		image_id = LingGongData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_LINGQI then				-- 灵骑
		image_id = LingQiData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN then				-- 手环
		image_id = ShouHuanData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_TALT then					-- 尾巴
		image_id = TailData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FLYPET then				-- 飞宠
		image_id = FlyPetData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FABAO then				-- 法宝
		image_id = FaBaoData.Instance:GetCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_FASHION then				-- 时装
		image_id = FashionData.Instance:GetFashionCurGradeImageId()
	elseif system_type == JINJIE_TYPE.JINJIE_TYPE_SHENBING then				-- 神兵
		image_id = FashionData.Instance:GetCurGradeImageId()
	end
...
function JinJieRewardData:GetSmallTargetShowData(system_type, callback)
	...
	data_list.system_type = system_type																				-- 类型
...
function JinJieRewardData:IsAdvance(system_type)
	local is_advance = false
	for k,v in pairs(ADVANCE_SYSTEM_TYPE) do
		if system_type == v then
			is_advance = true
			break
		end
	end
...
function JinJieRewardData:IsGoddess(system_type)
	local is_goddess = false
	for k,v in pairs(GODDESS_SYSTEM_TYPE) do
		if system_type == v then
			is_goddess = true
			break
		end
...
function JinJieRewardData:IsAppearance(system_type)
	local is_appearance = false
	for k,v in pairs(APPEARANCE_SYSTEM_TYPE) do
		if system_type == v then
			is_appearance = true
			break
		end
