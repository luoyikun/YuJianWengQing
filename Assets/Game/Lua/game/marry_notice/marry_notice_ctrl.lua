...
function MarryNoticeCtrl:OnMarryNotic(protocol)
	...
	if main_role_id ~= protocol.uid1 and main_role_id ~= protocol.uid2 then
		...
		ViewManager.Instance:FlushView(ViewName.MarryNoticeView, "info", {uid1 = protocol.uid1, name1 = protocol.name1, uid2 = protocol.uid2, name2 = protocol.name2, server_marry_times = protocol.server_marry_times,  prof1 = protocol.prof1, prof2 = protocol.prof2})
	end
end
...
function MarryNoticeCtrl:OnMarryZhuheShou(protocol)
	...
	if not self.blessing_view:IsOpen() then
		MainUICtrl.Instance:FlushTipsIcon(MainUIViewChat.IconList.MarryBlessing, true)
	end
end
