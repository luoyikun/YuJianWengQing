...
require("game/shenshou/shengqi_equip_strength_view")
require("game/shenshou/shengqi_equip_spirit_view")
require("game/shenshou/shengqi_kill_view")
...
function ShenShouCtrl:__init()
	...
	self.shengqi_strength_view = ShengQiEquipStrengthView.New(ViewName.ShengQiEquipStrength)
	self.shengqi_spirit_view = ShengQiEquipSpiritView.New(ViewName.ShengQiEquipSpirit)
	self.shengqi_kill_view = ShengQiKillView.New(ViewName.ShengQiKillView)

	self.shengqi_req_type = nil
	self:RegisterAllProtocols()
end

function ShenShouCtrl:__delete()
	ShenShouCtrl.Instance = nil
	if nil ~= self.shengqi_strength_view then
		self.shengqi_strength_view:DeleteMe()
		self.shengqi_strength_view = nil
	end
	if nil ~= self.shengqi_spirit_view then
		self.shengqi_spirit_view:DeleteMe()
		self.shengqi_spirit_view = nil
	end
	...
	if nil == self.shengqi_kill_view then
		self.shengqi_kill_view:DeleteMe()
		self.shengqi_kill_view = nil
	end
	if nil ~= self.bag_view then
	...
	if nil ~= self.shenshou_equip_tip then
		self.shenshou_equip_tip:DeleteMe()
		self.shenshou_equip_tip = nil
	end
	if nil ~= self.fuling_select_material_view then
		self.fuling_select_material_view:DeleteMe()
		self.fuling_select_material_view = nil
	end
	if nil ~= self.fuling_tips then
		self.fuling_tips:DeleteMe()
		self.fuling_tips = nil
	end
end
...
function ShenShouCtrl:RegisterAllProtocols()
	...
	self:RegisterProtocol(CSShengqiReq)
	self:RegisterProtocol(SCShengqiInfo, "OnShengQiEquipInfo")
	self:BindGlobalEvent(MainUIEventType.MAINUI_OPEN_COMLETE, BindTool.Bind1(self.MainuiOpenCreate, self))
end

function ShenShouCtrl:OnShengQiEquipInfo(protocol)
	self.data:OnShengQiEquipInfo(protocol)
	self.view:Flush()

	if self.shengqi_strength_view:IsOpen() then
		self.shengqi_strength_view:Flush()
	end
	if self.shengqi_spirit_view:IsOpen() then
		self.shengqi_spirit_view:Flush()
	end
	RemindManager.Instance:Fire(RemindName.ShengQi)
end

-- req_type 0 获取圣器所有信息  1 圣器激活操作  2 圣器开启附灵属性操作  3 圣器附灵操作  4 圣器强化操作
function ShenShouCtrl:SendShengQiEquipReq(req_type, param1, param2)
	if req_type == nil then return end

	self.shengqi_req_type = req_type
	local protocol = ProtocolPool.Instance:GetProtocol(CSShengqiReq)
...
function ShenShouCtrl:OpenSkillTip(index, cell)
	...
	self.shenshou_skill_tip:Open()	--现在open之后的代码是不执行的
end
...
function ShenShouCtrl:SetFulingCancelViewCallBack(callback)
	self.fuling_select_material_view:SetCallBack(callback , "Cancel")
end
...
function ShenShouCtrl:OpenShengQiequipSpiritView(data)
	self.shengqi_spirit_view:SetViewData(data)
	self.shengqi_spirit_view:Open()
end

function ShenShouCtrl:OpenShengQiequipStrengthView(data)
	self.shengqi_strength_view:SetViewData(data)
	self.shengqi_strength_view:Open()
end