...
function GeneralSkillView:__init(instance, parent)
	...
	self.skill_cd_time = {}
	for i = 1, 3 do
		self.skill_icon_pos_list[i] = self.node_list["Skill" .. i]
		self.skill_icons[i] = self.node_list["SkillIcon" .. i]
	end

	for i = 2, 3 do
		self.skill_cd_progress[i] = self.node_list["CDMask" .. i]
		self.skill_cd_progress[i].image.fillAmount = 0
		self.skill_cd_time[i] = self.node_list["CDText" .. i]
		self.skill_cd_time[i].text.text = 0
	end
	...
	-- 监听UI事件
	self.node_list["Skill1"].event_trigger_listener:AddPointerDownListener(BindTool.Bind(self.OnClickAttackDown, self, self.skill_infos[1]))
	self.node_list["Skill1"].event_trigger_listener:AddPointerUpListener(BindTool.Bind(self.OnClickAttackUp, self, self.skill_infos[1]))
	self.node_list["Skill2"].button:AddClickListener(BindTool.Bind(self.OnClickSkill, self, self.skill_infos[2]))
	self.node_list["Skill3"].button:AddClickListener(BindTool.Bind(self.OnClickSkill, self, self.skill_infos[3]))
...
function GeneralSkillView:OnRecvMainRoleInfo()
	local skillinfo = BianShenData.Instance:GetSkillCfg()
...
function GeneralSkillView:OnSkillChange(change_type, skill_id)
	...
	local skill_cfg = nil
	for i, v in ipairs(self.skill_infos) do
		skill_info = ConfigManager.Instance:GetAutoConfig("roleskill_auto").skillinfo[v.skill_id]
...
function GeneralSkillView:OnMainRoleUseSkill(skill_id)
	for i, v in ipairs(self.skill_infos) do
		if v.skill_id == skill_id and self.skill_cd_progress[i] then
			...
			self.skill_cd_progress[i].image.fillAmount = 0.0
			...
			if self.skill_cd_time[i] then
				self.skill_cd_time[i].text.text = math.ceil(cd)
				self.skill_cd_time[i]:SetActive(not (math.ceil(cd) == 0))
				v.countdonw2 = CountDown.Instance:AddCountDown(
					cd, 1.0, function(elapse_time, total_time)
						self.skill_cd_time[i].text.text = math.ceil(total_time - elapse_time)
						self.skill_cd_time[i]:SetActive(not (math.ceil(total_time - elapse_time) == 0))
...
function GeneralSkillView:OnFlush(param_t)
	...
	for i, v in ipairs(self.skill_infos) do
		if v.is_exist and nil ~= v.skill_icon then
			...
			self.skill_icons[i].image:LoadSpriteAsync(bundle, asset .. ".png")
		end
...
function GeneralSkillView:OnClickAttackUp(skill_info)
	...
	elseif math.abs(off_y) < 20 then
		...
		if off_y < 0 then
			...
			self:SelectObj(SceneObjType.Monster, SelectType.Alive)
		end
	end
