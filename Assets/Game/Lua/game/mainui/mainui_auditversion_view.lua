MainUIAuditVersionView = MainUIAuditVersionView or BaseClass(BaseRender)

function MainUIAuditVersionView:__init()
	self.attr_handlers = {
	...
	self.player_data_change_callback = BindTool.Bind1(self.AuditPlayerDataChangeCallback, self)
	...
	self.node_list["Recharge"].button:AddClickListener(BindTool.Bind(self.OnClickRecharge, self))
	self.node_list["Shop"].button:AddClickListener(BindTool.Bind(self.OnClickShop, self))
	self.node_list["BtnPackage"].button:AddClickListener(BindTool.Bind(self.OnClickPackage, self))
	self:Flush()
end

function MainUIAuditVersionView:__delete()
...
function MainUIAuditVersionView:OnClickRecharge()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function MainUIAuditVersionView:OnClickShop()
	-- ViewManager.Instance:Open(ViewName.Shop)
	ShopCtrl.Instance:OpenShopView()
end

function MainUIAuditVersionView:OnClickPackage()
	ViewManager.Instance:Open(ViewName.PackageView)
end

function MainUIAuditVersionView:AuditPlayerDataChangeCallback(attr_name, value, old_value)
...
function MainUIAuditVersionView:OnFlush()
...
function MainUIAuditVersionView:SetImage()
	if IS_AUDIT_VERSION then
		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_player_head_bg.png") then
			self.node_list["player_head_bg"]:SetActive(false)
			self.node_list["player_head_bg_url"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_player_head_bg.png")
			self.node_list["player_head_bg_url"].raw_image:LoadURLSprite(path)
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_icon_bg.png") then
			self.node_list["Icon_Bg_Shop"]:SetActive(false)
			self.node_list["Icon_Bg_Recharge"]:SetActive(false)
			self.node_list["Icon_Bg_Recharge_url"]:SetActive(true)
			self.node_list["Icon_Bg_Shop_url"]:SetActive(true)
			self.node_list["Icon_Bg_Package"]:SetActive(false)
			self.node_list["Icon_Bg_Package_url"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_icon_bg.png")
			self.node_list["Icon_Bg_Recharge_url"].raw_image:LoadURLSprite(path)
			self.node_list["Icon_Bg_Shop_url"].raw_image:LoadURLSprite(path)
			self.node_list["Icon_Bg_Package_url"].raw_image:LoadURLSprite(path)
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_icon_recharge.png") then
			self.node_list["ImgRecharge"]:SetActive(false)
			self.node_list["ImgRecharge_url"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_icon_recharge.png")
			self.node_list["ImgRecharge_url"].raw_image:LoadURLSprite(path)
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_icon_shop.png") then
			self.node_list["ImgShop"]:SetActive(false)
			self.node_list["ImgShop_url"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_icon_shop.png")
			self.node_list["ImgShop_url"].raw_image:LoadURLSprite(path)
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_icon_recharge_name.png") then
			self.node_list["ImgRechargeName"]:SetActive(false)
			self.node_list["ImgRechargeName_url"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_icon_recharge_name.png")
			self.node_list["ImgRechargeName_url"].raw_image:LoadURLSprite(path)
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_icon_shop_name.png") then
			self.node_list["ImgShopName"]:SetActive(false)
			self.node_list["ImgShopName_url"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_icon_shop_name.png")
			self.node_list["ImgShopName_url"].raw_image:LoadURLSprite(path)
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_icon_package.png") then
			self.node_list["ImgPackage"]:SetActive(false)
			self.node_list["ImgPackage_url"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_icon_package.png")
			self.node_list["ImgPackage_url"].raw_image:LoadURLSprite(path)
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_icon_package_name.png") then
			self.node_list["ImgPackageName"]:SetActive(false)
			self.node_list["ImgPackageName_url"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_icon_package_name.png")
			self.node_list["ImgPackageName_url"].raw_image:LoadURLSprite(path)
		end
	end
end

-- 设置目标血条
function MainUIAuditVersionView:SetHpPercent(percent)
	local value_hp = percent or 0
	self.hp_slider_top.value = value_hp
	self.hp_slider_bottom:DOValue(value_hp, 0.8, false)
end

function MainUIAuditVersionView:UpdateAttackMode(mode)
...
function MainUIAuditVersionView:OnFightPowerChanged()
	local vo = GameVoManager.Instance:GetMainRoleVo()
	self.node_list["TxtNumber"].text.text = vo.capability
end

function MainUIAuditVersionView:OnHPChanged()
	local vo = GameVoManager.Instance:GetMainRoleVo()
...
function MainUIAuditVersionView:OnLevelChanged()
	local vo = GameVoManager.Instance:GetMainRoleVo()
...
function MainUIAuditVersionView:RoleChangeProf()
...
-- 头像更换
function MainUIAuditVersionView:OnHeadChange()
	local vo = GameVoManager.Instance:GetMainRoleVo()
	...
	if IS_AUDIT_VERSION then
		if ResMgr.ExistedInStreaming("AgentAssets/Main/main_head_image" .. prof .. ".png") then
			self.node_list["Portrait"]:SetActive(false)
			self.node_list["RawPortrait"]:SetActive(true)

			local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/main_head_image" .. prof .. ".png")
			self.node_list["RawPortrait"].raw_image:LoadURLSprite(path)
		else
			load_callback()
		end
	else
...
function MainUIAuditVersionView:OnFightEffectChange(is_main_role)
...
-----------------MainUIAuditVersionSkillView-----------------------
MainUIAuditVersionSkillView = MainUIAuditVersionSkillView or BaseClass(BaseRender)
...
function MainUIAuditVersionSkillView:__init(instance, parent)
	-- 初始化
	self.parent = parent
	self.is_show_icons_image_url = {}
	self.skill_infos = {
		SkillInfo(),
		SkillInfo(),
		SkillInfo(),
	...
	self.skill_icons_url = {
		self.node_list["AttackIcon_url"],
		self.node_list["SkillImg_url1"],
		self.node_list["SkillImg_url2"],
		self.node_list["SkillImg_url3"],
		self.node_list["SkillImg_url4"],
		false,
		false,
		false,
		self.node_list["SkillImg_url9"],
		self.node_list["SkillImg_url10"],
	}
	...
	self.skill_lock_url = {
		false,
		self.node_list["SkillLock_url1"],
		self.node_list["SkillLock_url2"],
		self.node_list["SkillLock_url3"],
		self.node_list["SkillLock_url4"],
		false,
		false,
	...
	self.node_list["BtnSimple"]:SetActive(false)
	self.node_list["SavePower"]:SetActive(false)
	self.node_list["JumpContent"]:SetActive(false)
	self.node_list["HidePanel"]:SetActive(false)

	self:SetSpecialImage()
end

function MainUIAuditVersionSkillView:SetSpecialImage()
	if IS_AUDIT_VERSION then
		if ResMgr.ExistedInStreaming("AgentAssets/Main/skill_icon_bg.png") then
			for i = 1, 4 do
				if self.node_list["SkillIconBg" .. i] and self.node_list["SkillIconBg_url" .. i] then
					self.node_list["SkillIconBg" .. i]:SetActive(false)
					self.node_list["SkillIconBg_url" .. i]:SetActive(true)

					local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/skill_icon_bg.png")
					self.node_list["SkillIconBg_url" .. i].raw_image:LoadURLSprite(path)
				end
			end
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/special_skill_icon_bg.png") then
			for i = 9,10 do
				if self.node_list["SkillIconBg" .. i] and self.node_list["SkillIconBg_url" .. i] then
					self.node_list["SkillIconBg" .. i]:SetActive(false)
					self.node_list["SkillIconBg_url" .. i]:SetActive(true)

					local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/special_skill_icon_bg.png")
					self.node_list["SkillIconBg_url" .. i].raw_image:LoadURLSprite(path)
				end
			end
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/attack_skill_icon_bg.png") then
			if self.node_list["ImgAttackBg"] and self.node_list["ImgAttackBg_url"] then
				self.node_list["ImgAttackBg"]:SetActive(false)
				self.node_list["ImgAttackBg_url"]:SetActive(true)

				local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/attack_skill_icon_bg.png")
				self.node_list["ImgAttackBg_url"].raw_image:LoadURLSprite(path)
			end
		end

		if ResMgr.ExistedInStreaming("AgentAssets/Main/skill_lock_image.png") then
			for i = 1, 4 do
				if self.node_list["Skill" .. i .. "Lock"] and self.node_list["SkillLock_url" .. i] then
					self.node_list["Skill" .. i .. "Lock"]:SetActive(false)
					-- self.node_list["SkillLock_url" .. i]:SetActive(true)

					local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/skill_lock_image.png")
					self.node_list["SkillLock_url" .. i].raw_image:LoadURLSprite(path)
				end
			end
		end
	end
...
function MainUIAuditVersionSkillView:FlusChengZhuSkill()
	self:OnSkillChange()
end

function MainUIAuditVersionSkillView:__delete()
	PlayerData.Instance:UnlistenerAttrChange(self.player_data_change_callback)
	self.player_data_change_callback = nil
...
function MainUIAuditVersionSkillView:SetSkillLock(index, is_lock)
	if self.skill_icons[index] and not TipsCtrl.Instance:GetIsOpenNewSkill() then
		self.skill_icons[index]:SetActive(not is_lock and not self.is_show_icons_image_url[index])
	end
	if self.skill_icons_url[index] and not TipsCtrl.Instance:GetIsOpenNewSkill() then
		if index ~= 1 then
			self.skill_icons_url[index]:SetActive(not is_lock and IS_AUDIT_VERSION)
		end
	end
	if self.skill_lock[index] then
		if ResMgr.ExistedInStreaming("AgentAssets/Main/skill_lock_image.png") then
			self.skill_lock[index]:SetActive(is_lock and not IS_AUDIT_VERSION)
		else
			self.skill_lock[index]:SetActive(is_lock)
		end
	end
	if self.skill_lock_url[index] then
		if IS_AUDIT_VERSION and ResMgr.ExistedInStreaming("AgentAssets/Main/skill_lock_image.png") then
			self.skill_lock_url[index]:SetActive(is_lock)
		end
	end
...
function MainUIAuditVersionSkillView:ShowSkill6Effect(param)
	if false == param then
...
function MainUIAuditVersionSkillView:SetJinjieSkillInfo()
...
function MainUIAuditVersionSkillView:OnClickJinjieEquipSkill()
	ViewManager.Instance:Open(ViewName.AdvanceEquipSkillView)
end

--技能切换
function MainUIAuditVersionSkillView:OnClickQieHuan(skill_info,flag)
...
function MainUIAuditVersionSkillView:SetMojiePanelAnimation(value)
...
function MainUIAuditVersionSkillView:OnMainUIModeListChange(is_show)
	self:SetMojiePanelAnimation(false)
end

function MainUIAuditVersionSkillView:PortraitToggleChange(state, from_joystick)
	self:SetMojiePanelAnimation(false)
end

function MainUIAuditVersionSkillView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function MainUIAuditVersionSkillView:IsShowJumpBtn()
...
function MainUIAuditVersionSkillView:SetJumpButIsShow(value)
	if value then
		self:IsShowJumpBtn()
	else
		if self.node_list["JumpButton"] then
...
function MainUIAuditVersionSkillView:OnRecvMainRoleInfo()
...
function MainUIAuditVersionSkillView:CheckNuqiEff()
...
function MainUIAuditVersionSkillView:OnSkillChange(change_type, skill_id)
	local mojie_index = 0
...
function MainUIAuditVersionSkillView:OnMainRoleUseSkill(skill_id)
	local prof = PlayerData.Instance:GetRoleBaseProf()
	...
	elseif math.floor(skill_id / 10) == math.floor(self.skill_infos[1].skill_id / 10) then
		...
		if nil ~= skill_cfg then
			...
			if IS_AUDIT_VERSION then
				if ResMgr.ExistedInStreaming("AgentAssets/Main/skill_" .. skill_cfg.skill_icon ..".png") then
					self.skill_icons[1]:SetActive(false)
					self.node_list["AttackIcon_url"]:SetActive(true)

					local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/Skill_" .. skill_cfg.skill_icon ..".png")
					self.node_list["AttackIcon_url"].raw_image:LoadURLSprite(path)
				else
		...
		self.common_skill_timer = GlobalTimerQuest:AddDelayTimer(function()
			...
			if IS_AUDIT_VERSION then
				if ResMgr.ExistedInStreaming("AgentAssets/Main/Skill_" .. self.skill_infos[1].skill_icon ..".png") then
					self.skill_icons[1]:SetActive(false)
					self.node_list["AttackIcon_url"]:SetActive(true)

					local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/Skill_" .. self.skill_infos[1].skill_icon ..".png")
					self.node_list["AttackIcon_url"].raw_image:LoadURLSprite(path)
				else
					self.skill_icons[1].image:LoadSprite(bundle, asset .. ".png")
				end
...
function MainUIAuditVersionSkillView:OnFlush(param_t)
	...
	for i, v in ipairs(self.skill_infos) do
		...
		if nil ~= v.skill_icon then
			if v.is_exist or i == 9 or i == 10 then
				...
				if IS_AUDIT_VERSION and self.skill_icons_url[i] then
					if ResMgr.ExistedInStreaming("AgentAssets/Main/Skill_" .. v.skill_icon ..".png") then
						self.skill_icons[i]:SetActive(false)
						self.is_show_icons_image_url[i] = true
						local path = ResUtil.GetAgentAssetPath("AgentAssets/Main/Skill_" .. v.skill_icon ..".png")
						self.skill_icons_url[i].raw_image:LoadURLSprite(path)
					else
						self.is_show_icons_image_url[i] = false
						self.skill_icons[i].image:LoadSprite(bundle, asset .. ".png")
					end
				else
					self.skill_icons[i].image:LoadSprite(bundle, asset .. ".png")
...
function MainUIAuditVersionSkillView:ClickSkill5Change(index)
...
function MainUIAuditVersionSkillView:OnClickAttackDown(skill_info)
	click_attack_down = UnityEngine.Input.mousePosition
end

function MainUIAuditVersionSkillView:OnClickAttackUp(skill_info)
...
function MainUIAuditVersionSkillView:SelectObj(obj_type, select_type)
	-- 获取所有可选对象
...
function MainUIAuditVersionSkillView:OnClickSkill5Up(skill_info)
...
function MainUIAuditVersionSkillView:OnClickSkill5Down(skill_info)
...
function MainUIAuditVersionSkillView:OnClickSkill(skill_info)
	self:SetMojiePanelAnimation(false)
...
function MainUIAuditVersionSkillView:OnClickLanding()
...
function MainUIAuditVersionSkillView:FlushJumpEnergyShow()
...
function MainUIAuditVersionSkillView:FlushEnergyTime()
...
--刷新跳跃显示
function MainUIAuditVersionSkillView:FlushJumpState(qinggong_index)
...
-- 倒计时函数
function MainUIAuditVersionSkillView:JumpProgress(elapse_time, total_time)
...
function MainUIAuditVersionSkillView:FlushQinggongShow()
...
function MainUIAuditVersionSkillView:OnMainRoleJumpState(state)
	if state then
...
function MainUIAuditVersionSkillView:Update()
	-- Refresh jump time
	local vo = GameVoManager.Instance:GetMainRoleVo()
...
function MainUIAuditVersionSkillView:GetSkillButtonPosition()
	return self.skill_icon_pos_list
end

local old_type = nil
function MainUIAuditVersionSkillView:OnMainRoleSwitchScene(area_type)
...
function MainUIAuditVersionSkillView:DeleteAreaTip(area_type)
	if area_type == SceneConvertionArea.WAY_TO_SAFE then
		if self.enter_safe_area_animator and self.enter_safe_area_animator.isActiveAndEnabled then
...
function MainUIAuditVersionSkillView:ClickGeneralSkill()
...
function MainUIAuditVersionSkillView:SetGuildStation(is_active)
	if self.is_guild_station then
		self.is_guild_station = is_active
	end
end

function MainUIAuditVersionSkillView:FlushGeneralSkill(is_in)
	...
	if self.node_list["BtnBianShen"] then
		...
		if BianShenData.Instance:CheckShowSkill() and not self.is_guild_station and is_can_bianshen then
			...
			if value == -1 and has_general_skill then
				...
				-- self.node_list["BtnBianShen"]:SetActive(true)
...
-- 获取变身引导
function MainUIAuditVersionSkillView:GetMainBianShen()
	if self.node_list["BtnBianShen"] then
		return self.node_list["BtnBianShen"], BindTool.Bind(self.ClickGeneralSkill, self)
	end
end

function MainUIAuditVersionSkillView:GeneralBianShenCd()
...
function MainUIAuditVersionSkillView:UpdateGeneralSkill(elapse_time, total_time)
...
function MainUIAuditVersionSkillView:UpdateGeneralCD(elapse_time, total_time)
	local daiff_value = math.floor(total_time - elapse_time)
	if self.general_skill and not IsNil(self.general_skill.cd_time.gameObject)  then
		self.general_skill.cd_time.text.text = TimeUtil.FormatSecond(daiff_value, 2)
	end
...
-- 变身按钮计时器结束回调
function MainUIAuditVersionSkillView:GeneralComplereFun(is_general)
	self.general_skill.cd_time.text.text = ""
	CountDown.Instance:RemoveCountDown(self.general_skill.countdonw)
...
function MainUIAuditVersionSkillView:SetBtnBianShenEffect(is_nb, is_ruo)
...
--引导用名将变身技能
function MainUIAuditVersionSkillView:GetClickGeneralSkillCallBack()
	return BindTool.Bind(self.ClickGeneralSkill, self)
end

function MainUIAuditVersionSkillView:ChangeBianShenEffect()
	local main_role = Scene.Instance:GetMainRole()
...
function MainUIAuditVersionSkillView:OnSwitchSpecSkills()
...
function MainUIAuditVersionSkillView:OnShowActivitySkill(attach_obj)
...
function MainUIAuditVersionSkillView:ClearActivitySkill()
...
function MainUIAuditVersionSkillView:OnSwitchActivitySkills(duration_time)
...
-- 功能引导仙女技能
function MainUIAuditVersionSkillView:GetSkill6()
	if self.node_list["Skill6"] then
		return self.node_list["Skill6"], BindTool.Bind(self.OnClickSkill, self, self.skill_infos[6])
	end
end

function MainUIAuditVersionSkillView:GetQingGongBtn()
	if self.node_list["JumpButton"] then
		return self.node_list["JumpButton"]
	end
end

function MainUIAuditVersionSkillView:GetQingGongDown()
	if self.node_list["LandingButton"] then
		return self.node_list["LandingButton"]
	end
end

function MainUIAuditVersionSkillView:ShowQingGongGuideSkillEffect(is_show, is_qinggong_down)
