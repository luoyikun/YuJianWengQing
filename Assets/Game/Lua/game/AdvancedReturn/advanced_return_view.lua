AdvancedReturnView = AdvancedReturnView or BaseClass(BaseView)

function AdvancedReturnView:__init()
	self.ui_config = {
...
function AdvancedReturnView:__delete()

end

function AdvancedReturnView:ReleaseCallBack()
...
function AdvancedReturnView:CloseCallBack()
	if self.model then
		self.model:DeleteMe()
		self.model = nil
	end
...
function AdvancedReturnView:LoadCallBack()
...
function AdvancedReturnView:OpenCallBack()
...
function AdvancedReturnView:OnRoleDrag(data)
	if self.model then
		self.model:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function AdvancedReturnView:GoShoplevelDan()
	local act_item , act_grade = AdvancedReturnData.Instance:GetReturnShowItemCfg()
	local item_cfg = ItemData.Instance:GetItemConfig(act_item) 
	if item_cfg == nil then
		return
	end
	local price = AdvancedReturnData.Instance:GetReturnNeedGoldCfg()
	...
	local func = function ()
		if role_money >= price then
				if bags_grid_num >= 1 then
					AdvancedReturnCtrl.Instance:SendRandShopBuyReq(RA_OPEN_SERVER_ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_UPGRADE_RETURN,act_item)
				else
...
function AdvancedReturnView:OnClickClose()
	self:Close()
end
function AdvancedReturnView:FlushTextInfo()
	local upgrade_return_info = AdvancedReturnData.Instance:GetUpGradeReturnInfo()
	self.act_type = upgrade_return_info.act_type

	local info_grade, bubble, asset, res_id = AdvancedReturnData.Instance:GetImageResPath(self.act_type)
...
function AdvancedReturnView:SetMainAsset(bundle, asset ,res_id)
...
--移动坐骑，达到尾焰拖尾效果
function AdvancedReturnView:UpdateMountPosition()
...
function AdvancedReturnView:CancelMountMoveTimeQuest()
...
function AdvancedReturnView:OpenJinJieView()
	local index = 0
	if self.act_type == TYPE_UPGRADE_RETURN.MOUNT_UPGRADE_RETURN then
...
function AdvancedReturnView:GetNumberOfCells()
	local list = AdvancedReturnData.Instance:GetUpGradeReturnList()
	return #list or 0
end

function AdvancedReturnView:RefreshCell(cell, data_index)
	data_index = data_index + 1
	local reward_cell = self.cell_list[cell]
	if reward_cell == nil then
		reward_cell = ReturnRewardsItem.New(cell.gameObject)
		reward_cell.parent_view = self
		self.cell_list[cell] = reward_cell
	end
	local data_list = AdvancedReturnData.Instance:GetUpGradeReturnList()
	reward_cell:SetIndex(data_index)
	reward_cell:SetData(data_list[data_index])
end

function AdvancedReturnView:OnFlush()
	self:FlushTextInfo()
	local act_type = AdvancedReturnData.Instance:GetUpGradeReturnActType()
	self.node_list["Name"].text.text = Language.Activity.UpGradeReturn[act_type]
	self:SetReMainTime()
	local is_show = AdvancedReturnData.Instance:ActivetyFuanHuanIsShow()
	local max_grade = AdvancedReturnData.Instance:GetMaxLevelGrade()
	if is_show and max_grade > self.current_grade then
		...
		local act_itemid, act_grade = AdvancedReturnData.Instance:GetReturnShowItemCfg()
		if act_grade > 0 then
		...
		local act_info = AdvancedReturnData.Instance:GetUpGradeReturnInfo()
		...
		local need_gold = AdvancedReturnData.Instance:GetReturnNeedGoldCfg()
...
function AdvancedReturnView:SetReMainTime()
...
ReturnRewardsItem = ReturnRewardsItem or BaseClass(BaseCell)

function ReturnRewardsItem:__init()
...
function ReturnRewardsItem:__delete()
	if self.item then
		self.item:DeleteMe()
		self.item = nil
	end
...
function ReturnRewardsItem:OnGetReward()
	KaifuActivityCtrl.Instance:SendRandActivityOperaReq(RA_OPEN_SERVER_ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_UPGRADE_RETURN, RA_JINJIE_RETURN_OPERA_TYPE.RA_JINJIE_RETURN_OPERA_TYPE_FETCH, self.data.seq)
end

function ReturnRewardsItem:OnFlush()
	if self.data == nil and self.parent_view.act_type == nil then
		...
		self.node_list["ImgHasGet"]:SetActive(tonumber(self.data.fetch_reward_flag) == 1)
		self.node_list["RewardButton"]:SetActive(tonumber(self.data.fetch_reward_flag) ~= 1)
