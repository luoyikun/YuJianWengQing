...
function RuneAwakenTipsView:LoadCallBack()
	self.node_list["BGButton"].button:AddClickListener(BindTool.Bind(self.OnClickClose, self))
end
...
function RuneAwakenTipsView:OpenCallBack()
	...
	if 1 == self.is_property then
		self.node_list["TopImg"].image:LoadSprite(ResPath.GetRuneRes("awaken_icon_" .. self.awaken_type .. 2))
	end
	if 0 == self.is_property then
		self.node_list["TopImg"].image:LoadSprite(ResPath.GetRuneRes("awaken_icon_" .. self.awaken_type))
	end
...
function RuneAwakenTipsView:FlushContent(awaken_seq_index)
	...
	if 5 <= awaken_type then
		self.node_list["SliderProgress"].text.text = Language.Rune.AwakenReward
		local str = string.format("<color=#00ff00>%d</color>", self.awaken_type_info.add_value)	
		self.node_list["SliderText"].text.text = str
	-- 属性类型
	else
		-- 符文塔等级
	...
	self.node_list["TypeNameTxt"].text.text = self.awaken_type_info.awaken_name
	self.node_list["DescribeTxt"].text.text = self.awaken_type_info.awaken_dec
