...
CrossServerData.CS_TYPE_T = {
	...
	ACTIVITY_TYPE.CROSS_ACTIVITY_TYPE_FISHING,
}
...
function CrossServerData:SetIsReqDisconnecting(req_disconnecting)
	self.req_disconnecting = req_disconnecting
end

function CrossServerData:GetIsReqDisconnecting()
	return self.req_disconnecting
end
...
function CrossServerData:SetServerId(server_id)
	self.server_id = server_id
end

function CrossServerData:GetServerId()
...
function CrossServerData:CheckCanOpenInCross(view_name)
	if view_name == ViewName.Ranking
		or view_name == ViewName.MarryBaby
		or view_name == ViewName.LittlePetView
		or view_name == ViewName.CoupleHomeView
		or view_name == ViewName.BiaoBaiQiang
		or view_name == ViewName.Market
		-- or view_name == ViewName.Scoiety 
		...
		or view_name == ViewName.GuildWarehouseView
		or view_name == ViewName.GuildWageView then
	...
	-- 	--and view_name ~= ViewName.ChatGuild
	-- 	and view_name ~= ViewName.Map
	-- 	and view_name ~= ViewName.BuffPandectTips
	-- 	and view_name ~= ViewName.KuafuTaskRecordView
	-- 	and view_name ~= ViewName.BossSkillWarning
	-- 	and view_name ~= ViewName.TipsFocusBossView then
	-- 		return false, Language.Common.CantOpenInCross
	-- end
	-- return true
end