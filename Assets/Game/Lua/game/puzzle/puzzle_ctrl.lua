...
function PuzzleCtrl:__init()
	...
	RemindManager.Instance:Bind(self.remind_change, RemindName.PuzzleView)
end
...
function PuzzleCtrl:OnFastFilpResult(result)
	...
	if self.view and self.view:IsOpen() then
		if result == 0 then
			...
			self.delay_time = GlobalTimerQuest:AddDelayTimer(BindTool.Bind2(self.BeginFastFilp, self), 0.5)
		end
	end
end
...
function PuzzleCtrl:RemindChangeCallBack(remind_name, num)
	if remind_name == RemindName.PuzzleView then
		ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_FANFAN, num > 0)
	end
end

function PuzzleCtrl:ActivityChangeCallBack(activity_type, status)
	if activity_type == ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_FANFAN then
		-- 活动开启之后才请求
