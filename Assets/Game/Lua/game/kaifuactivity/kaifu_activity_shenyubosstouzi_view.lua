ShenyuBossTouZiView = ShenyuBossTouZiView or BaseClass(BaseRender)

function ShenyuBossTouZiView:__init()
	self.cell_list = {}
	local list_delegate = self.node_list["RewardList"].list_simple_delegate
	list_delegate.NumberOfCellsDel = BindTool.Bind(self.GetNumberOfCells, self)
	list_delegate.CellRefreshDel = BindTool.Bind(self.RefreshCell, self)	

	local price = KaifuActivityData.Instance:GetShenYuBossTouZiPrice()
	...
	if cur_day > -1 then
		UnityEngine.PlayerPrefs.SetInt("ShenYuBossTouZiView" .. real_role_id, cur_day)
...
function ShenyuBossTouZiView:OnInvestment()
	local func = function ()
		InvestCtrl.Instance:SendTouzijihuaFbBossOperate(TOUZIJIHUA_FB_BOSS_OPERATE_TYPE.TOUZIJIHUA_FB_BOSS_OPERATE_SHENYU_BOSS_BUY)
	end
...
function ShenyuBossTouZiView:__delete()
...
function ShenyuBossTouZiView:OnFlush()
	local hai_ke_fan_huan_txt, yi_fan_huan_txt, li_ji_ling_qu_txt = KaifuActivityData.Instance:GetShenYuBossTouZiAfterGold()
...
function ShenyuBossTouZiView:GetNumberOfCells()
	local data_list = KaifuActivityData.Instance:GetShenYuBossTouZiDataList()
	return #data_list
end

function ShenyuBossTouZiView:RefreshCell(cell, data_index)
	local data_list = KaifuActivityData.Instance:GetShenYuBossTouZiDataList()
	data_index = data_index + 1
	local reward_cell = self.cell_list[cell]
	if reward_cell == nil then
		reward_cell = ShenyuBossTouZiViewItem.New(cell.gameObject)
...
function ShenyuBossTouZiView:SetReMainTime()
	...
	if self.day_count_down == nil then
		function diff_time_func(elapse_time, total_time)
			...
			has_buy = InvestData.Instance:CheckIsActiveShenYuBossByID(1)
...
function ShenyuBossTouZiView:GetDifferTimeOpenSever()
...
------------------------------------------------------------------
ShenyuBossTouZiViewItem = ShenyuBossTouZiViewItem or BaseClass(BaseCell)

function ShenyuBossTouZiViewItem:__init()
...
function ShenyuBossTouZiViewItem:__delete()
	if self.item then
		self.item:DeleteMe()
		self.item = nil
	end
...
function ShenyuBossTouZiViewItem:OnGetReward()
	local now_kill_num = InvestData.Instance:GetShenYuBossKillNum()
	if now_kill_num >= self.data.kill_num and not InvestData.Instance:CheckIsActiveShenYuBossByID(1) then
		SysMsgCtrl.Instance:ErrorRemind(Language.Activity.TouZiLiJiLingQu)
		return
	end	
	InvestCtrl.Instance:SendTouzijihuaFbBossOperate(TOUZIJIHUA_FB_BOSS_OPERATE_TYPE.TOUZIJIHUA_FB_BOSS_OPERATE_SHENYU_BOSS_REWARD, self.data.index)
end

function ShenyuBossTouZiViewItem:OnQianWang()
	ViewManager.Instance:Close(ViewName.TouziActivityView)
	ViewManager.Instance:Open(ViewName.ShenYuBossView, TabIndex.kf_boss)
end

function ShenyuBossTouZiViewItem:OnFlush()
	...
	self.node_list["Txt_need_levle"].text.text = string.format(Language.Activity.ShenYuLeiJiJiSha, self.data.kill_num, color, now_kill_num, self.data.kill_num)
	self.node_list["Txt_1"]:SetActive(now_kill_num < self.data.kill_num)
	self.node_list["Effect"]:SetActive(InvestData.Instance:CheckIsActiveShenYuBossByID(1))
	UI:SetButtonEnabled(self.node_list["Btn_Get"], InvestData.Instance:CheckIsActiveShenYuBossByID(1))

	if InvestData.Instance:CheckIsFetchedShenYuBossByID(self.data.index + 1) then
