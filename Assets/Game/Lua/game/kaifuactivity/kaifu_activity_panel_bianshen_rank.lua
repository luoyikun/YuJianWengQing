...
function BianShenRank:__init()
	...
	local rest_time, next_time = ActivityData.Instance:GetActivityResidueTime(RA_OPEN_SERVER_ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_SPECIAL_APPEARANCE_RANK)
	...
	self.node_list["TxtBianShenCount"].text.text = KaifuActivityData.Instance:GetSpecialAppearanceRoleChangeTimes()

	self.item_cell = ItemCell.New()
	self.item_cell:SetInstanceParent(self.node_list["CellItemReward"])

	local item_cell_data = KaifuActivityData.Instance:GetSpecialAppearanceRankJoinRewardCfg()
	self.item_cell:SetData(item_cell_data)

	self.reward_list = KaifuActivityData.Instance:GetSpecialAppearanceRankCfg()

	self.node_list["Help"].button:AddClickListener(BindTool.Bind(self.OpenHelp,self))

	if self.node_list["Title"] and self.reward_list[1] and self.reward_list[1].title_change2 then
		local bundle, asset = ResPath.GetTitleIcon(self.reward_list[1].title_change2)
		self.node_list["Title"].image:LoadSprite(bundle, asset, function()
				self.node_list["Title"].image:SetNativeSize()
			end)
		TitleData.Instance:LoadTitleEff(self.node_list["Title"], self.reward_list[1].title_change2, true)
	end
end
...
function BianShenRank:OnFlush()
	...
	local rank_list = KaifuActivityData.Instance:GetSpecialAppearanceRankList()
	self.node_list["TxtBianShenCount"].text.text = KaifuActivityData.Instance:GetSpecialAppearanceRoleChangeTimes()
...
function BianShenRank:SetTime(rest_time)
...
function BianShenRank:GetNumberOfCells()
	return KaifuActivityData.Instance:GetSpecialAppearanceRankCount()
end

function BianShenRank:RefreshCell(cell, cell_index)
	local contain_cell = self.contain_cell_list[cell]

	if contain_cell == nil then
		contain_cell = BianShenRankCell.New(cell.gameObject, self)
...
function BianShenRank:OpenHelp()
	local tips_id = 315
...
function BianShenRankCell:SetItemData(data, index)
	self.reward_data = data or {}
	self.item_cell_index = index
	self:Flush()
end

function BianShenRankCell:SetRankData(data)
	self.rank_data = data
end

function BianShenRankCell:OnFlush()
	...
	self.node_list["TxtPlayername"].text.text = 0
	...
	if self.data.user_name ~= "" and self.data.change_num ~= 0 then
