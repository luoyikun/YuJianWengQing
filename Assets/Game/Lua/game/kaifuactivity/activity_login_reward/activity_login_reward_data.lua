...
function ActivityPanelLoginRewardData:__init()
	...
	self.login_reward_info_list = {
		[ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LOGIN_GIFT] = nil,
	...
	self.login_reward_cfg_list = {
		[ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LOGIN_GIFT] = ServerActivityData.Instance:GetCurrentRandActivityConfig().login_gift_0,
	...
	self.login_reward_cfg_list_count = {
		[ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LOGIN_GIFT] = GetListNum(self.login_reward_cfg_list[ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LOGIN_GIFT]),
...
function ActivityPanelLoginRewardData:SetLoginRewardInfo_0(protocol)
	...
	self.login_reward_info_list[ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LOGIN_GIFT] = {
...
function ActivityPanelLoginRewardData:GetShowListTab(act_id)
	local result_list = {}
	local cfg = self:GetLoginRewardCfg(act_id)
	local count = 1
	for k,v in pairs(cfg) do
		table.insert(result_list,TableCopy(v))
		result_list[#result_list].get_flag = self:GetRewardStatus(act_id, v.need_login_days)
		-- count = count + 1
	end
	self.login_reward_cfg_list_count[act_id] = #result_list
	table.sort(result_list, SortTools.KeyLowerSorters("get_flag","need_login_days"))
	return result_list
end
...
function ActivityPanelLoginRewardData:GetLoginGiftRemind0()
	return self:GetRemind(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LOGIN_GIFT)
end
