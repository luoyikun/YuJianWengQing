BaojidayView = BaojidayView or BaseClass(BaseRender)

--暴击日
function BaojidayView:LoadCallBack()
...
function BaojidayView:__init()
	self:FlushUI()
end

function BaojidayView:__delete()

end

function BaojidayView:OnFlush()
	self:FlushUI()
end

function BaojidayView:FlushUI()
	local act_type = KaifuActivityData.Instance:GetBaojiDayActType()
...
function BaojidayView:OpenFanhuan()
	local act_type = KaifuActivityData.Instance:GetBaojiDayActType()
	if act_type == 1 then
