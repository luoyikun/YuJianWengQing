DegreeRewardsView = DegreeRewardsView or BaseClass(BaseRender)

function DegreeRewardsView:__init()
	self.degree_data = {}
	self.model_list = {}
	self.cell_list = {}
	self.act_type = 1
...
function DegreeRewardsView:__delete()
...
function DegreeRewardsView:OpenCallBack()
...
function DegreeRewardsView:OnFlush()
	self:SetReMainTime()
	self:FlushRoleModelList()
	local list = KaifuActivityData.Instance:GetUpGradeReturnList()
	if #list > 0 then
		for i = 1, #list do
...
function DegreeRewardsView:FlushTextInfo()
	local upgrade_return_info = KaifuActivityData.Instance:GetUpGradeReturnInfo()
...
function DegreeRewardsView:OpenJinJieView()
	local index = 0
	if self.act_type == TYPE_UPGRADE_RETURN.MOUNT_UPGRADE_RETURN then
...
function DegreeRewardsView:SendRankListReq()
	if self.act_type == TYPE_UPGRADE_RETURN.MOUNT_UPGRADE_RETURN then
...
function DegreeRewardsView:FlushRoleModelList()
	local list = {}
...
function DegreeRewardsView:SetReMainTime()
...
function DegreeRewardsView:GetNumberOfCells()
	return #self.degree_data or 0
end

function DegreeRewardsView:RefreshCell(cell, data_index)
	data_index = data_index + 1
	local reward_cell = self.cell_list[cell]
	if reward_cell == nil then
		reward_cell = DegreeRewardsItem.New(cell.gameObject)
...
function DegreeRewardsView:OpenRankView()
	if self.act_type == TYPE_UPGRADE_RETURN.MOUNT_UPGRADE_RETURN then
...
------------------------------------------------------------------
DegreeRewardsItem = DegreeRewardsItem or BaseClass(BaseCell)

function DegreeRewardsItem:__init()
...
function DegreeRewardsItem:__delete()
	if self.item then
		self.item:DeleteMe()
		self.item = nil
	end
...
function DegreeRewardsItem:OnGetReward()
...
function DegreeRewardsItem:OnFlush()
