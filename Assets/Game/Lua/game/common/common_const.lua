...
COMMON_CONSTS =
{
	...
	XIN_SHOU_LEVEL = 120,							-- 新手保护等级
	CHAT_LEVEL_LIMIT = 78,							-- 聊天等级限制
	PRIVATE_CHAT_LEVEL_LIMIT = 78,					-- 私聊等级限制
	SHUIJING_LEVEL_LIMIT = 150,						-- 进入水晶幻境等级限制

	MAX_PRIVITE_OBJ_NUM = 50,						-- 最大私聊对象数量
	...
	GUILD_ICON_CHANGE_LV = 78,						--修改公会头像等级

	MAX_FALL_MSG_NUM = 200,							-- 最大珍稀掉落信息数量
	MAX_EQUIPMENT_GRID_NUM = 12,					-- 最大装备格子数量
	MAX_NEW_EQUIPMENT_GRID_NUM = 8,					-- 最大觉醒装备格子数量
	MAX_CAN_FORGE_EQUIP_NUM = 10,					-- 最大可锻造装备数量（0 - 10）
	E_INDEX_MAX = 9,								-- 转职装备数（0 - 9）
	BAIZHAN_E_INDEX_MAX = 9,						-- 百战装备数（0 - 9）
	MAX_STONE_COUNT = 6,							-- 每个装备中宝石的最大数量
	MAX_ZHUANZHI_STONE_SLOT = 6,					-- 每个转职装备中宝石的最大数量
	MAX_ZHUANZHI_FULING_ATTR_COUNT = 4, 			-- 转职附灵最大属性数
	EQUIP_BAPTIZE_ONE_PART_MAX_BAPTIZE_NUM  = 6, 	-- 一个部位最大的洗炼条数

	MAX_STONE_EQUIP_PART = 11,						-- 宝石的位置
	...
	DEFAULT_SPEED = 700,							--默认移动速度

	NPC_HUSONG_RECEIVE_ID = 410,					--护送接收任务npcId
	NPC_HUSONG_DONE_ID = 411,						--护送完成任务npcId
	NPC_CAMP_ID = 201,								--阵营npcId
	NPC_STORAGE_ID = 185,							--仓库npcId
	NPC_DRUGSTORE_ID = 186,							--药店npcId
	NPC_MARRY_ID = 302,								--月老npcId
	...
	VIRTUAL_ITEM_BINDGOL = 65533,					--虚拟物品 绑定元宝
	VIRTUAL_ITEM_JIFEN = 65532,						-- 虚拟物品 积分
	VIRTUAL_ITEM_MOJINT = 90002,					-- 魔晶
	VIRTUAL_ITEM_SPIRIT_JIFEN = 90000,				-- 虚拟物品 仙宠猎取积分
	VIRTUAL_ITEM_SPIRIT_SOUL = 90010,				-- 虚拟物品 仙宠命魂魂力
	VIRTUAL_ITEM_WAREHOUSE_SCORE = 90029,			-- 虚拟物品 仙盟仓库积分
	
	VIRTUAL_ITEM_HORNOR = 90071,					--虚拟物品 荣誉
	VIRTUAL_ITEM_GONGXIAN = 90009,					--虚拟物品 仙盟贡献
	...
	VIRTUAL_KF_ITEM_HORNOR = 90004,					--虚拟物品 跨服荣誉
	...
	FEIXIAN_MAX_XIANPIN_NUM = 11,					--飞仙装备传奇属性最大数量
	...
	MAGICAL_PRECIOUS_CHAPTER_COUNT = 16,			--魔龙秘宝章节数
	MAGICAL_PRECIOUS_CHAPTER_FLAG_COUNT = 5,		--魔龙秘宝章数
	MAGICAL_PRECIOUS_CHAPTER_REWARD_INDEX_COUNT = 16,-- 魔龙秘宝章数任务数

	GREATE_SOLDIER_SLOT_MAX_COUNT = 5,				-- 最多的将位槽数

	FOOTPRINT_CREATE_GAP_TIME = 0.3,				-- 足迹生成间隔时间
	SHUIBO_SHOW_DELAY_TIME = 0.3,					-- 波纹延迟显示时间

	OPEN_SPIRIT_EXPLOER = 50,                        -- 精灵探险开启等级
	OPEN_SPIRIT_PLUNDER = 50,						 -- 精灵掠夺开启等级
	...
	WATER_HEIGHT = 1.2,								-- 水深
	REMIND_LEVEL = 80,								-- 红点显示等级

	CHONGCI_MIN_DISTANCE = 3 * 3,						-- 冲刺最短距离
	CHONGCI_MAX_DISTANCE = 20 * 20,						-- 冲刺最长距离
	CHONGCI_SPEED = 50,									-- 冲刺速度

	WELFARE_FIND_ACTIVITY_MAX_NUM = 31,					-- 福利活动找回种类

	XIN_SHOU_CUN_SCENE_ID = 101,					-- 跨服中转站 新手村
	MAX_QING_GONG_COUNT = 4,							-- 轻功最大连跳次数

	SPRITE_SEARCH_TYPE = 602,						-- 精灵搜索类型
