JueXingFourView = JueXingFourView or BaseClass(BaseRender)

local JUE_XING_FOUR_BALL_NUM = 10
local ball_effect_scale = 0.7
local ball_red_pos = 34

function JueXingFourView:__init(instance)
	for i=1, JUE_XING_FOUR_BALL_NUM do
	...
	self.ball_index = 1
	self.zhuan_index = 7		-- 这个界面是7转也是2觉醒
	self.is_can_zhuanzhi = false

	self:InitView()
end

function JueXingFourView:__delete()
...
function JueXingFourView:LoadCallBack()

end

function JueXingFourView:InitView()
	local zhuanzhi_info = ZhuanZhiData.Instance:GetSingleZhuanZhiInfo(ZHUANZHI_TIME.ZHUANZHI_TIME_NINE)
...
function JueXingFourView:OnFlush()
	local zhuanzhi_info = ZhuanZhiData.Instance:GetSingleZhuanZhiInfo(ZHUANZHI_TIME.ZHUANZHI_TIME_NINE)
	...
	if zhuan == ZHUANZHI_TIME.ZHUANZHI_TIME_SEVEN - 1 
		and zhuanzhi_info < JUE_XING_FOUR_BALL_NUM
		...
		local ball_red_flag = ZhuanZhiData.Instance:GetBallRedPointRemind(ZHUANZHI_TIME.ZHUANZHI_TIME_NINE, zhuanzhi_info + 1)
	...
	self.node_list["StartTask"]:SetActive(btn_state and task_cfg ~= nil and zhuan < self.zhuan_index and zhuan <= ZHUANZHI_TIME.ZHUANZHI_TIME_SEVEN, btn_show)
...
function JueXingFourView:OnClickBall(index)
	TipsCtrl.Instance:ShowZhuanZhiTips(ZHUANZHI_TIME.ZHUANZHI_TIME_NINE, index, ZHUANZHI_TIME.ZHUANZHI_TIME_EIGHT)
end

function JueXingFourView:OnClickTaskBtn()
...
function JueXingFourView:OnClickGoFb()
	local task_cfg, zhuanzhi_task_status = TaskData.Instance:GetNowZhuanZhiTask()
	if nil == task_cfg then
		return
	end
...
function JueXingFourView:SetBallEffect(is_show, pos, scale)
	if self.effect_obj then
...
function JueXingFourView:SetBallRedPoint(is_show, pos)
...
function JueXingFourView:DoPanelTweenPlay()
