JueXingTwoView = JueXingTwoView or BaseClass(BaseRender)

local JUE_XING_TWO_BALL_NUM = 12
local ball_effect_scale = 0.7
local ball_red_pos = 32

function JueXingTwoView:__init(instance)
	for i=1, JUE_XING_TWO_BALL_NUM do
		self.node_list["cell_" .. i].button:AddClickListener(BindTool.Bind(self.OnClickBall, self, i))
	end
...
function JueXingTwoView:__delete()
...
function JueXingTwoView:LoadCallBack()

end

function JueXingTwoView:InitView()
	-- local zhuanzhi_info = ZhuanZhiData.Instance:GetSingleZhuanZhiInfo(ZHUANZHI_TIME.ZHUANZHI_TIME_SEVEN)
...
function JueXingTwoView:OnFlush()
	local zhuanzhi_info = ZhuanZhiData.Instance:GetSingleZhuanZhiInfo(ZHUANZHI_TIME.ZHUANZHI_TIME_SEVEN)
	...
	if zhuanzhi_info then
		for i=1,JUE_XING_TWO_BALL_NUM do
			UI:SetGraphicGrey(self.node_list["Item" .. i], zhuanzhi_info < i)
		end

		
		if zhuan == ZHUANZHI_TIME.ZHUANZHI_TIME_FIVE - 1
			and zhuanzhi_info < JUE_XING_TWO_BALL_NUM
			and zhuanzhi_task_status == TASK_STATUS.ACCEPT_PROCESS then
			
			local pos = self.node_list["cell_" .. zhuanzhi_info + 1].transform.anchoredPosition3D
			local scale = ball_effect_scale
			local red_pos = Vector3(pos.x + ball_red_pos, pos.y + ball_red_pos, 0)
			local ball_red_flag = ZhuanZhiData.Instance:GetBallRedPointRemind(ZHUANZHI_TIME.ZHUANZHI_TIME_SEVEN, zhuanzhi_info + 1)
	...
	self.node_list["StartTask"]:SetActive(btn_state and zhuan < self.zhuan_index and zhuan <= ZHUANZHI_TIME.ZHUANZHI_TIME_SEVEN and btn_show)
...
function JueXingTwoView:OnClickBall(index)
	TipsCtrl.Instance:ShowZhuanZhiTips(ZHUANZHI_TIME.ZHUANZHI_TIME_SEVEN, index, ZHUANZHI_TIME.ZHUANZHI_TIME_FIVE)
end

function JueXingTwoView:OnClickTaskBtn()
...
function JueXingTwoView:OnClickGoFb()
	local task_cfg, zhuanzhi_task_status = TaskData.Instance:GetNowZhuanZhiTask()
	if nil == task_cfg then
		return
	end
...
function JueXingTwoView:SetBallEffect(is_show, pos, scale)
	if self.effect_obj then
...
function JueXingTwoView:SetBallRedPoint(is_show, pos)
...
function JueXingTwoView:DoPanelTweenPlay()
	UITween.MoveShowPanel(self.node_list["BottumView"], Vector3(-115, -100, 0), TWEEN_TIME, DG.Tweening.Ease.InOutSine)
