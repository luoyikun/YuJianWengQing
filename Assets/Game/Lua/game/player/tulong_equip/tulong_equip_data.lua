...
function TulongEquipData:__init()
	...
	RemindManager.Instance:Register(RemindName.TulongEquipGroup, BindTool.Bind(self.GetTulongAllRemind, self))
end
...
function TulongEquipData:__delete()
	...
	RemindManager.Instance:UnRegister(RemindName.TulongEquipGroup)
...
function TulongEquipData:GetTulongEquipCfg(tab_index, index)
	if tab_index == 2 then
		if self.cs_level_cfg_list[index] then
			return self.cs_level_cfg_list[index]
		end
	else	
		if self.level_cfg_list[index] then
			return self.level_cfg_list[index]
		end
	end
end
...
function TulongEquipData:GetNextUpShishenAttr(index, level, key)
	local cur_level_cfg = self:GetChuanshiCfg(index, level)
	local cur_value = nil ~= cur_level_cfg and cur_level_cfg[key] or 0
	for i = 1, 10 do
		local level_cfg = self:GetChuanshiCfg(index, level + i)
...
function TulongEquipData:GetTulongAllRemind()
	if 1 == self:GetShenEquipRemind() or 1 == self:GetCSShenEquipRemind() then
		return 1
	end
	return 0
end
...
function TulongEquipData:SetCombineServerFlag(is_active_flag)
	self.combine_server_flag = is_active_flag
end

function TulongEquipData:GetCombineServerFlag()
	return self.combine_server_flag or 0
end
