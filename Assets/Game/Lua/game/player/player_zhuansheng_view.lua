...
function PlayerZhuanShengView:__init(instance, parent_view)
	...
	for i = 1, 8 do
		...
		self.item_list[i].button:AddClickListener(BindTool.Bind(self.OnClickEquipItem, self, i))
	end
...
function PlayerZhuanShengView:AddListenEvent()
	...
	self.node_list["BtnKill"].button:AddClickListener(BindTool.Bind(self.OnClickGoKill, self))
end
...
function PlayerZhuanShengView:FlushItemList(zhuansheng_info,zhuansheng_cfg)
	...
	for i = 1, 8 do
		...
		if data.item_id <= 0 then
			...
			if item_cfg then
				self.item_list[i].image:LoadSprite(ResPath.GetItemIcon(item_cfg.icon_id))
			end
