...
MojieData.Attr = {"gong_ji", "fang_yu", "max_hp", "ming_zhong", "shan_bi",  "bao_ji", "jian_ren", "per_baoji", "per_pofang", "per_baoji_hurt", "pvp_jianshang", "per_mianshang"}
MojieData.MOJIE_EVENT = "mojie_event"	--魔戒信息变化
MojieData.ITEM_ID_T = {[0] = 26700, 26701, 26702, 26703}
MojieData.SKILL_T = {70}
function MojieData:__init()
	...
	self.model_gift_id = -1
	self.model_gift_bag_index = -1
	self.ring_view_index = 1
	self:IntiMojieInfo()
...
function MojieData:SetMojieGiftNum(num)
	self.mojie_gift_num = num
end

function MojieData:GetMojieGiftNum()
	return self.mojie_gift_num
end
...
function MojieData:SetRingIndex(index)
	self.ring_view_index = index
end

function MojieData:GetRingIndex()
	return self.ring_view_index
end


-----------模型自选礼包
function MojieData:SetModelGiftBagIndex(bag_index)
	self.model_gift_bag_index = bag_index
end

function MojieData:GetModelGiftBagIndex()
	return self.model_gift_bag_index
end

function MojieData:SetModelGiftId(item_id)
	self.model_gift_id = item_id
end

function MojieData:GetModelGiftId()
	return self.model_gift_id
end

function MojieData:SetModelGiftData(data)
	self.model_gift_data = data
end

function MojieData:GetModelGiftData(data)
	return self.model_gift_data
end

function MojieData:SetFromView(from_view)
	self.from_view = from_view
end

function MojieData:GetFromView()
