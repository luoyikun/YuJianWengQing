...
function FuBenInFoView:LoadCallBack()
	...
	self.phase_info_content = self.node_list["PhaseInfoContent"]
	self.phase_info_view = FuBenInfoPhaseView.New(self.phase_info_content)

	self.exp_info_content = self.node_list["ExpInfoContent2"]
	...
	self.tower_info_content = self.node_list["TowerInfoContent"]
	self.tower_info_view = FuBenInfoTowerView.New(self.tower_info_content)

	self.vip_info_content = self.node_list["VipInfoContent"]
	self.vip_info_view = FuBenInfoVipView.New(self.vip_info_content)

	self.leave_fb_content = self.node_list["LeaveFBContent"]
