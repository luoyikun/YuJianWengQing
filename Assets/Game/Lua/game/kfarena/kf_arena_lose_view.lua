KFArenaLoseView = KFArenaLoseView or BaseClass(BaseView)

function KFArenaLoseView:__init(instance)
...
function KFArenaLoseView:__delete()

end

function KFArenaLoseView:LoadCallBack()
	self.node_list["BtnConfirm"].button:AddClickListener(BindTool.Bind(self.OnClick, self))
end

function KFArenaLoseView:ReleaseCallBack()
...
function KFArenaLoseView:OpenCallBack()
	self:Flush()
end

function KFArenaLoseView:OnFlush()
...
function KFArenaLoseView:OnClick()
	self:Close()
