...
function HotStringChatData:__init()
	...
	self.choose_answer = -1
	self.is_start_gather = false
	self.server_id = 0
	self.target_id = 0
	...
	self.gather_cfg = ConfigManager.Instance:GetAutoConfig("hotspring_auto").gather_cfg or {}
	self.piting_list = {}
	self.question_info = {
...
function HotStringChatData:GetHotSpringGatherCfg()
...
function HotStringChatData:SetPlayerInfo(info)
	...
	self.start_fresh_gather_time = info.start_fresh_gather_time
end

-- 获得刷新时间
function HotStringChatData:GetFlushGatherTime()
	return self.start_fresh_gather_time
end

-- 获得已经使用的采集次数
function HotStringChatData:GetUseGatherTime()
	return self.gather_times or 0
end

function HotStringChatData:GetRemainGatherTime()
	local time = HotStringChatData.Instance:GetMaxGatherTime() - HotStringChatData.Instance:GetUseGatherTime()
	return time
end
...
function HotStringChatData:GetShuangXiuJingYan()
	return math.abs(self.add_addition)
end
...
function HotStringChatData:SetChooseAnswer(answer)
	self.choose_answer = answer
end

function HotStringChatData:GetChooseAnswer()
	return self.choose_answer
end

function HotStringChatData:SetIsStartGather(enable)
	self.is_start_gather = enable
end

function HotStringChatData:GetIsStartGather()
	return self.is_start_gather
end
...
-- 经验值加成
function HotStringChatData:GetInteractionAdd()
	return self.other_cfg[1].interaction_add or 0
end

-- 获得最大采集次数
function HotStringChatData:GetMaxGatherTime()
	return self.other_cfg[1].max_gather_times or 0
end
