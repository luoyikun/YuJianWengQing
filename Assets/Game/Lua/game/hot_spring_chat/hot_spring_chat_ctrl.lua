...
function HotStringChatCtrl:ActivityChangeCallback(activity_type, status, next_time, open_type)
	if activity_type == ACTIVITY_TYPE.KF_HOT_SPRING then
		if self.info_view then
			self.info_view:FlushSkillActive()
		end
	end
end
...
function HotStringChatCtrl:OnSCHotspringPlayerInfo(protocol)
	...
	self.info_view:Flush("time_info")
end
...
function HotStringChatCtrl:OnChangeRepair(state)
	self.info_view:OnChangeRepair(state)
end
...
function HotStringChatCtrl:OnSCHSSendPartnerInfo(protocol)
	...
	if protocol.partner_id ~= 0 then
		...
		HotStringChatData.Instance:SetIsStartGather(false)
...
function HotStringChatCtrl:SetSelcetAnswer(index)
	if self.question_view:IsOpen() then
		self.question_view:ShowSelectAnswer(index)
	end
end

function HotStringChatCtrl:SetToggleIsOn(index, is_on)
	if self.info_view then
		self.info_view:SetToggleIsOn(index, is_on)
	end
end
