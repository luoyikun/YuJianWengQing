...
function LittlePetData:__init()
	...
	self.feed_stuff_cfg = ListToMap(feed_cfg,"quality_type", "feed_level")
	...
	self.cur_scene_show_baby_id = 0                  -- 当前场景显示的宠物id
...
function LittlePetData:OnSCLittlePetNotifyInfo(protocol)
	if protocol.param_type == LITTLE_PET_NOTIFY_INFO_TYPE.LITTLE_PET_NOTIFY_INFO_SCORE and self.all_info_list then
		local old_score = self.all_info_list.score or 0
		self.all_info_list.score = protocol.param1              --积分信息
		local delta_score = self.all_info_list.score - old_score
		if delta_score and delta_score > 0 then
			TipsFloatingManager.Instance:ShowFloatingTips(string.format(Language.Marriage.GetJiFenTip, delta_score))
		end
...
--得到背包中宠物喂养材料列表
function LittlePetData:GetBagLittlePetFeedDataList()
	local item_list = ItemData.Instance:GetItemListByBigType(GameEnum.ITEM_BIGTYPE_OTHER)
	local feed_list = {}

	for _, v in pairs(item_list) do
		if self:IsFeedItem(v.item_id) then
			table.insert(feed_list, v)
		end
	end
...
function LittlePetData:IsFeedItem(item_id)
	if item_id >= 27380 and item_id <= 27383 then
		return true
	else
		return false
	end
...
function LittlePetData:GetBagLittlePetDataList()
	...
	for _, v in pairs(bag_item_data_list) do
		...
		if nil ~= item_cfg then
			if GameEnum.ITEM_BIGTYPE_EQUIPMENT == big_type and item_cfg.sub_type == GameEnum.USE_TYPE_LITTLE_PET then
...
function LittlePetData:GetBagLittlePetRecycleDataList()
	local bag_item_data_list = ItemData.Instance:GetBagItemDataList()
	local pet_list = {}
	...
	for _, v in pairs(bag_item_data_list) do
		local item_cfg, big_type = ItemData.Instance:GetItemConfig(v.item_id)
		if nil ~= item_cfg then
			if self:IsFeedItem(v.item_id) or GameEnum.ITEM_BIGTYPE_EQUIPMENT == big_type
				and (item_cfg.sub_type == GameEnum.USE_TYPE_LITTLE_PET or EquipData.IsLittlePetToyType(item_cfg.sub_type)) then
				table.insert(pet_list, v)
			end
		end
	end
...
function LittlePetData:GetLittlePetRecycleData()
	...
	local pet_list = self:GetBagLittlePetRecycleDataList()
	-- local feed_list = self:GetBagLittlePetFeedDataList()
	-- local sort_pet_toy_list = self:GetBagBestLittlePet(pet_toy_list)
	local sort_pet_list = self:GetBagBestLittlePet(pet_list)
	-- local sort_feed_list = self:GetSortFeedList(feed_list)
	-- for i,v in ipairs(sort_pet_toy_list) do
	--  table.insert(sort_pet_list, v)
	-- end
	-- for i,v in ipairs(sort_feed_list) do
	--  table.insert(sort_pet_list, v)
	-- end
	return sort_pet_list
end

function LittlePetData:ClearRecycleConditon()
	self.recycle_type_color_list = {}
end

function LittlePetData:GetLittlePetEquipMaxColorByType(sub_type)
	if self.recycle_type_color_list[sub_type] == nil then
		self.recycle_type_color_list[sub_type] = 1
	end
	local list = self:GetLittlePetRecycleData()
	for k,v in pairs(list) do
		local item_cfg, big_type = ItemData.Instance:GetItemConfig(v.item_id)

		if item_cfg and item_cfg.sub_type == sub_type then
			if self.recycle_type_color_list[sub_type] < item_cfg.color then
				self.recycle_type_color_list[sub_type] = item_cfg.color
			end
		end
	end
	return self.recycle_type_color_list[sub_type]
end

function LittlePetData:GetSortFeedList(data_list)
	if data_list == nil or nil == next(data_list) then
		return {}
	end
	...
	table.sort(data_list, function (a, b)
		...
		if item_cfg_a and item_cfg_b then
...
-- 获取仓库数据
function LittlePetData:GetWarehouseList(data_list)
	if nil == next(data_list) then
		return {}
	end

	for k, v in pairs(data_list) do
		v.power = self:CalPetBaseFightPower(false, v.item_id) or 0
	end
	table.sort(data_list, SortTools.KeyUpperSorter("power"))
	return data_list
end
...
function LittlePetData:GetLittlePetItemQualityTypeByItemID(item_id)
	local little_pet_cfg = self:GetLittlePetCfg()
	for k,v in pairs(little_pet_cfg) do
		if v.active_item_id == item_id then
			return v.quality_type or 0
		end
	end
	return 0
end
...
function LittlePetData:CalPetFeedFightPower(quality_type, level)
	local attr_cfg = self:GetFeedAttrCfgByLevel(quality_type, level)
...
function LittlePetData:GetEquipPetCfgDataList(lover_flag)
	...
	for k,v in pairs(pet_list) do
		...
		local item_cfg = ItemData.Instance:GetItemConfig(pet_cfg[v.id].active_item_id)
		local temp_list = {}
		...
		if item_cfg then
			temp_list.color = item_cfg.color or 0
		end
...
-- 宠物回收红点
function LittlePetData:CheckHomeRecycleIsShowRedPoint()
	local bag_pet_list = self:GetBagLittlePetDataList()
	local my_equip_list = self:GetHomeEquipPetDataList(false)
	...
	-- 当前装备小宠物不满时，不提醒
	if #my_equip_list < PERSON_CULTIVATE_MAX_NUM then
		return false
	end

	-- 当前装备满小宠物，背包有比装备的最低品质宠物 品质更差的宠物，红点提醒
	if #my_equip_list == PERSON_CULTIVATE_MAX_NUM then
		local my_equip_power_list = {}
		for k,v in pairs(my_equip_list) do
			local data = v
			local base_power = LittlePetData.Instance:CalPetBaseFightPower(true, data.id)
			table.insert(my_equip_power_list, base_power)
		end
		local min = -1
		for k1, v1 in pairs (my_equip_power_list) do 
			-- local min = -1
			if min < 0 or min > v1 then
				min = v1
			end
		end
		for k2,v2 in pairs(bag_pet_list) do 
			local bag_item_power = self:CalPetBaseFightPower(false, v2.item_id)
			if min > bag_item_power then
				return true
			end
		end
	end
...
function LittlePetData:GetLittlePetHomeRemind()
	...
	-- if self:CheckHomeRecycleIsShowRedPoint() then
	--  return 1
	-- end
	return 0
end
...
function LittlePetData:CheckIsHigherPowerPet(data, small_power)
	if data == nil then
		return
	end
	local select_power = self:CalPetBaseFightPower(false, data.item_id) or 0
	local item_quality_type = self:GetLittlePetItemQualityTypeByItemID(data.item_id)

	if data.param then
		local select_add_power = self:CalPetFeedFightPower(item_quality_type, data.param.param1)
		if select_add_power then
			select_power = select_power + select_add_power
		end
	end

	return select_power > small_power
end

function LittlePetData:HomeEquipPowerPet()
	local small_power = 0
	local my_equip_list = self:GetHomeEquipPetDataList(false)
	for i = 1, PERSON_CULTIVATE_MAX_NUM do
		if my_equip_list[i] == nil then
			...
			local quality_type = self:GetLittlePetItemQualityTypeByItemID(my_equip_list[i].item_id)
			local equip_add_power = self:CalPetFeedFightPower(quality_type, my_equip_list[i].feed_level)
			if equip_add_power then
				equip_power = equip_power + equip_add_power
			end

			if small_power < equip_power then
				small_power = equip_power
			end
		end
	end
...
function LittlePetData:CheckIsHigherPetEquip(data, item_cfg)
	if data == nil or item_cfg == nil then 
		return false
	end

	return self:CheckIsHigherPowerToy(data.item_id, item_cfg)
end
...
--根据喂养等级获取宠物喂养配置
function LittlePetData:GetFeedLevelCfg(quality_type, feed_level)
	if nil == feed_level then 
		return
	end
	return self.feed_stuff_cfg[quality_type][feed_level]
end

--根据喂养等级获取喂养材料
function LittlePetData:GetGridUpgradeStuffDataListByLevel(quality_type, feed_level)
	local stuff_list = {}
	local cfg = self:GetFeedLevelCfg(quality_type, feed_level)
	if cfg == nil then
		return stuff_list
	end

	if self.feed_stuff_cfg then
		local num = #self.feed_stuff_cfg
		for i = 0, num do
			local data = {}
...
function LittlePetData:GetFeedAttrCfgByIndex(index, is_lover, item_id)
	...
	local quality_type = self:GetLittlePetItemQualityTypeByItemID(item_id)
	local feed_cfg = self:GetFeedLevelCfg(quality_type, feed_level)
...
function LittlePetData:GetFeedAttrCfgByLevel(quality_type, feed_level)
	local feed_cfg = {}
	feed_cfg = self:GetFeedLevelCfg(quality_type, feed_level)
	return feed_cfg
end

function LittlePetData:GetMaxFeedLevel(quality_type)
	local max_feed_level = 0
	if self.feed_stuff_cfg[quality_type] ~= nil and next(self.feed_stuff_cfg[quality_type]) then 
		max_feed_level = self.feed_stuff_cfg[quality_type][#self.feed_stuff_cfg[quality_type]].feed_level 
	end
	return max_feed_level
end

--根据等级判断是否可以喂养宠物
function LittlePetData:CanFeedPetByFeedLevel(quality_type, feed_level, id)
	local feed_flag = 1
	local max_feed_level = self:GetMaxFeedLevel(quality_type)
	local cfg = self:GetLittlePetCfg()
	if feed_level < max_feed_level then
		local stuff_list = self:GetGridUpgradeStuffDataListByLevel(quality_type, feed_level)
		if stuff_list ~= nil and next(stuff_list) ~= nil then
			if cfg[id] and stuff_list[cfg[id].quality_type] then
				local data = stuff_list[cfg[id].quality_type]
...
function LittlePetData:GetLittlePetFeedRemind()
	...
	for k,v in pairs(equip_pet_list) do
		if v.feed_level then
			local item_id = self:GetLittlePetItemIDByID(v.id)
			local quality_type = self:GetLittlePetItemQualityTypeByItemID(item_id)
			local item_cfg, _ = ItemData.Instance:GetItemConfig(item_id)
			if item_cfg then
				feed_flag = self:CanFeedPetByFeedLevel(quality_type, v.feed_level, v.id)
			end
...
--得到背包中宠物玩具列表(根据玩具部位1~4, 上限颜色)
function LittlePetData:GetBagLittlePetToyDataListByToyPart(toy_part, max_color)
	local equip_list = ItemData.Instance:GetItemListByBigType(GameEnum.ITEM_BIGTYPE_EQUIPMENT)
	local pet_toy_list = {}
	for _, v in pairs(equip_list) do
		local item_cfg, big_type = ItemData.Instance:GetItemConfig(v.item_id)
		if nil ~= item_cfg and GameEnum.ITEM_BIGTYPE_EQUIPMENT == big_type
			and item_cfg.sub_type == LITTLE_PET_TOY_PART_ITEM_TYPE[toy_part]
			and item_cfg.color <= max_color then
			table.insert(pet_toy_list, v)
		end
	end
...
--单个宠物玩具部位总战力
function LittlePetData:GetSinglePetToyPartPower(pet_index, pet_info_type, toy_part_index, flag, ignore)
	local power = 0
	...
	if not ignore then
		local equip_pet_cfg_list = self:GetAllEquipPetCfgDataList()
		local equip_pet_cfg = nil
		if equip_pet_cfg_list then
			for k,v in pairs(equip_pet_cfg_list) do
				if v.index == pet_index then
					equip_pet_cfg = v
				end
			end
		end
		local item_cfg, _ = ItemData.Instance:GetItemConfig(equip_part_list.equipment_id)
		if equip_pet_cfg and item_cfg then
			if item_cfg.color > equip_pet_cfg.color then
				power = 0
			end
		end
	end
...
function LittlePetData:SinglePetIsCanEquipToy(part_num, list)
	...
	local list = self:GetBagLittlePetToyDataListByToyPart(part_num, pet_equip_info.color)
...
function LittlePetData:IsHaveFreeTimesByInfo()
	...
	if self.all_info_list and self.all_info_list.last_free_chou_timestamp and other_cfg[1] then
		...
		is_free = (free_time + free_jian_ge * 3600) - sever_time
	end
	return is_free
end

--抽奖按钮一次红点
function LittlePetData:ErnieRedPointOne()
	local flag = false
	if self:IsHaveFreeTimesByInfo() <= 0 then
		flag = true
	end
...
--抽奖按钮十次红点
function LittlePetData:ErnieRedPointTen()
	local flag = false
...
function LittlePetData:GetReplaceOneID()
	local replacement_id = 0
	local other_cfg = self:GetOtherCfg()
	if nil == other_cfg or nil == next(other_cfg) then
		return replacement_id
	end
	return other_cfg[1].replacement_id or 0
end

function LittlePetData:GetReplaceTenID()
	local open_box_30_use_itemid = 0
	local other_cfg = self:GetOtherCfg()
	if nil == other_cfg or nil == next(other_cfg) then
		return open_box_30_use_itemid
	end
	return other_cfg[1].open_box_30_use_itemid or 0
end
...
function LittlePetData:GetLittlePetShopRemind()
	if not OpenFunData.Instance:CheckIsHide("petview") then
		return 0
	end
	
	if self:ErnieRedPointOne() then
		return 1
	end

	if self:ErnieRedPointTen() then
		return 1
	end
	--宠物仓库红点
...
function LittlePetData:SetLittlePetWarehouseList(list)
	if #list > 1 then
		list = self:GetWarehouseList(list)
	end
...
function LittlePetData:GetLittlePetRewardList(list)
	...
	for k,v in pairs(list) do
		...
		local is_pet_item = self:IsLittlePetItem(item_id, true)
		if is_pet_item then
...
function LittlePetData:IsLittlePetItem(item_id, flag)
	...
	--宠物
	if GameEnum.ITEM_BIGTYPE_EQUIPMENT == big_type and cfg.sub_type == GameEnum.USE_TYPE_LITTLE_PET then
		is_pet_item = true
	end
...
--场景中显示的宝宝id
function LittlePetData:SetSceneShowBabyId(id)
	self.cur_scene_show_baby_id = id or 0
end

function LittlePetData:GetSceneShowBabyid()
	return self.cur_scene_show_baby_id
end
