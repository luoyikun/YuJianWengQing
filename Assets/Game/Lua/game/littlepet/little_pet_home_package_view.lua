...
function LittlePetHomePackageView:__init()
    self.ui_config = {
       ...
       {"uis/views/littlepetview_prefab","ShowHomePackagePetView"}
    }
    self.is_modal = true    
...
function LittlePetHomePackageView:LoadCallBack()
    ...
    self.node_list["Bg"].rect.sizeDelta = Vector3(574,590,0)
...
function LittlePetHomePackageView:PackageRefreshCell(cell, data_index)
    ...
    for i = 1, PACKAGE_ROW do
        ...
        local bag_index = self.data_list[index + 1] and self.data_list[index + 1].index or nil
        group:ListenClick(i, BindTool.Bind(self.HandlePackageOnClick, self, data, group, i, index, bag_index))
...
function LittlePetHomePackageView:HandlePackageOnClick(data, group, group_index, data_index, bag_index)
    ...
    local bag_index = bag_index or ItemData.Instance:GetItemIndex(item_id)
    if lower_flag then
