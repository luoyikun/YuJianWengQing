...
function AchieveView:__init()
	--Toggle
	self.title_toggle = self.node_list["TitleToggle"].toggle
	self.overview_toggle = self.node_list["OverviewToggle"].toggle
	...
	local title_content = self.node_list["TitleView"]
	title_content.uiprefab_loader:Wait(function(obj)
		obj = U3DObject(obj)
		self.title_view = AchieveTitleView.New(obj)
	end)

	local overview_content = self.node_list["OverviewView"]
	...
	--红点
	self.red_point_list = {
		[RemindName.Achieve_Overview] =self.node_list["ImgRedPoint1"],
		[RemindName.Achieve_Title] = self.node_list["ImgRedPoint2"],
	}
...
function AchieveView:RemindChangeCallBack(key, value)
	if self.red_point_list[key] then
		self.red_point_list[key]:SetActive(value > 0)
	end
end

function AchieveView:FlushRedPint()
	for k,v in pairs(self.red_point_list) do
		self.red_point_list[k]:SetActive(RemindManager.Instance:GetRemind(k))
	end
end
