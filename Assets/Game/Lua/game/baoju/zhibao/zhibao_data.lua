...
function ZhiBaoData:__init()
	...
	self.zhibao_huanhua_stuff_table = ListToMap(self.zhibao_huanhua, "stuff_id")
	self.zhibao_huanhua_type_grade_table = ListToMap(self.zhibao_huanhua, "huanhua_type", "grade")
	...
	RemindManager.Instance:Register(RemindName.Baoju, BindTool.Bind(self.GetAllBaojuRemind, self))
...
function ZhiBaoData:__delete()
	...
	RemindManager.Instance:UnRegister(RemindName.Baoju)
...
function ZhiBaoData:SyncZhiBaoInfo(protocol)
	...
	if self.zhibao_level == 300 then
		TipsCtrl.Instance:ShowSystemMsg(Language.BaoJu.ZhiBaoExpYiMan)
	end
end
...
function ZhiBaoData:GetExpRatio()
	local ratio = 1
	local activedegree_cfg = ConfigManager.Instance:GetAutoConfig("activedegree_auto")
	local exp_factor = activedegree_cfg.other[1].exp_factor or 1
	local level = GameVoManager.Instance:GetMainRoleVo().level
	local ratio = PlayerData.Instance:GetFBExpByLevel(level)
	if ratio then
		ratio = ratio * exp_factor
	end
	-- for _, v in ipairs(self.ratio_cfg) do
	-- 	if level < v.min_level then
	-- 		break
	-- 	end
	-- 	ratio = v.exp_ratio * exp_factor
	-- end
	return ratio
end
...
-- 
function ZhiBaoData:GetAllBaojuRemind()
	local zhibao_upgrade = self:GetZhiBaoUpgradeRemind()
	if zhibao_upgrade == 1 then
		return 1
	end
	local zhibao_active = self:GetZhiBaoActiveRemind()
	if zhibao_active == 1 then
		return 1
	end
	-- local medal_remind = MedalData.Instance:GetMedalRemind()
	local jingjie_remind = JingJieData.Instance:GetjingjieRemind()
	if jingjie_remind == 1 then
		return 1
	end
...
function ZhiBaoData:CheckZhiBaoCanUpgrade()
	local flag = false
	--检测功能是否开启
	if not OpenFunData.Instance:CheckIsHide("baoju_zhibao_upgrade") then
		return flag
	end
...
function ZhiBaoData:HuanHuaRedPoint()
	...
	--检测功能是否开启
	if not OpenFunData.Instance:CheckIsHide("baoju") then
		self.red_point_list["HuanHua"] = false
		return
	end
...
function ZhiBaoData:GetHuanHuaLevelCfg(huanhua_type, is_next, level)
	...
	local c = self.zhibao_huanhua_type_grade_table[huanhua_type]
	if c then
		return c[level]
	else
		return nil
...
function ZhiBaoData:GetRewardFetchFlag(index)
	...
	local bit_list = bit:ll2bn(self.active_degree_info.activedegree_fetch_flag)
...
function ZhiBaoData:GetActiveRewardInfo()
	local final_data = {}
	--第一个是寻宝用的，不读
	if self.active_degree_info == nil or self.active_degree_info.reward_flags == nil then return final_data end
...
function ZhiBaoData:FindZhiBaoHuanHuaByStuffID(stuff_id)
	local v = self.zhibao_huanhua_stuff_table[stuff_id]
	if v then
		return self:GetHuanHuaLevelCfg(v.huanhua_type, false, 1)
	else
		return nil
	end
end
...
function ZhiBaoData:GetDailyActiveRewardInfo()
	...
	active_reward_info.reward_state = reward_state
...
function ZhiBaoData:IsShowActiveRewardRedPoint()
	...
	if has_reach >= 1 then
		for i = 1, has_reach do
			if active_reward_info.reward_on_day_flag_list[i] == 0 then
				return true
			end
		end
	end
