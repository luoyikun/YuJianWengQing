...
function PlayPawnView:ReleaseCallBack()
	self.node_list["image_pawn_anim"] = nil
...
function PlayPawnView:LoadCallBack()
	-- 查找组件
 	self.img_animator = self.node_list["image_pawn_anim"].animator
...
function PlayPawnView:OpenCallBack()
 	self.isflag = true
	self.node_list["image_pawn_anim"]:SetActive(true)
...
function PlayPawnView:OnFlush()
	...
	if curr_score > 0 then
		...
		self.node_list["image_pawn_anim"]:SetActive(false)
