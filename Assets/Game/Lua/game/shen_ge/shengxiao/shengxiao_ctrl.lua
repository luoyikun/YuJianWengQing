...
function ShengXiaoCtrl:__init()
	...
	self.shengxiao_suit_view = ShengXiaoSuitView.New(ViewName.ShengXiaoSuitView)
	self.shengxiao_equip_bag = ShengXiaoEquipBag.New(ViewName.ShengXiaoEquipBag)
	...
	self.resolve_view = ShengXiaoResolveView.New(ViewName.ShengXiaoResolveView)
...
function ShengXiaoCtrl:__delete()
	...
	if self.shengxiao_suit_view then
		self.shengxiao_suit_view:DeleteMe()
		self.shengxiao_suit_view = nil
	end
...
function ShengXiaoCtrl:SetShengXiaoEquipIndex(index)
	self.shengxiao_equip_bag:SetSlotIndex(index)
end
...
function ShengXiaoCtrl:SetSuitOpenCallBack(index)
	self.shengxiao_suit_view:SetOpenCallBack(index)
	self.shengxiao_suit_view:Open()
end
...
function ShengXiaoCtrl:SendZodiacRecycleEquip(count, index_list)
	local send_protocol = ProtocolPool.Instance:GetProtocol(CSChineseZodiacRecycle)
	send_protocol.count = count or 0
	send_protocol.index_list = index_list or 0
	send_protocol:EncodeAndSend()
end

function ShengXiaoCtrl:SendZodiacTakeOffEquipRequest(zodiac_type, equip_type)
	local send_protocol = ProtocolPool.Instance:GetProtocol(CSChineseZodiacTakeOffEquip)
	send_protocol.zodiac_type = zodiac_type or 0
	send_protocol.equip_type = equip_type or 0
	send_protocol:EncodeAndSend()
end
