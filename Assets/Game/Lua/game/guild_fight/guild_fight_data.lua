...
function GuildFightData:GetRankList()
	return self.global_info.rank_list
end
function GuildFightData:GetGuildFightRankList()
	local data_list = {}
	local list_cfg = GuildFightData.Instance:GetRankList()
...
function GuildFightData:GetMyRank(guild_name)
...
function GuildFightData:GetLuckRewardItem()
	if self.config then
		local other_cfg = self.config.other[1]
		if other_cfg then
			return other_cfg.lucky_item
		end
	end
end
	...
	if index > 0 then
		...
		if guild_name ~= "" then
			if guild_id == info.guild_id then
				guild_name = ToColorStr(info.guild_name, TEXT_COLOR.BLUE)
			else
...
function GuildFightData:GetMaxGongHui()
	local data_list  = self:GetConfig().personal_credit_reward
	for k,v in pairs(data_list) do
		if #data_list == v.reward_index then
			return v.reward_credit_min
		end
	end
end