	...
	if sp_type == GuaJiTaData.SP_TYPE.TYPE then
		...
		for i = 1, 1 do
			...
			if item_id then
				...
				self.one_img.image:LoadSprite(bundle, asset)
		...
		if sp_type == GuaJiTaData.SP_TYPE.SLOT then
			...
			self.one_img.image:LoadSprite("uis/views/guajitaview/images_atlas", "img_uplevel")
...
function GuajiTaFbJieSuoView:CalTime()
	...
	self.gj_time_quest = GlobalTimerQuest:AddRunQuest(function()
		...
		if timer_cal <= 0 then
			...
			UITween.MoveToShowPanel(self.node_list["ImgOne"], Vector3(0, 0, 0), Vector3(x, y, 0), 2, nil, function()
