TipSpecialSkillView = TipSpecialSkillView or BaseClass(BaseView)

local ZHUDONG_TYPE = 70

function TipSpecialSkillView:__init()
	self.ui_config = {{"uis/views/tips/specialskilltip_prefab", "SpecialSkillView"}}
...
-- 创建完调用
function TipSpecialSkillView:LoadCallBack()
	self.node_list["BtnCloseButton"].button:AddClickListener(BindTool.Bind(self.OnClickCloseButton, self))
end

function TipSpecialSkillView:OnClickCloseButton()
	self:Close()
end

function TipSpecialSkillView:SetData(skill_id, skill_level, has_active)
...
function TipSpecialSkillView:OnFlush(param_list)
	...
	local skill_type = self.skill_id == ZHUDONG_TYPE and Language.Common.ZhuDongSkill or Language.Common.BeiDongSkill
