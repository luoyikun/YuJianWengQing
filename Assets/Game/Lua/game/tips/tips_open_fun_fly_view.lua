...
--主界面右上角图标大小与间距
local TopRightBtnWidth = 78
local TopRightBtnHeight = 78
local TopRightBtnScale = 0.9					--图标缩放
local TopRightBtnSpacing = 17

--主界面右下角图标大小与间距
local BottomRightBtnWidth = 82
local BottomRightBtnHeight = 82
...
function TipOpenFunctionFlyView:LoadCallBack()
	self.icon_go = self.node_list["icon_go"]

	self.icon_text = self.node_list["icon_text"]
	self.block = self.node_list["Block"]
	self.frame = self.node_list["Frame"]

	self.node_list["block_go"].button:AddClickListener(BindTool.Bind(self.CloseWindow, self))
	self.node_list["Block"].button:AddClickListener(BindTool.Bind(self.BlockClick, self))
end
...
function TipOpenFunctionFlyView:CloseCallBack()
	...
	GlobalEventSystem:Fire(OpenFunEventType.OPEN_PAUSE, false)
...
function TipOpenFunctionFlyView:InitIcon()
	...
	if icon ~= "" then
		local bundle, asset = ResPath.GetMainIcon(icon)
	...
	--初始化文字
	self.icon_text.text.text = self.cfg.show_tips
end
...
function TipOpenFunctionFlyView:BlockClick()
	...
	local right_button_ani_state = main_view:GetRightButtonsVisible()
	...
	if not self.top_right_complete then
		self.top_right_complete = right_button_ani_state == 1
	end
