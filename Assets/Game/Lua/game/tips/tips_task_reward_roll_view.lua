...
function TipsTaskRewardRollView:LoadCallBack()
	...
	self.time = self.node_list["Time"]
	self.wheel = self.node_list["Wheel"]
	for i = 1, CellCount do
		self.reward_cells[i] = ItemCell.New()
		self.reward_cells[i]:SetInstanceParent(self.node_list["Reward"..i])
		self.reward_cells[i]:SetToggleGroup(self.toggle_group)
	end

	for i = 1,CellCount do
		self.imageshow[i] = self.node_list["Image"..i]
	end
			...
			if TaskData.Instance:GetRewardRollInfo()[1].index then
				...
				tween1:OnComplete(function ()
					if self.wheel then
						...
						self.imageshow[TaskData.Instance:GetRewardRollInfo()[1].index + 1]:SetActive(true)
	...
	if task_cfg then
		if com_type == "accepted_add" then
			...
			elseif TASK_RI_AUTO and task_cfg.task_type == TASK_TYPE.RI and DayCounterData.Instance:GetDayCount(DAY_COUNT.DAYCOUNT_ID_COMMIT_DAILY_TASK_COUNT) > 0 then
				TaskCtrl.Instance:DoTask(task_id)
			elseif TASK_HUAN_AUTO and task_cfg.task_type == TASK_TYPE.HUAN and TaskData.Instance:GetPaohuanTaskInfo().commit_times > 0 then
				TaskCtrl.Instance:DoTask(task_id)
			elseif TASK_ZHUANZHI_AUTO and task_cfg.task_type == TASK_TYPE.ZHUANZHI then
...
function TipsTaskRewardRollView:ShowRoll(task_cfg)
	if self:IsOpen() then
		...
		if task_cfg.condition == TASK_COMPLETE_CONDITION.HUG or task_cfg.task_id == FAkE_TRUCK then
			TaskCtrl.Instance:DoTask(task_cfg.task_id)
		else
