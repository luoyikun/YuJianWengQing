...
function ChatFilter:CheckBackground()
	...
	local sign = MD52.GetMD5(spid .. content .. now_server_time .. key)
	local real_url = string.format("%s/api/keyword.php?keyword=%s&spid=%s&time=%s&sign=%s",
									GlobalUrl, content, spid, tostring(now_server_time), sign)
