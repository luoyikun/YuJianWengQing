...
function CoolChatCtrl:SetAvatarFrameImage(avatar_frame_variable, avatar_type, show_variable)
	if avatar_type == nil then
		show_variable:SetActive(true)
		return
	end
	if show_variable then
		show_variable:SetActive(avatar_type <= -1)
	end
	if avatar_frame_variable then
		if avatar_type <= -1 then
			return
		end
		avatar_frame_variable.image:LoadSprite(ResPath.GetHeadFrameIcon(avatar_type))
	end
end
