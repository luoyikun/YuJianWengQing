...
function JinYinTaData:GetHistoryRewardInfo(reward_index)
	local level_lottery_reward = self:GetRewardCfgByDay()
	if level_lottery_reward[reward_index].is_record_history == 1 then
...
function JinYinTaData:GetLeijiJiangli()
	...
	for k,v in pairs(rand_config) do
		list[k] = {}
		list[k].opengame_day = v.opengame_day
		list[k].reward_index = v.reward_index
		list[k].flag = self:IsGetReward(v.reward_index)
		list[k].total_times = v.total_times
		list[k].vip_level_limit = v.vip_level_limit
...
function JinYinTaData:GetJinYinTaRemind()
	...
	local has_key = self:GetKeyRemind()
	if mianfei <= 0 then
		return 1
	end
	if get_count > 0 then 
...
function JinYinTaData:GetKeyRemind()
