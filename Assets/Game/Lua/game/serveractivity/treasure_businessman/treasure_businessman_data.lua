...
function TreasureBusinessmanData:GetReturnReward()
...
function TreasureBusinessmanData:GetIsAllBuy()
	for k,v in pairs(self.zhenbaoge_item_list) do
		if v ~= 0 then
			return true
		end
	end
...
function TreasureBusinessmanData:GetZhenBaogeRemind()
	if not ActivityData.Instance:GetActivityIsOpen(ACTIVITY_TYPE.RAND_ACTIVITY_TREASURE_BUSINESSMAN) then
		return 0
	end

	local reward_data = self:GetReturnReward()
	local cur_num = TreasureBusinessmanData.Instance:GetServerFlushTimes() or 0
	
	for i = 1, 6 do
		local fetch_flag = reward_data[i].fetch_flag
		local can_get = cur_num >= reward_data[i].cfg.can_fetch_times
		-- print_error(can_get, fetch_flag)
		if can_get and fetch_flag ~= 1 then
			return 1
		end
	end

	local nexttime = self:GetNextFlushTimeStamp() - TimeCtrl.Instance:GetServerTime() --智扬说现在只有可领取才提示红点
