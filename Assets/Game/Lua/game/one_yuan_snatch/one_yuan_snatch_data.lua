...
function OneYuanSnatchData:__init()
	...
	self.cfg = ConfigManager.Instance:GetAutoConfig("cloud_purchase_auto") or {}
	self.greate_soldier_cfg = ConfigManager.Instance:GetAutoConfig("greate_soldier_config_auto").greate_soldier or {}
...
function OneYuanSnatchData:GetShowModelCfg()
	local cfg = self:GetIntergralCfg()
	local show_cfg = {}
	if cfg then
		for k,v in pairs(cfg) do
			if v and v.current_prize == 1 then
				show_cfg = v
			end
		end
	end
