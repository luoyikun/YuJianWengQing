...
function SnatchLogView:OnFlush()
	if self.node_list["TopListView"] and self.node_list["TopListView"].scroller then
		self.node_list["TopListView"].scroller:ReloadData(0)
	end

	if self.node_list["DownListView"] and self.node_list["DownListView"].scroller then
...
function SnatchLogCell:OnFlush()
	...
	self.node_list["TxtInfo"].text.text = text
end
