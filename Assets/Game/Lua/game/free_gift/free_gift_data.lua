...
ZERO_GIFT_OPERATE_TYPE =
{
	...
	ZERO_GIFT_FETCH_REWARD_ITEM = 2,
	ZERO_GIFT_BUY_GOD_COSTUME = 3,					 -- 购买四阶神装
	ZERO_GIFT_FETCH_GOD_COSTUME_REWARD_ITEM = 4,	 -- 领取四阶神装奖励，参数1 seq   参数2 等级阶段
}
...
function FreeGiftData:CanShowZeroGift()
	...
	if open_day > self.phase_cfg[0].open_day + 2 then 
		return false
	end
...
function FreeGiftData:SetFreeGiftSign(enable)
	self.is_show_label = enable
end

function FreeGiftData:GetFreeGiftSign()
	local is_not_all_buy = false
	if self.phase_list then
		if self.phase_list[0] and self.phase_list[0].state then
			local server_time = TimeCtrl.Instance:GetServerTime()
			if self.phase_list[0].state ~= ZERO_GIFT_STATE.HAD_BUY_STATE and self.phase_list[0].state ~= ZERO_GIFT_STATE.HAD_FETCHE_STATE
				and self.phase_list[0].timestamp - server_time > 0 then
				is_not_all_buy = true
			end
		end
	end
	return self.is_show_label and is_not_all_buy
end