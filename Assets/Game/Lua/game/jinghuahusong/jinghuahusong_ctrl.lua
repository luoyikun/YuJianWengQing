...
require("game/jinghuahusong/jinghuahusong_view")
-- 护送精华
...
function JingHuaHuSongCtrl:__init()
	...
	self.view = JingHuaHuSongView.New(ViewName.CrystalEscortView)
...
function JingHuaHuSongCtrl:RegisterAllProtocols()
	self:RegisterProtocol(SCCrossHusongShuijingInfo, "OnJinghuaHusongViewChange")			--护送状态改变
	self:RegisterProtocol(SCCrossHusongShuijingGatherInfo, "OnSCCrossHusongShuijingGatherInfo")	--灵石数量
	self:RegisterProtocol(CSCrossHusongShuijingOpera)
end

--继续护送精华
function JingHuaHuSongCtrl:ContinueJingHuaHuSong(jinghua_type)
	jinghua_type = jinghua_type or self.data.cur_jinghua_type
	if self.data:GetMainRoleState() == -1 then -- JH_HUSONG_STATUS.NONE
...
function JingHuaHuSongCtrl:SetJingHuaHuSongInfo(protocol)
	...
	if protocol.param1 == -1  then										-- param1 == -1表示精华采集物刷新。
		if GameVoManager.Instance:GetMainRoleVo().level >= self.data:GetRemindLevel() and not self.data:IsAllCommit() and ActivityData.Instance:IsAchieveLevelInLimintConfigById(ACTIVITY_TYPE.JINGHUA_HUSONG) then	-- 如果等级足够且未全部采集完，则提醒玩家
...
function JingHuaHuSongCtrl:OnJinghuaHusongViewChange(protocol)
	...
	self.data:SetAllJingHuaHuSongInfo(protocol)
	if nil ~= role then		
		if role:IsMainRole() then
			self.data:SetMainRoleState(protocol.husong_status)
			if protocol.husong_type and protocol.husong_type ~= -1 then
				self.data:SetCurJingHuaType(protocol.husong_type)
			end
			role:SetAttr("husong_type", protocol.husong_type) 		--设置护送类型，以更换护送图标
			role:SetAttr("husong_status", protocol.husong_status) 	--设置状态，以更换护送图标
			local main_role = Scene.Instance:GetMainRole()
			if protocol.husong_status ~= JH_HUSONG_STATUS.NONE then
				self:ContinueJingHuaHuSong(protocol.husong_type)
			end
		end
	end
...
function JingHuaHuSongCtrl:MoveToHuShongCommitNpc(ignore_vip)
	GuajiCtrl.Instance:MoveToNpc(self.data:GetCommitNpc(), nil, self.data:GetGatherSceneId(), ignore_vip, 1)
end

function JingHuaHuSongCtrl:OnSCCrossHusongShuijingGatherInfo(protocol)
	self.data:SetJingHuaHuSongTimeInfo(protocol)
	if protocol.type == 1 then
		if GameVoManager.Instance:GetMainRoleVo().level >= self.data:GetNeedJoinActLevel() and not self.data:IsAllCommit() and ActivityData.Instance:IsAchieveLevelInLimintConfigById(ACTIVITY_TYPE.JINGHUA_HUSONG) then	-- 如果等级足够且未全部采集完，则提醒玩家
			TipsCtrl.Instance:OpenFocusJingHuaHuSongTip()				-- 弹出提醒框
		end
	end
	if self.view and self.view:IsOpen() then
	...
	if detail_view and detail_view:IsOpen() then
		detail_view:Flush()
	end
end
...
function JingHuaHuSongCtrl:MoveToGather(ignore_vip, jinghua_type)
	...
	-- local func = function()
		...
		-- CrossServerCtrl.Instance:SendCrossStartReq(ACTIVITY_TYPE.JINGHUA_HUSONG, nil, nil,nil,self.data:GetGatherPosX(jinghua_type),self.data:GetGatherPosY(jinghua_type))
		GuajiCtrl.Instance:MoveToPos(self.data:GetGatherSceneId(jinghua_type), self.data:GetGatherPosX(jinghua_type),self.data:GetGatherPosY(jinghua_type))
	-- end
	-- local scene_key = PlayerData.Instance:GetAttr("scene_key") or 0
	-- if not scene_key ~= 0 and not self.data:GetGatherSceneId(jinghua_type) == Scene.Instance:GetSceneId() then
...
function JingHuaHuSongCtrl:GetIntoCrossShuiJing(jinghua_type)
	ViewManager.Instance:CloseAll()
	CrossServerCtrl.Instance:SendCrossStartReq(ACTIVITY_TYPE.JINGHUA_HUSONG,jinghua_type)
end

--提交物品
function JingHuaHuSongCtrl.SendCommitReq()
	local protocol = ProtocolPool.Instance:GetProtocol(CSCrossHusongShuijingOpera)
...
--请求精华护送信息
function JingHuaHuSongCtrl:SendGetCrystalInfoReq()
	local protocol = ProtocolPool.Instance:GetProtocol(CSCrossHusongShuijingOpera)
	protocol.operate_type = COMMON_OPERATE_TYPE.COT_JINGHUA_HUSONG_COMMIT_OPE
	protocol.param1 = 0
	protocol.param2 = 0
...
function JingHuaHuSongCtrl:CanGatherJingHua(gahter_id)
	if self.data:GetMainRoleState() ~= JH_HUSONG_STATUS.NONE or self.data:IsAllCommit() then
		return false
	else
		return true
	end
