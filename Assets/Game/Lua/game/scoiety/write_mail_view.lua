...
function WriteMailView:__init()
	...
	self.node_list["BtnSend"].button:AddClickListener(BindTool.Bind(self.ClickSend, self))
	self.node_list["BtnReturn"].button:AddClickListener(BindTool.Bind(self.ClickReturn, self))
	self.node_list["BtnClickFriendList"].button:AddClickListener(BindTool.Bind(self.ClickFriendList, self))
	self.node_list["BtnName"].button:AddClickListener(BindTool.Bind(self.ClickFriendList, self))

		--获取控件
	self.cell_list = {}

	for i = 1, PAGE_COUNT do
		self["Page" .. i] = self.node_list["PageToggle" .. i]
		self["Page" .. i].toggle:AddValueChangedListener(BindTool.Bind(self.ChangePage, self, i - 1))

		self.node_list["Page" .. i].toggle:AddClickListener(BindTool.Bind(self.BagJumpPage, self, 1))
	end
	...
	for i = 1, 3 do
		...
		send_item:SetInstanceParent(self.node_list["Item" .. i])
		send_item.button:AddClickListener(BindTool.Bind(self.SendItemClick, self, send_item))
		send_item:SetData(nil)
		send_item:ShowHighLight(false)
		table.insert(self.send_item_list, send_item)
	end

	self.node_list["TextName"].text.text = Language.Society.SelectName
end
...
function WriteMailView:SetFriendName(param)
	...
	self.node_list["TextName"].text.text = self.player_name
end
...
function MailItemCellGroup:__init()
	self.cells = {}
	for i = 1, BAG_ROW do
		local cell = ItemCell.New(self.node_list["Item" .. i])
