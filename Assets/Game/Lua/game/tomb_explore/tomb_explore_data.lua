...
function TombExploreData:GetTombFbBuffTime()
...
function TombExploreData:GetTombActivityRewardCfg(index)
	return self.cfg.task_list[index].reward_item[0]
end
...
function TombExploreData:GetTianXianRewardEq(wrold_level)
	local data_list = ConfigManager.Instance:GetAutoConfig("activitywanglingexplore_auto").show_item
	for k,v in pairs(data_list) do
		if v.level_down <= wrold_level and wrold_level <= v.level_up then
			return v
		end
	end
	return {}
...
function TombExploreData:SetWangLingExploreBossInfo(protocol)
...
function TombExploreData:GetWangLingExploreBossInfo()
