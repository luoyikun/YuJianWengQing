CheckWeiYanView = CheckWeiYanView or BaseClass(BaseRender)
function CheckWeiYanView:__init(instance)
	self.fight_text = CommonDataManager.FightPower(self, self.node_list["TxtZhanli"])
end

function CheckWeiYanView:__delete()
	self.weiyan_attr = nil
	self.fight_text = nil
end


function CheckWeiYanView:DoPanelTweenPlay()
	UITween.MoveShowPanel(self.node_list["RightFrame"], CheckData.TweenPosition.RightFrame , TWEEN_TIME, DG.Tweening.Ease.InOutSine)
end


function CheckWeiYanView:OnFlush()
	if self.weiyan_attr then
		local grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(self.weiyan_attr.grade)
		...
		local shuxingdan_count = self.weiyan_attr.shuxingdan_list[0]
		...
		if self.fight_text and self.fight_text.text then
			self.fight_text.text.text = self.weiyan_attr.capability
		end
...
function CheckWeiYanView:SetAttr()
	local check_attr = CheckData.Instance:UpdateAttrView()
	if check_attr and check_attr.weiyan_attr then
		self.weiyan_attr = check_attr.weiyan_attr
		self:Flush()
	end
end

function CheckWeiYanView:SetModle(image_cfg)
