CheckTouShiView = CheckTouShiView or BaseClass(BaseRender)
function CheckTouShiView:__init(instance)
	self.fight_text = CommonDataManager.FightPower(self, self.node_list["TxtZhanli"])
end

function CheckTouShiView:__delete()
	self.toushi_attr = nil
	self.fight_text = nil
end


function CheckTouShiView:DoPanelTweenPlay()
	UITween.MoveShowPanel(self.node_list["RightFrame"], CheckData.TweenPosition.RightFrame , TWEEN_TIME, DG.Tweening.Ease.InOutSine)
end


function CheckTouShiView:OnFlush()
	if self.toushi_attr then
		local grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(self.toushi_attr.grade)
		...
		local zizhi_hp = zi_zhi_cfg.maxhp * self.toushi_attr.shuxingdan_count 
		local zizhi_gongji = zi_zhi_cfg.gongji * self.toushi_attr.shuxingdan_count
		local zizhi_fangyu = zi_zhi_cfg.fangyu * self.toushi_attr.shuxingdan_count
		...
		if self.fight_text and self.fight_text.text then
			self.fight_text.text.text = self.toushi_attr.capability
		end
...
function CheckTouShiView:SetAttr()
	local check_attr = CheckData.Instance:UpdateAttrView()
	if check_attr and check_attr.toushi_attr then
		self.toushi_attr = check_attr.toushi_attr
		self:Flush()
	end
end

function CheckTouShiView:SetModle()
