CheckLingZhuView = CheckLingZhuView or BaseClass(BaseRender)
function CheckLingZhuView:__init(instance)
	self.fight_text = CommonDataManager.FightPower(self, self.node_list["TxtZhanli"])
end

function CheckLingZhuView:__delete()
	self.lingzhu_attr = nil
	self.fight_text = nil
end


function CheckLingZhuView:DoPanelTweenPlay()
	UITween.MoveShowPanel(self.node_list["RightFrame"], CheckData.TweenPosition.RightFrame , TWEEN_TIME, DG.Tweening.Ease.InOutSine)
end


function CheckLingZhuView:OnFlush()
	if self.lingzhu_attr then
		local grade_info = LingZhuData.Instance:GetLingZhuGradeCfgInfoByGrade(self.lingzhu_attr.grade)
		...
		local shuxingdan_count = self.lingzhu_attr.shuxingdan_list[0]
		...
		if self.fight_text and self.fight_text.text then
			self.fight_text.text.text = self.lingzhu_attr.capability
		end
...
function CheckLingZhuView:SetAttr()
	local check_attr = CheckData.Instance:UpdateAttrView()
	if check_attr and check_attr.lingzhu_attr then
		self.lingzhu_attr = check_attr.lingzhu_attr
		self:Flush()
	end
end

function CheckLingZhuView:SetModle(image_cfg)
