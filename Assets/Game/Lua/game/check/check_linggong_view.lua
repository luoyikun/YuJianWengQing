...
function CheckLingGongView:__delete()
	self.linggong_attr = nil
	self.fight_text = nil
end


function CheckLingGongView:DoPanelTweenPlay()
...
function CheckLingGongView:OnFlush()
	if self.linggong_attr then
		local grade_info = LingGongData.Instance:GetLingGongGradeCfgInfoByGrade(self.linggong_attr.grade)
		...
		local shuxingdan_count = self.linggong_attr.shuxingdan_list[0]
		...
		if self.fight_text and self.fight_text.text then
			self.fight_text.text.text = self.linggong_attr.capability
		end
...
function CheckLingGongView:SetAttr()
	...
	if check_attr and check_attr.linggong_attr then
		self.linggong_attr = check_attr.linggong_attr
		self:Flush()
	end
end

function CheckLingGongView:SetModle(image_cfg)
