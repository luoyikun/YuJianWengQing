CheckFlyPetView = CheckFlyPetView or BaseClass(BaseRender)
function CheckFlyPetView:__init(instance)
	self.fight_text = CommonDataManager.FightPower(self, self.node_list["TxtZhanli"])
end

function CheckFlyPetView:__delete()
	self.flypet_attr = nil
	self.fight_text = nil
end


function CheckFlyPetView:DoPanelTweenPlay()
	UITween.MoveShowPanel(self.node_list["RightFrame"], CheckData.TweenPosition.RightFrame , TWEEN_TIME, DG.Tweening.Ease.InOutSine)
end

function CheckFlyPetView:OnFlush()
	if self.flypet_attr then
		local grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(self.flypet_attr.grade)
		...
		local shuxingdan_count = self.flypet_attr.shuxingdan_list[0]
		...
		if self.fight_text and self.fight_text.text then
			self.fight_text.text.text = self.flypet_attr.capability
		end
...
function CheckFlyPetView:SetAttr()
	local check_attr = CheckData.Instance:UpdateAttrView()
	if check_attr and check_attr.flypet_attr then
		self.flypet_attr = check_attr.flypet_attr
		self:Flush()
	end
end

function CheckFlyPetView:SetModle(image_cfg)
