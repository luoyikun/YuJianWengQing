CheckYaoShiView = CheckYaoShiView or BaseClass(BaseRender)
function CheckYaoShiView:__init(instance)
	self.fight_text = CommonDataManager.FightPower(self, self.node_list["TxtZhanli"])
end

function CheckYaoShiView:__delete()
	self.yaoshi_attr = nil
	self.fight_text = nil
end


function CheckYaoShiView:DoPanelTweenPlay()
	UITween.MoveShowPanel(self.node_list["RightFrame"], CheckData.TweenPosition.RightFrame , TWEEN_TIME, DG.Tweening.Ease.InOutSine)
end


function CheckYaoShiView:OnFlush()
	if self.yaoshi_attr then
		local grade_info = WaistData.Instance:GetWaistGradeCfgInfoByGrade(self.yaoshi_attr.grade)
		...
		local zizhi_hp = zi_zhi_cfg.maxhp * self.yaoshi_attr.shuxingdan_count 
		local zizhi_gongji = zi_zhi_cfg.gongji * self.yaoshi_attr.shuxingdan_count
		local zizhi_fangyu = zi_zhi_cfg.fangyu * self.yaoshi_attr.shuxingdan_count
		...
		if self.fight_text and self.fight_text.text then
			self.fight_text.text.text = self.yaoshi_attr.capability
		end
...
function CheckYaoShiView:SetAttr()
	local check_attr = CheckData.Instance:UpdateAttrView()
	if check_attr and check_attr.yaoshi_attr then
		self.yaoshi_attr = check_attr.yaoshi_attr
		self:Flush()
	end
end

function CheckYaoShiView:SetModle()
