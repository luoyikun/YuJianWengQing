...
function DaFuHaoCtrl:ActivityChangeCallback(activity_type, status, next_time, open_type)
	if activity_type ~= DaFuHaoDataActivityId.ID then
		return
	end

	if status == ACTIVITY_STATUS.OPEN then
		self:SendGetGatherInfoReq()
	else
...
function DaFuHaoCtrl:OnDaFuHaoInfo(protocol)
	...
	if self.old_gather_total_times ~= protocol.gather_total_times then
		...
		if 0 == protocol.is_turn and 10 == protocol.gather_total_times and Scene.Instance:GetSceneType() == SceneType.DaFuHao then
			self.roll_view:Open()
		end
	end
...
function DaFuHaoCtrl:SetDaFuHaoIcon()
