...
function HefuActivityView:LoadCallBack()
	...
	local list_delegate = self.node_list["ScrollerToggleList"].list_simple_delegate
...
function HefuActivityView:RefreshCell(cell, data_index)
	...
	if tab_btn == nil then
		...
		tab_btn:SetToggleGroup(self.node_list["ScrollerToggleList"].toggle_group)
...
function HefuActivityView:FlushLeftTabListView()
	...
	if self.node_list["ScrollerToggleList"].scroller.isActiveAndEnabled then
		if self.cur_day ~= TimeCtrl.Instance:GetCurOpenServerDay() or self.cur_tab_list_length ~= #list then
			self.node_list["ScrollerToggleList"].scroller:ReloadData(0)
		else
			self.node_list["ScrollerToggleList"].scroller:RefreshActiveCellViews()
		end
	end
...
function HefuActivityView:OpenChildPanel()
	...
	if nil == panel then
		local async_loader = AllocAsyncLoader(self, "panel_loader_" .. self.cur_sub_type)
		async_loader:Load(
			...
			function(obj)
				...
				obj.transform:SetParent(self.node_list["RightContent"],false)
				obj = U3DObject(obj)
