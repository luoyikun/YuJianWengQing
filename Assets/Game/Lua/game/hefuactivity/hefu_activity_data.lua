...
function HefuActivityData:__init()
	...
	self.is_open_hefu_jijin = true
	self.is_open_hefu_touzi = true
	self.luckly_click = false
	self.luckly_toggle = false
	--RemindManager.Instance:Register(RemindName.HeFu, BindTool.Bind(self.GetCombineServerRemind, self))
	RemindManager.Instance:Register(RemindName.QiangGou, BindTool.Bind(self.GetShowQiangGouRedPoint, self))
	RemindManager.Instance:Register(RemindName.LucklyTurn, BindTool.Bind(self.GetLucklyTurnRedPoint, self))
	RemindManager.Instance:Register(RemindName.ChongZhiRank, BindTool.Bind(self.GetShowChongZhiRedPoint, self))
end

function HefuActivityData:__delete()
	HefuActivityData.Instance = nil
	RemindManager.Instance:UnRegister(RemindName.QiangGou)
	RemindManager.Instance:UnRegister(RemindName.LucklyTurn)
	RemindManager.Instance:UnRegister(RemindName.ChongZhiRank)
end
...
function HefuActivityData:IsCombineSubActivityOpen(sub_type)
	for k, v in pairs(self.open_act_id_list) do
		if sub_type == v.sub_type then
			return true
		end 
	end
	return false
...
function HefuActivityData:GetShowQiangGouRedPoint()
	-- 每天提醒一次的假红点 
	return 0
end

function HefuActivityData:IsShowOnceADayRedPoint(remind_name)
	if remind_name == RemindName.QiangGou and not self:IsCombineSubActivityOpen(COMBINE_SERVER_ACTIVITY_SUB_TYPE.CSA_SUB_TYPE_RANK_QIANGGOU) then
		return false
	elseif remind_name == RemindName.ChongZhiRank and not self:IsCombineSubActivityOpen(COMBINE_SERVER_ACTIVITY_SUB_TYPE.CSA_SUB_TYPE_CHONGZHI_RANK) then
		return false
	end
	if not RemindManager.Instance:GetOnceADayRemindList(remind_name) then
		return true
	end
	return false
end

function HefuActivityData:GetShowChongZhiRedPoint()
	-- 每天提醒一次的假红点 
	return 0
end
...
function HefuActivityData:SetCombineRoleInfo(protocol)
	...
	self.combine_person_info.rank_combine_chongzhi_num = protocol.rank_combine_chongzhi_num
	self.combine_person_info.rank_combine_consume_num = protocol.rank_combine_consume_num
...
function HefuActivityData:IsHeFuFirstCombine()
	local flag = false
	if self:IsCombineSubActivityOpen(COMBINE_SERVER_ACTIVITY_SUB_TYPE.CSA_SUB_TYPE_GONGCHENGZHAN) and self.combine_server_data and self.combine_server_data.csa_gcz_winner_roleid
...
function HefuActivityData:IsHeFuFirstGuildWar()
	local flag = false
	if self:IsCombineSubActivityOpen(COMBINE_SERVER_ACTIVITY_SUB_TYPE.CSA_SUB_TYPE_XIANMENGZHAN) and self.combine_server_data and self.combine_server_data.csa_xmz_winner_roleid
...
-- 消费排行
function HefuActivityData:GetConsubeRank()
	return self.combine_person_info.rank_combine_consume_num
end
...
-- 充值排行
function HefuActivityData:GetChongZhiRank()
	return self.combine_person_info.rank_combine_chongzhi_num
end
...
function HefuActivityData:GetLoginGiftCfgByDay(day)
	...
	for k,v in pairs(login_gift) do
		if v.need_login_days == day then
			...
			data.sort_key = TableCopy(self:GetLoginRewardKey())
		end
	end
	return data
end
...
function HefuActivityData:GetLoginRewardFlag()
	...
	data["is_vip"] = GameVoManager.Instance:GetMainRoleVo().vip_level > 0 and true or false
	--第三个格子
	if self.combine_person_info.has_fetch_accumulate_reward == 0 then
		if other[1].need_accumulate_days <= self.combine_person_info.login_days then
			data["is_need_accumulate_days"] = true
			data[3] = true
		else
			data["is_need_accumulate_days"] = false
			data[3] = false
		end
	else
		data[3] = false
...
function HefuActivityData:GetLoginRewardKey()
	local other = self:GetCurrentCombineActivityConfig().other
	local data = {}
	if self.combine_person_info == nil or next(self.combine_person_info) == nil then return end
	data[1] = self.combine_person_info.fetch_common_reward_flag[32 - self.combine_person_info.login_days + 1] == 0 and 1 or 4
	data[2] = self.combine_person_info.fetch_vip_reward_flag[32 - self.combine_person_info.login_days + 1] == 0 and 2 or 7
	if GameVoManager.Instance:GetMainRoleVo().vip_level > 0 then
		data[2] = self.combine_person_info.fetch_vip_reward_flag[32 - self.combine_person_info.login_days + 1] == 0 and 2 or 5
	end
	if self.combine_person_info.has_fetch_accumulate_reward == 0 then
		if other[1].need_accumulate_days <= self.combine_person_info.login_days then
			data[3] = 3
...
function HefuActivityData:GetLucklyTurnRedPoint()
	if not self:IsCombineSubActivityOpen(COMBINE_SERVER_ACTIVITY_SUB_TYPE.CSA_SUB_TYPE_ROLL) then
		return 0
	end
	local chongzhi_count, total_chongzhi_count = HefuActivityData.Instance:GetRollChongZhiCount()
	local roll_cost =  math.floor(chongzhi_count / self:GetRollCost())
	if roll_cost <= 0 then
		if not RemindManager.Instance:GetOnceADayRemindList(RemindName.LucklyTurn) then
			return 1
		end
	end
	return roll_cost
...
function HefuActivityData:SetFoundationData(protocol)
	for i = 1, COMBINE_SERVER_ACTIVITY_SUB_TYPE.CSA_SUB_TYPE_JIJIN do
...
function HefuActivityData:IsShowHeFuJiJinRedPoint()
	...
	if not self:IsCombineSubActivityOpen(COMBINE_SERVER_ACTIVITY_SUB_TYPE.CSA_SUB_TYPE_JIJIN) then
		return false
	end
	local num = HefuActivityData.Instance:GetHeFuJiJinNum() or 0
	if num > 0 then
		for i = 1, num do
			if self.foundation_data[i] then
				if 0 ~= self.foundation_data[i].buy_level then
					...
					can_reward = not self.is_open_hefu_jijin
				end
			end
		end
	end
...
function HefuActivityData:IsOpenHeFuJiJin()
	self.is_open_hefu_jijin = true
end

function HefuActivityData:IsShowHeFuTouZiRedPoint()
	local can_reward = false
	if not self:IsCombineSubActivityOpen(COMBINE_SERVER_ACTIVITY_SUB_TYPE.CSA_SUB_TYPE_TOUZI) then
		return false
	end
	...
	if info and next(info) then
		...
		if info.csa_touzijihua_buy_flag == 1 then
			...
			can_reward = not self.is_open_hefu_touzi
		end
	end
	return can_reward
end

function HefuActivityData:IsOpenHeFuTouZi()
	self.is_open_hefu_touzi = true
...
function HefuActivityData:GetCombineServerBossCfg()
	...
	for i = 1, #self.boss_info do
		if self.boss_info[i] and self.boss_info[i].boss_type == 0 and self.boss_info[i].boss_id > 0 then
...
function HefuActivityData:GetActivityCombineServerBossCfg()
	local temp_cfg = {}
	for i = 1, #self.boss_info do
		if self.boss_info[i] and self.boss_info[i].boss_type == 1 and self.boss_info[i].boss_id > 0 then
			table.insert(temp_cfg, self.boss_info[i])
		end
	end
	local need_cfg = {}
...
function HefuActivityData:GetAllCombineBossList()
	...
	for i = 1, #self.boss_info do
		if self.boss_info[i] and self.boss_info[i].boss_type == boss_type and self.boss_info[i].boss_id > 0 then
			table.insert(temp_cfg, self.boss_info[i])
		end
	end
	local need_cfg = {}
...
function HefuActivityData:GetBossShowLevel()
	local level_list = self:GetCombineServerBossConfig().boss_level_show or {}
	local world_level = RankData.Instance:GetWordLevel()
	for k, v in ipairs(level_list) do
		if world_level >= v.world_level_1 and world_level <= v.world_level_2 then
			return v.boss_level
		end
	end
end
...
function HefuActivityData:SetLucklyTurnClick(enable)
	self.luckly_click = enable
end

function HefuActivityData:GetLucklyTurn()
	return self.luckly_click
end

function HefuActivityData:SetLucklyTurnToggle(enable)
	self.luckly_toggle = enable
end

function HefuActivityData:GetLucklyTurnToggle()
	return self.luckly_toggle
end
