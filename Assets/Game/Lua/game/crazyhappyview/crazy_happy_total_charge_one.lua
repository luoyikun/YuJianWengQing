TotalChargeOne =  TotalChargeOne or BaseClass(BaseRender)
--累计充值
function TotalChargeOne:__init()
	self.contain_cell_list = {}
end

function TotalChargeOne:__delete()
	self.list_view = nil
...
function TotalChargeOne:OpenCallBack()
	KaifuActivityCtrl.Instance:SendRandActivityOperaReq(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHIONE,
	...
	local rest_time, next_time = ActivityData.Instance:GetActivityResidueTime(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHIONE)
	...
	self.reward_list = CrazyHappyData.Instance:GetOpenActTotalChargeOneRewardCfg(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHIONE)
	local list_delegate = self.node_list["ScrollerListView"].list_simple_delegate
	list_delegate.NumberOfCellsDel = BindTool.Bind(self.GetNumberOfCells, self)
	list_delegate.CellRefreshDel = BindTool.Bind(self.RefreshCell, self)
end

function TotalChargeOne:CloseCallBack()
	if self.least_time_timer then
		CountDown.Instance:RemoveCountDown(self.least_time_timer)
		self.least_time_timer = nil
	end
...
function TotalChargeOne:ClickReChange()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function TotalChargeOne:OnFlush()
	self.reward_list = CrazyHappyData.Instance:GetOpenActTotalChargeOneRewardCfg(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHIONE)
	...
	local info = CrazyHappyData.Instance:GetTotalChargeInfo(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHIONE)
	local money = 0
...
function TotalChargeOne:SetTime(elapse_time, total_time)
...
function TotalChargeOne:GetNumberOfCells()
	return #self.reward_list
end

function TotalChargeOne:RefreshCell(cell, cell_index)
	local contain_cell = self.contain_cell_list[cell]

	if contain_cell == nil then
		contain_cell = TotalChargeOneCell.New(cell.gameObject, self)
...
----------------------------TotalChargeOneCell---------------------------------
TotalChargeOneCell = TotalChargeOneCell or BaseClass(BaseCell)

local MAX_REWARD_NUM = 4

function TotalChargeOneCell:__init()
	self.item_cell_list = {}
...
function TotalChargeOneCell:__delete()
...
function TotalChargeOneCell:OnFlush()
...
function TotalChargeOneCell:OnClickGet()
	KaifuActivityCtrl.Instance:SendRandActivityOperaReq(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHIONE, RA_NEW_TOTAL_CHARGE_OPERA_TYPE.RA_NEW_TOTAL_CHARGE_OPERA_TYPE_FETCH_REWARD, self.data.seq)
end