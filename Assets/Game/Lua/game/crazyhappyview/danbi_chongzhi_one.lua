DanBiChongZhiOne = DanBiChongZhiOne or BaseClass(BaseRender)
--单笔充值
function DanBiChongZhiOne:__init()
	self.contain_cell_list = {}
	self.reward_list = {}
end

function DanBiChongZhiOne:__delete()
...
function DanBiChongZhiOne:OpenCallBack()
	local list_delegate = self.node_list["ScrollerListView"].list_simple_delegate
	list_delegate.NumberOfCellsDel = BindTool.Bind(self.GetNumberOfCells, self)
	list_delegate.CellRefreshDel = BindTool.Bind(self.RefreshCell, self)

	local rest_time, next_time = ActivityData.Instance:GetActivityResidueTime(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_DANBICHONGZHIONE)
	-- local opengameday = TimeCtrl.Instance:GetCurOpenServerDay()
	self.reward_list = CrazyHappyData.Instance:GetSingleChargeInfoOne(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_DANBICHONGZHIONE)
...
function DanBiChongZhiOne:CloseCallBack()
	if self.least_time_timer then
		CountDown.Instance:RemoveCountDown(self.least_time_timer)
		self.least_time_timer = nil
	end
...
function DanBiChongZhiOne:ClickReChange()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function DanBiChongZhiOne:GetNumberOfCells()
	return #self.reward_list
end

function DanBiChongZhiOne:RefreshCell(cell, cell_index)
	local contain_cell = self.contain_cell_list[cell]
	
	if contain_cell == nil then
		contain_cell = DanBiChongZhiOneCell.New(cell.gameObject, self)
...
function DanBiChongZhiOne:SetTime(rest_time)
...
function DanBiChongZhiOne:OnFlush()
...
------------------------------DanBiChongZhiOneCell-------------------------------------
DanBiChongZhiOneCell = DanBiChongZhiOneCell or BaseClass(BaseCell)

local MAX_CELL_NUM = 4

function DanBiChongZhiOneCell:__init()
	self.charge_value = 0
	self.item_cell_list = {}
...
function DanBiChongZhiOneCell:__delete()
...
function DanBiChongZhiOneCell:SetChargeValue(value)
	self.charge_value = value
end

function DanBiChongZhiOneCell:OnFlush()
	...
	if next(self.data) then
		...
		if reward_list and nil == reward_list.item_1_id then
...
function DanBiChongZhiOneCell:OnClickGet()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end