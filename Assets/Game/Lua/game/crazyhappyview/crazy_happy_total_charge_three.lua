TotalChargeThree =  TotalChargeThree or BaseClass(BaseRender)
--累计充值
function TotalChargeThree:__init()
	self.contain_cell_list = {}
end

function TotalChargeThree:__delete()
	self.list_view = nil
...
function TotalChargeThree:OpenCallBack()
	KaifuActivityCtrl.Instance:SendRandActivityOperaReq(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHITHREE,
	...
	local rest_time, next_time = ActivityData.Instance:GetActivityResidueTime(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHITHREE)
	...
	self.reward_list = CrazyHappyData.Instance:GetOpenActTotalChargeThreeRewardCfg(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHITHREE)
...
function TotalChargeThree:CloseCallBack()
	if self.least_time_timer then
		CountDown.Instance:RemoveCountDown(self.least_time_timer)
		self.least_time_timer = nil
	end
...
function TotalChargeThree:ClickReChange()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function TotalChargeThree:OnFlush()
	self.reward_list = CrazyHappyData.Instance:GetOpenActTotalChargeThreeRewardCfg(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHITHREE)
	...
	local info = CrazyHappyData.Instance:GetTotalChargeInfo(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHITHREE)
	local money = 0
...
function TotalChargeThree:SetTime(elapse_time, total_time)
...
function TotalChargeThree:GetNumberOfCells()
	return #self.reward_list
end

function TotalChargeThree:RefreshCell(cell, cell_index)
	local contain_cell = self.contain_cell_list[cell]

	if contain_cell == nil then
		contain_cell = TotalChargeThreeCell.New(cell.gameObject, self)
...
----------------------------TotalChargeThreeCell---------------------------------
TotalChargeThreeCell = TotalChargeThreeCell or BaseClass(BaseCell)

local MAX_REWARD_NUM = 4

function TotalChargeThreeCell:__init()
	self.item_cell_list = {}
...
function TotalChargeThreeCell:__delete()
...
function TotalChargeThreeCell:OnFlush()
...
function TotalChargeThreeCell:OnClickGet()
	KaifuActivityCtrl.Instance:SendRandActivityOperaReq(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_LEIJICHONGZHITHREE, RA_NEW_TOTAL_CHARGE_OPERA_TYPE.RA_NEW_TOTAL_CHARGE_OPERA_TYPE_FETCH_REWARD, self.data.seq)
end