...
require("game/yizhandaodi/yizhan_finish_reward_view")
...
function YiZhanDaoDiCtrl:__init()
	...
	self.yizhan_finish_reward_view = YiZhanFinishRewardView.New(ViewName.YiZhanFinishRewardView)
...
function YiZhanDaoDiCtrl:__delete()
	...
	if self.yizhan_finish_reward_view then
		self.yizhan_finish_reward_view:DeleteMe()
		self.yizhan_finish_reward_view = nil
	end
...
function YiZhanDaoDiCtrl:RegisterAllProtocols()
	...
	-- self:RegisterProtocol(SCYiZhanDaoDiRankTopInfo, "OnYiZhanDaoDiRankTopInfo")			-- 活动前3名
end
...
-- function YiZhanDaoDiCtrl:OnYiZhanDaoDiRankTopInfo(protocol)
-- 	self.data:SetYiZhanDaoDiRankTopInfo(protocol)
-- end
...
function YiZhanDaoDiCtrl:OnYiZhanDaoDiUserInfo(protocol)
	...
	if nil ~= fuben_icon_view and fuben_icon_view:IsOpen() then
		fuben_icon_view:Flush("yizhandaodiguwu_animator")
	end

	if protocol.show_jiesuan == 1 then
		ViewManager.Instance:Open(ViewName.YiZhanFinishRewardView)
	end		
end
