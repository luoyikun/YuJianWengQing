...
function JuBaoPenData:IsShowRedPoint()
	...
	if OpenFunData.Instance:CheckIsHide("jubaopen") then
		...
		if 0 == remind and self.reward_lun <= max_lun and ActivityData.Instance:GetActivityIsOpen(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_CORNUCOPIA) then
...
function JuBaoPenData:CheckIsShow()
	...
	local is_open, tips =  OpenFunData.Instance:CheckIsHide("jubaopen")
	-- 不知道为什么要加首充取反的显示 not is_first
	if self.reward_lun <= max_lun and ActivityData.Instance:GetActivityIsOpen(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_CORNUCOPIA) and is_open then 
	 	return true
	end
	return false
end
