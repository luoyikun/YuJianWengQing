...
function GuildBonfireView:LoadCallBack()
	...
	self.node_list["AddWood"].button:AddClickListener(BindTool.Bind(self.OnContribution, self))
	self.node_list["GetBonfire"].button:AddClickListener(BindTool.Bind(self.OnPray, self))
	self.node_list["InfoPanel"].event_trigger_listener:AddPointerClickListener(BindTool.Bind(self.Close, self))
...
function GuildBonfireView:OnFlush()
	if nil ~= self.target_obj and nil ~= self.target_obj:GetVo() and nil ~= self.target_obj:GetVo().param then
		...
		self.node_list["AddWood"]:SetActive(guild_id == self.target_obj:GetVo().param)
