...
function AutoVoiceView:ChangeTime()
	...
	self.node_list["TxtTimeText"].text.text = string.format("(%s)", self.time_value)
end
...
function AutoVoiceView:SendFeesAudioMsg()
	AudioGVoice.StopRecorder(function (is_succeed, file_id, str)
		...
		if is_succeed then
			...
			if nil ~= self.curr_send_channel then
				ChatCtrl.SendChannelChat(self.curr_send_channel, content, content_type)
			else
...
function AutoVoiceView:SendPrivateAudioMsg(message, content_type)
	...
	if private_role_id > SPECIAL_CHAT_ID.ALL then
		...
		if ViewManager.Instance:IsOpen(ViewName.ChatGuild) then
			ViewManager.Instance:FlushView(ViewName.ChatGuild, "new_chat", {false, private_role_id})
		end
	else
