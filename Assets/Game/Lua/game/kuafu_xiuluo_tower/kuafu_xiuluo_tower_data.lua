...
function KuaFuXiuLuoTowerData:GetScore()
	if self.xiuluo_cfg then
		return self.xiuluo_cfg.score
	end
end
...
function KuaFuXiuLuoTowerData:GetCanGetRewardUI()
	...
	local max_num = GetListNum(self.xiuluo_cfg.score)
	if self.score >= self.xiuluo_cfg.score[max_num].score then
		return self.xiuluo_cfg.score[max_num], max_num
	end
	return nil
end
...
function KuaFuXiuLuoTowerData:OnXiuLuoSelfInfo(protocol)
	...
	if nil ~= fuben_icon_view and fuben_icon_view:IsOpen() then
		fuben_icon_view:Flush("xiuluota_wudi")
	end
end
...
function KuaFuXiuLuoTowerData:SetCossXiuluoTowerRankInfo(protocol)
	self.rank_count = protocol.rank_count
	self.rank_layer_list = protocol.rank_list
end

function KuaFuXiuLuoTowerData:GetCossXiuluoTowerRankInfoList()
	return self.rank_layer_list
end

function KuaFuXiuLuoTowerData:GetCossXiuluoTowerRankNum()
...
function KuaFuXiuLuoTowerData:GetTitleCfg()
	if self.xiuluo_cfg then
		return self.xiuluo_cfg.rank_title
	end
end

function KuaFuXiuLuoTowerData:GetRongYaoReward()
	local data = {}
	if self.xiuluo_cfg then 
		for k,v in pairs(self.xiuluo_cfg.score) do
			if self.score >= v.score then
				data[k] = {}
				for i = 1, 3 do
					table.insert(data[k], v["reward_item" .. i])
				end
			end
		end
	end
	...
	local reward_bind_list = {}
	for key, value in pairs(data) do
		if value then
			for k,v in pairs(value) do
				reward_id_list[v.item_id] = reward_id_list[v.item_id] or 0
				reward_id_list[v.item_id] = reward_id_list[v.item_id] + v.num
				reward_bind_list[v.item_id] = v.is_bind
			end
		end
	end

	local item_list = {}
	for m, n in pairs(reward_id_list) do
		if m and n and reward_bind_list[m] then
			table.insert(item_list, {is_bind = reward_bind_list[m], item_id = m, num = n})
		end
	end
	return item_list
end

function KuaFuXiuLuoTowerData:SetCossXiuluoTowerBossInfo(protocol)
	self.monster_list.monster_id = protocol.monster_id
	self.monster_list.max_hp = protocol.max_hp
	self.monster_list.cur_hp = protocol.cur_hp
end

function KuaFuXiuLuoTowerData:GetCossXiuluoTowerBossInfo()
