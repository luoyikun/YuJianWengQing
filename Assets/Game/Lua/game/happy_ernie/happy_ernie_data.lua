...
function HappyErnieData:GetOpenTakeTimeCfg()
	...
	happy_ernie_cfg = ActivityData.Instance:GetRandActivityConfig(cfg.huanleyaojiang, ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_HAPPYERNIE) or {}
	return happy_ernie_cfg
end
...
function HappyErnieData:GetHappyErnieRewardConfig()
	...
	reward_cfg = ActivityData.Instance:GetRandActivityConfig(cfg.huanleyaojiang_reward, ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_HAPPYERNIE) or {}
	return reward_cfg
end
...
function HappyErnieData:GetHappyErnieRewardItemConfig()
	...
	for i, v in ipairs(happy_ernie_activity_reward_cfg) do
		if self:GetIsFetchFlag(v.index) then		--self.reward_flag[self.reward_list_length - i + 1] == 1
...
function HappyErnieData:SetRAHappyErnieInfo(protocol)
	self.happy_ernie_next_free_tao_timestamp = protocol.ra_huanleyaojiang_next_free_tao_timestamp
...
function HappyErnieData:SetRAHappyErnieTaoResultInfo(protocol)
    ...
    self.happy_ernie_tao_seq = protocol.huanleyaojiang_tao_seq
end
...
function HappyErnieData:FlushHallRedPoindRemind()
	...
	if remind_num > 0 then
		ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_HAPPYERNIE, true)
	else
		ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_HAPPYERNIE, not self.is_open)
	end
end

function HappyErnieData:SetIsOpen()
	self.is_open = true
end