...
function HappyErnieCtrl:ActivityChange(activity_type, status, next_time, open_type)
	if activity_type == ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_HAPPYERNIE then
		-- 活动开启之后才请求
		if status == ACTIVITY_STATUS.OPEN then
			KaifuActivityCtrl.Instance:SendRandActivityOperaReq(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_HAPPYERNIE, RA_HUANLE_YAOJIANG_OPERA_TYPE.RA_HUANLEYAOJIANG_OPERA_TYPE_QUERY_INFO, 0, 0)
		end
	end
end
...
function HappyErnieCtrl:RemindChangeCallBack(remind_name, num)
	if remind_name == RemindName.HappyErnieRemind and ActivityData.Instance:GetActivityIsOpen(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_HAPPYERNIE)  then
