...
function LuckyDrawData:__init()
	...
	RemindManager.Instance:Register(RemindName.RemindLuckyDraw, BindTool.Bind(self.GetLuckyDrawRemind, self))
end

function LuckyDrawData:GetLuckyDrawRemind()
	local num = 0
	local cfg = ServerActivityData.Instance:GetCurrentRandActivityConfig().other
	if cfg and cfg[1] then
		num = cfg[1].tianming_free_chou_times - self.free_chou_times
	end
	return num
end

function LuckyDrawData:__delete()
	self.first_remind_num = 0
	RemindManager.Instance:UnRegister(RemindName.RemindLuckyDraw)
	LuckyDrawData.Instance = nil
end

function LuckyDrawData:SetLuckyDrawInfo(protocol)
	self.total_add_lot = protocol.total_add_lot
	...
	-- ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_LUCKYDRAW, self.free_chou_times < 3)
end
...
function LuckyDrawData:GetFreeChouTimes()
	return self.free_chou_times
end

function LuckyDrawData:GetAddLotTimes()
	return self.total_add_lot
end
...
function LuckyDrawData:GetNeedPayMoney()
	...
	local need_pay_money = 0
	if self:GetConsumeCfg(add_num) and self:GetConsumeCfg(add_num).chou_consume_gold then
		need_pay_money = self:GetConsumeCfg(add_num).chou_consume_gold
	end
