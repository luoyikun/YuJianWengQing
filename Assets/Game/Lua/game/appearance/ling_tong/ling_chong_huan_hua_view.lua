-- 仙域-外观-灵童幻化-AppearanceHuanHuaContent
...
function LingChongHuanHuaView:__init()
	...
	self.def_index = TabIndex.appearance_lingtong
...
function LingChongHuanHuaView:LoadCallBack()
	local tab_cfg = {
		{name = Language.Common.Huanhua, bundle = "uis/images_atlas", asset = "tab_icon_lingtong", tab_index = TabIndex.appearance_lingtong, remind_id = RemindName.LingChongHuanHua},
	}
	...
	RemindManager.Instance:Fire(RemindName.LingChongHuanHua)
...
function LingChongHuanHuaView:OnRoleDrag(data)
	if UIScene.role_model then
		UIScene:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function LingChongHuanHuaView:SetRendering(value)
...
function LingChongHuanHuaView:ClickAddGold()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function LingChongHuanHuaView:ClickClose()
	self:Close()

	local ling_chong_view = nil
	if AppearanceCtrl.Instance.view then
		ling_chong_view = AppearanceCtrl.Instance.view:GetLingChongView()
	end

	if ling_chong_view and ling_chong_view:IsOpen() then
		ling_chong_view:UITween()
	end
end

--点击目标战力
function LingChongHuanHuaView:ClickSuperPower()
	local cfg = self.list_data[self.select_index]
	local image_id = cfg and cfg.image_id
	local index = image_id or 0
	local data = LingChongData.Instance:GetSpecialHuanHuaShowData(index)
...
function LingChongHuanHuaView:ClickUpGrade()
	...
	local is_active = LingChongData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local huanhua_cfg = LingChongData.Instance:GetHuanHuaCfgInfo(data.image_id)
	...
	if not is_active then
		...
		UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_TONG, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_IMAGE_UPGRADE, data.image_id)
	end
end
...
function LingChongHuanHuaView:ClickUsed()
	...
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_TONG, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, data.image_id)
end
...
function LingChongHuanHuaView:FlushContent()
	...
	local now_level = LingChongData.Instance:GetHuanHuaGrade(data.image_id)
	...
	local attr0 = LingChongData.Instance:GetHuanHuaCfgInfo(data.image_id)
	local attr1 = LingChongData.Instance:GetHuanHuaCfgInfo(data.image_id, now_level + 1)
	local max_grade = LingChongData.Instance:GetSpecialImageMaxUpLevelById(data.image_id)
	self:SetAttr(now_level, attr0, attr1, max_grade, data.image_id)

	local is_show_super = LingChongData.Instance:IsShowSuperPower(data.image_id)
	local is_active_super = LingChongData.Instance:GetStarIsShowSuperPower(data.image_id)
	...
	if is_show_super and not is_active_super then
		local need_reach_level = LingChongData.Instance:GetActiveSuperPowerNeedLevel(data.image_id)
	...
	local is_active = LingChongData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local is_used = LingChongData.Instance:GetHuanHuaIdIsUsed(data.image_id)
...
function LingChongHuanHuaView:SetAttr(special_image_grade, attr0, attr1, max_grade, special_index)
	...
	local active_grade, attr_type, attr_value = LingChongData.Instance:GetHuanHuaSpecialAttrActiveType(nil, special_index)
	if active_grade and attr_type and attr_value then
		if special_image_grade < active_grade then
			...
			for i = special_image_grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = LingChongData.Instance:GetHuanHuaSpecialAttrActiveType(i, special_index)
...
function LingChongHuanHuaView:FlushModel()
	...
	transform.rotation = Quaternion.Euler(4, -172, 0)
	UIScene:SetCameraTransform(transform)
	UIScene:SetTerraceBg(nil, nil, {position = Vector3(-136, -300, 0)}, nil)

	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetLingChongModel(data.res_id_h)
...
function LingChongHuanHuaView:ShowIndexCallBack(index, index_nodes)
...
function LingChongHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function LingChongHuanHuaView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "all" then
			if v and v.id then
				local index, num = LingChongData.Instance:CanHuanhuaIndexByImageId(v.id)
				if index then
					self.select_index = index
					local cfg_list = LingChongData.Instance:GetHuanHuaCfgList()
...
function LingChongHuanHuaView:UITween()
...
function LingChongHuanHuaCell:ListenClick(handler)
...
function LingChongHuanHuaCell:SetData(data)
	if nil == data then
		return
	end
	...
	if LingChongData.Instance:GetHuanHuaGrade(data.image_id) < LingChongData.Instance:GetSpecialImgMaxLevel() and
