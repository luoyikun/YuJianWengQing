...
function LingChongContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self.medal_skill_index = LingChongData.Instance:GetMedalIndex()
	self:GetSkill()
end

function LingChongContentView:LoadCallBack(instance)
...
function LingChongContentView:CloseCallBack()
	if self.count_down ~= nil then
		CountDown.Instance:RemoveCountDown(self.count_down)
		self.count_down = nil
	end
...
function LingChongContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = LingChongData.Instance:GetSkillCfgById(i - 1) or {}
		if self.cur_data and next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = LingChongData.Instance:GetSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetLingTongSkillIcon(i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickLingChongSkill, self, i))
	end
end
...
-- 点击头饰成长按钮
function LingChongContentView:ClickGrowup()
	local lingchong_info = LingChongData.Instance:GetLingChongInfo()
	if nil == lingchong_info or nil == lingchong_info.grade then return end
	local is_activie = lingchong_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "lingchongchengzhang", {item_id = LingChongShuXingDanId.ChengZhangDanId})
	end
end
...
function LingChongContentView:ClickZiZhi()
	local lingchong_info = LingChongData.Instance:GetLingChongInfo()
	if nil == lingchong_info or nil == lingchong_info.grade then return end
	local is_activie = lingchong_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "lingchong_zizhi", {item_id = LingChongShuXingDanId.ZiZhiDanId})
	end
end

-- 点击进阶装备
function LingChongContentView:OnClickEquipBtn()
	local lingchong_info = LingChongData.Instance:GetLingChongInfo()
	if nil ~= lingchong_info and nil ~= lingchong_info.grade then
		if lingchong_info.grade <= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL then
			...
			ViewManager.Instance:Open(ViewName.AppearacneEquipView, TabIndex.appearance_lingtong)
		end
	end
end

-- 点击头饰技能
function LingChongContentView:ClickLingChongSkill(index)
	if index == self.medal_skill_index + 1 then
		ViewManager.Instance:Open(ViewName.TipXunZhangView, nil, "lingchongskill", {index = index - 1})
	else
		if self.is_shake_skill and index == 1 then
			self.node_list["Special"].animator:SetBool("IsShake", false)
			self.is_shake_skill = nil
		end
		ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "lingchongskill", {index = index - 1})
	end
end
...
function LingChongContentView:ClickUse()
	...
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_TONG, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, grade_info.image_id)
end
...
function LingChongContentView:ClickUpgrade(is_auto_upgrade)
	local lingchong_info = LingChongData.Instance:GetLingChongInfo()
	if nil == lingchong_info then return end
	...
	local close_func = function()
		...
		if not is_auto_upgrade then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_TONG, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_TONG, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE,grade_info.pack_num, is_auto_buy)
		end
	end
	...
	local is_have_zhishengdan, item_id = LingChongData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, lingchong_info.grade), ok_callback, close_func)
	    return
	end
...
function LingChongContentView:OpenCallBack()
	...
	if nil ~= lingchong_info then
		...
		if lingchong_info.grade == 1 then
			...
			local is_huanhua_img = LingChongData.Instance:IsHuanHuaImage(lingchong_info.used_imageid)
			if is_huanhua_img then
				...
				local image_cfg = LingChongData.Instance:GetLingChongImageCfgInfoByImageId(lingchong_info.used_imageid)
				if nil == image_cfg then return end
				-- show_grade为客户端阶数由0开始，服务器阶数由1开始，所以要加1
				self.select_grade = image_cfg.show_grade + 1
			end
...
function LingChongContentView:FlushSkillIcon()
	local lingchong_info = LingChongData.Instance:GetLingChongInfo() or {}
	local lingchong_skill_list = lingchong_info.skill_level_list
	if nil == lingchong_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node, lingchong_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = LingChongData.Instance:GetSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = LingChongData.Instance:GetSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local lingchong_info = LingChongData.Instance:GetLingChongInfo()
	if lingchong_info ~= nil or next(lingchong_info) ~= nil then
		if lingchong_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end

	local cur_skill_data = LingChongData.Instance:GetSkillCfgById(self.medal_skill_index) or {}
	...
	local next_data_cfg = LingChongData.Instance:GetSkillCfgById(self.medal_skill_index, cur_grade + 1)
...
function LingChongContentView:FlushModel()
	...
	local image_cfg = LingChongData.Instance:GetLingChongImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>灵童_ID:" .. grade_info.image_id .. "</color>")
...
function LingChongContentView:FlushLeft()
	...
	-- 刷新使用状态
	local is_used = temp_grade_info.image_id == lingchong_info.used_imageid
	...
	-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = LingChongData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(LingChongData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(LingChongData.Instance:IsShowGrowupRemind())
	self.node_list["ShowEquipRemind"]:SetActive(LingChongData.Instance:CalAllEquipRemind() > 0)
	local is_show_huanhua_remind = LingChongData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(lingchong_info)
	self:SetBtnGrowup(lingchong_info)
	self:SetBtnEquip(lingchong_info)
end
...
function LingChongContentView:FlushRight()
	...
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = LingChongData.Instance:GetLingChongGradeCfgInfoByGrade(lingchong_info.grade + 1)
	local bless_value_ratio = lingchong_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = LingChongData.Instance:GetLingChongMaxGrade()
	local active_grade, attr_type, attr_value = LingChongData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if lingchong_info.grade < active_grade then
			...
			for i = lingchong_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = LingChongData.Instance:GetSpecialAttrActiveType(i)
	...
	-- -- 特殊属性
	-- -- UI:SetGraphicGrey(self.node_list["TxtSpecialAttr"], grade_info.shanbi == 0)
	-- local str_special_attr = ""
	-- if grade_info.shanbi == 0 then
	-- 	str_special_attr = string.format(Language.MultiMount.SpecialAttribute[2] .. Language.MultiMount.LevelStart, 5)
	-- else
	-- 	local level, add_attr = LingChongData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	-- 	if level and add_attr then
	-- 		str = string.format(Language.Advance.NextAttr, CommonDataManager.GetDaXie(level - 1), add_attr/100)
	-- 	end
	-- 	str_special_attr = string.format(Language.MultiMount.SpecialAttribute[2], grade_info.shanbi / 100) .. str
	-- end
	...
	-- if grade_info and next_grade_info and lingchong_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", lingchong_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(lingchong_info, grade_info)
end

--祝福值提示 清空祝福值 清空时间
function LingChongContentView:ClearBlessTip(lingchong_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if lingchong_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = lingchong_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function LingChongContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end
...
-- 设置进阶按钮状态
function LingChongContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function LingChongContentView:SetBtnZiZhi(lingchong_info)
 	local is_activie = lingchong_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function LingChongContentView:SetBtnGrowup(lingchong_info)
	local is_activie = lingchong_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
-- 设置装备按钮状态
function LingChongContentView:SetBtnEquip(lingchong_info)
	local is_activie = lingchong_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL
	UI:SetGraphicGrey(self.node_list["BtnEquipButton"], not is_activie)
	self.node_list["TextEquip"].text.text = is_activie and Language.MultiMount.LingChongEquipType or 
...
function LingChongContentView:FlushItem()
	...
	self.node_list["RemindBtn"]:SetActive(LingChongData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end
...
function LingChongContentView:FlushRestChallengeTime()
...
function LingChongContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function LingChongContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_LINGCHONG
...
function LingChongContentView:ClickActIcon()
...
function LingChongContentView:SetActTime()
...
function LingChongContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function LingChongContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function LingChongContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function LingChongContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function LingChongContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function LingChongContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function LingChongContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function LingChongContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function LingChongContentView:RemoveCountDown()
...
--打开大目标面板
function LingChongContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_LINGCHONG)
end

--打开小目标面板
function LingChongContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_LINGCHONG
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_LINGCHONG, callback)
