...
function LingQiContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self:GetSkill()
end

function LingQiContentView:LoadCallBack(instance)
...
function LingQiContentView:CloseCallBack()
	if self.count_down ~= nil then
		CountDown.Instance:RemoveCountDown(self.count_down)
		self.count_down = nil
	end
...
function LingQiContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = LingQiData.Instance:GetSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = LingQiData.Instance:GetSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetLingQiSkillIcon(i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickLingQiSkill, self, i))
	end
end
...
-- 点击灵骑成长按钮
function LingQiContentView:ClickGrowup()
	local lingqi_info = LingQiData.Instance:GetLingQiInfo()
	if nil == lingqi_info or nil == lingqi_info.grade then return end
	local is_activie = lingqi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "lingqichengzhang", {item_id = LingQiShuXingDanId.ChengZhangDanId})
	end
end
...
function LingQiContentView:ClickZiZhi()
	local lingqi_info = LingQiData.Instance:GetLingQiInfo()
	if nil == lingqi_info or nil == lingqi_info.grade then return end
	local is_activie = lingqi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "lingqi_zizhi", {item_id = LingQiShuXingDanId.ZiZhiDanId})
	end
end

-- 点击进阶装备
function LingQiContentView:OnClickEquipBtn()
	local lingqi_info = LingQiData.Instance:GetLingQiInfo()
	if nil ~= lingqi_info and nil ~= lingqi_info.grade then
		if lingqi_info.grade <= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL then
			...
			ViewManager.Instance:Open(ViewName.AppearacneEquipView, TabIndex.appearance_lingqi)
		end
	end
end

-- 点击灵骑技能
function LingQiContentView:ClickLingQiSkill(index)
	if self.is_shake_skill and index == 1 then
		self.node_list["Special"].animator:SetBool("IsShake", false)
		self.is_shake_skill = nil
	end
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "lingqiskill", {index = index - 1})
end
...
function LingQiContentView:ClickUse()
	...
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_QI, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, grade_info.image_id)
end
...
function LingQiContentView:ClickUpgrade(is_auto_upgrade)
	local lingqi_info = LingQiData.Instance:GetLingQiInfo()
	if nil == lingqi_info then return end
	...
	local close_func = function()
		...
		if not is_auto_upgrade then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_QI, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_QI, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE,grade_info.pack_num, is_auto_buy)
		end
	end
	...
	local is_have_zhishengdan, item_id = LingQiData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, lingqi_info.grade), ok_callback, close_func)
	    return
	end
...
function LingQiContentView:OpenCallBack()
	...
	if nil ~= lingqi_info then
		local is_huanhua_img = LingQiData.Instance:IsHuanHuaImage(lingqi_info.used_imageid)
		--默认选择已使用的形象阶数
		if lingqi_info.grade == 1 then
			--零阶和一阶使用相同形象
			self.select_grade = lingqi_info.grade
		else
			-- if lingqi_info.used_imageid > GameEnum.MOUNT_SPECIAL_IMA_ID then			-- 使用了幻化形象
			-- 	self.select_grade = lingqi_info.grade
			if is_huanhua_img then
				...
				local image_cfg = LingQiData.Instance:GetLingQiImageCfgInfoByImageId(lingqi_info.used_imageid)
				if nil == image_cfg then return end
				-- show_grade为客户端阶数由0开始，服务器阶数由1开始，所以要加1
				self.select_grade = image_cfg.show_grade + 1
			end
...
function LingQiContentView:FlushSkillIcon()
	local lingqi_skill_list = LingQiData.Instance:GetLingQiInfo().skill_level_list

	if nil == lingqi_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node, lingqi_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = LingQiData.Instance:GetSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = LingQiData.Instance:GetSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local lingqi_info = LingQiData.Instance:GetLingQiInfo()
	if lingqi_info ~= nil or next(lingqi_info) ~= nil then
		if lingqi_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end
...
function LingQiContentView:FlushModel()
	...
	local image_cfg = LingQiData.Instance:GetLingQiImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>灵骑_ID:" .. grade_info.image_id .. "</color>")
...
function LingQiContentView:FlushLeft()
	...
	-- 刷新使用状态
	local is_used = temp_grade_info.image_id == lingqi_info.used_imageid
	...
	-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = LingQiData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(LingQiData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(LingQiData.Instance:IsShowGrowupRemind())
	self.node_list["ShowEquipRemind"]:SetActive(LingQiData.Instance:CalAllEquipRemind() > 0)
	local is_show_huanhua_remind = LingQiData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(lingqi_info)
	self:SetBtnGrowup(lingqi_info)
	self:SetBtnEquip(lingqi_info)
end
...
function LingQiContentView:FlushRight()
	...
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = LingQiData.Instance:GetLingQiGradeCfgInfoByGrade(lingqi_info.grade + 1)
	local bless_value_ratio = lingqi_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = LingQiData.Instance:GetLingQiMaxGrade()
	local active_grade, attr_type, attr_value = LingQiData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if lingqi_info.grade < active_grade then
			...
			for i = lingqi_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = LingQiData.Instance:GetSpecialAttrActiveType(i)
	...
	-- else
	-- 	local level, add_attr = LingQiData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	...
	-- if grade_info and next_grade_info and lingqi_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", lingqi_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(lingqi_info, grade_info)
end

--祝福值提示 清空祝福值 清空时间
function LingQiContentView:ClearBlessTip(lingqi_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if lingqi_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = lingqi_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function LingQiContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end
...
-- 设置进阶按钮状态
function LingQiContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function LingQiContentView:SetBtnZiZhi(lingqi_info)
 	local is_activie = lingqi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function LingQiContentView:SetBtnGrowup(lingqi_info)
	local is_activie = lingqi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
-- 设置装备按钮状态
function LingQiContentView:SetBtnEquip(lingqi_info)
	local is_activie = lingqi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL
	UI:SetGraphicGrey(self.node_list["BtnEquipButton"], not is_activie)
	self.node_list["TextEquip"].text.text = is_activie and Language.MultiMount.LingQiEquipType or 
...
function LingQiContentView:FlushItem()
	...
	self.node_list["RemindBtn"]:SetActive(LingQiData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end
...
function LingQiContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function LingQiContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_LINGQI
...
function LingQiContentView:ClickActIcon()
...
function LingQiContentView:SetActTime()
...
function LingQiContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function LingQiContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function LingQiContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function LingQiContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function LingQiContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function LingQiContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function LingQiContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function LingQiContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function LingQiContentView:RemoveCountDown()
...
--打开大目标面板
function LingQiContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_LINGQI)
end

--打开小目标面板
function LingQiContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_LINGQI
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_LINGQI, callback)
