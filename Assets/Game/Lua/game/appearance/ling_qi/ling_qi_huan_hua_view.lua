-- 仙域-外观-灵骑幻化-AppearanceHuanHuaContent
LingQiHuanHuaView = LingQiHuanHuaView or BaseClass(BaseView)

function LingQiHuanHuaView:__init()
	self.ui_config = {
	...
	self.def_index = TabIndex.appearance_lingqi_huanhua
...
function LingQiHuanHuaView:__delete()

end

function LingQiHuanHuaView:ReleaseCallBack()
...
function LingQiHuanHuaView:LoadCallBack()
	local tab_cfg = {
		{name = Language.Common.Huanhua, bundle = "uis/images_atlas", asset = "tab_icon_lingqi", tab_index = TabIndex.appearance_lingqi_huanhua, remind_id = RemindName.LingQiHuanHua},
	}
	...
	RemindManager.Instance:Fire(RemindName.LingQiHuanHua)
...
function LingQiHuanHuaView:OnRoleDrag(data)
	if UIScene.role_model then
		UIScene:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function LingQiHuanHuaView:SetRendering(value)
...
function LingQiHuanHuaView:ClickAddGold()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function LingQiHuanHuaView:ClickClose()
	self:Close()

	local ling_qi_view = nil
	if AppearanceCtrl.Instance.view then
		ling_qi_view = AppearanceCtrl.Instance.view:GetLingQiView()
	end

	if ling_qi_view and ling_qi_view:IsOpen() then
		ling_qi_view:UITween()
	end
end

--点击目标战力
function LingQiHuanHuaView:ClickSuperPower()
	local cfg = self.list_data[self.select_index]
	local image_id = cfg and cfg.image_id
	local index = image_id or 0
	local data = LingQiData.Instance:GetSpecialHuanHuaShowData(index)
	TipsCtrl.Instance:ShowSpecialHuanHuaViewView(data)
end

-- 激活升级形象
function LingQiHuanHuaView:ClickUpGrade()
	...
	local is_active = LingQiData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local huanhua_cfg = LingQiData.Instance:GetHuanHuaCfgInfo(data.image_id)
	...
	if not is_active then
		...
		UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_QI, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_IMAGE_UPGRADE, data.image_id)
	end
end

-- 使用形象
function LingQiHuanHuaView:ClickUsed()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_QI, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, data.image_id)
end

function LingQiHuanHuaView:GetNumberOfCells()
	return #self.list_data
end

function LingQiHuanHuaView:RefreshCell(cell, data_index)
	data_index = data_index + 1
	
	local huanhua_cell = self.cell_list[cell]
	if nil == huanhua_cell then
		huanhua_cell = LingQiHuanHuaCell.New(cell.gameObject)
...
function LingQiHuanHuaView:ClickItem(cell)
	if nil == cell then
		return
	end
...
function LingQiHuanHuaView:FlushView()
	self:FlushContent()
end

function LingQiHuanHuaView:FlushContent()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	...
	local now_level = LingQiData.Instance:GetHuanHuaGrade(data.image_id)
	...
	local attr0 = LingQiData.Instance:GetHuanHuaCfgInfo(data.image_id)
	local attr1 = LingQiData.Instance:GetHuanHuaCfgInfo(data.image_id, now_level + 1)
	local max_grade = LingQiData.Instance:GetSpecialImageMaxUpLevelById(data.image_id)
	self:SetAttr(now_level, attr0, attr1, max_grade, data.image_id)

	local is_show_super = LingQiData.Instance:IsShowSuperPower(data.image_id)
	local is_active_super = LingQiData.Instance:GetStarIsShowSuperPower(data.image_id)
	...
	if is_show_super and not is_active_super then
		local need_reach_level = LingQiData.Instance:GetActiveSuperPowerNeedLevel(data.image_id)
	...
	local max_level = LingQiData.Instance:GetSpecialImgMaxLevel()
	local is_active = LingQiData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local is_used = LingQiData.Instance:GetHuanHuaIdIsUsed(data.image_id)
...
function LingQiHuanHuaView:SetAttr(special_image_grade, attr0, attr1, max_grade, special_index)
	...
	local active_grade, attr_type, attr_value = LingQiData.Instance:GetHuanHuaSpecialAttrActiveType(nil, special_index)
	if active_grade and attr_type and attr_value then
		if special_image_grade < active_grade then
			...
			for i = special_image_grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = LingQiData.Instance:GetHuanHuaSpecialAttrActiveType(i, special_index)
...
function LingQiHuanHuaView:FlushModel()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	...
	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetLingQiModel(data.res_id, true)
...
function LingQiHuanHuaView:OpenCallBack()
	self.select_index = 1
	self:Flush()
	self.list_data = LingQiData.Instance:GetHuanHuaCfgList() or {}
...
function LingQiHuanHuaView:CloseCallBack()
	if nil ~= self.data_listen then
...
function LingQiHuanHuaView:ShowIndexCallBack(index, index_nodes)
...
function LingQiHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function LingQiHuanHuaView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "all" then
			if v and v.id then
				local index, num = LingQiData.Instance:CanHuanhuaIndexByImageId(v.id)
				if index then
					self.select_index = index
					local cfg_list = LingQiData.Instance:GetHuanHuaCfgList()
...
function LingQiHuanHuaView:UITween()
...
---------------------------------------------------------------------------------------------------
LingQiHuanHuaCell = LingQiHuanHuaCell or BaseClass(BaseCell)
function LingQiHuanHuaCell:__init()

end

function LingQiHuanHuaCell:__delete()
	
end

function LingQiHuanHuaCell:ListenClick(handler)
	self.node_list["HuanHuaItem"].toggle:AddClickListener(handler)
end

function LingQiHuanHuaCell:SetToggleGroup(group)
	self.root_node.toggle.group = group
end

function LingQiHuanHuaCell:SetToggleIsOn(is_on)
	self.root_node.toggle.isOn = is_on
end

function LingQiHuanHuaCell:SetData(data)
	if nil == data then
		return
	end
	...
	if LingQiData.Instance:GetHuanHuaGrade(data.image_id) < LingQiData.Instance:GetSpecialImgMaxLevel() and
