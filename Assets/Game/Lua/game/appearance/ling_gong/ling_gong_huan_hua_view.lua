-- 仙域-外观-灵弓幻化-AppearanceHuanHuaContent
LingGongHuanHuaView = LingGongHuanHuaView or BaseClass(BaseView)

function LingGongHuanHuaView:__init()
	self.ui_config = {
	...
	self.def_index = TabIndex.appearance_linggong_huanhua
...
function LingGongHuanHuaView:__delete()
	
end

function LingGongHuanHuaView:ReleaseCallBack()
...
function LingGongHuanHuaView:LoadCallBack()
	local tab_cfg = {
		{name = Language.Common.Huanhua, bundle = "uis/images_atlas", asset = "tab_icon_linggong", tab_index = TabIndex.appearance_linggong_huanhua, remind_id = RemindName.LingGongHuanHua},
	}
	...
	RemindManager.Instance:Fire(RemindName.LingGongHuanHua)
...
function LingGongHuanHuaView:OnRoleDrag(data)
	if UIScene.role_model then
		UIScene:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function LingGongHuanHuaView:SetRendering(value)
...
function LingGongHuanHuaView:ClickAddGold()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function LingGongHuanHuaView:ClickClose()
	self:Close()

	local ling_gong_view = nil
	if AppearanceCtrl.Instance.view then
		ling_gong_view = AppearanceCtrl.Instance.view:GetLingGongView()
	end

	if ling_gong_view and ling_gong_view:IsOpen() then
		ling_gong_view:UITween()
	end
end

--点击目标战力
function LingGongHuanHuaView:ClickSuperPower()
	local cfg = self.list_data[self.select_index]
	local image_id = cfg and cfg.image_id
	local index = image_id or 0
	local data = LingGongData.Instance:GetSpecialHuanHuaShowData(index)
	TipsCtrl.Instance:ShowSpecialHuanHuaViewView(data)
end

-- 激活升级形象
function LingGongHuanHuaView:ClickUpGrade()
	...
	local is_active = LingGongData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local huanhua_cfg = LingGongData.Instance:GetHuanHuaCfgInfo(data.image_id)
	...
	if not is_active then
		...
		UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_GONG, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_IMAGE_UPGRADE, data.image_id)
	end
end

-- 使用形象
function LingGongHuanHuaView:ClickUsed()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_GONG, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, data.image_id)
end

function LingGongHuanHuaView:GetNumberOfCells()
	return #self.list_data
end

function LingGongHuanHuaView:RefreshCell(cell, data_index)
	data_index = data_index + 1
	
	local huanhua_cell = self.cell_list[cell]
	if nil == huanhua_cell then
		huanhua_cell = LingGongHuanHuaCell.New(cell.gameObject)
...
function LingGongHuanHuaView:ClickItem(cell)
	if nil == cell then
		return
	end
...
function LingGongHuanHuaView:FlushView()
	self:FlushContent()
end

function LingGongHuanHuaView:FlushContent()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	...
	local now_level = LingGongData.Instance:GetHuanHuaGrade(data.image_id)
	...
	local attr0 = LingGongData.Instance:GetHuanHuaCfgInfo(data.image_id)
	local attr1 = LingGongData.Instance:GetHuanHuaCfgInfo(data.image_id, now_level + 1)
	local max_grade = LingGongData.Instance:GetSpecialImageMaxUpLevelById(data.image_id)
	self:SetAttr(now_level, attr0, attr1, max_grade, data.image_id)

	local is_show_super = LingGongData.Instance:IsShowSuperPower(data.image_id)
	local is_active_super = LingGongData.Instance:GetStarIsShowSuperPower(data.image_id)
	...
	if is_show_super and not is_active_super then
		local need_reach_level = LingGongData.Instance:GetActiveSuperPowerNeedLevel(data.image_id)
	...
	local max_level = LingGongData.Instance:GetSpecialImgMaxLevel()
	local is_active = LingGongData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local is_used = LingGongData.Instance:GetHuanHuaIdIsUsed(data.image_id)
...
function LingGongHuanHuaView:SetAttr(special_image_grade, attr0, attr1, max_grade, special_index)
	...
	local active_grade, attr_type, attr_value = LingGongData.Instance:GetHuanHuaSpecialAttrActiveType(nil, special_index)
	if active_grade and attr_type and attr_value then
		if special_image_grade < active_grade then
			...
			for i = special_image_grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = LingGongData.Instance:GetHuanHuaSpecialAttrActiveType(i, special_index)
...
function LingGongHuanHuaView:FlushModel()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	...
	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetLingGongModel(data.res_id_h)
...
function LingGongHuanHuaView:OpenCallBack()
	self.select_index = 1
	self:Flush()
	self.list_data = LingGongData.Instance:GetHuanHuaCfgList() or {}
...
function LingGongHuanHuaView:CloseCallBack()
	if nil ~= self.data_listen then
...
function LingGongHuanHuaView:ShowIndexCallBack(index, index_nodes)
...
function LingGongHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function LingGongHuanHuaView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "all" then
			if v and v.id then
				local index, num = LingGongData.Instance:CanHuanhuaIndexByImageId(v.id)
				if index then
					self.select_index = index
					local cfg_list = LingGongData.Instance:GetHuanHuaCfgList()
...
function LingGongHuanHuaView:UITween()
...
---------------------------------------------------------------------------------------------------
LingGongHuanHuaCell = LingGongHuanHuaCell or BaseClass(BaseCell)
function LingGongHuanHuaCell:__init()

end

function LingGongHuanHuaCell:__delete()
	
end

function LingGongHuanHuaCell:ListenClick(handler)
	self.node_list["HuanHuaItem"].toggle:AddClickListener(handler)
end

function LingGongHuanHuaCell:SetToggleGroup(group)
	self.root_node.toggle.group = group
end

function LingGongHuanHuaCell:SetToggleIsOn(is_on)
	self.root_node.toggle.isOn = is_on
end

function LingGongHuanHuaCell:SetData(data)
	if nil == data then
		return
	end
	...
	if LingGongData.Instance:GetHuanHuaGrade(data.image_id) < LingGongData.Instance:GetSpecialImgMaxLevel() and
