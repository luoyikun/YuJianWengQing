...
function WaistContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self:GetSkill()
end

function WaistContentView:LoadCallBack(instance)
...
function WaistContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = WaistData.Instance:GetYaoShiSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = WaistData.Instance:GetYaoShiSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetYaoShiSkillIcon(i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickYaoShiSkill, self, i))
	end
end
...
-- 点击腰饰成长按钮
function WaistContentView:ClickGrowup()
	local yaoshi_info = WaistData.Instance:GetYaoShiInfo()
	if nil == yaoshi_info or nil == yaoshi_info.grade then return end
	local is_activie = yaoshi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "waistchengzhang", {item_id = YaoShiShuXingDanId.ChengZhangDanId})
	end
end
...
function WaistContentView:ClickZiZhi()
	local yaoshi_info = WaistData.Instance:GetYaoShiInfo()
	if nil == yaoshi_info or nil == yaoshi_info.grade then return end
	local is_activie = yaoshi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "waist_zizhi", {item_id = YaoShiShuXingDanId.ZiZhiDanId})
	end
end

-- 点击腰饰技能
function WaistContentView:ClickYaoShiSkill(index)
	if self.is_shake_skill and index == 1 then
		self.node_list["Special"].animator:SetBool("IsShake", false)
		self.is_shake_skill = nil
	end
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "yaoshiskill", {index = index - 1})
end
...
function WaistContentView:ClickUse()
	...
	WaistCtrl.Instance:SendYaoShiReq(YAOSHI_OPERA_TYPE.YAOSHI_OPERA_TYPE_USE_IMAGE, grade_info.image_id)
end
...
function WaistContentView:ClickUpgrade(is_auto_upgrade)
	local yaoshi_info = WaistData.Instance:GetYaoShiInfo()
	if nil == yaoshi_info then return end
	...
	local close_func = function()
		...
		if not is_auto_upgrade then
			WaistCtrl.Instance:SendYaoShiReq(YAOSHI_OPERA_TYPE.YAOSHI_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			WaistCtrl.Instance:SendYaoShiReq(YAOSHI_OPERA_TYPE.YAOSHI_OPERA_TYPE_UPGRADE, grade_info.pack_num, is_auto_buy)
		end
	end
	...
	if KaifuActivityData.Instance:IsOpenAdvanceReturnActivity() and self.is_can_tip then
		local open_advance_one = KaifuActivityData.Instance:GetOpenAdvanceType(TYPE_UPGRADE_RETURN.WAIST_UPGRADE_RETURN)
		local open_advance_two = KaifuActivityData.Instance:GetOpenAdvanceTypeTwo(TYPE_UPGRADE_RETURN.WAIST_UPGRADE_RETURN)
	...
	local is_have_zhishengdan, item_id = WaistData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, yaoshi_info.grade), ok_callback, close_func)
	    return
	end
...
function WaistContentView:OpenCallBack()
	...
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.WAIST)
...
function WaistContentView:FlushSkillIcon()
	local yaoshi_skill_list = WaistData.Instance:GetYaoShiInfo().skill_level_list

	if nil == yaoshi_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node, yaoshi_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = WaistData.Instance:GetYaoShiSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = WaistData.Instance:GetYaoShiSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local yaoshi_info = WaistData.Instance:GetYaoShiInfo()
	if yaoshi_info ~= nil or next(yaoshi_info) ~= nil then
		if yaoshi_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end
...
function WaistContentView:FlushModel()
	...
	local grade_info = WaistData.Instance:GetWaistGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end
	local image_cfg = WaistData.Instance:GetWaistImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>腰饰_ID:" .. grade_info.image_id .. "</color>")

	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetWaistModel(image_cfg.res_id)
...
function WaistContentView:FlushLeft()
	...
	-- 刷新使用状态
	local is_used = temp_grade_info.image_id == yaoshi_info.used_imageid
	...
	self.node_list["BtnRightButton"]:SetActive(is_show_right)
		...
		-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = WaistData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(WaistData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(WaistData.Instance:IsShowGrowupRemind())
	local is_show_huanhua_remind = WaistData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(yaoshi_info)
	self:SetBtnGrowup(yaoshi_info)
end
...
function WaistContentView:FlushRight()
	...
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = WaistData.Instance:GetWaistGradeCfgInfoByGrade(yaoshi_info.grade + 1)
	local bless_value_ratio = yaoshi_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = WaistData.Instance:GetYaoShiMaxGrade()
	local active_grade, attr_type, attr_value = WaistData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if yaoshi_info.grade < active_grade then
			...
			for i = yaoshi_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = WaistData.Instance:GetSpecialAttrActiveType(i)
	...
	-- else
	-- 	local level, add_attr = WaistData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	...
	-- if grade_info and next_grade_info and yaoshi_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", yaoshi_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(yaoshi_info, grade_info)
end

-- 设置进阶按钮状态
function WaistContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function WaistContentView:SetBtnZiZhi(yaoshi_info)
 	local is_activie = yaoshi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function WaistContentView:SetBtnGrowup(yaoshi_info)
	local is_activie = yaoshi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
--祝福值提示 清空祝福值 清空时间
function WaistContentView:ClearBlessTip(yaoshi_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if yaoshi_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = yaoshi_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function WaistContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end
...
function WaistContentView:FlushItem()
	...
	self.node_list["RemindBtn"]:SetActive(WaistData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end
...
function WaistContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function WaistContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_YAOSHI
	...
	-- local yaoshi_info = WaistData.Instance:GetYaoShiInfo()
	-- if yaoshi_info == nil or yaoshi_info.grade == nil then
	-- 	return
	-- end
	
	-- local cur_img_grade = yaoshi_info.grade - 1
...
function WaistContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function WaistContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function WaistContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function WaistContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function WaistContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function WaistContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function WaistContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function WaistContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function WaistContentView:RemoveCountDown()
...
--打开大目标面板
function WaistContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_YAOSHI)
end

--打开小目标面板
function WaistContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_YAOSHI
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_YAOSHI, callback)
	TipsCtrl.Instance:ShowTimeLimitTitleView(data)
end

function WaistContentView:ToggleOnClick()
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.WAIST) == 0 and 1 or 0
	SettingData.Instance:SetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.WAIST, flag)
	self.node_list["HighLight"]:SetActive(flag == 1)
	if flag == 1 then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.Hide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.WAIST])
	else
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.UnHide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.WAIST])
	end
