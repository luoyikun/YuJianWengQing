-- 仙域-外观-飞宠幻化-AppearanceHuanHuaContent
FlyPetHuanHuaView = FlyPetHuanHuaView or BaseClass(BaseView)

function FlyPetHuanHuaView:__init()
	self.ui_config = {
	...
	self.def_index = TabIndex.appearance_tail_huanhua
...
function FlyPetHuanHuaView:__delete()
	

end

function FlyPetHuanHuaView:ReleaseCallBack()
...
function FlyPetHuanHuaView:LoadCallBack()
	local tab_cfg = {
		{name = Language.Common.Huanhua, bundle = "uis/images_atlas", asset = "tab_icon_flypet", tab_index = TabIndex.appearance_tail_huanhua, remind_id = RemindName.FlyPetHuanHua},
	}
	...
	RemindManager.Instance:Fire(RemindName.FlyPetHuanHua)
...
function FlyPetHuanHuaView:OnRoleDrag(data)
	if UIScene.role_model then
		UIScene:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function FlyPetHuanHuaView:SetRendering(value)
...
function FlyPetHuanHuaView:ClickAddGold()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function FlyPetHuanHuaView:ClickClose()
	self:Close()

	local fly_pet_view = nil
	if AppearanceCtrl.Instance.view then
		fly_pet_view = AppearanceCtrl.Instance.view:GetFlyPetView()
	end

	if fly_pet_view and fly_pet_view:IsOpen() then
		fly_pet_view:UITween()
	end
end


--点击目标战力
function FlyPetHuanHuaView:ClickSuperPower()
	local cfg = self.list_data[self.select_index]
	local image_id = cfg and cfg.image_id
	local index = image_id or 0
	local data = FlyPetData.Instance:GetSpecialHuanHuaShowData(index)
	TipsCtrl.Instance:ShowSpecialHuanHuaViewView(data)
end

-- 激活升级形象
function FlyPetHuanHuaView:ClickUpGrade()
	...
	local is_active = FlyPetData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local huanhua_cfg = FlyPetData.Instance:GetHuanHuaCfgInfo(data.image_id)
	...
	if not is_active then
		...
		UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.FLY_PET, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_IMAGE_UPGRADE, data.image_id)
	end
end

-- 使用形象
function FlyPetHuanHuaView:ClickUsed()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.FLY_PET, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, data.image_id)
end

function FlyPetHuanHuaView:GetNumberOfCells()
	return #self.list_data
end

function FlyPetHuanHuaView:RefreshCell(cell, data_index)
	data_index = data_index + 1
	
	local huanhua_cell = self.cell_list[cell]
	if nil == huanhua_cell then
		huanhua_cell = FlyPetHuanHuaCell.New(cell.gameObject)
...
function FlyPetHuanHuaView:ClickItem(cell)
	if nil == cell then
		return
	end
...
function FlyPetHuanHuaView:FlushView()
	self:FlushContent()
end

function FlyPetHuanHuaView:FlushContent()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	...
	local now_level = FlyPetData.Instance:GetHuanHuaGrade(data.image_id)
	...
	local attr0 = FlyPetData.Instance:GetHuanHuaCfgInfo(data.image_id)
	local attr1 = FlyPetData.Instance:GetHuanHuaCfgInfo(data.image_id, now_level + 1)
	local max_grade = FlyPetData.Instance:GetSpecialImageMaxUpLevelById(data.image_id)
	self:SetAttr(now_level, attr0, attr1, max_grade, data.image_id)

	local is_show_super = FlyPetData.Instance:IsShowSuperPower(data.image_id)
	local is_active_super = FlyPetData.Instance:GetStarIsShowSuperPower(data.image_id)
	...
	if is_show_super and not is_active_super then
		local need_reach_level = FlyPetData.Instance:GetActiveSuperPowerNeedLevel(data.image_id)
	...
	local max_level = FlyPetData.Instance:GetSpecialImgMaxLevel()
	local is_active = FlyPetData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local is_used = FlyPetData.Instance:GetHuanHuaIdIsUsed(data.image_id)
...
function FlyPetHuanHuaView:SetAttr(special_image_grade, attr0, attr1, max_grade, special_index)
	...
	local active_grade, attr_type, attr_value = FlyPetData.Instance:GetHuanHuaSpecialAttrActiveType(nil, special_index)
	if active_grade and attr_type and attr_value then
		if special_image_grade < active_grade then
			...
			for i = special_image_grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = FlyPetData.Instance:GetHuanHuaSpecialAttrActiveType(i, special_index)
...
function FlyPetHuanHuaView:FlushModel()
	local data = self.list_data[self.select_index]
	if nil == data then
		return
	end
	...
	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetFlyPetModel(data.res_id)
...
function FlyPetHuanHuaView:OpenCallBack()
	self.select_index = 1
	self:Flush()
	self.list_data = FlyPetData.Instance:GetHuanHuaCfgList() or {}
...
function FlyPetHuanHuaView:CloseCallBack()
	if nil ~= self.data_listen then
...
function FlyPetHuanHuaView:ShowIndexCallBack(index, index_nodes)
	...
	local callback = function ()
		...
		local transform = RoleModel.GetModelCameraSettingByType(MODEL_CAMERA_TYPE.TIPS, "flypet")
...
function FlyPetHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function FlyPetHuanHuaView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "all" then
			if v and v.id then
				local index, num = FlyPetData.Instance:CanHuanhuaIndexByImageId(v.id)
				if index then
					self.select_index = index
					local cfg_list = FlyPetData.Instance:GetHuanHuaCfgList()
...
function FlyPetHuanHuaView:UITween()
...
---------------------------------------------------------------------------------------------------
FlyPetHuanHuaCell = FlyPetHuanHuaCell or BaseClass(BaseCell)
function FlyPetHuanHuaCell:__init()

end

function FlyPetHuanHuaCell:__delete()
	
end

function FlyPetHuanHuaCell:ListenClick(handler)
	self.node_list["HuanHuaItem"].toggle:AddClickListener(handler)
end

function FlyPetHuanHuaCell:SetToggleGroup(group)
	self.root_node.toggle.group = group
end

function FlyPetHuanHuaCell:SetToggleIsOn(is_on)
	self.root_node.toggle.isOn = is_on
end

function FlyPetHuanHuaCell:SetData(data)
	if nil == data then
		return
	end
	...
	if FlyPetData.Instance:GetHuanHuaGrade(data.image_id) < FlyPetData.Instance:GetSpecialImgMaxLevel() and
