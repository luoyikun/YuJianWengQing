-- 仙域-外观-飞宠-Content
FlyPetContentView = FlyPetContentView or BaseClass(BaseRender)

local SHOWSPECGRADE = 10

function FlyPetContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}

	self.medal_skill_index = FlyPetData.Instance:GetMedalIndex()
	self:GetSkill()
end

function FlyPetContentView:LoadCallBack(instance)
...
function FlyPetContentView:__delete()
...
function FlyPetContentView:CloseCallBack()
	if self.count_down ~= nil then
		CountDown.Instance:RemoveCountDown(self.count_down)
		self.count_down = nil
	end
...
function FlyPetContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = FlyPetData.Instance:GetSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = FlyPetData.Instance:GetSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetSkillIcon("FlyPetSkill_" .. i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickFlyPetSkill, self, i))
	end
end

-- 点击飞宠幻化按钮
function FlyPetContentView:ClickHuanHua()
	ViewManager.Instance:Open(ViewName.FlyPetHuanHua)
end

-- 点击头饰成长按钮
function FlyPetContentView:ClickGrowup()
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if nil == flypet_info or nil == flypet_info.grade then return end
	local is_activie = flypet_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then		TipsCtrl.Instance:ShowSystemMsg(string.format(Language.MultiMount.UpGradeStartThree, Language.Common.NumToChs[APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN]))
	else
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "flypetchengzhang", {item_id = FlyPetShuXingDanId.ChengZhangDanId})
	end
end

-- 点击头饰资质按钮
function FlyPetContentView:ClickZiZhi()
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if nil == flypet_info or nil == flypet_info.grade then return end
	local is_activie = flypet_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "flypet_zizhi", {item_id = FlyPetShuXingDanId.ZiZhiDanId})
	end
end

-- 点击进阶装备
function FlyPetContentView:OnClickEquipBtn()
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if nil ~= flypet_info and nil ~= flypet_info.grade then
		if flypet_info.grade <= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL then
			...
			ViewManager.Instance:Open(ViewName.AppearacneEquipView, TabIndex.appearance_flypet)
		end
	end
end

-- 点击头饰技能
function FlyPetContentView:ClickFlyPetSkill(index)
	if index == self.medal_skill_index + 1 then
		ViewManager.Instance:Open(ViewName.TipXunZhangView, nil, "flypet_skill", {index = index - 1})
	else
		if self.is_shake_skill and index == 1 then
			self.node_list["Special"].animator:SetBool("IsShake", false)
			self.is_shake_skill = nil
		end	
		ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "flypet_skill", {index = index - 1})
	end
end

function FlyPetContentView:ClickUse()
	local grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.FLY_PET, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, grade_info.image_id)
end

function FlyPetContentView:ClickLeft()
	self.select_grade = self.select_grade - 1
	self:FlushView()
end

function FlyPetContentView:ClickRight()
	self.select_grade = self.select_grade + 1
	self:FlushView()
end

function FlyPetContentView:ClickUpgrade(is_auto_upgrade)
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if nil == flypet_info then return end
	local grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(flypet_info.grade)
	if nil == grade_info then return end

	local close_func = function()
		--获取下一级，不存在则满级
		local next_grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(flypet_info.grade + 1)
		...
		if not is_auto_upgrade then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.FLY_PET, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.FLY_PET, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE, grade_info.pack_num, is_auto_buy)
		end
	end
	...
	local is_have_zhishengdan, item_id = FlyPetData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, flypet_info.grade), ok_callback, close_func)
	    return
	end
...
function FlyPetContentView:OpenCallBack()
	self.is_upgrade_state = false
	self:SetBtnState(false, self.is_max)
	self.node_list["AutoToggle"].toggle.isOn = false

	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.FLYPET)
	self.node_list["HighLight"]:SetActive(flag == 1)

	self.old_select_grade = 0
	self.select_grade = 0
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if nil ~= flypet_info then
		local is_huanhua_img = FlyPetData.Instance:IsHuanHuaImage(flypet_info.used_imageid)
		--默认选择已使用的形象阶数
		if flypet_info.grade == 1 then
			--零阶和一阶使用相同形象
			self.select_grade = flypet_info.grade
		else
			if is_huanhua_img then
				...
				local image_cfg = FlyPetData.Instance:GetFlyPetImageCfgInfoByImageId(flypet_info.used_imageid)
				if nil == image_cfg then return end
				-- show_grade为客户端阶数由0开始，服务器阶数由1开始，所以要加1
				self.select_grade = image_cfg.show_grade + 1
			end
...
function FlyPetContentView:FlushView()
	self:FlushLeft()
	self:FlushRight()
	self:FlushSkillIcon()
end

function FlyPetContentView:FlushSkillIcon()
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if flypet_info == nil then
		return
	end
	local flypet_skill_list = flypet_info.skill_level_list

	if nil == flypet_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node, flypet_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = FlyPetData.Instance:GetSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = FlyPetData.Instance:GetSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if flypet_info ~= nil or next(flypet_info) ~= nil then
		if flypet_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end

	local cur_skill_data = FlyPetData.Instance:GetSkillCfgById(self.medal_skill_index) or {}
	local cur_grade = cur_skill_data and cur_skill_data.skill_level or 1
	local bundle, asset = ResPath.GetAppearMedalLevelImage(cur_grade)
	self.skill_list[self.medal_skill_index + 1].icon.image:LoadSprite(bundle, asset)
	local next_data_cfg = FlyPetData.Instance:GetSkillCfgById(self.medal_skill_index, cur_grade + 1)
	if nil == next_data_cfg then
		self.node_list["TimeLimit"]:SetActive(false)
		self.node_list["TxtTimePanel"]:SetActive(false)
	end

	local temp_grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(self.select_grade)
	if temp_grade_info and temp_grade_info.image_id then
		local image_info = FlyPetData.Instance:GetFlyPetImageCfgInfoByImageId(temp_grade_info.image_id)
		if image_info then 
			local cur_skill_data = FlyPetData.Instance:GetSkillCfgById(self.medal_skill_index) or {}
			...
			self.node_list["Name"].text.text = temp_grade_info.gradename .. "·" .. ToColorStr(image_info.image_name .. skill_name, SOUL_NAME_COLOR[image_info.colour])
		end
	end
end

function FlyPetContentView:FlushModel()
	local grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end
	local image_cfg = FlyPetData.Instance:GetFlyPetImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>头饰_ID:" .. grade_info.image_id .. "</color>")
...
function FlyPetContentView:FlushLeft()
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if nil == flypet_info then return end

	--初始记录旧阶数
	if self.old_grade < 0 then
		self.old_grade = flypet_info.grade
	end

	if self.old_grade ~= flypet_info.grade then
		--进阶成功
	...
	-- 刷新使用状态
	local is_used = temp_grade_info.image_id == flypet_info.used_imageid
	if flypet_info.grade >= self.select_grade then
	...
	--刷新左右箭头
	local last_temp_grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(self.select_grade - 1)
	local is_show_left = true
	if nil == last_temp_grade_info or (flypet_info.grade > 1 and temp_grade_info.show_grade == 1) then
		--没有上一阶属性或者处于第一阶(服务器第二阶)不显示左箭头
		is_show_left = false
	end
	self.node_list["BtnLeftButton"]:SetActive(is_show_left)

	local next_temp_grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(self.select_grade + 1)
	local is_show_right = true
	if nil == next_temp_grade_info or self.select_grade > flypet_info.grade then
		is_show_right = false
	end
	self.node_list["BtnRightButton"]:SetActive(is_show_right)

	-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = FlyPetData.Instance:CanSkillUpLevelList()
	if can_upgrade_skill_list then
		for i = 1, 3 do 
			self.node_list["UpLevelEffect" .. i]:SetActive(can_upgrade_skill_list[i] ~= nil)
		end
	end
	self.node_list["ShowMedalUplevel"]:SetActive(can_upgrade_skill_list[3] ~= nil)

	self.node_list["EffectZiZhi"]:SetActive(FlyPetData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(FlyPetData.Instance:IsShowGrowupRemind())
	self.node_list["ShowEquipRemind"]:SetActive(FlyPetData.Instance:CalAllEquipRemind() > 0)
	local is_show_huanhua_remind = FlyPetData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(flypet_info)
	self:SetBtnGrowup(flypet_info)
	self:SetBtnEquip(flypet_info)
end

function FlyPetContentView:FlushRight()
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if nil == flypet_info then return end
	local grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(flypet_info.grade)
	if nil == grade_info then return end
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(flypet_info.grade + 1)
	local bless_value_ratio = flypet_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = FlyPetData.Instance:GetFlyPetMaxGrade()
	local active_grade, attr_type, attr_value = FlyPetData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if flypet_info.grade < active_grade then
			...
			for i = flypet_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = FlyPetData.Instance:GetSpecialAttrActiveType(i)
	...
	-- for k, v in pairs(switch_attr_list)
	...
	-- -- 特殊属性
	-- -- UI:SetGraphicGrey(self.node_list["TxtSpecialAttr"], grade_info.jianshang_per == 0)
	-- local str_special_attr = ""
	-- if grade_info.jianshang_per == 0 then
	-- 	str_special_attr = string.format(Language.MultiMount.SpecialAttribute[6] .. Language.MultiMount.LevelStart, 5) 
	-- else
	-- 	local level, add_attr = FlyPetData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	-- 	if level and add_attr then
	-- 		str = string.format(Language.Advance.NextAttr, CommonDataManager.GetDaXie(level - 1), add_attr/100)
	-- 	end
	-- 	str_special_attr = string.format(Language.MultiMount.SpecialAttribute[6], grade_info.jianshang_per / 100) .. str
	-- end
	...
	-- if grade_info and next_grade_info and flypet_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", flypet_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(flypet_info, grade_info)
end

--祝福值提示 清空祝福值 清空时间
function FlyPetContentView:ClearBlessTip(flypet_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if flypet_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = flypet_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function FlyPetContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end

function FlyPetContentView:UpGradeResult(result)
...
-- 设置进阶按钮状态
function FlyPetContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function FlyPetContentView:SetBtnZiZhi(flypet_info)
 	local is_activie = flypet_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function FlyPetContentView:SetBtnGrowup(flypet_info)
	local is_activie = flypet_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
-- 设置装备按钮状态
function FlyPetContentView:SetBtnEquip(flypet_info)
	local is_activie = flypet_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL
	UI:SetGraphicGrey(self.node_list["BtnEquipButton"], not is_activie)
	self.node_list["TextEquip"].text.text = is_activie and Language.MultiMount.FlyPetEquipType or 
			string.format(Language.MultiMount.UpGradeStart, Language.Common.NumToChs[APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL])
end

function FlyPetContentView:FlushItem()
	local flypet_info = FlyPetData.Instance:GetFlyPetInfo()
	if nil == flypet_info then return end
	local grade_info = FlyPetData.Instance:GetFlyPetGradeCfgInfoByGrade(flypet_info.grade)
	if nil == grade_info then return end
	...
	self.node_list["RemindBtn"]:SetActive(FlyPetData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end

function FlyPetContentView:PlayEffect()
...
function FlyPetContentView:OnFlush(param_t)
	self:JinJieReward()
	for k, v in pairs(param_t) do
		if k == "all" then
			self:FlushView()
			self:FlushItem()
		elseif k == "flypet" then
			self:FlushView()
		elseif k == "flypet_upgrade" then
			self:UpGradeResult(v[1])
		elseif k == "flypet_item_change" then
			self:FlushItem()
		end
	end
end

function FlyPetContentView:FlushRestChallengeTime()
...
function FlyPetContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function FlyPetContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_FLYPET
...
function FlyPetContentView:ClickActIcon()
...
function FlyPetContentView:SetActTime()
...
function FlyPetContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function FlyPetContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function FlyPetContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function FlyPetContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function FlyPetContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function FlyPetContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function FlyPetContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function FlyPetContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function FlyPetContentView:RemoveCountDown()
...
--打开大目标面板
function FlyPetContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_FLYPET)
end

--打开小目标面板
function FlyPetContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_FLYPET
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_FLYPET, callback)
	TipsCtrl.Instance:ShowTimeLimitTitleView(data)
end

function FlyPetContentView:ToggleOnClick()
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.FLYPET) == 0 and 1 or 0
	SettingData.Instance:SetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.FLYPET, flag)
	self.node_list["HighLight"]:SetActive(flag == 1)
	if flag == 1 then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.Hide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.FLYPET])
	else
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.UnHide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.FLYPET])
	end
