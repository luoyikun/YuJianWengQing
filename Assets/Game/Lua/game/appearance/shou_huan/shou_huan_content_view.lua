-- 仙域-外观-手环-Content
ShouHuanContentView = ShouHuanContentView or BaseClass(BaseRender)

local SHOWSPECGRADE = 10
function ShouHuanContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self:GetSkill()
end

function ShouHuanContentView:LoadCallBack(instance)
...
function ShouHuanContentView:__delete()
...
function ShouHuanContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = ShouHuanData.Instance:GetSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = ShouHuanData.Instance:GetSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetSkillIcon("ShouHuanSkill_" .. i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickShouHuanSkill, self, i))
	end
end

-- 点击手环幻化按钮
function ShouHuanContentView:ClickHuanHua()
	ViewManager.Instance:Open(ViewName.ShouHuanHuanHua)
end

-- 点击手环成长按钮
function ShouHuanContentView:ClickGrowup()
	local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	if nil == shouhuan_info or nil == shouhuan_info.grade then return end
	local is_activie = shouhuan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "shouhuanchengzhang", {item_id = ShouHuanShuXingDanId.ChengZhangDanId})
	end
end

-- 点击手环资质按钮
function ShouHuanContentView:ClickZiZhi()
	local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	if nil == shouhuan_info or nil == shouhuan_info.grade then return end
	local is_activie = shouhuan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "shouhuan_zizhi", {item_id = ShouHuanShuXingDanId.ZiZhiDanId})
	end
end

-- 点击手环技能
function ShouHuanContentView:ClickShouHuanSkill(index)
	if self.is_shake_skill and index == 1 then
		self.node_list["Special"].animator:SetBool("IsShake", false)
		self.is_shake_skill = nil
	end
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "shouhuan_skill", {index = index - 1})
end

function ShouHuanContentView:ClickUse()
	local grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.SHOU_HUAN, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, grade_info.image_id)
end

function ShouHuanContentView:ClickLeft()
	self.select_grade = self.select_grade - 1
	self:FlushView()
end

function ShouHuanContentView:ClickRight()
	self.select_grade = self.select_grade + 1
	self:FlushView()
end

function ShouHuanContentView:ClickUpgrade(is_auto_upgrade)
	local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	if nil == shouhuan_info then return end
	local grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(shouhuan_info.grade)
	if nil == grade_info then return end

	local close_func = function()
		--获取下一级，不存在则满级
		local next_grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(shouhuan_info.grade + 1)
		...
		if not is_auto_upgrade then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.SHOU_HUAN, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.SHOU_HUAN, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE,grade_info.pack_num, is_auto_buy)
		end
	end
	...
	local is_have_zhishengdan, item_id = ShouHuanData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, shouhuan_info.grade), ok_callback, close_func)
	    return
	end
...
function ShouHuanContentView:OpenCallBack()
	...
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.SHOUHUAN)
	self.node_list["HighLight"]:SetActive(flag == 1)

	self.old_select_grade = 0
	self.select_grade = 0
	local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	local is_huanhua_img = ShouHuanData.Instance:IsHuanHuaImage(shouhuan_info.used_imageid)
	if nil ~= shouhuan_info then
		--默认选择已使用的形象阶数
		if shouhuan_info.grade == 1 then
			--零阶和一阶使用相同形象
			self.select_grade = shouhuan_info.grade
		else
			if is_huanhua_img then
				local grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(shouhuan_info.grade)
				if nil == grade_info then return end
				local image_cfg = ShouHuanData.Instance:GetShouHuanImageCfgInfoByImageId(grade_info.image_id)
				if nil == image_cfg then return end
				-- show_grade为客户端阶数由0开始，服务器阶数由1开始，所以要加1
				self.select_grade = image_cfg.show_grade + 1
			else
				local image_cfg = ShouHuanData.Instance:GetShouHuanImageCfgInfoByImageId(shouhuan_info.used_imageid)
				if nil == image_cfg then return end
				-- show_grade为客户端阶数由0开始，服务器阶数由1开始，所以要加1
				self.select_grade = image_cfg.show_grade + 1
			end
...
function ShouHuanContentView:FlushView()
	self:FlushLeft()
	self:FlushRight()
	self:FlushSkillIcon()
end

function ShouHuanContentView:FlushSkillIcon()
	local shouhuan_skill_list = ShouHuanData.Instance:GetShouHuanInfo().skill_level_list

	if nil == shouhuan_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node, shouhuan_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = ShouHuanData.Instance:GetSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = ShouHuanData.Instance:GetSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	if shouhuan_info ~= nil or next(shouhuan_info) ~= nil then
		if shouhuan_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end
...
function ShouHuanContentView:FlushModel()
	local grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end
	local image_cfg = ShouHuanData.Instance:GetShouHuanImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>手环_ID:" .. grade_info.image_id .. "</color>")

	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetShouHuanModel(image_cfg.res_id)
...
function ShouHuanContentView:FlushLeft()
	local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	if nil == shouhuan_info then return end

	--初始记录旧阶数
	if self.old_grade < 0 then
		self.old_grade = shouhuan_info.grade
	end

	if self.old_grade ~= shouhuan_info.grade then
		--进阶成功
	...
	local temp_grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(self.select_grade)
	if nil == temp_grade_info then return end
	local image_info = ShouHuanData.Instance:GetShouHuanImageCfgInfoByImageId(temp_grade_info.image_id)
	...
	-- 刷新使用状态
	local is_used = temp_grade_info.image_id == shouhuan_info.used_imageid
	if shouhuan_info.grade >= self.select_grade then
	...
	--刷新左右箭头
	local last_temp_grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(self.select_grade - 1)
	local is_show_left = true
	if nil == last_temp_grade_info or (shouhuan_info.grade > 1 and temp_grade_info.show_grade == 1) then
		--没有上一阶属性或者处于第一阶(服务器第二阶)不显示左箭头
		is_show_left = false
	end
	self.node_list["BtnLeftButton"]:SetActive(is_show_left)

	local next_temp_grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(self.select_grade + 1)
	local is_show_right = true
	if nil == next_temp_grade_info or self.select_grade > shouhuan_info.grade then
		is_show_right = false
	end
	self.node_list["BtnRightButton"]:SetActive(is_show_right)

	-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = ShouHuanData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(ShouHuanData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(ShouHuanData.Instance:IsShowGrowupRemind())
	local is_show_huanhua_remind = ShouHuanData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(shouhuan_info)
	self:SetBtnGrowup(shouhuan_info)
end

function ShouHuanContentView:FlushRight()
	local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	if nil == shouhuan_info then return end
	local grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(shouhuan_info.grade)
	if nil == grade_info then return end
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(shouhuan_info.grade + 1)
	local bless_value_ratio = shouhuan_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = ShouHuanData.Instance:GetShouHuanMaxGrade()
	local active_grade, attr_type, attr_value = ShouHuanData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if shouhuan_info.grade < active_grade then
			...
			for i = shouhuan_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = ShouHuanData.Instance:GetSpecialAttrActiveType(i)
	...
	local attr = ShouHuanData.Instance:UseChengZhandDanAddBaseAttr(grade_info)
	-- 转换显示样式
	...
	-- -- 特殊属性
	-- -- UI:SetGraphicGrey(self.node_list["TxtSpecialAttr"], grade_info.mingzhong == 0)
	-- local str_special_attr = ""
	-- if grade_info.mingzhong == 0 then
	-- 	str_special_attr = string.format(Language.MultiMount.SpecialAttribute[1] .. Language.MultiMount.LevelStart, 5)
	-- else
	-- 	local level, add_attr = ShouHuanData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	-- 	if level and add_attr then
	-- 		str = string.format(Language.Advance.NextAttr, CommonDataManager.GetDaXie(level - 1), add_attr/100)
	-- 	end
	-- 	str_special_attr = string.format(Language.MultiMount.SpecialAttribute[1], grade_info.mingzhong / 100) .. str
	-- end
	...
	-- if grade_info and next_grade_info and shouhuan_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", shouhuan_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(shouhuan_info, grade_info)
end

--祝福值提示 清空祝福值 清空时间
function ShouHuanContentView:ClearBlessTip(shouhuan_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if shouhuan_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = shouhuan_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function ShouHuanContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end

function ShouHuanContentView:UpGradeResult(result)
...
-- 设置进阶按钮状态
function ShouHuanContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function ShouHuanContentView:SetBtnZiZhi(shouhuan_info)
 	local is_activie = shouhuan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function ShouHuanContentView:SetBtnGrowup(shouhuan_info)
	local is_activie = shouhuan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
function ShouHuanContentView:FlushItem()
	local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	if nil == shouhuan_info then return end
	local grade_info = ShouHuanData.Instance:GetShouHuanGradeCfgInfoByGrade(shouhuan_info.grade)
	if nil == grade_info then return end
	...
	self.node_list["RemindBtn"]:SetActive(ShouHuanData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end

function ShouHuanContentView:PlayEffect()
...
function ShouHuanContentView:OnFlush(param_t)
	self:JinJieReward()
	for k, v in pairs(param_t) do
		if k == "all" then
			self:FlushView()
			self:FlushItem()
		elseif k == "shouhuan" then
			self:FlushView()
		elseif k == "shouhuan_upgrade" then
			self:UpGradeResult(v[1])
		elseif k == "shouhuan_item_change" then
			self:FlushItem()
		end
	end
end

function ShouHuanContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function ShouHuanContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN
	...
	-- local shouhuan_info = ShouHuanData.Instance:GetShouHuanInfo()
	-- if shouhuan_info == nil or shouhuan_info.grade == nil then
	-- 	return
	-- end
	
	-- local cur_img_grade = shouhuan_info.grade - 1
...
function ShouHuanContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function ShouHuanContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function ShouHuanContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function ShouHuanContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function ShouHuanContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function ShouHuanContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function ShouHuanContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function ShouHuanContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function ShouHuanContentView:RemoveCountDown()
...
--打开大目标面板
function ShouHuanContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN)
end

--打开小目标面板
function ShouHuanContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_SHOUHUAN, callback)
	TipsCtrl.Instance:ShowTimeLimitTitleView(data)
end

function ShouHuanContentView:ToggleOnClick()
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.SHOUHUAN) == 0 and 1 or 0
	SettingData.Instance:SetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.SHOUHUAN, flag)
	self.node_list["HighLight"]:SetActive(flag == 1)
	if flag == 1 then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.Hide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.SHOUHUAN])
	else
