...
function MaskContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self:GetSkill()
end

function MaskContentView:LoadCallBack(instance)
...
function MaskContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = MaskData.Instance:GetMaskSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = MaskData.Instance:GetMaskSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetMaskSkillIcon(i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickMaskSkill, self, i))
	end
end
...
-- 点击面饰成长按钮
function MaskContentView:ClickGrowup()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if nil == mask_info or nil == mask_info.grade then return end
	local is_activie = mask_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "maskchengzhang", {item_id = MaskShuXingDanId.ChengZhangDanId})
	end
end
...
function MaskContentView:ClickZiZhi()
	local mask_info = MaskData.Instance:GetMaskInfo()
	if nil == mask_info or nil == mask_info.grade then return end
	local is_activie = mask_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "mask_zizhi", {item_id = MaskShuXingDanId.ZiZhiDanId})
	end
end

-- 点击面饰技能
function MaskContentView:ClickMaskSkill(index)
...
function MaskContentView:ClickUse()
	...
	MaskCtrl.Instance:SendMaskReq(MASK_OPERA_TYPE.MASK_OPERA_TYPE_USE_IMAGE, grade_info.image_id)
end
...
function MaskContentView:ClickUpgrade(is_auto_upgrade)
	local mask_info = MaskData.Instance:GetMaskInfo()
	if nil == mask_info then return end
	...
	local close_func = function()
		...
		if not is_auto_upgrade then
			 MaskCtrl.Instance:SendMaskReq(MASK_OPERA_TYPE.MASK_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			 MaskCtrl.Instance:SendMaskReq(MASK_OPERA_TYPE.MASK_OPERA_TYPE_UPGRADE, grade_info.pack_num, is_auto_buy)
		end
	end
	...
	if KaifuActivityData.Instance:IsOpenAdvanceReturnActivity() and self.is_can_tip then
		local open_advance_one = KaifuActivityData.Instance:GetOpenAdvanceType(TYPE_UPGRADE_RETURN.MASK_UPGRADE_RETURN)
		local open_advance_two = KaifuActivityData.Instance:GetOpenAdvanceTypeTwo(TYPE_UPGRADE_RETURN.MASK_UPGRADE_RETURN)
	...
	local is_have_zhishengdan, item_id = MaskData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, mask_info.grade), ok_callback, close_func)
	    return
	end
...
function MaskContentView:OpenCallBack()
	...
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.MASK)
...
function MaskContentView:FlushSkillIcon()
	local maks_skill_list = MaskData.Instance:GetMaskInfo().skill_level_list
	if nil == maks_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node,maks_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = MaskData.Instance:GetMaskSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = MaskData.Instance:GetMaskSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local mask_info = MaskData.Instance:GetMaskInfo()
	if mask_info ~= nil or next(mask_info) ~= nil then
		if mask_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end
...
function MaskContentView:FlushModel()
	local grade_info = MaskData.Instance:GetMaskGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end
	local image_cfg = MaskData.Instance:GetMaskImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>面饰_ID:" .. grade_info.image_id .. "</color>")

	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetMaskModel(image_cfg.res_id)
...
function MaskContentView:FlushLeft()
	...
	--刷新使用状态
	local is_used = temp_grade_info.image_id == mask_info.used_imageid
	...
	-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = MaskData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(MaskData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(MaskData.Instance:IsShowGrowupRemind())
	local is_show_huanhua_remind = MaskData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(mask_info)
	self:SetBtnGrowup(mask_info)
end
...
function MaskContentView:FlushRight()
	...
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info =  MaskData.Instance:GetMaskGradeCfgInfoByGrade(mask_info.grade + 1)
	local bless_value_ratio = mask_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = MaskData.Instance:GetMaskMaxGrade()
	local active_grade, attr_type, attr_value = MaskData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if mask_info.grade < active_grade then
			...
			for i = mask_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = MaskData.Instance:GetSpecialAttrActiveType(i)
	...
	-- else
	-- 	local level, add_attr = MaskData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	...
	-- -- if grade_info and next_grade_info and mask_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", mask_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(mask_info, grade_info)
end

--祝福值提示 清空祝福值 清空时间
function MaskContentView:ClearBlessTip(mask_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if mask_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = mask_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function MaskContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end
...
-- 设置进阶按钮状态
function MaskContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function MaskContentView:SetBtnZiZhi(mask_info)
 	local is_activie = mask_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function MaskContentView:SetBtnGrowup(mask_info)
	local is_activie = mask_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
function MaskContentView:FlushItem()
	...
	self.node_list["RemindBtn"]:SetActive(MaskData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end
...
function MaskContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function MaskContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_MASK
	...
	-- local mask_info = MaskData.Instance:GetMaskInfo()
	-- if mask_info == nil or mask_info.grade == nil then
	-- 	return
	-- end
	
	-- local cur_img_grade = mask_info.grade - 1
...
function MaskContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function MaskContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function MaskContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function MaskContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function MaskContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function MaskContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function MaskContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function MaskContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function MaskContentView:RemoveCountDown()
...
--打开大目标面板
function MaskContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_MASK)
end

--打开小目标面板
function MaskContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_MASK
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_MASK, callback)
	TipsCtrl.Instance:ShowTimeLimitTitleView(data)
end

function MaskContentView:ToggleOnClick()
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.MASK) == 0 and 1 or 0
	SettingData.Instance:SetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.MASK, flag)
	self.node_list["HighLight"]:SetActive(flag == 1)
	if flag == 1 then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.Hide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.MASK])
	else
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.UnHide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.MASK])
	end
