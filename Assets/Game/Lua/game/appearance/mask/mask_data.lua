...
MaskShuXingDanId = {
	ZiZhiDanId = 22122,			-- 面饰资质丹
	ChengZhangDanId = 22129,	-- 面饰成长丹
}
...
function MaskData:__init()
	...
	self.mask_cfg = ConfigManager.Instance:GetAutoConfig("mask_auto")
	self.mask_skill_cfg = self.mask_cfg.mask_skill
	self.other_cfg = self.mask_cfg.other[1]
	self.mask_grade_cfg = ListToMap(self.mask_cfg.grade, "grade")
	self.mask_image_list_cfg = ListToMap(self.mask_cfg.image_list, "image_id")
	self.mask_special_img_cfg = ListToMap(self.mask_cfg.special_img, "image_id")
	self.mask_special_img_item_cfg = ListToMap(self.mask_cfg.special_img, "item_id")
	self.mask_special_image_upgrade_cfg = ListToMap(self.mask_cfg.special_image_upgrade, "special_img_id", "grade")
	self.huanhua_special_cap_add = ListToMap(self.mask_cfg.huanhua_special_cap_add, "huanhua_id")			--幻化特殊战力加成

	self.special_grade_max_level = self:CalcSpecialImgMaxLevel()

	RemindManager.Instance:Register(RemindName.Mask, BindTool.Bind(self.ShowMaskRemind, self))
	RemindManager.Instance:Register(RemindName.MaskHuanHua, BindTool.Bind(self.CalcHuanHuaRemind, self))
end

function MaskData:__delete()
	RemindManager.Instance:UnRegister(RemindName.Mask)
	RemindManager.Instance:UnRegister(RemindName.MaskHuanHua)
	MaskData.Instance = nil
end

function MaskData:SetMaskInfo(info)
	if self.mask_info and self.mask_info.grade_bless_val then
		local diff = info.grade_bless_val - self.mask_info.grade_bless_val
		local bless_cfg =  AdvanceData.Instance:GetBlessBaojiCfg(UPGRADE_BAOJI_TYPE.UPGRADE_TYPE_MASK)
	...
	self.mask_info = info
	self.mask_info.shuxingdan_count = info.shuxingdan_list[SHUXINGDAN_SLOT_TYPE.SHUXINGDAN_SLOT_TYPE_ZIZHI]			-- 资质丹
	self.mask_info.chengzhangdan_count = info.shuxingdan_list[SHUXINGDAN_SLOT_TYPE.SHUXINGDAN_SLOT_TYPE_CHENGZHANG]	-- 成长丹
...
function MaskData:GetMaskImage()
	return self.mask_image_list_cfg
end
...
function MaskData:GetSpecialImageCfg()
	return self.mask_special_img_cfg
end
...
-- 获取当前等级可使用的最大成长丹数量
function MaskData:GetMaxChengZhangDanCount(grade)
	local max_num = 0
	if nil == self.mask_info then
		return max_num
	end
	...
	local grade_cfg = self:GetMaskGradeCfgInfoByGrade(grade)
...
function MaskData:CanHuanhuaIndexByImageId(image_id)
...
-- 获取特殊形象配置
function MaskData:GetSpecialImage()
	return self.mask_special_img_cfg
end
...
function MaskData:GetSpecialImageMaxUpLevelById(image_id)
	...
	for k, v in pairs(self.mask_cfg.special_image_upgrade) do
...
--计算资质丹红点
function MaskData:IsShowZiZhiRemind()
	if self.mask_info then
		if self.mask_info.grade <= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN then
			return false
		end
		if self.mask_info.shuxingdan_count >= self:GetMaxShuXingDanCount(self.mask_info.grade) then
			return false
		end
		if ItemData.Instance:GetItemNumInBagById(MaskShuXingDanId.ZiZhiDanId) > 0 then
			return true
		end	
	end
...
-- 计算成长丹按钮红点
function MaskData:IsShowGrowupRemind()
	if self.mask_info then
		if self.mask_info.grade <= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN then
			return false
		end
		if self.mask_info.chengzhangdan_count >= self:GetMaxChengZhangDanCount(self.mask_info.grade) then
			return false
		end
		if ItemData.Instance:GetItemNumInBagById(MaskShuXingDanId.ChengZhangDanId) > 0 then
			return true
		end
	end
	return false
...
-- 计算进阶按钮红点
function MaskData:IsShowUpgradeBtnRemind()
	if not self.mask_info or not self.mask_info.grade then
		return false
	end

	local mask_cfg = self:GetMaskGradeCfgInfoByGrade(self.mask_info.grade)
	if not mask_cfg  then
		return false
	end

	-- 不清空祝福值时和可进阶时，显示红点
	if mask_cfg.is_clear_bless == APPEARANCE_CLEAR_BLESS.NOT_CLEAR then
		local item_id = mask_cfg.upgrade_stuff_id
		local item_id2 = mask_cfg.upgrade_stuff2_id
		local need_item_num = mask_cfg.upgrade_stuff_count
...
-- 计算面饰侧边栏红点显示
function MaskData:ShowMaskRemind()
	local is_mask_huanhua = self:CalcHuanHuaRemind() == 1 and true or false
	local is_can_active_jinjie_reward = JinJieRewardData.Instance:SystemIsShowRedPoint(JINJIE_TYPE.JINJIE_TYPE_MASK)
...
-- 获得特殊属性更高的
function MaskData:GetGradeAndSpecialAttr()
	local cfg = self:GetMaskGradeCfgInfoByGrade(self.mask_info.grade)
	for k, v in ipairs(self.mask_cfg.grade) do
...
-- 面饰的最大阶数，服务器阶数
function MaskData:GetMaskMaxGrade()
	return #self.mask_grade_cfg
end
...
function MaskData:CanSkillUpLevelList()
	...
	local list = {}
	if nil == self.mask_info.grade or self.mask_info.grade <= 0 then
		return list
	end

	if nil == self.mask_info.skill_level_list then
		return list
	end

	for i, j in pairs(self.mask_skill_cfg) do
		if j.uplevel_stuff_num <= ItemData.Instance:GetItemNumInBagById(j.uplevel_stuff_id)
			and self.mask_info.skill_level_list[j.skill_idx] == (j.skill_level - 1)
			and j.grade <= self.mask_info.grade and j.skill_type ~= 0 then
...
-- 是否有技能可以升级
function MaskData:IsCanLevelSkill()
...
-- 获取当前点击的面饰技能的配置 通过技能索引和技能等级来确定一个技能的所有属性
function MaskData:GetMaskSkillCfgById(skill_idx, level, mask_info)
	local mask_info = mask_info or self.mask_info
	local level = level or mask_info.skill_level_list[skill_idx]

	for k, v in pairs(self.mask_skill_cfg) do
...
function MaskData:GetSpecialAttrActiveType(cur_grade)
	cur_grade = cur_grade or self.mask_info.grade or 0
	return AppearanceData.Instance:GetSpecialAttrActiveType(self.mask_cfg.grade, cur_grade)
end

function MaskData:GetHuanHuaSpecialAttrActiveType(grade, index)
	local grade = grade or self.mask_info.special_img_grade_list[index] or 0
	return AppearanceData.Instance:GetSpecialAttrActiveType(self.mask_cfg.special_image_upgrade, grade, index)
end
...
-- 使用成长丹增加基础属性
function MaskData:UseChengZhandDanAddBaseAttr(info)
	local attribute = CommonDataManager.GetAttributteByClass(info)
	local shuxingdan_cfg = AppearanceData.Instance:GetShuXingDanInfo(MaskShuXingDanCfgType.Type, SHUXINGDAN_SLOT_TYPE.SHUXINGDAN_SLOT_TYPE_CHENGZHANG)
	if nil == shuxingdan_cfg or nil == self.mask_info or nil == attribute then
		return
	end

	local attr = {}
	for k, v in pairs(attribute) do
		attr[k] = math.ceil((self.mask_info.chengzhangdan_count * shuxingdan_cfg.attr_per / 10000 + 1) * v)
	end
	return attr
end

function MaskData:IsHaveZhiShengDanInGrade() 
  if self.mask_info and next(self.mask_info) then
    ...
    for k, v in pairs(zhishengdan_list) do
      ...
      if item_cfg.use_type == 100 and item_cfg.param2 == self.mask_info.grade then 		
...
function MaskData:SuperPowerIsOpenByCfg()
...
--特殊星星是否显示
function MaskData:GetStarIsShowSuperPower(huanhua_id)
	local is_show = false
...
--超级战力是否显示
function MaskData:IsShowSuperPower(huanhua_id)
	local is_show = false
	local is_open = self:SuperPowerIsOpenByCfg()
	if not is_open then
		return is_show
...
--获取单个幻化形象特殊战力配置
function MaskData:GetSingleHuanHuaSpecialCapAddList(huanhua_id)
	local list = {}
	if nil == huanhua_id or nil == self.huanhua_special_cap_add or nil == self.huanhua_special_cap_add[huanhua_id] then
		return list
	end
...
--获取激活超级形象的要求等级
function MaskData:GetActiveSuperPowerNeedLevel(huanhua_id)
	local level = 0
...
--特殊战力面板显示数据
function MaskData:GetSpecialHuanHuaShowData(huanhua_id)
...
--当前进阶等级对应的image_id
function MaskData:GetCurGradeImageId()
	local image_id = 0
	local cfg = self:GetMaskGradeCfgInfoByGrade()
	if cfg then
...
function MaskData:GetCurGradeBaseFightPowerAndAddPer()
	local power = 0
	local huanhua_add_per = 0

	local grade = self:GetGrade()
	local cur_grade = grade == 0 and 1 or grade
	local attr_cfg = self:GetMaskGradeCfgInfoByGrade(cur_grade)
...
--计算进阶奖励红点
function MaskData:CalcJinJieRewardRemind()
	local is_can_active_jinjie_reward = JinJieRewardData.Instance:SystemIsShowRedPoint(JINJIE_TYPE.JINJIE_TYPE_MASK)
...
--获取相关总属性值
function MaskData:GetAttrSum(info)
	local attr = CommonStruct.Attribute()
	if nil == info or nil == info.grade or info.grade <= 0 then
		return attr
	end

	local base_grade_cfg = self:GetMaskGradeCfgInfoByGrade(info.grade)
	...
	if is_active_big_target then
		...
		local big_target_add_attr = JinJieRewardData.Instance:GetSingleAttrCfgAttrAddPer(JINJIE_TYPE.JINJIE_TYPE_MASK)
...
-- 获取已激活特殊形象的总属性
function MaskData:GetSpecialImageAttrSum(info)
	...
	local big_target_img_id = JinJieRewardData.Instance:GetSingleRewardCfgParam0(JINJIE_TYPE.JINJIE_TYPE_MASK)
...
--超级战力属性
function MaskData:SuperPowerAttr(huanhua_id, cur_grade)
...
--获取当前使用形象image_id
function MaskData:GetUsedImageId()
	local image_id = 0
	if self.mask_info and self.mask_info.used_imageid then
		image_id = self.mask_info.used_imageid
	end
	return image_id
end

-- 获取特殊形象进阶信息
function MaskData:GetSpecialImageUpgradeInfo(image_id, grade)
...
function MaskData:IsHidden()
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.MASK)		--0为不隐藏，1为隐藏
	return flag == 1
end