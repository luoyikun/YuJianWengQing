...
function TouShiContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self:GetSkill()
end

function TouShiContentView:LoadCallBack(instance)
...
function TouShiContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = TouShiData.Instance:GetTouShiSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = TouShiData.Instance:GetTouShiSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetTouShiSkillIcon(i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickTouShiSkill, self, i))
	end
end
...
-- 点击头饰成长按钮
function TouShiContentView:ClickGrowup()
	local toushi_info = TouShiData.Instance:GetTouShiInfo()
	if nil == toushi_info or nil == toushi_info.grade then return end
	local is_activie = toushi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "toushichengzhang", {item_id = TouShiShuXingDanId.ChengZhangDanId})
	end
end
...
function TouShiContentView:ClickZiZhi()
	local toushi_info = TouShiData.Instance:GetTouShiInfo()
	if nil == toushi_info or nil == toushi_info.grade then return end
	local is_activie = toushi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "toushi_zizhi", {item_id = TouShiShuXingDanId.ZiZhiDanId})
	end
end

-- 点击头饰技能
function TouShiContentView:ClickTouShiSkill(index)
	if self.is_shake_skill and index == 1 then
		self.node_list["Special"].animator:SetBool("IsShake", false)
		self.is_shake_skill = nil
	end
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "toushiskill", {index = index - 1})
end
...
function TouShiContentView:ClickUse()
	...
	TouShiCtrl.Instance:SendTouShiReq(TOUSHI_OPERA_TYPE.TOUSHI_OPERA_TYPE_USE_IMAGE, grade_info.image_id)
end
...
function TouShiContentView:ClickUpgrade(is_auto_upgrade)
	local toushi_info = TouShiData.Instance:GetTouShiInfo()
	if nil == toushi_info then return end
	...
	local close_func = function()
		...
		if not is_auto_upgrade then
			TouShiCtrl.Instance:SendTouShiReq(TOUSHI_OPERA_TYPE.TOUSHI_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			TouShiCtrl.Instance:SendTouShiReq(TOUSHI_OPERA_TYPE.TOUSHI_OPERA_TYPE_UPGRADE,grade_info.pack_num, is_auto_buy)
		end
	end
	...
	if KaifuActivityData.Instance:IsOpenAdvanceReturnActivity() and self.is_can_tip then
		local open_advance_one = KaifuActivityData.Instance:GetOpenAdvanceType(TYPE_UPGRADE_RETURN.TOUSHI_UPGRADE_RETURN)
		local open_advance_two = KaifuActivityData.Instance:GetOpenAdvanceTypeTwo(TYPE_UPGRADE_RETURN.TOUSHI_UPGRADE_RETURN)
	...
	local is_have_zhishengdan, item_id = TouShiData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, toushi_info.grade), ok_callback, close_func)
	    return
	end
...
function TouShiContentView:OpenCallBack()
	...
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.TOUSHI)
...
function TouShiContentView:FlushSkillIcon()
	local toushi_skill_list = TouShiData.Instance:GetTouShiInfo().skill_level_list

	if nil == toushi_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node,toushi_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = TouShiData.Instance:GetTouShiSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = TouShiData.Instance:GetTouShiSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local toushi_info = TouShiData.Instance:GetTouShiInfo()
	if toushi_info ~= nil or next(toushi_info) ~= nil then
		if toushi_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end
...
function TouShiContentView:FlushModel()
	local grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end
	local image_cfg = TouShiData.Instance:GetTouShiImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>头饰_ID:" .. grade_info.image_id .. "</color>")

	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetTouShiModel(image_cfg.res_id)
...
function TouShiContentView:FlushLeft()
	...
	-- 刷新使用状态
	local is_used = temp_grade_info.image_id == toushi_info.used_imageid
	...
	-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = TouShiData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(TouShiData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(TouShiData.Instance:IsShowGrowupRemind())
	local is_show_huanhua_remind = TouShiData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(toushi_info)
	self:SetBtnGrowup(toushi_info)
end
...
function TouShiContentView:FlushRight()
	...
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = TouShiData.Instance:GetTouShiGradeCfgInfoByGrade(toushi_info.grade + 1)
	local bless_value_ratio = toushi_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = TouShiData.Instance:GetTouShiMaxGrade()
	local active_grade, attr_type, attr_value = TouShiData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if toushi_info.grade < active_grade then
			...
			for i = toushi_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = TouShiData.Instance:GetSpecialAttrActiveType(i)
	...
	-- else
	-- 	local level, add_attr = TouShiData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	...
	-- if grade_info and next_grade_info and toushi_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", toushi_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(toushi_info, grade_info)
end

--祝福值提示 清空祝福值 清空时间
function TouShiContentView:ClearBlessTip(toushi_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if toushi_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = toushi_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function TouShiContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end
...
-- 设置进阶按钮状态
function TouShiContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function TouShiContentView:SetBtnZiZhi(toushi_info)
	local is_activie = toushi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function TouShiContentView:SetBtnGrowup(toushi_info)
	local is_activie = toushi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
function TouShiContentView:FlushItem()
	...
	self.node_list["RemindBtn"]:SetActive(TouShiData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end
...
function TouShiContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function TouShiContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_TOUSHI
	...
	-- local toushi_info = TouShiData.Instance:GetTouShiInfo()
	-- if toushi_info == nil or toushi_info.grade == nil then
	-- 	return
	-- end
	
	-- local cur_img_grade = toushi_info.grade - 1
...
function TouShiContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function TouShiContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function TouShiContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function TouShiContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function TouShiContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function TouShiContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function TouShiContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function TouShiContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function TouShiContentView:RemoveCountDown()
...
--打开大目标面板
function TouShiContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_TOUSHI)
end

--打开小目标面板
function TouShiContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_TOUSHI
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_TOUSHI, callback)
	TipsCtrl.Instance:ShowTimeLimitTitleView(data)
end

function TouShiContentView:ToggleOnClick()
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.TOUSHI) == 0 and 1 or 0
	SettingData.Instance:SetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.TOUSHI, flag)
	self.node_list["HighLight"]:SetActive(flag == 1)
	if flag == 1 then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.Hide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.TOUSHI])
	else
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.UnHide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.TOUSHI])
	end
