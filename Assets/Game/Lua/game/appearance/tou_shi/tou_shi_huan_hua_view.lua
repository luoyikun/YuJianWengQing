-- 仙域-外观-头饰幻化-AppearanceHuanHuaContent
...
function TouShiHuanHuaView:__init()
	...
	self.def_index = TabIndex.appearance_toushi_huanhua
	self.play_audio = true
	self.full_screen = true
	self.cell_list = {}
	self.prefab_preload_id = 9
end
...
function TouShiHuanHuaView:LoadCallBack()
	local tab_cfg = {
		{name = Language.Common.Huanhua, bundle = "uis/images_atlas", asset = "tab_icon_toushi", tab_index = TabIndex.appearance_toushi_huanhua, remind_id = RemindName.TouShiHuanHua},
	}
	...
	RemindManager.Instance:Fire(RemindName.TouShiHuanHua)
...
function TouShiHuanHuaView:OnRoleDrag(data)
	if UIScene.role_model then
		UIScene:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function TouShiHuanHuaView:SetRendering(value)
...
function TouShiHuanHuaView:ClickAddGold()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function TouShiHuanHuaView:ClickClose()
	self:Close()

	local toushi_view = nil
	if AppearanceCtrl.Instance.view then
		toushi_view = AppearanceCtrl.Instance.view:GetTouShiView()
	end

	if toushi_view and toushi_view:IsOpen() then
		toushi_view:UITween()
	end
end

--点击目标战力
function TouShiHuanHuaView:ClickSuperPower()
	local cfg = self.list_data[self.select_index]
	local image_id = cfg and cfg.image_id
	local index = image_id or 0
	local data = TouShiData.Instance:GetSpecialHuanHuaShowData(index)
...
function TouShiHuanHuaView:ClickUpGrade()
	...
	local is_active = TouShiData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local huanhua_cfg = TouShiData.Instance:GetHuanHuaCfgInfo(data.image_id)
	...
	if not is_active then
		...
		TouShiCtrl.Instance:SendTouShiReq(TOUSHI_OPERA_TYPE.TOUSHI_OPERA_TYPE_SPECIAL_IMAGE_UPGRADE, data.image_id)
	end
end
...
function TouShiHuanHuaView:ClickUsed()
	...
	TouShiCtrl.Instance:SendTouShiReq(TOUSHI_OPERA_TYPE.TOUSHI_OPERA_TYPE_USE_IMAGE, image_id)
end
...
function TouShiHuanHuaView:FlushContent()
	...
	local now_level = TouShiData.Instance:GetHuanHuaGrade(data.image_id)
	local name = "<color=" .. SOUL_NAME_COLOR[item_cfg.color] .. "> " .. data.image_name .. "</color>"
	self.node_list["TxtName"].text.text = "Lv." .. now_level .. name

	-- local special_image_grade = TouShiData.Instance:GetHuanHuaGrade(data.image_id)
	local attr0 = TouShiData.Instance:GetHuanHuaCfgInfo(data.image_id)
	local attr1 = TouShiData.Instance:GetHuanHuaCfgInfo(data.image_id, now_level + 1)
	local max_grade = TouShiData.Instance:GetSpecialImageMaxUpLevelById(data.image_id)
	...
	local is_show_super = TouShiData.Instance:IsShowSuperPower(data.image_id)
	local is_active_super = TouShiData.Instance:GetStarIsShowSuperPower(data.image_id)
	...
	if is_show_super and not is_active_super then
		local need_reach_level = TouShiData.Instance:GetActiveSuperPowerNeedLevel(data.image_id)
	...
	local is_active = TouShiData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local is_used = TouShiData.Instance:GetHuanHuaIdIsUsed(data.image_id, true)
...
function TouShiHuanHuaView:SetAttr(special_image_grade, attr0, attr1, max_grade, special_index)
...
function TouShiHuanHuaView:FlushModel()
	...
	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetTouShiModel(data.res_id)
...
function TouShiHuanHuaView:ShowIndexCallBack(index, index_nodes)
...
function TouShiHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function TouShiHuanHuaView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "all" then
			if v and v.id then
				local index, num = TouShiData.Instance:CanHuanhuaIndexByImageId(v.id)
				if index then
					self.select_index = index
					local cfg_list = TouShiData.Instance:GetHuanHuaCfgList()
...
function TouShiHuanHuaView:UITween()
...
function TouShiHuanHuaCell:ListenClick(handler)
...
function TouShiHuanHuaCell:SetData(data)
	if nil == data then
		return
	end
	...
	if TouShiData.Instance:GetHuanHuaGrade(data.image_id) < TouShiData.Instance:GetSpecialImgMaxLevel() and
