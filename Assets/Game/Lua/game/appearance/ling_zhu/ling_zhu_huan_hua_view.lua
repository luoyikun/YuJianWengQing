...
function LingZhuHuanHuaView:__init()
	...
	self.def_index = TabIndex.appearance_lingzhu_huanhua
...
function LingZhuHuanHuaView:LoadCallBack()
	local tab_cfg = {
		{name = Language.Common.Huanhua, bundle = "uis/images_atlas", asset = "tab_icon_lingzhu", tab_index = TabIndex.appearance_lingzhu_huanhua, remind_id = RemindName.LingZhuHuanHua},
	}
	...
	RemindManager.Instance:Fire(RemindName.LingZhuHuanHua)
...
function LingZhuHuanHuaView:OnRoleDrag(data)
	if UIScene.role_model then
		UIScene:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function LingZhuHuanHuaView:SetRendering(value)
...
function LingZhuHuanHuaView:ClickAddGold()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function LingZhuHuanHuaView:ClickClose()
	self:Close()

	local ling_zhu_view = nil
	if AppearanceCtrl.Instance.view then
		ling_zhu_view = AppearanceCtrl.Instance.view:GetLingZhuView()
	end

	if ling_zhu_view and ling_zhu_view:IsOpen() then
		ling_zhu_view:UITween()
	end
end


--点击目标战力
function LingZhuHuanHuaView:ClickSuperPower()
	local cfg = self.list_data[self.select_index]
	local image_id = cfg and cfg.image_id
	local index = image_id or 0
	local data = LingZhuData.Instance:GetSpecialHuanHuaShowData(index)
...
function LingZhuHuanHuaView:ClickUpGrade()
	...
	local is_active = LingZhuData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local huanhua_cfg = LingZhuData.Instance:GetHuanHuaCfgInfo(data.image_id)
	...
	if not is_active then
		...
		UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_ZHU, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_IMAGE_UPGRADE, data.image_id)
	end
end
...
function LingZhuHuanHuaView:ClickUsed()
	...
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_ZHU, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, data.image_id)
end
...
function LingZhuHuanHuaView:FlushContent()
	...
	local now_level = LingZhuData.Instance:GetHuanHuaGrade(data.image_id)
	...
	local attr0 = LingZhuData.Instance:GetHuanHuaCfgInfo(data.image_id)
	local attr1 = LingZhuData.Instance:GetHuanHuaCfgInfo(data.image_id, now_level + 1)
	local max_grade = LingZhuData.Instance:GetSpecialImageMaxUpLevelById(data.image_id)
	self:SetAttr(now_level, attr0, attr1, max_grade, data.image_id)

	local is_show_super = LingZhuData.Instance:IsShowSuperPower(data.image_id)
	local is_active_super = LingZhuData.Instance:GetStarIsShowSuperPower(data.image_id)
	...
	if is_show_super and not is_active_super then
		local need_reach_level = LingZhuData.Instance:GetActiveSuperPowerNeedLevel(data.image_id)
	...
	local is_active = LingZhuData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local is_used = LingZhuData.Instance:GetHuanHuaIdIsUsed(data.image_id)
...
function LingZhuHuanHuaView:SetAttr(special_image_grade, attr0, attr1, max_grade, special_index)
	...
	local active_grade, attr_type, attr_value = LingZhuData.Instance:GetHuanHuaSpecialAttrActiveType(nil, special_index)
	if active_grade and attr_type and attr_value then
		if special_image_grade < active_grade then
			...
			for i = special_image_grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = LingZhuData.Instance:GetHuanHuaSpecialAttrActiveType(i, special_index)
...
function LingZhuHuanHuaView:FlushModel()
	...
	PrefabPreload.Instance:StopLoad(self.prefab_preload_id)
	local bundle, asset = ResPath.GetLingZhuModel(data.res_id, true)
...
function LingZhuHuanHuaView:ShowIndexCallBack(index, index_nodes)
	...
	local callback = function ()
		...
		local transform = RoleModel.GetModelCameraSettingByType(MODEL_CAMERA_TYPE.TIPS, "lingzhu")
...
function LingZhuHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function LingZhuHuanHuaView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "all" then
			if v and v.id then
				local index, num = LingZhuData.Instance:CanHuanhuaIndexByImageId(v.id)
				if index then
					self.select_index = index
					local cfg_list = LingZhuData.Instance:GetHuanHuaCfgList()
...
function LingZhuHuanHuaView:UITween()
...
function LingZhuHuanHuaCell:ListenClick(handler)
...
function LingZhuHuanHuaCell:SetData(data)
	if nil == data then
		return
	end
	...
	if LingZhuData.Instance:GetHuanHuaGrade(data.image_id) < LingZhuData.Instance:GetSpecialImgMaxLevel() and
