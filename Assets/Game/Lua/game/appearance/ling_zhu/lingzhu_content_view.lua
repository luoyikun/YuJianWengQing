...
function LingZhuContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self:GetSkill()
end

function LingZhuContentView:LoadCallBack(instance)
...
function LingZhuContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = LingZhuData.Instance:GetSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = LingZhuData.Instance:GetSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetLingZhuSkillIcon(i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickLingZhuSkill, self, i))
	end
end
...
-- 点击灵珠成长按钮
function LingZhuContentView:ClickGrowup()
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	if nil == lingzhu_info or nil == lingzhu_info.grade then return end
	local is_activie = lingzhu_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "lingzhuchengzhang", {item_id = LingZhuShuXingDanId.ChengZhangDanId})
	end
end
...
function LingZhuContentView:ClickZiZhi()
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	if nil == lingzhu_info or nil == lingzhu_info.grade then return end
	local is_activie = lingzhu_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "lingzhu_zizhi", {item_id = LingZhuShuXingDanId.ZiZhiDanId})
	end
end

-- 点击灵珠技能
function LingZhuContentView:ClickLingZhuSkill(index)
	if self.is_shake_skill and index == 1 then
		self.node_list["Special"].animator:SetBool("IsShake", false)
		self.is_shake_skill = nil
	end	
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "lingzhuskill", {index = index - 1})
end
...
function LingZhuContentView:ClickUse()
	...
	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_ZHU, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, grade_info.image_id)
end
...
function LingZhuContentView:ClickUpgrade(is_auto_upgrade)
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	if nil == lingzhu_info then return end
	...
	local close_func = function()
		...
		if not is_auto_upgrade then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_ZHU, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.LING_ZHU, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE,grade_info.pack_num, is_auto_buy)
		end
	end
	...
	local is_have_zhishengdan, item_id = LingZhuData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, lingzhu_info.grade), ok_callback, close_func)
	    return
	end
...
function LingZhuContentView:OpenCallBack()
	...
	self.select_grade = 0
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	local is_huanhua_img = LingZhuData.Instance:IsHuanHuaImage(lingzhu_info.used_imageid)
	if nil ~= lingzhu_info then
		...
		if lingzhu_info.grade == 1 then
			...
			if is_huanhua_img then
				...
				local image_cfg = LingZhuData.Instance:GetLingZhuImageCfgInfoByImageId(lingzhu_info.used_imageid)
				if nil == image_cfg then return end
				-- show_grade为客户端阶数由0开始，服务器阶数由1开始，所以要加1
				self.select_grade = image_cfg.show_grade + 1
			end
...
function LingZhuContentView:FlushSkillIcon()
	local lingzhu_skill_list = LingZhuData.Instance:GetLingZhuInfo().skill_level_list

	if nil == lingzhu_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node, lingzhu_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = LingZhuData.Instance:GetSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = LingZhuData.Instance:GetSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	if lingzhu_info ~= nil or next(lingzhu_info) ~= nil then
		if lingzhu_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end
...
function LingZhuContentView:FlushModel()
	...
	local image_cfg = LingZhuData.Instance:GetLingZhuImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>灵珠_ID:" .. grade_info.image_id .. "</color>")
...
function LingZhuContentView:FlushLeft()
	...
	-- 刷新使用状态
	local is_used = temp_grade_info.image_id == lingzhu_info.used_imageid
	...
	-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = LingZhuData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(LingZhuData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(LingZhuData.Instance:IsShowGrowupRemind())
	local is_show_huanhua_remind = LingZhuData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(lingzhu_info)
	self:SetBtnGrowup(lingzhu_info)
end
...
function LingZhuContentView:FlushRight()
	...
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = LingZhuData.Instance:GetLingZhuGradeCfgInfoByGrade(lingzhu_info.grade + 1)
	local bless_value_ratio = lingzhu_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = LingZhuData.Instance:GetLingZhuMaxGrade()
	local active_grade, attr_type, attr_value = LingZhuData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if lingzhu_info.grade < active_grade then
			...
			for i = lingzhu_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = LingZhuData.Instance:GetSpecialAttrActiveType(i)
	...
	-- print_error("switch_attr_list",switch_attr_list)
	...
	-- else
	-- 	local level, add_attr = LingZhuData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	...
	-- if grade_info and next_grade_info and lingzhu_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", lingzhu_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(lingzhu_info, grade_info)
end

--祝福值提示 清空祝福值 清空时间
function LingZhuContentView:ClearBlessTip(lingzhu_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if lingzhu_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = lingzhu_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function LingZhuContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end
...
-- 设置进阶按钮状态
function LingZhuContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function LingZhuContentView:SetBtnZiZhi(lingzhu_info)
 	local is_activie = lingzhu_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function LingZhuContentView:SetBtnGrowup(lingzhu_info)
	local is_activie = lingzhu_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
function LingZhuContentView:FlushItem()
	...
	self.node_list["RemindBtn"]:SetActive(LingZhuData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end
...
function LingZhuContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function LingZhuContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_LINGZHU
	...
	-- local lingzhu_info = LingZhuData.Instance:GetLingZhuInfo()
	-- if lingzhu_info == nil or lingzhu_info.grade == nil then
	-- 	return
	-- end
	
	-- local cur_img_grade = lingzhu_info.grade - 1
...
function LingZhuContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function LingZhuContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function LingZhuContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function LingZhuContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function LingZhuContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function LingZhuContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function LingZhuContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function LingZhuContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function LingZhuContentView:RemoveCountDown()
...
--打开大目标面板
function LingZhuContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_LINGZHU)
end

--打开小目标面板
function LingZhuContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_LINGZHU
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_LINGZHU, callback)
