...
function QilinBiContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self:GetSkill()
end

function QilinBiContentView:LoadCallBack(instance)
...
function QilinBiContentView:CloseCallBack()
	if self.count_down ~= nil then
		CountDown.Instance:RemoveCountDown(self.count_down)
		self.count_down = nil
	end
...
function QilinBiContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = QilinBiData.Instance:GetQiLinBiSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = QilinBiData.Instance:GetQiLinBiSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetQiLinBiSkillIcon(i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickQiLinBiSkill, self, i))
	end
end
...
-- 点击麒麟臂成长按钮
function QilinBiContentView:ClickGrowup()
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if nil == qilinbi_info or nil == qilinbi_info.grade then return end
	local is_activie = qilinbi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "qilinbichengzhang", {item_id = QilinBiShuXingDanId.ChengZhangDanId})
	end
end
...
function QilinBiContentView:ClickZiZhi()
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if nil == qilinbi_info or nil == qilinbi_info.grade then return end
	local is_activie = qilinbi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "qilinbi_zizhi",  {item_id = QilinBiShuXingDanId.ZiZhiDanId})
	end
end

-- 点击进阶装备
function QilinBiContentView:OnClickEquipBtn()
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if nil ~= qilinbi_info and nil ~= qilinbi_info.grade then
		if qilinbi_info.grade <= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL then
			...
			ViewManager.Instance:Open(ViewName.AppearacneEquipView, TabIndex.appearance_qilinbi)
		end
	end
end

-- 点击麒麟臂技能
function QilinBiContentView:ClickQiLinBiSkill(index)
	if self.is_shake_skill and index == 1 then
		self.node_list["Special"].animator:SetBool("IsShake", false)
		self.is_shake_skill = nil
	end	
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "qilinbiskill", {index = index - 1})
end
...
function QilinBiContentView:ClickUse()
	...
	QilinBiCtrl.Instance:SendQiLinBiReq(QILINBI_OPERA_TYPE.QILINBI_OPERA_TYPE_USE_IMAGE, grade_info.image_id)
end
...
function QilinBiContentView:ClickUpgrade(is_auto_upgrade)
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if nil == qilinbi_info then return end
	...
	local close_func = function()
		...
		if not is_auto_upgrade then
			QilinBiCtrl.Instance:SendQiLinBiReq(QILINBI_OPERA_TYPE.QILINBI_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			QilinBiCtrl.Instance:SendQiLinBiReq(QILINBI_OPERA_TYPE.QILINBI_OPERA_TYPE_UPGRADE, grade_info.pack_num, is_auto_buy)
		end
	end
	...
	local is_have_zhishengdan, item_id = QilinBiData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, qilinbi_info.grade), ok_callback, close_func)
	    return
	end
...
function QilinBiContentView:OpenCallBack()
	...
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.QILINBI)
...
function QilinBiContentView:FlushSkillIcon()
	local qilinbi_skill_list = QilinBiData.Instance:GetQilinBiInfo().skill_level_list

	if nil == qilinbi_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node, qilinbi_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = QilinBiData.Instance:GetQiLinBiSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = QilinBiData.Instance:GetQiLinBiSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local qilinbi_info = QilinBiData.Instance:GetQilinBiInfo()
	if qilinbi_info ~= nil or next(qilinbi_info) ~= nil then
		if qilinbi_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end
...
function QilinBiContentView:FlushModel()
	...
	-- print_warning("<color=#07F862FF>麒麟臂_ID:" .. grade_info.image_id .. "</color>")
	-- 麒麟臂模型根据角色的性别加载不同的模型
...
function QilinBiContentView:FlushLeft()
	...
	--刷新使用状态
	local is_used = temp_grade_info.image_id == qilinbi_info.used_imageid
	...
	-- 刷新左侧技能列表特效
	local can_upgrade_skill_list = QilinBiData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(QilinBiData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(QilinBiData.Instance:IsShowGrowupRemind())
	self.node_list["ShowEquipRemind"]:SetActive(QilinBiData.Instance:CalAllEquipRemind() > 0)
	local is_show_huanhua_remind = QilinBiData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(qilinbi_info)
	self:SetBtnGrowup(qilinbi_info)
	self:SetBtnEquip(qilinbi_info)
end
...
function QilinBiContentView:FlushRight()
	...
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = QilinBiData.Instance:GetQilinBiGradeCfgInfoByGrade(qilinbi_info.grade + 1)
	local bless_value_ratio = qilinbi_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = QilinBiData.Instance:GetQiLinBiMaxGrade()
	local active_grade, attr_type, attr_value = QilinBiData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if qilinbi_info.grade < active_grade then
			...
			for i = qilinbi_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = QilinBiData.Instance:GetSpecialAttrActiveType(i)
	...
	-- else
	-- 	local level, add_attr = QilinBiData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	...
	-- if grade_info and next_grade_info and qilinbi_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", qilinbi_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(qilinbi_info, grade_info)
end


--祝福值提示 清空祝福值 清空时间
function QilinBiContentView:ClearBlessTip(qilinbi_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if qilinbi_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = qilinbi_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function QilinBiContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end
...
-- 设置进阶按钮状态
function QilinBiContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function QilinBiContentView:SetBtnZiZhi(qilinbi_info)
 	local is_activie = qilinbi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function QilinBiContentView:SetBtnGrowup(qilinbi_info)
	local is_activie = qilinbi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
-- 设置装备按钮状态
function QilinBiContentView:SetBtnEquip(qilinbi_info)
	local is_activie = qilinbi_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL
	UI:SetGraphicGrey(self.node_list["BtnEquipButton"], not is_activie)
	self.node_list["TextEquip"].text.text = is_activie and Language.MultiMount.QiLinBiEquipType or 
...
function QilinBiContentView:FlushItem()
	...
	self.node_list["RemindBtn"]:SetActive(QilinBiData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end
...
function QilinBiContentView:UITween()
	...
	UITween.MoveShowPanel(self.node_list["BtnGropPanel"], Vector3(72, 430, 0), 0.7)
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function QilinBiContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_QILINBI
...
function QilinBiContentView:ClickActIcon()
...
function QilinBiContentView:SetActTime()
...
function QilinBiContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function QilinBiContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function QilinBiContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function QilinBiContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function QilinBiContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function QilinBiContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function QilinBiContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function QilinBiContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function QilinBiContentView:RemoveCountDown()
...
--打开大目标面板
function QilinBiContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_QILINBI)
end

--打开小目标面板
function QilinBiContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_QILINBI
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_QILINBI, callback)
	TipsCtrl.Instance:ShowTimeLimitTitleView(data)
end

function QilinBiContentView:ToggleOnClick()
	local flag = SettingData.Instance:GetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.QILINBI) == 0 and 1 or 0
	SettingData.Instance:SetAdvanceTypeHideFlag(ADVANCE_HIDE_TYPE.QILINBI, flag)
	self.node_list["HighLight"]:SetActive(flag == 1)
	if flag == 1 then
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.Hide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.QILINBI])
	else
		SysMsgCtrl.Instance:ErrorRemind(Language.Common.UnHide .. Language.Advance.AdvanceHideType[ADVANCE_HIDE_TYPE.QILINBI])
	end
