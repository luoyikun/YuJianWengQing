-- 仙域-外观-麒麟臂幻化-AppearanceHuanHuaContent
...
function QilinBiHuanHuaView:__init()
	...
	self.def_index = TabIndex.appearance_qilinbi_huanhua
...
function QilinBiHuanHuaView:LoadCallBack()
	local tab_cfg = {
		{name = Language.Common.Huanhua, bundle = "uis/images_atlas", asset = "tab_icon_qilinbi", tab_index = TabIndex.appearance_qilinbi_huanhua, remind_id = RemindName.QilinBiHuanHua},
	}
	...
	RemindManager.Instance:Fire(RemindName.QilinBiHuanHua)
...
function QilinBiHuanHuaView:OnRoleDrag(data)
	if UIScene.role_model then
		UIScene:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function QilinBiHuanHuaView:SetRendering(value)
...
function QilinBiHuanHuaView:ClickAddGold()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function QilinBiHuanHuaView:ClickClose()
	self:Close()

	local qilinbi_view = nil
	if AppearanceCtrl.Instance.view then
		qilinbi_view = AppearanceCtrl.Instance.view:GetQiLinBiView()
	end

	if qilinbi_view and qilinbi_view:IsOpen() then
		qilinbi_view:UITween()
	end
end

--点击目标战力
function QilinBiHuanHuaView:ClickSuperPower()
	local cfg = self.list_data[self.select_index]
	local image_id = cfg and cfg.image_id
	local index = image_id or 0
	local data = QilinBiData.Instance:GetSpecialHuanHuaShowData(index)
...
function QilinBiHuanHuaView:ClickUpGrade()
	...
	local is_active = QilinBiData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local huanhua_cfg = QilinBiData.Instance:GetHuanHuaCfgInfo(data.image_id)
	...
	if not is_active then
		...
		QilinBiCtrl.Instance:SendQiLinBiReq(QILINBI_OPERA_TYPE.QILINBI_OPERA_TYPE_SPECIAL_IMAGE_UPGRADE, data.image_id)
	end
end
...
function QilinBiHuanHuaView:ClickUsed()
	...
	QilinBiCtrl.Instance:SendQiLinBiReq(QILINBI_OPERA_TYPE.QILINBI_OPERA_TYPE_USE_IMAGE, image_id)
end
...
function QilinBiHuanHuaView:FlushContent()
	...
	local now_level = QilinBiData.Instance:GetHuanHuaGrade(data.image_id)
	...
	local attr0 = QilinBiData.Instance:GetHuanHuaCfgInfo(data.image_id)
	local attr1 = QilinBiData.Instance:GetHuanHuaCfgInfo(data.image_id, now_level + 1)
	local max_grade = QilinBiData.Instance:GetSpecialImageMaxUpLevelById(data.image_id)
	self:SetAttr(now_level, attr0, attr1, max_grade, data.image_id)

	local is_show_super = QilinBiData.Instance:IsShowSuperPower(data.image_id)
	local is_active_super = QilinBiData.Instance:GetStarIsShowSuperPower(data.image_id)
	...
	if is_show_super and not is_active_super then
		local need_reach_level = QilinBiData.Instance:GetActiveSuperPowerNeedLevel(data.image_id)
	...
	local is_active = QilinBiData.Instance:GetHuanHuaIsActiveByImageId(data.image_id)
	local is_used = QilinBiData.Instance:GetHuanHuaIdIsUsed(data.image_id, true)
...
function QilinBiHuanHuaView:SetAttr(special_image_grade, attr0, attr1, max_grade, special_index)
	...
	local active_grade, attr_type, attr_value = QilinBiData.Instance:GetHuanHuaSpecialAttrActiveType(nil, special_index)
	if active_grade and attr_type and attr_value then
		if special_image_grade < active_grade then
			...
			for i = special_image_grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = QilinBiData.Instance:GetHuanHuaSpecialAttrActiveType(i, special_index)
...
function QilinBiHuanHuaView:ShowIndexCallBack(index, index_nodes)
...
function QilinBiHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function QilinBiHuanHuaView:OnFlush(param_t)
	for k, v in pairs(param_t) do
		if k == "all" then
			if v and v.id then
				local index, num = QilinBiData.Instance:CanHuanhuaIndexByImageId(v.id)
				if index then
					self.select_index = index
					local cfg_list = QilinBiData.Instance:GetHuanHuaCfgList()
...
function QilinBiHuanHuaView:UITween()
...
function QilinBiHuanHuaCell:ListenClick(handler)
...
function QilinBiHuanHuaCell:SetData(data)
	if nil == data then
		return
	end
	...
	if QilinBiData.Instance:GetHuanHuaGrade(data.image_id) < QilinBiData.Instance:GetSpecialImgMaxLevel() and
