-- 仙域-外观-尾焰-Content
WeiYanContentView = WeiYanContentView or BaseClass(BaseRender)

local SHOWSPECGRADE = 10
function WeiYanContentView:__init(instance)
	self.tesu_index = 0
	self.skill_list = {}
	self:GetSkill()
end

function WeiYanContentView:LoadCallBack(instance)
...
function WeiYanContentView:__delete()
...
function WeiYanContentView:CloseCallBack()
	if self.count_down ~= nil then
		CountDown.Instance:RemoveCountDown(self.count_down)
		self.count_down = nil
	end
...
function WeiYanContentView:GetSkill()
	for i = 1, 4 do
		local cur_level = 0
		local next_level = 1
		local skill = nil
		self.cur_data = WeiYanData.Instance:GetSkillCfgById(i - 1) or {}
		if next(self.cur_data) then
			cur_level = self.cur_data.skill_level
			next_level = cur_level + 1
		end
		self.next_data_cfg = WeiYanData.Instance:GetSkillCfgById(i - 1, next_level) or {}
		...
		local bundle, asset = ResPath.GetSkillIcon("WeiYanSkill_" .. i)
		...
		skill.toggle:AddClickListener(BindTool.Bind(self.ClickWeiYanSkill, self, i))
	end
end

-- 点击尾焰幻化按钮
function WeiYanContentView:ClickHuanHua()
	ViewManager.Instance:Open(ViewName.WeiYanHuanHua)
end

-- 点击尾焰成长按钮
function WeiYanContentView:ClickGrowup()
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	if nil == weiyan_info or nil == weiyan_info.grade then return end
	local is_activie = weiyan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
	if not is_activie then
		...
		ViewManager.Instance:Open(ViewName.TipChengZhang, nil, "weiyanchengzhang", {item_id = WeiYanShuXingDanId.ChengZhangDanId})
	end
end

-- 点击尾焰资质按钮
function WeiYanContentView:ClickZiZhi()
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	if nil == weiyan_info or nil == weiyan_info.grade then return end
	local is_activie = weiyan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
	if not is_activie  then
		...
		ViewManager.Instance:Open(ViewName.TipZiZhi, nil, "weiyan_zizhi", {item_id = WeiYanShuXingDanId.ZiZhiDanId})
	end
end

-- 点击进阶装备
function WeiYanContentView:OnClickEquipBtn()
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	if nil ~= weiyan_info and nil ~= weiyan_info.grade then
		if weiyan_info.grade <= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL then
			...
			ViewManager.Instance:Open(ViewName.AppearacneEquipView, TabIndex.appearance_weiyan)
		end
	end
end

-- 点击尾焰技能
function WeiYanContentView:ClickWeiYanSkill(index)
	if self.is_shake_skill and index == 1 then
		self.node_list["Special"].animator:SetBool("IsShake", false)
		self.is_shake_skill = nil
	end	
	ViewManager.Instance:Open(ViewName.TipSkillUpgrade, nil, "weiyan_skill", {index = index - 1})
end

function WeiYanContentView:ClickUse()
	local grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end

	UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.WEI_YAN, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_USE_IMAGE, 0, grade_info.image_id)
end

function WeiYanContentView:ClickLeft()
	self.select_grade = self.select_grade - 1
	self:FlushView()
end

function WeiYanContentView:ClickRight()
	self.select_grade = self.select_grade + 1
	self:FlushView()
end

function WeiYanContentView:ClickUpgrade(is_auto_upgrade)
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	if nil == weiyan_info then return end
	local grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(weiyan_info.grade)
	if nil == grade_info then return end

	local close_func = function()
		--获取下一级，不存在则满级
		local next_grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(weiyan_info.grade + 1)
		...
		if not is_auto_upgrade then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.WEI_YAN, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE, 1, is_auto_buy)
			return
		end
		...
		if self.is_upgrade_state then
			UpgradeCtrl.Instance:SendUpgradeReq(UPGRADE_TYPE.WEI_YAN, UPGRADE_OPERA_TYPE.UPGRADE_OPERA_TYPE_UPGRADE,grade_info.pack_num, is_auto_buy)
		end
	end
	...
	local is_have_zhishengdan, item_id = WeiYanData.Instance:IsHaveZhiShengDanInGrade()
	...
	if is_have_zhishengdan and self.is_can_tip and item then
	    ...
	    TipsCtrl.Instance:ShowCommonAutoView(nil, string.format(Language.Advance.IsUseZhiShengDan, weiyan_info.grade), ok_callback, close_func)
	    return
	end
...
function WeiYanContentView:OpenCallBack()
	...
	self.select_grade = 0
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	local is_huanhua_img = WeiYanData.Instance:IsHuanHuaImage(weiyan_info.used_imageid)
	if nil ~= weiyan_info then
		--默认选择已使用的形象阶数
		if weiyan_info.grade == 1 then
			--零阶和一阶使用相同形象
			self.select_grade = weiyan_info.grade
		else
			if is_huanhua_img then
				...
				local image_cfg = WeiYanData.Instance:GetWeiYanImageCfgInfoByImageId(weiyan_info.used_imageid)
				if nil == image_cfg then return end
				-- show_grade为客户端阶数由0开始，服务器阶数由1开始，所以要加1
				self.select_grade = image_cfg.show_grade + 1
			end
...
function WeiYanContentView:FlushView()
	self:FlushLeft()
	self:FlushRight()
	self:FlushSkillIcon()
end

function WeiYanContentView:FlushSkillIcon()
	local weiyan_skill_list = WeiYanData.Instance:GetWeiYanInfo().skill_level_list

	if nil == weiyan_skill_list then
		return
	end
	...
	for k, v in pairs(self.skill_list) do
		...
		if node then
			UI:SetGraphicGrey(node, weiyan_skill_list[k - 1] == 0)
		end
	end
	local cur_level = 0
	local next_level = 1
	local cur_data = WeiYanData.Instance:GetSkillCfgById(self.tesu_index - 1) or {}
	if next(cur_data) then
		cur_level = cur_data.skill_level
		next_level = cur_level + 1
	end
	local next_data_cfg = WeiYanData.Instance:GetSkillCfgById(self.tesu_index - 1, next_level) or {}
	...
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	if weiyan_info ~= nil or next(weiyan_info) ~= nil then
		if weiyan_info.grade > SHOWSPECGRADE then
			self.node_list["SpecialSkill"]:SetActive(true)
			self.node_list["SpecialSkillText"]:SetActive(true)
		end
	end
...
function WeiYanContentView:FlushModel()
	local grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(self.select_grade)
	if nil == grade_info then return end
	local image_cfg = WeiYanData.Instance:GetWeiYanImageCfgInfoByImageId(grade_info.image_id)
	if nil == image_cfg then return end
	-- print_warning("<color=#07F862FF>尾焰_ID:" .. grade_info.image_id .. "</color>")
	...
	local call_back = function(model, obj)
		...
		if advance_transform_cfg then
			UIScene:MoveObjResetPos(advance_transform_cfg)
		else
...
function WeiYanContentView:FlushLeft()
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	if nil == weiyan_info then return end

	--初始记录旧阶数
	if self.old_grade < 0 then
		self.old_grade = weiyan_info.grade
	end

	if self.old_grade ~= weiyan_info.grade then
		--进阶成功
	...
	local temp_grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(self.select_grade)
	if nil == temp_grade_info then return end
	local image_info = WeiYanData.Instance:GetWeiYanImageCfgInfoByImageId(temp_grade_info.image_id)
	...
	-- 刷新使用状态
	local is_used = temp_grade_info.image_id == weiyan_info.used_imageid
	if weiyan_info.grade >= self.select_grade then
	...
	--刷新左右箭头
	local last_temp_grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(self.select_grade - 1)
	local is_show_left = true
	if nil == last_temp_grade_info or (weiyan_info.grade > 1 and temp_grade_info.show_grade == 1) then
		--没有上一阶属性或者处于第一阶(服务器第二阶)不显示左箭头
		is_show_left = false
	end
	self.node_list["BtnLeftButton"]:SetActive(is_show_left)

	local next_temp_grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(self.select_grade + 1)
	local is_show_right = true
	if nil == next_temp_grade_info or self.select_grade > weiyan_info.grade then
		is_show_right = false
	end
	self.node_list["BtnRightButton"]:SetActive(is_show_right)

	--刷新下面技能列表特效
	local can_upgrade_skill_list = WeiYanData.Instance:CanSkillUpLevelList()
	...
	self.node_list["EffectZiZhi"]:SetActive(WeiYanData.Instance:IsShowZiZhiRemind())
	self.node_list["EffectBtnGrowup"]:SetActive(WeiYanData.Instance:IsShowGrowupRemind())
	self.node_list["ShowEquipRemind"]:SetActive(WeiYanData.Instance:CalAllEquipRemind() > 0)
	local is_show_huanhua_remind = WeiYanData.Instance:CalcHuanHuaRemind() == 1 and true or false
	self.node_list["HuanhuaRedPoint"]:SetActive(is_show_huanhua_remind)
	self:SetBtnZiZhi(weiyan_info)
	self:SetBtnGrowup(weiyan_info)
	self:SetBtnEquip(weiyan_info)
end

function WeiYanContentView:FlushRight()
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	if nil == weiyan_info then return end
	local grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(weiyan_info.grade)
	if nil == grade_info then return end
	-- 获取下一阶属性，如果存在则不是最大等级
	local next_grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(weiyan_info.grade + 1)
	local bless_value_ratio = weiyan_info.grade_bless_val / grade_info.bless_val_limit
	...
	local max_grade = WeiYanData.Instance:GetWeiYanMaxGrade()
	local active_grade, attr_type, attr_value = WeiYanData.Instance:GetSpecialAttrActiveType()
	if active_grade and attr_type and attr_value then
		if weiyan_info.grade < active_grade then
			...
			for i = weiyan_info.grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = WeiYanData.Instance:GetSpecialAttrActiveType(i)
	...
	-- -- 特殊属性
	-- -- UI:SetGraphicGrey(self.node_list["TxtSpecialAttr"], grade_info.per_jingzhun == 0)
	-- local str_special_attr = ""
	-- if grade_info.per_jingzhun == 0 then
	-- 	str_special_attr = string.format(Language.MultiMount.SpecialAttribute[8] .. Language.MultiMount.LevelStart, 5)
	-- else
	-- 	local level, add_attr = WeiYanData.Instance:GetGradeAndSpecialAttr()
	-- 	local str = ""
	-- 	if level and add_attr then
	-- 		str = string.format(Language.Advance.NextAttr, CommonDataManager.GetDaXie(level - 1), add_attr/100)
	-- 	end
	-- 	str_special_attr = string.format(Language.MultiMount.SpecialAttribute[8], grade_info.per_jingzhun / 100) .. str
	-- end
	...
	-- if grade_info and next_grade_info and weiyan_info.grade_bless_val ~= 0 then
	...
	if next_grade_info then
		...
		self.node_list["TxtProgress"].text.text = string.format("%s/%s", weiyan_info.grade_bless_val, grade_info.bless_val_limit)
	else
		self.is_max = true
	...
	self:SetBtnState(self.is_upgrade_state, self.is_max)
	self:FlushItem()
	self:ClearBlessTip(weiyan_info, grade_info)
end

--祝福值提示 清空祝福值 清空时间
function WeiYanContentView:ClearBlessTip(weiyan_info, grade_cfg)
	-- 最大等级
	if self.is_max then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
	...
	-- 当祝福值为0要清空计时器
	if weiyan_info.grade_bless_val == 0 then
		if nil ~= self.clear_count_down then
			CountDown.Instance:RemoveCountDown(self.clear_count_down)
			self.clear_count_down = nil
		end
	...
	if APPEARANCE_CLEAR_BLESS.CLEAR == grade_cfg.is_clear_bless then
		...
		local clear_bless_time = weiyan_info.clear_upgrade_time
...
-- 计时器,用于取消默认延迟1s显示
function WeiYanContentView:ClickTimer(remaining_time)
	remaining_time = remaining_time - 1
	local tmp_time = TimeUtil.FormatSecond(remaining_time - 1)
	self.node_list["TxtBlessTip"].text.text = string.format(Language.MultiMount.ClearBlessStr,tostring(tmp_time))
end

function WeiYanContentView:UpGradeResult(result)
...
-- 设置进阶按钮状态
function WeiYanContentView:SetBtnState(is_upgrade_state, is_max)
...
-- 设置资质丹按钮状态
function WeiYanContentView:SetBtnZiZhi(weiyan_info)
 	local is_activie = weiyan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.ZIZHIDAN
...
-- 设置成长丹按钮状态
function WeiYanContentView:SetBtnGrowup(weiyan_info)
	local is_activie = weiyan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.CHENGZHANGDAN
...
-- 设置装备按钮状态
function WeiYanContentView:SetBtnEquip(weiyan_info)
	local is_activie = weiyan_info.grade - 1 >= APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL
	UI:SetGraphicGrey(self.node_list["BtnEquipButton"], not is_activie)
	self.node_list["TextEquip"].text.text = is_activie and Language.MultiMount.WeiYanEquipType or 
			string.format(Language.MultiMount.UpGradeStart, Language.Common.NumToChs[APPEARANCE_SHUXINGDAN_LIMIT.EQUIPLEVEL])
end

function WeiYanContentView:FlushItem()
	local weiyan_info = WeiYanData.Instance:GetWeiYanInfo()
	if nil == weiyan_info then return end
	local grade_info = WeiYanData.Instance:GetWeiYanGradeCfgInfoByGrade(weiyan_info.grade)
	if nil == grade_info then return end
	...
	self.node_list["RemindBtn"]:SetActive(WeiYanData.Instance:IsShowUpgradeBtnRemind() and not self.is_upgrade_state)
end

function WeiYanContentView:PlayEffect()
...
function WeiYanContentView:OnFlush(param_t)
	self:JinJieReward()
	for k, v in pairs(param_t) do
		if k == "all" then
			self:FlushView()
			self:FlushItem()
		elseif k == "weiyan" then
			self:FlushView()
		elseif k == "weiyan_upgrade" then
			self:UpGradeResult(v[1])
		elseif k == "weiyan_item_change" then
			self:FlushItem()
		end
	end
end

function WeiYanContentView:UITween()
...
--------------------------------------------------进阶奖励相关显示---------------------------------------------------
--进阶奖励相关
function WeiYanContentView:JinJieReward()
	local system_type = JINJIE_TYPE.JINJIE_TYPE_WEIYAN
...
function WeiYanContentView:ClickActIcon()
...
function WeiYanContentView:SetActTime()
...
function WeiYanContentView:ClearJinJieFreeData(target_type)
...
--大目标 变动显示
function WeiYanContentView:BigTargetNotConstantData(system_type, target_type)
...
--小目标 变动显示
function WeiYanContentView:SmallTargetNotConstantData(system_type, target_type)
...
--小目标固定显示
function WeiYanContentView:SmallTargetConstantData(system_type, target_type)
...
--大目标固定显示
function WeiYanContentView:BigTargetConstantData(system_type, target_type)
...
--刷新免费时间
function WeiYanContentView:FulshJinJieFreeTime(end_time, target_type)
	if end_time == 0 then
		self:ClearJinJieFreeData(target_type)
		return
	end
...
--设置进阶时间
function WeiYanContentView:SetJinJieFreeTime(time, target_type)
	if time > 0 then
...
--免费时间显示
function WeiYanContentView:FreeTimeShow(time, target_type)
	if target_type and target_type == JIN_JIE_REWARD_TARGET_TYPE.SMALL_TARGET then --小目标
...
--移除倒计时
function WeiYanContentView:RemoveCountDown()
...
--打开大目标面板
function WeiYanContentView:OnClickJinJieAward()
	JinJieRewardCtrl.Instance:OpenJinJieAwardView(JINJIE_TYPE.JINJIE_TYPE_WEIYAN)
end

--打开小目标面板
function WeiYanContentView:OnClickOpenSmallTarget()
	local function callback()
		local param1 = JINJIE_TYPE.JINJIE_TYPE_WEIYAN
	...
	local data = JinJieRewardData.Instance:GetSmallTargetShowData(JINJIE_TYPE.JINJIE_TYPE_WEIYAN, callback)
