...
function YunbiaoData:GetBuyHusonGold()
	return ConfigManager.Instance:GetAutoConfig("husongcfg_auto").buy_times_cfg
end

function YunbiaoData:GetHuSongSceneId()
	return ConfigManager.Instance:GetAutoConfig("husongcfg_auto").other[1].escort_scene
end
...
function YunbiaoData:GetRewardConfig()
	...
	local husong_cfg = ConfigManager.Instance:GetAutoConfig("husongcfg_auto")

	if reward_config and husong_cfg then
		for i = 1, 5 do
			local factor = husong_cfg.task_reward_factor_list[i] and husong_cfg.task_reward_factor_list[i].factor or 0
			local level = GameVoManager.Instance:GetMainRoleVo().level
			local cfg_exp = PlayerData.Instance:GetFBExpByLevel(level)
			local exp_factor = husong_cfg.other[1].exp_factor
			local exp = math.floor(cfg_exp * exp_factor * factor / 100)
