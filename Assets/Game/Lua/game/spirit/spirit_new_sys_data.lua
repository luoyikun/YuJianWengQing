...
function SpiritNewSysData:CreateUnFightLevelList(protocol)
	...
	local cfg = ConfigManager.Instance:GetAutoConfig("jingling_auto").uplevel
	for i=0,3 do
		self.unfight_level_list[i] = {}
		local level_attr = SpiritData.Instance:GetSpiritLevelCfgByLevel(i,1)
		if protocol[i] then
			local grade = protocol[i].param and protocol[i].param.strengthen_level
			self.grade_list[i] = grade
			grade = cfg[grade + 1] and grade or grade - 1
			if grade > 1 then
				self.unfight_level_list[i].maxhp = level_attr.maxhp * 1 + cfg[grade + 1].maxhp
				self.unfight_level_list[i].gongji = level_attr.gongji * 1 + cfg[grade + 1].gongji
				self.unfight_level_list[i].fangyu = level_attr.fangyu * 1 + cfg[grade + 1].fangyu
			else
				self.unfight_level_list[i].maxhp = level_attr.maxhp * 1 
				self.unfight_level_list[i].gongji = level_attr.gongji * 1 
				self.unfight_level_list[i].fangyu = level_attr.fangyu * 1 
			end
		else
...
function SpiritNewSysData:CreateLevelList(protocol)
	...
	for i = 0, 3 do
		...
		if protocol[i] then
			...
			self.grade_list[i] = grade
			grade = cfg[grade + 1] and grade or grade - 1
			local level_attr = SpiritData.Instance:GetSpiritLevelCfgByLevel(i, grade)
			if nil ~= level_attr then
				self.level_list[i].maxhp = level_attr.maxhp
				self.level_list[i].gongji = level_attr.gongji
				self.level_list[i].fangyu = level_attr.fangyu
			end
		end
	end
end
...
function SpiritNewSysData:GetUseItem(cur_index)
	...
	cfg = ConfigManager.Instance:GetAutoConfig("jingling_auto").uplevel
	local grade = 0
...
function SpiritNewSysData:CreateAptitudeLevelList(protocol)
	if next(protocol) == nil then return end
	local cfg = ConfigManager.Instance:GetAutoConfig("jingling_auto").talent_attr
	for i=0,3 do
...
function SpiritNewSysData:GetAptitudeUseItem(cur_index)
	...
	cfg = ConfigManager.Instance:GetAutoConfig("jingling_auto").talent_attr
	local grade = 0
