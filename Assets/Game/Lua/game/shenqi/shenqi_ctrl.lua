...
function ShenqiCtrl:OnShenqiSingleInfo(protocol)
	...
	self:CheckBaoJia()
	if ViewManager.Instance:IsOpen(ViewName.ShenQiEffectTips) then
		ViewManager.Instance:FlushView(ViewName.ShenQiEffectTips)
	end
end
...
function ShenqiCtrl:MainuiOpenCreate()
	 -- 请求信息红点需要信息判断
