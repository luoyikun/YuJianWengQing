...
function ShenqiData:SetShenqiImageInfo(protocol)
	self.shenqi_all_info.info_type = protocol.info_type
	self.shenqi_all_info.texiao_open_flag = protocol.texiao_open_flag or 0
	if self.shenqi_all_info.info_type == SHENQI_SC_INFO_TYPE.SHENQI_SC_INFO_TYPE_SHENBING then
...
function ShenqiData:GetIsCanXiangQian(id, list, cfg)
	...
	local cur_stuff_list = self:GetXiangQianStuffListById(id, list, cfg)
		...
		for _, v in pairs(bag_data) do
			...
			if next(bag_stuff_cfg) and next(cur_stuff_list_cfg) then
...
-- 根据品质获取分解材料
function ShenqiData:GetFenjieListbyQuality(quality_list)
	self.fenjie_list = {}
	local data_list = ShenqiData.Instance:GetCanFenjieStuff()
	if data_list and quality_list then
		for k,v in pairs(data_list) do
			for k1,v1 in pairs(quality_list) do
				if v.quality == v1 then
					self.fenjie_list[k] = v
				end
			end
		end
	end
	return self.fenjie_list
end
...
function ShenqiData:GetResCfgByIamgeID(index, role_vo)
	...
	for k,v in pairs(shenbing_image_cfg) do
		if index == v.id and v["resource_id_" .. prof .. role_vo.sex] then
			return v["resource_id_" .. prof .. role_vo.sex]
		end
	end
	return 0
end
...
function ShenqiData:GetDataResCfgByIamgeID(data)
	local shenbing_image_cfg = self:GetShenbingImageCfg()
	local prof = PlayerData.Instance:GetRoleBaseProf(data.prof)
	for k,v in pairs(shenbing_image_cfg) do
		if data.shengbing_img_id == v.id and v["resource_id_" .. prof .. data.sex] then
			return v["resource_id_" .. prof .. data.sex]
		end
	end
	return 0
end
...
function ShenqiData:GetIsShowSbJianLingRp()
	...
	local shenqi_all_info1 = self:GetShenqiAllInfo()
	local shenbing_upgrade_cfg = self:GetShenbingUpgradeCfg()
	if shenbing_upgrade_cfg == nil or shenqi_all_info1 == nil 
		or shenqi_other_cfg1 == nil or nil == next(shenqi_all_info1.shenbing_list) then 
		return false 
	end
	...
	for i = 1, ShenqiData.ShenqiInfo.ShenBingNum do
		...
		local cur_level1 = shenqi_info1.level
		local next_level = cur_level1 + 1 < max_level and cur_level1 + 1 or max_level
		--999不触发红点机制
		local cur_level_exp1 = 9999
		local data = self:GetCfgByLevel(next_level, shenbing_upgrade_cfg)
		if cur_level1 ~= next_level then
...
function ShenqiData:GetBaoJiaUpdateRedPoint(index)
	...
	if baojia_upgrade_cfg == nil or shenqi_other_cfg == nil 
		or shenqi_all_info == nil or nil == next(shenqi_all_info.baojia_list) then 
		return false 
	end
	...
	local cfg = self:GetCfgByLevel(next_level, baojia_upgrade_cfg)
	if cfg == nil or nil == next(cfg) then return false end
	local cur_level_exp = cfg.need_exp
	local check_flag = false
...
function ShenqiData:GetIsShowBjQiLingRp()
	...
	for i = 1, ShenqiData.ShenqiInfo.BaoJiaNum do
		...
		local max_level = #baojia_upgrade_cfg 
		local next_level = cur_level + 1 < max_level and cur_level + 1 or max_level
		local data = self:GetCfgByLevel(next_level, baojia_upgrade_cfg)
...
function ShenqiData:GetAllJiangLingAttributeByIndex(index)
	local list = CommonStruct.Attribute()
	local list2 = CommonStruct.Attribute()
	local cfg = self.shenqi_all_info.shenbing_list[index]
	local shenbing_upgrade_cfg = self:GetShenbingUpgradeCfg()
	local data_exp = 0
	local level = 0
	local need_exp = shenbing_upgrade_cfg[1].need_exp
	if nil == cfg then
		return list, list2, data_exp, need_exp, level
	end

	local cur_exp = cfg.exp
	local cur_level = cfg.level
	level = cur_level or 0
 	data_exp = cur_exp or 0
	--当前属性
	--这里是升级属性
	local cur_jianling_cfg = shenbing_upgrade_cfg[cur_level]
	local max_level = #shenbing_upgrade_cfg 
	local next_level = cur_level + 1 < max_level and cur_level + 1 or max_level
	local next_cfg = shenbing_upgrade_cfg[next_level]
	need_exp = next_cfg.need_exp or 0

	if nil ~= cur_jianling_cfg then
		local attr_list = CommonDataManager.GetAttributteByClass(cur_jianling_cfg)
		list = CommonDataManager.AddAttributeAttr(list, attr_list)
	end

	if nil~= next_cfg then
			local attr_list1 = CommonDataManager.GetAttributteByClass(next_cfg)
			list2 = CommonDataManager.LerpAttributeAttr(list, attr_list1)
	end
	...
	for k,v in pairs(cfg.quality_list) do
		if 0 ~= v then
			...
			local temp_list = CommonDataManager.GetAttributteByClass(temp)
			list = CommonDataManager.AddAttributeAttr(list, temp_list)
		end
	end

	--激活属性
	local is_active1, is_active2 = self:GetJiangLingTeXiaoByIndex(index)
	if is_active1 and (not is_active2) then
		...
		local actives_list = CommonDataManager.GetAttributteByClass(active_list)
		list = CommonDataManager.AddAttributeAttr(list, actives_list)
	elseif is_active1 and is_active2 then
		local active_cfg = self:GetShenbingTexiaoCfg()
		if nil == active_cfg then
			return list, list2, data_exp, need_exp, level
		end

		local active_list = active_cfg[index + QIBINGNUM]
		if nil == active_list then
			return list, list2, data_exp, need_exp, level
		end
...
function ShenqiData:GetAllBaoJiaAttributeByIndex(index)
	...
	if nil ~= cur_baojia_cfg then
		local attr_list = CommonDataManager.GetAttributteByClass(cur_baojia_cfg)
		list = CommonDataManager.AddAttributeAttr(list, attr_list)
	end
	...
	--激活属性
	local is_active1, is_active2 = self:GetBaoJiaTeXiaoByIndex(index)
	if is_active and (not is_active2) then
	...
	elseif is_active1 and is_active2 then
		...
		local active_list = active_cfg[index + DUNJIANUM]
		if nil == active_list then
			return list, list2, data_exp, need_exp, level
		end
...
function ShenqiData:GetNeedQualitycfg(choose_type,index)
	local list_cfg  = {}
	if choose_type == ShenqiData.ChooseType.JianLing then
		list_cfg = self:GetShenbingImageCfg()
	elseif choose_type == ShenqiData.ChooseType.BaoJia then
		list_cfg = self:GetBaojiaImageCfg()
	end
	for k,v in pairs(list_cfg) do 
		if index == v.id then
			return v.need_quality
		end
	end
end
...
function ShenqiData:IsCanGo(index, choose_type)
	local data = nil
	local need_quality = 0
	if choose_type == ShenqiData.ChooseType.JianLing then
		data = self:GetJianLingInfo(index)
		need_quality = self:GetNeedQualitycfg(choose_type,index)
	elseif choose_type == ShenqiData.ChooseType.BaoJia then
	...
	for k,_ in pairs(data.quality_list) do
		if need_quality > data.quality_list[k] then
			return false
		end
	end
...
function ShenqiData:GetJiangLingTeXiaoByIndex(index)
	if nil ~= self.shenqi_all_info.shenbing_texiao_flag then
		return (self.shenqi_all_info.shenbing_texiao_flag[64 - index] == 1), (self.shenqi_all_info.shenbing_texiao_flag[64 - index - QIBINGNUM] == 1)
	end
...
function ShenqiData:GetBaoJiaTeXiaoByIndex(index)
	if nil ~= self.shenqi_all_info.baojia_texiao_flag then
		return (self.shenqi_all_info.baojia_texiao_flag[64 - index] == 1), (self.shenqi_all_info.baojia_texiao_flag[64 - index - DUNJIANUM] == 1)
	end
...
function ShenqiData:GetJianLingChangeRemind()
	...
	if OpenFunData.Instance:CheckIsHide("shenbing") and self:GetIsShowSbJianLingRp() then
		flag = 1
	end
...
function ShenqiData:GetBaoJiaChangeRemind()
	...
	for i =1 ,ShenqiData.ShenqiInfo.BaoJiaNum do
		local can_update = ShenqiData.Instance:GetBaoJiaUpdateRedPoint(i)
		local can_inlay = ShenqiData.Instance:GetIsShowBjXiangQiangRpByIndex(i)
		if can_update or can_inlay then
			return 1
		end
	end

	if OpenFunData.Instance:CheckIsHide("baojia") and self:GetIsShowBjQiLingRp() then
		flag = 1
	end
...
function ShenqiData:GetJianLingActiveByIndex(index)
	...
	if nil == quality or nil == quality.quality_list then
		return false
	end
...
function ShenqiData:GetBaoJiaActiveByIndex(index)
	local is_active1, is_active2 = self:GetBaoJiaTeXiaoByIndex(index)
	if is_active and is_active2 then
		return false
	end	
	--品质计算
	...
	local num = ItemData.Instance:GetItemNumInBagById(data[index].active_texiao_stuff_id)
	local is_active1, is_active2 = ShenqiData.Instance:GetBaoJiaTeXiaoByIndex(index)
	if is_active1 and is_active2 then
		return false
	end 
...
function ShenqiData:SetFJColor(fj_level)
	self.shenqi_color = fj_level
end

function ShenqiData:GetFJColor()
	return self.shenqi_color or {}
end

function ShenqiData:SetFenjielist(item,index)
	local key = nil
	for k, v in pairs(self.fenjie_list) do 
		if v.item_id == item and v.index == index then
			key = k
			break
		end
	end
	self.fenjie_list[key] = nil
end

function ShenqiData:SetFenjielist1(data)
		table.insert(self.fenjie_list, data)
end

function ShenqiData:GetFenjielist()
	return self.fenjie_list
end

function ShenqiData:GetJianLingTaoZhuangAttr(select_index)
	local cfg = self:GetShenbingTexiaoCfg()
	local attribute = CommonStruct.AttributeNoUnderline()
	local is_active, is_active1 = ShenqiData.Instance:GetJiangLingTeXiaoByIndex(select_index)
	if is_active == false and is_active1 == false then
		return attribute
	end

	if is_active1 then 
		select_index = select_index + 3
	end
	local info = cfg[select_index]
	if not info then 
		return attribute
	end

	attribute = CommonDataManager.GetAttributteNoUnderline(info)

	local per_attribute = CommonDataManager.GetRolePercentAttrNoUnderline(info)
	attribute = CommonDataManager.AddAttributeAttrNoUnderLine(attribute, per_attribute)

	return attribute
end

function ShenqiData:GetBaoJiaTaoZhuangAttr(select_index)
	local cfg = self:GetBaojiaTexiaoCfg()
	local attribute = CommonStruct.AttributeNoUnderline()
	local is_active, is_active1 = ShenqiData.Instance:GetBaoJiaTeXiaoByIndex(select_index)
	if is_active == false and is_active1 == false then
		return attribute
	end
...
function ShenqiData:GetShenBingNumCfg()
	local data_list = ListToMap(self:GetShenbingImageCfg(),"id") 
...
function ShenqiData:GetBaoJiaNumCfg()
	local data_list = ListToMap(self:GetBaojiaImageCfg(), "id")
	local open_day = TimeCtrl.Instance:GetCurOpenServerDay()
	local num = 0
	for k,v in pairs(data_list) do 
		if open_day >= v.open_day then
...
function ShenqiData:GetShenBingCfg()
	local data_list = ListToMap(self:GetShenbingImageCfg(),"id") 
	local open_day = TimeCtrl.Instance:GetCurOpenServerDay()
	local id_list = {}
	for k,v in pairs(data_list) do 
...
function ShenqiData:GetBaoJiaCfg()
	local data_list = ListToMap(self:GetBaojiaImageCfg(),"id") 
	local open_day = TimeCtrl.Instance:GetCurOpenServerDay()
	local id_list = {}
	for k,v in pairs(data_list) do 
