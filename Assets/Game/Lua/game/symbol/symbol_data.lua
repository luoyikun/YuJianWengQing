SymbolData = SymbolData or BaseClass(BaseEvent)
...
function SymbolData:__init()
	...
	local element_heart_cfg = ConfigManager.Instance:GetAutoConfig("element_heart_cfg_auto")
	local chou_cfg = element_heart_cfg.chou_cfg
	self.element_heart_openc_cfg = element_heart_cfg.element_heart_openc_fg
	self.element_heart_openc_cfg2 = ListToMap(self.element_heart_openc_cfg, "id")
	self.other_cfg = element_heart_cfg.other[1]
	self.element_heart_level_cfg = ListToMap(element_heart_cfg.element_heart_level, "level")
	self.element_heart_grade_cfg = ListToMap(element_heart_cfg.element_heart_grade, "grade")
	self.xilian_slot_opene_cfg = ListToMapList(element_heart_cfg.xilian_slot_open, "element_id")
	self.xilian_type_cfg = ListToMapList(element_heart_cfg.xilian_type, "element_id")
	self.order_cfg = ListToMapList(element_heart_cfg.order, "type")
	self.lock_consume_cfg = ListToMap(element_heart_cfg.lock_consume, "lock_num", "consume_color")
	self.element_shuxing_addition_cfg = ListToMapList(element_heart_cfg.element_shuxing_addition, "element_shuxing_type")
	self.xilian_consume_cfg = element_heart_cfg.xilian_consume
	self.item_cfg = ListToMap(element_heart_cfg.item_cfg, "item_id")
	self.item_type_cfg = ListToMapList(element_heart_cfg.item_cfg, "type")
	self.upgrade_limit_cfg = ListToMap(element_heart_cfg.upgrade_limit, "element_id")
	...
	self.element_texture_level_cfg = ListToMap(element_heart_cfg.element_texture_level,"wuxing_type","grade")
	self.inlay_cfg = ListToMap(element_heart_cfg.inlay,"e_index")
	...
	self.element_equip_shop_cfg = ListToMap(element_heart_cfg.element_equip_shop, "seq")
	self.element_equip_level_cfg = ListToMap(element_heart_cfg.element_equip_level, "real_level")
	self.element_equip_shuxing_cfg = ListToMap(element_heart_cfg.element_equip_shuxing, "item_id")
	...
	-- RemindManager.Instance:Register(RemindName.SymbolYuanZhuang, BindTool.Bind(self.GetSymbolYuanZhuangRemind, self))
...
function SymbolData:__delete()
	...
	-- RemindManager.Instance:UnRegister(RemindName.SymbolYuanZhuang)
...
-- 获取id对应的配置
function SymbolData:GetElementLimitByID(element_id)
	return self.element_heart_openc_cfg2[element_id] and self.element_heart_openc_cfg2[element_id]
end
...
function SymbolData:GetEquipByWuxing(index)
	return self.inlay_cfg[index].wuxing_type
end

--获取装备消耗的物品信息
function SymbolData:GetYSStuffCfg(wuxing_type)
	local cfg = DeepCopy(self.ys_stuff_cfg[wuxing_type])
	table.sort(cfg, function(a, b)
			local weight_a, weight_b = 0, 0
			if ItemData.Instance:GetItemNumInBagById(a.item_id) > 0 then
				weight_a = weight_a + 10
			end
			if ItemData.Instance:GetItemNumInBagById(b.item_id) > 0 then
...
function SymbolData:GetXiLianDefaultInfo()
	local stuff_cfg = {}
	local consume_cfg = self.xilian_consume_cfg
	...
	if not next(stuff_cfg) then
		stuff_cfg = consume_cfg[SymbolData.XiLianStuffColor.BLUE + 1]
	end
	return stuff_cfg
end
...
--获取洗练锁定配置
function SymbolData:GetElementXiLianLockCfg(lock_num, comsume_color)
	return self.lock_consume_cfg[lock_num + 1][comsume_color]
end
...
function SymbolData:GetSyebolOneChouJiangItemId()
	return self.other_cfg.one_chou_need_item or 0
end

function SymbolData:GetSyebolTenChouJiangItemId()
	return self.other_cfg.ten_chou_need_item or 0
end
...
function SymbolData:GetSymbolYuanSuRemind()
	for k,v in pairs(self.element_heart_info.element_list) do
		local next_cfg = self:GetElementHeartLevelCfg(v.element_level + 1)
		-- if v.element_level > 0 and v.next_product_timestamp <= TimeCtrl.Instance:GetServerTime() then
		-- 	return 1
		-- end
		if v.element_level > 0 and next_cfg then
			if self:GetHasTuijianElementFoods(v.wuxing_type) then
				return 1
			end
...
function SymbolData:GetSymbolYuanHuoRemind()
	...
	for k,__ in pairs(self.inlay_cfg) do
		for _,v1 in pairs(self.element_heart_info.element_list) do
			if v1.grade > 0 and self.element_texture_info.charm_list[k].wuxing_type == v1.wuxing_type
				and self:GetElementTextureInfo(k).grade < self:GetElementTextureMaxLevel() then
				table.insert(wuxing_type_list, v1)
				break
			end
		end
	end

	for _,v in pairs(wuxing_type_list) do
		local food_cfg = self.ys_stuff_cfg[v.wuxing_type]
		local num = ItemData.Instance:GetItemNumInBagById(food_cfg.item_id)
		--local shenzhuang_list = ForgeData.Instance:GetZhuanzhiEquipAll()
		for _,v1 in pairs(food_cfg) do
			num = num + ItemData.Instance:GetItemNumInBagById(v1.item_id)
			if num > 0 and v.element_level > 0 then
				return 1
			end
		end
	end
...
function SymbolData:GetSymbolYuanYongRemind()
	...
	local one_chou_item_id = self:GetSyebolOneChouJiangItemId()
	local ten_chou_item_id = self:GetSyebolTenChouJiangItemId()
	if ItemData.Instance:GetItemNumInBagById(one_chou_item_id) > 0 or ItemData.Instance:GetItemNumInBagById(ten_chou_item_id) > 0 then
		return 1
	end
	return 0
end
...
function SymbolData:GetSymbolYuanShiRemind()
	...
	local max_levle = self:GetElementMaxGrade()
	for k,v in pairs(self.element_heart_info.element_list) do
		if v.grade - 1 >= 0 and v.grade - 1 < max_levle then
			local last_cfg = SymbolData.Instance:GetElementInfo(v.id - 1)
			local limit_cfg = SymbolData.Instance:GetUpgradeLimitById(v.id)
			if nil == last_cfg or last_cfg.grade >= limit_cfg.last_element_level + 1 then
				local need_item_num = self:GetElementHeartCfgByGrade(v.grade).need_item_num
