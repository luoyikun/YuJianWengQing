...
function CrossFishingView:InitFishBait()
	...
	self.node_list["BtnAdd"].button:AddClickListener(BindTool.Bind(self.OnBuyFishBaitHandler, self))
end
...
function CrossFishingView:FlushFishBait()
	...
	if fish_bait_cfg then
		...
		if self.node_list["TxtFishBait"] then
			self.node_list["TxtFishBait"].text.text = string.format(Language.Fishing.LabelFishBait, item_num)
		end
		self.node_list["ImgRedPoint"]:SetActive(item_num <= 0)
		if item_num > 0 then
			self.bait_timer = nil
		end
	end
