...
function CrossFishingData:GetFishingFishCfgByType(fish_type)
	local fish_cfg = self:GetFishingFishCfg()
	if fish_cfg[fish_type] then
		return fish_cfg[fish_type]
	end
end
...
function CrossFishingData:GetFishingScoreRewardCfgByStage(stage)
	local list = TableCopy(self:GetFishingScoreRewardCfg())
	local level = GameVoManager.Instance:GetMainRoleVo().level
	local exp = PlayerData.Instance:GetFBExpByLevel(level)
	local cfg = self:GetFishingCfg()
	if cfg and next(cfg) ~= nil then
		local exp_factor = cfg.other[1].exp_factor or 1
		for k,v in pairs(list) do
			if v.stage == stage then
				v.exp_reward = exp * v.exp_factor_stage
				return v
			end
		end
	end
...
function CrossFishingData:GetMaxRewardScore()
	local reward_cfg = self:GetFishingScoreRewardCfg()
	if reward_cfg then
		local max = #reward_cfg
		return reward_cfg[max].need_score
	end
	return 0
end
...
function CrossFishingData:IsFishing(pos_x, pos_y)
	local cfg = self:GetFishinglocationCfg()
...
function CrossFishingData:IsCanExchange()
	...
	local user_info = self:GetFishingUserInfo()
	if nil == cfg or nil == user_info then
		return false
	end
	...
	for i = 1, GetListNum(cfg) do
		...
		if nil ~= temp then
			...
			for k = 1, GameEnum.FISHING_FISH_TYPE_MAX_COUNT - 1 do
				if user_info.fish_num_list and user_info.fish_num_list[k + 1] then
					local num = user_info.fish_num_list[k + 1]
...
function CrossFishingData:SetIsPassTime(enble)
	self.is_pass_time = enble
end

function CrossFishingData:GetIsPassTime()
	return self.is_pass_time
end