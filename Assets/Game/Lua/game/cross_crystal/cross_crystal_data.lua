...
function CrossCrystalData:SetCrystalInfo(info)
	...
	self.info.big_shuijing_next_flush_time = info.big_shuijing_next_flush_time --至尊水晶下次刷新时间

	self.scene_info.big_shuijing_num = info.big_shuijing_num							-- 至尊水晶数量
	self.scene_info.big_shuijing_next_flush_time = info.big_shuijing_next_flush_time	-- 至尊水晶下次刷新时间
end

function CrossCrystalData:SetCrystalSceneInfo(info)
	self.scene_info.big_shuijing_num = info.big_shuijing_num							-- 至尊水晶数量
	self.scene_info.big_shuijing_next_flush_time = info.big_shuijing_next_flush_time	-- 至尊水晶下次刷新时间
end
...
function CrossCrystalData:GetCrystalSceneInfo()
...
function CrossCrystalData:GetOtherCfg()
	return self.cfg.other[1]
end

function CrossCrystalData:GetActivityShuiJingFree()
	if self.cfg and self.cfg.other then
		return self.cfg.other[1].free_relive_count
	end
	return 0
end

function CrossCrystalData:GetActivityShuiJingType(id)
	if self.cfg and self.cfg.task then
		for k,v in pairs(self.cfg.task) do
...
function CrossCrystalData:GetShuiJingInfoById(task_id)
	if task_id == 0 then  		--采集4次任意水晶
		return self.gather_cfg
	elseif task_id == 1 then 	--新增采集1次的任意大水晶
		return self:GetBigShuiJingInfoList()[1]
	elseif task_id == 2 then 	--采集3次任意大水晶
		return self:GetBigShuiJingInfoList()
	elseif task_id == 3 then    --采集1次钻石大水晶
	...
	-- elseif task_id == 3 then 	--采集1次至尊水晶 		--屏蔽
...
function CrossCrystalData:GetMinGatherId(index)
	local shui_jing_info = CrossCrystalData.Instance:GetShuiJingInfoById(index) or {}
	local distance_list = {}
	if #shui_jing_info < 1 then  --只有一个表
		return shui_jing_info.gather_id
	else  --含有多个表
		for k,v in pairs(shui_jing_info) do
			if v and v.gather_id then
				local list = {}
		...
		if distance_list[1] and distance_list[1].gather_id then
...
function CrossCrystalData:GetMinDistGatherPos(task_id)
	...
	local shuijing_list = self:GetShuiJingInfoById(task_id) or {}
...
function CrossCrystalData:GetCurGatherCount(shuijing_type)
	if shuijing_type == SHUIJING_ID[1] then
		return self.task_info.gather_shuijing_total_num
	elseif shuijing_type == SHUIJING_ID[2] then
		return self.task_info.gather_big_shuijing_total_num
	elseif shuijing_type == SHUIJING_ID[3] then
		return self.task_info.gather_diamond_big_shuijing_num
	elseif shuijing_type == SHUIJING_ID[4] then
		return self.task_info.gather_best_shuijing_count
	end
end
...
function CrossCrystalData:CheckIsComplete(task_id)
	local count = self.cfg["task"][task_id + 1].gather_count
	local shuijing_type = self:GetActivityShuiJingType(task_id)
	if shuijing_type == 0 then
		return self.task_info.gather_shuijing_total_num >= count
	elseif shuijing_type == 1 then
		return self.task_info.gather_big_shuijing_total_num >= count
	elseif shuijing_type == 2 then
		return self.task_info.gather_diamond_big_shuijing_num >= count
	elseif shuijing_type == 3 then
...
function CrossCrystalData:GetShuijingExp(task_id)
	...
	-- local exp_factor = self.cfg.other[1].exp_factor or 1
	local exp_factor = 1
	for k,v in pairs(self.cfg["task"]) do
		if v.task_id == task_id then
			exp_factor = v.exp_factor_task or 1
		end
	end
	local level = GameVoManager.Instance:GetMainRoleVo().level
	local exp = PlayerData.Instance:GetFBExpByLevel(level)
	return exp * exp_factor
end
