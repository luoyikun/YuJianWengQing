KFPVPViewRank = KFPVPViewRank or BaseClass(BaseRender)

local Count = 4

local ListViewDelegate = ListViewDelegate

function KFPVPViewRank:__init()
	self.is_modal = true
	...
	self.rank_info = {}
	self:LoadSingleTitle(KuafuPVPData.Instance:GetTitleId() or 0)
	self:FlushFp()
	self:Flush()
end

function KFPVPViewRank:LoadCallBack()
	
end

function KFPVPViewRank:OpenCallBack()
	self:Flush()
	KuafuPVPCtrl.Instance:SendGetCross3V3RankListReq(1)
end

function KFPVPViewRank:__delete()
...
function KFPVPViewRank:OnFlush()
	self.rank_info = KuafuPVPData.Instance:GetRankList()
...
function KFPVPViewRank:FlushSelfInfo()
	...
	self.info_self = KuafuPVPData.Instance:GetActivityInfo()
	if self.rank_info and self.info_self then
		self.rank_info_myself.ji_fen.text.text = self.info_self.challenge_score
		...
		local config = KuafuPVPData.Instance:GetRankByScore(self.info_self.challenge_score)
		if config then
		...
		-- local bundle, asset = AvatarManager.GetDefAvatar((vo.prof % 10), false, vo.sex)
		-- self.rank_info_myself.touxiang.image:LoadSprite(bundle, asset)
	end
end

function KFPVPViewRank:FlushPage(page)
	...
	for i = 1, Count do
		...
		local config = KuafuPVPData.Instance:GetRankByScore(info.rank_value)
		if config then
		...
		-- local bundle, asset = AvatarManager.GetDefAvatar((info.prof % 10), false, info.sex)
		-- self.rank_info_list[i].touxiang.image:LoadSprite(bundle, asset)
	end
end

function KFPVPViewRank:ClearPage()
	for i = 1, Count do
...
function KFPVPViewRank:SwitchRankListPage(key)
	if "next" == key then
...
function KFPVPViewRank:OnClickJumpPage()
end

function KFPVPViewRank:InputEnd(count)
	self.curret_page = count or self.curret_page
	self:FlushPage(self.curret_page)
end

function KFPVPViewRank:OnClickGetReward()
end

function KFPVPViewRank:OnClickHelp()
	TipsCtrl.Instance:ShowHelpTipView(314)
end


function KFPVPViewRank:FlushModel()
...
function KFPVPViewRank:FlushFp()
	local shizhuang_id = KuafuPVPData.Instance:GetShiZhuangId() or 0
	...
	local title_id = KuafuPVPData.Instance:GetTitleId() or 0
...
function KFPVPViewRank:LoadSingleTitle(title_id)
