KFPVPViewGongXun = KFPVPViewGongXun or BaseClass(BaseRender)

function KFPVPViewGongXun:__init(instance)
...
function KFPVPViewGongXun:ReleaseCallBack()
...
function KFPVPViewGongXun:OpenCallBack()
	self:Flush()
end


function KFPVPViewGongXun:GetNumberOfCells()
	return KuafuPVPData.Instance:GetGongXunRewardCount()
end

function KFPVPViewGongXun:RefreshCell(cell, cell_index)
	cell_index = cell_index + 1
	local jifen_reward = KuafuPVPData.Instance:GetJiFenRewardCfg()
	local target_cell = self.cell_list[cell]
	if target_cell == nil then
		target_cell = GongXunCell.New(cell.gameObject)
...
function KFPVPViewGongXun:OnFlush()
	self.node_list["ListView"].scroller:ReloadData(0)
end

---------------------------GongXunCell----------------------
GongXunCell = GongXunCell or BaseClass(BaseCell)

function GongXunCell:__init()
...
function GongXunCell:ClickReward()
	KuafuPVPCtrl.Instance:OnCrossMultiuserChallengeFetchGongxunReward(self.data.seq)
end

function GongXunCell:OnFlush()
	...
	local gongxun_num = KuafuPVPData.Instance:GetRewardGongxun()
	local list = KuafuPVPData.Instance:GetPvPGongXunRewardIsGetFlag()
