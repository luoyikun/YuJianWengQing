...
function SecretTreasureHuntingData:GetCanFetchFlag(index)
	return self.reward_flag and self.reward_flag[32 - index] and self.reward_flag[32 - index] >= 1
end
...
function SecretTreasureHuntingData:FlushHallRedPoindRemind()
	...
	if remind_num > 0 then
		ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MIJINGXUNBAO3, true)
	else
		ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MIJINGXUNBAO3, not self.is_open)
	end
end

function SecretTreasureHuntingData:SetIsOpen()
	self.is_open = true
end
