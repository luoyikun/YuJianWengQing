...
function ConsumeRewardData:__init()
	...
	local is_open = ActivityData.Instance:GetActivityIsOpen(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_CONSUME_GOLD_FANLI)
	ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_CONSUME_GOLD_FANLI, is_open)
	self.is_open = false
...
function ConsumeRewardData:SetConsumeRewardGiftInfo(protocol)
	...
	if is_open then
		if can_reward then
			...
			ActivityData.Instance:SetActivityRedPointState(ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_CONSUME_GOLD_FANLI, not self.is_open)
		end
	end
end
...
function ConsumeRewardData:SetIsOpenCousumeReWard()
	self.is_open = true
end