BossEnterTips = BossEnterTips or BaseClass(BaseView)
-- 多人经验本购买次数
function BossEnterTips:__init()
	self.ui_config = {{"uis/views/bossview_prefab", "BossEnterTips"}}
...
function BossEnterTips:__delete()

end

function BossEnterTips:ReleaseCallBack()
...
function BossEnterTips:LoadCallBack(index, loaded_times)
...
function BossEnterTips:OpenCallBack()

end

function BossEnterTips:ShowIndexCallBack()
	self:Flush()
end

function BossEnterTips:OnClickCancel()
	self:Close()
end

function BossEnterTips:OnClickGoToVip()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.VIP)
	ViewManager.Instance:Open(ViewName.VipView)
	self:Close()
end

-- function BossEnterTips:OnClickYes()
...
function BossEnterTips:CloseCallBack()
	self.ok_func = nil
end
...
function BossEnterTips:SetData(pay_money, buy_times, max_times, next_max_times, vip_power_id, callback, tableback, text_type, desc)
...
function BossEnterTips:OnFlush(param_t, index)
	...
	-- 	end
		self.node_list["TipContent"].text.text = Language.Boss.BabyBossEnterTimesLimit
	-- else
	...
	self.node_list["TextCurCount"].text.text = string.format(Language.Boss.EnterTimes, self.max_buy_times)
	self.node_list["NextCurCount"].text.text = string.format(Language.Boss.EnterTimes, next_times)
