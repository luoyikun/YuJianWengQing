...
function TianshenhutiCtrl:__init()
	if TianshenhutiCtrl.Instance ~= nil then
		print_error("[TianshenhutiCtrl] attempt to create singleton twice!")
		return
	end
...
function TianshenhutiCtrl:OnTianshenhutiRollResult(protocol)
	self.data:SetBoxChouJiangResult(protocol)
	local chest_shop_mode_type = nil
	if LAST_ROLL_TYPE == 1 then
		chest_shop_mode_type = CHEST_SHOP_MODE.TIAN_SHEN_HUTI_BOX_SCORE
	elseif LAST_ROLL_TYPE == 2 then
		chest_shop_mode_type = CHEST_SHOP_MODE.TIAN_SHEN_HUTI_BOX_GOID_1
	elseif LAST_ROLL_TYPE == 3 then
		chest_shop_mode_type = CHEST_SHOP_MODE.TIAN_SHEN_HUTI_BOX_GOID_10
	end
	if not chest_shop_mode_type then return end
	TipsCtrl.Instance:ShowTreasureView(chest_shop_mode_type)
end

--周末装备相关请求结果
function TianshenhutiCtrl:OnTianshenhutiReqResult(protocol)
	-- self.tianshenhuti_box_show_view:SetData(protocol.new_equip)
	self.data:SetTianshenhutiResult(protocol)
	TipsCtrl.Instance:ShowTreasureView(CHEST_SHOP_MODE.TIAN_SHEN_HUTI_BOX_GET_EQUIP)
...
function TianshenhutiCtrl:OnTianshenhutiCombineOneKeyResult(protocol)
	...
	self.data:SetTianshenhutiOneKeyResult(protocol)
	TipsCtrl.Instance:ShowTreasureView(CHEST_SHOP_MODE.TIAN_SHEN_HUTI_BOX_GET_EQUIP_ONE_KEY)
	-- self.tianshenhuti_box_show_view:SetData(protocol.new_equip)
end
