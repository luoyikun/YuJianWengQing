...
function TianshenhutiData:__init()
	...
	self.box_look_cfg = ConfigManager.Instance:GetAutoConfig("tianshenhuti_auto").box_look
...
-- 设置宝箱抽奖结果
function TianshenhutiData:SetBoxChouJiangResult(protocol)
	self.box_reward_list = protocol.reward_list
end

-- 获取宝箱抽奖结果
function TianshenhutiData:GetBoxChouJiangResult()
	local box_reward_list = {}
	for k, v in pairs(self.box_reward_list) do
		local equip_cfg = self:GetEquipCfgByEquipId(v.item_id)
		if equip_cfg then
			local item_data = {}
			item_data.item_id = equip_cfg.item_id
			table.insert(box_reward_list, item_data)
		end
	end
	return box_reward_list
end
...
function TianshenhutiData:GetBoxLookCfg()
	return self.box_look_cfg
end

function TianshenhutiData:GetEquipCfgByEquipId(equip_id)
	return self.equip_id_cfg[equip_id]
end

function TianshenhutiData:GetEquipItemIdCfgByCfg(equip_id)
	if self.equip_cfg[equip_id] then
		local data = {item_id = self.equip_cfg[equip_id].item_id, is_bind = 0}
		return data
	end
	return nil
end

function TianshenhutiData:GetEquipCfgByItemId(item_id)
...
function TianshenhutiData:SetTianshenhutiResult(protocol)
	self.item_data = protocol.new_equip
end

function TianshenhutiData:GetTianshenhutiResult()
	local result_list = {}
	for k, v in pairs(self.item_data) do
		...
		if equip_cfg then
			local item_data = {}
			item_data.item_id = equip_cfg.item_id
			table.insert(result_list, item_data)
		end
	end
	return result_list
end

function TianshenhutiData:SetTianshenhutiOneKeyResult(protocol)
	self.one_key_item_data = protocol.new_equip
end

function TianshenhutiData:GetTianshenhutiOneKeyResult()
	local one_key_list = {}
	for k, v in pairs(self.one_key_item_data) do
		local equip_cfg = self:GetEquipCfgByEquipId(v.item_id)
		if equip_cfg then
			local item_data = {}
			item_data.item_id = equip_cfg.item_id
			table.insert(one_key_list, item_data)
		end
	end
	return one_key_list
end
...
-- 根据装备id获取是否显示升级箭头
function TianshenhutiData:IsShowUpArrow(equip_id)
	local equip_cfg = self:GetEquipCfgByEquipId(equip_id)
	if not equip_cfg then
		return false
	end

	local equip_info = self.equip_list[equip_cfg.slot_index]
	if not equip_info then
		return true
	end

	local equip_info_cfg = self:GetEquipCfgByEquipId(equip_info.item_id)
	if not equip_info_cfg then
		return false
	end

	if equip_info_cfg.level < equip_cfg.level then
		return true
	end
...
function TianshenhutiData:GetTianshenhutiBoxRemind()
 ...
 --   			end, self.next_free_roll_time - TimeCtrl.Instance:GetServerTime())
	...
	if is_show then
		if self.other_cfg and self.other_cfg.super_roll_item_id and self.other_cfg.batch_roll_item_id and self.other_cfg.super_roll_item_num and self.other_cfg.batch_roll_item_num then
			local super_roll_item_num = ItemData.Instance:GetItemNumInBagById(self.other_cfg.super_roll_item_id)
			local is_have_key_one = super_roll_item_num >= self.other_cfg.super_roll_item_num
			local batch_roll_item_num = ItemData.Instance:GetItemNumInBagById(self.other_cfg.batch_roll_item_id)
			local is_have_key_two = batch_roll_item_num >= self.other_cfg.batch_roll_item_num
			if is_have_key_one or is_have_key_two then
				return 1
			end
		end
	end
...
function TianshenhutiData:GetTianshenhutiOherCfg()
	return self.other_cfg
end

-- 设置是否播放抽奖动画
function TianshenhutiData:SetPlayAniState(value)
	self.is_play_ani = value
end

-- 获取是否播放抽奖动画
function TianshenhutiData:GetPlayAniState()
...
function TianshenhutiData:GetCurItemInfo(item_id)
	local item_cfg = self:GetEquipCfgByItemId(item_id)
	if item_cfg then
		local equip_info = self.equip_list[item_cfg.slot_index]
		if equip_info == nil or equip_info.item_id <= 0 then 
			return nil 
		end 
		local equip_cfg = self:GetEquipCfgByEquipId(equip_info.item_id)
		if equip_cfg then
			local item_data = {}
			item_data.item_id = equip_cfg.item_id
