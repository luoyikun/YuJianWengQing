...
function FunctionGuide:__init()
	...
	self.is_qinggong_guiding = false
end
...
function FunctionGuide:OnLoadingComplete()
	GlobalTimerQuest:AddDelayTimer(function()
		...
		self.finished_open_fun = GlobalEventSystem:Bind(OpenFunEventType.OPEN_PAUSE, BindTool.Bind(self.OpenFunPause, self))
...
function FunctionGuide:OnRoleAttrValueChange(key, new_value, old_value)
	...
	if key == "level" then
		local guide_cfg = self:GetGuideCfgByTrigger(GuideTriggerType.LevelUp, new_value)
		if guide_cfg then
...
function FunctionGuide:OpenFunPause(state)
...
function FunctionGuide:CheckConfigGuide()
	...
	if self.cur_guide_step == 1 and step_cfg.step_type ~= GuideStepType.FindUi then
		...
		if not self.is_close_all then
			...
			if self.cur_guide_cfg.id ~= 48 then --离线奖励的引导不关闭界面
	...
	elseif step_cfg.step_type == GuideStepType.Arrow or step_cfg.step_type == GuideStepType.GirlGuide then --指引方式为找到ui
		...
		if ui == NextGuideStepFlag then		--直接跳至下一引导的标记
			...
			if ui then
				...
				if (step_cfg.ui_name == "qinggong_btn" or step_cfg.ui_name == "qinggong_down_btn") and self.cur_guide_step >= 3 then
					self:SaveQingGongGuide(ui, callback, step_cfg)
				else
...
function FunctionGuide:GetCurGuideStep()
	return self.cur_guide_step or nil
end
...
function FunctionGuide:GetIsQingGongGuide()
	self.is_qinggong_guiding = self.cur_guide_cfg and self.cur_guide_cfg.id == 47 or false
	return self.is_qinggong_guiding
end

function FunctionGuide:SetQingGongGuideState(state)
	self.is_qinggong_guiding = state
end

function FunctionGuide:SaveQingGongGuide(ui, callback, step_cfg)
	self.qinggong_delay_call_ui = ui
	self.qinggong_delay_call_callback = callback
	self.qinggong_delay_call_step_cfg = step_cfg
end

function FunctionGuide:GoNextQingGongGuideStep()
	self:UpdateGuideStep(self.qinggong_delay_call_ui, self.qinggong_delay_call_callback, self.qinggong_delay_call_step_cfg)
end