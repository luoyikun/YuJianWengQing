ShenCiFightMountHuanHuaView = ShenCiFightMountHuanHuaView or BaseClass(BaseView)


function ShenCiFightMountHuanHuaView:UIsMove()
...
function ShenCiFightMountHuanHuaView:__init()
	self.ui_config = {
	...
	self.def_index = TabIndex.shenci_fightmount_huan_hua
	self.play_audio = true
...
function ShenCiFightMountHuanHuaView:__delete()
end

function ShenCiFightMountHuanHuaView:ReleaseCallBack()
...
function ShenCiFightMountHuanHuaView:SetRendering(value)
	BaseView.SetRendering(self, value)
end

function ShenCiFightMountHuanHuaView:LoadCallBack()
...
function ShenCiFightMountHuanHuaView:ShowIndexCallBack(index, index_nodes)
...
function ShenCiFightMountHuanHuaView:ClickSuperPower()
	local special_cfg = FightMountData.Instance:GetSpecialImageCfg(self.index)
	local image_id = special_cfg and special_cfg.image_id or 0
	local index = image_id or 0
	local data = FightMountData.Instance:GetSpecialHuanHuaShowData(index)
	if data then
		TipsCtrl.Instance:ShowSpecialHuanHuaViewView(data)
	end
end

function ShenCiFightMountHuanHuaView:GetNumberOfCells()
	local cfg_list = FightMountData.Instance:GetShenCiHuanHuaCfgList()
	return #cfg_list
end

function ShenCiFightMountHuanHuaView:RefreshCell(cell, cell_index)
	local mount_special_image = FightMountData.Instance:GetShenCiHuanHuaCfgList()
	if nil == mount_special_image[cell_index + 1] or nil == mount_special_image[cell_index + 1].image_id then
		return
	end
	local is_show = FightMountData.Instance:CanShenCiHuanhuaUpgradeList()[mount_special_image[cell_index + 1].image_id] ~= nil
	...
	data.zhuanzhi_prof = mount_special_image[cell_index + 1].zhuanzhi_prof
...
function ShenCiFightMountHuanHuaView:HandleAddGold()
	VipData.Instance:SetOpenType(OPEN_VIP_RECHARGE_TYPE.RECHANRGE)
	ViewManager.Instance:Open(ViewName.VipView)
end

function ShenCiFightMountHuanHuaView:PlayerDataChangeCallback(attr_name, value, old_value)
...
function ShenCiFightMountHuanHuaView:OpenCallBack()
...
function ShenCiFightMountHuanHuaView:CloseCallBack()
...
function ShenCiFightMountHuanHuaView:OnRoleDrag(data)
	if UIScene.role_model then
		UIScene:Rotate(0, -data.delta.x * 0.25, 0)
	end
end

function ShenCiFightMountHuanHuaView:ItemDataChangeCallback()
end

function ShenCiFightMountHuanHuaView:OnFlush(param_list)
...
function ShenCiFightMountHuanHuaView:OnFlushCommonAdvance(key, value)
	if value and value.id then
		local index, num = FightMountData.Instance:CanShenCiHuanhuaIndexByImageId(value.id)
		if index then
			self.grade = nil
			self.index = index
			local cfg_list = FightMountData.Instance:GetShenCiHuanHuaCfgList()
	...
	local special_image_cfg = FightMountData.Instance:GetSpecialImagesCfg()
	if nil == special_image_cfg or nil == special_image_cfg[self.index] then
		return
	end
	local upgrade_cfg = FightMountData.Instance:GetSpecialImageUpgradeInfo(special_image_cfg[self.index].image_id)
	local info_list = FightMountData.Instance:GetFightMountInfo()
...
function ShenCiFightMountHuanHuaView:OpenFlush()
	self:UIsMove()
...
--点击激活按钮
function ShenCiFightMountHuanHuaView:OnClickActivate()
...
--点击升级按钮
function ShenCiFightMountHuanHuaView:OnClickUpGrade()
	local attr_cfg = FightMountData.Instance:GetSpecialImageUpgradeInfo(self.index)
	local cfg_grade = attr_cfg and FightMountData.Instance:GetSpecialImageMaxUpLevelById(attr_cfg.special_img_id) or 0
...
--点击使用当前形象
function ShenCiFightMountHuanHuaView:OnClickUseIma()
	FightMountCtrl.Instance:SendUseFightMountImage(self.index + GameEnum.MOUNT_SPECIAL_IMA_ID)
end

function ShenCiFightMountHuanHuaView:OnClickListCell(mount_special_data, index, mount_cell)
...
--获取激活坐骑符数量
function ShenCiFightMountHuanHuaView:GetHaveProNum(item_id, need_num)
...
function ShenCiFightMountHuanHuaView:SetSpecialImageAttr(mount_special_data, index)
...
function ShenCiFightMountHuanHuaView:SetFightMountModle(index)
...
function ShenCiFightMountHuanHuaView:SetOtherSpecialImageAttr(mount_special_data, index)
	local image_cfg = FightMountData.Instance:GetSpecialImageCfg(index)
	local attr_cfg = FightMountData.Instance:GetSpecialImageUpgradeInfo(mount_special_data.image_id)
	local info_list = FightMountData.Instance:GetFightMountInfo()
	local bit_list = info_list.active_special_image_flag
	...
	local is_show_super = FightMountData.Instance:IsShowSuperPower(mount_special_data.image_id)
	local is_active_super = FightMountData.Instance:GetStarIsShowSuperPower(mount_special_data.image_id)
	local need_reach_level = FightMountData.Instance:GetActiveSuperPowerNeedLevel(mount_special_data.image_id)
	...
	local special_image_grade = FightMountData.Instance:GetSingleSpecialImageGrade(mount_special_data.image_id)
	local attr0 = FightMountData.Instance:GetSpecialImageUpgradeInfo(index)
	local attr1 = FightMountData.Instance:GetSpecialImageUpgradeInfo(index, special_image_grade, true)
	local max_grade = FightMountData.Instance:GetSpecialImageMaxUpLevelById(mount_special_data.image_id)
...
function ShenCiFightMountHuanHuaView:SetAttr(special_image_grade, attr0, attr1, max_grade, special_index)
	...
	local active_grade, attr_type, attr_value = FightMountData.Instance:GetHuanHuaSpecialAttrActiveType(nil, special_index)
	if active_grade and attr_type and attr_value then
		if special_image_grade < active_grade then
			...
			for i = special_image_grade + 1, max_grade do
				local next_active_grade, next_attr_type, next_attr_value = FightMountData.Instance:GetHuanHuaSpecialAttrActiveType(i, special_index)
...
--设置激活按钮显示和隐藏
function ShenCiFightMountHuanHuaView:IsShowActivate(image_id)
...
--设置升级按钮显示和隐藏
function ShenCiFightMountHuanHuaView:IsShowUpGrade(image_id)
	if image_id == nil then
		return
	end
...
--升级按钮是否置灰
function ShenCiFightMountHuanHuaView:IsGrayUpgradeButton(index)
	...
	local image_id = FightMountData.Instance:GetSpecialImageMaxUpLevelById(mount_special_image[index].image_id)
...
function FightMountHuanHuaCell:ShowLabel(data, image_id)
	if image_id == nil then
		return
	end
