...
local PLAYER_MULTI_MOUNT_FLAG = nil
...
function MountCtrl:MultiMountFlushMountState()
	PLAYER_MULTI_MOUNT_FLAG = GameVoManager.Instance:GetMainRoleVo().multi_mount_res_id > 0 and 1 or 0
end
...
function MountCtrl:CheckMountUpOrDownInCg()
	...
	if CgManager.Instance:IsCgIng() then
		if 1 == PLAYER_MOUNT_FLAG or 1 == PLAYER_MULTI_MOUNT_FLAG then
...
function MountCtrl:SendGoonMountReq(mount_flag, is_from_cg)
	if not is_from_cg and CgManager.Instance:IsCgIng() and mount_flag == 1 then
		return
	end
	...
	if PLAYER_MOUNT_FLAG == nil then
		PLAYER_MOUNT_FLAG = role_vo.mount_appeid > 0 and 1 or 0
	end
	if mount_flag ~= PLAYER_MOUNT_FLAG then
		-- 是否是变身形象
		if mount_flag and mount_flag == 1 then
			local bianshen_param = role_vo.bianshen_param
