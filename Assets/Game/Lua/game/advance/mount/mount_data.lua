...
function MountData:__init()
	...
	self.special_upgrade_cfg = ListToMap(self.mount_cfg.special_image_upgrade, "special_img_id", "grade")
	...
	self.grade_info_cfg = self.mount_cfg.grade
...
function MountData:SetMountInfo(protocol)
	if self.mount_info.grade_bless_val then
		...
		local bless_cfg =  AdvanceData.Instance:GetBlessBaojiCfg(UPGRADE_BAOJI_TYPE.UPGRADE_TYPE_MOUNT)
	...
	self.mount_info.active_special_image_flag = bit:uc2b(protocol.active_special_image)
end
...
function MountData:SetGradeBlessVal(num)
	if self.mount_info then
		self.mount_info.grade_bless_val = num
	end
end

--获得特殊属性值
function MountData:GetSpecialAttrActiveType(cur_grade)
	cur_grade = cur_grade or self.mount_info.grade or 0
	return AdvanceData.Instance:GetSpecialAttrActiveType(self.mount_cfg.grade, cur_grade)
end

--获得幻化特殊属性值
function MountData:GetHuanHuaSpecialAttrActiveType(grade, index)
...
-- 获取可显示的幻化列表
function MountData:GetHuanHuaCfgList()
	local huanhua_list = {}
...
function MountData:GetShenCiHuanHuaCfgList()
	local huanhua_list = {}
	local open_server_day = TimeCtrl.Instance:GetCurOpenServerDay()
	local main_vo = GameVoManager.Instance:GetMainRoleVo()
	for _, v in ipairs(self.mount_cfg.special_img) do
...
function MountData:CanHuanhuaIndexByImageId(image_id)
...
function MountData:CanShenCiHuanhuaIndexByImageId(image_id)
...
function MountData:IsCanHuanhuaDayAndLevel(index)
...
function MountData:IsCanShenCiHuanhuaDayAndLevel(index)
...
function MountData:GetSpecialImageAttrSum(mount_info)
	...
	local info_grade = self:GetMountGradeCfg(mount_info.grade)
	if info_grade then
		sum_attr_list.chengzhangdan_count = special_chengzhangdan_count + info_grade.chengzhangdan_limit
		sum_attr_list.shuxingdan_count = special_shuxingdan_count + info_grade.shuxingdan_limit
		sum_attr_list.equip_limit = special_equip_limit + info_grade.equip_level_limit
	end
...
function MountData:GetDanAttr(mount_info)
	...
	local mount_shuxingdan_cfg = AdvanceData.Instance:GetShuXingDanCfg(MountShuXingDanCfgType.Type)
	if mount_shuxingdan_cfg and next(mount_shuxingdan_cfg) then
		attr_list.gong_ji = attr_list.gong_ji + mount_shuxingdan_cfg.gongji * mount_info.shuxingdan_count
		attr_list.fang_yu = attr_list.fang_yu + mount_shuxingdan_cfg.fangyu * mount_info.shuxingdan_count
		attr_list.max_hp = attr_list.max_hp + mount_shuxingdan_cfg.maxhp * mount_info.shuxingdan_count
	end

	return attr_list
end

function MountData:GetMountAttrSum(mount_info, next_level)
	local mount_info = mount_info or self:GetMountInfo()
	...
	attr.max_hp = mount_grade_cfg.maxhp or 0
...
function MountData:IsShowZizhiRedPoint()
	local mount_shuxingdan_cfg = AdvanceData.Instance:GetShuXingDanCfg(MountShuXingDanCfgType.Type)
	if not mount_shuxingdan_cfg or not next(mount_shuxingdan_cfg) then
		return false
	end
	if self.mount_info.grade == nil then
		return false
	end

	if self.mount_info.grade < mount_shuxingdan_cfg.order_limit then
		return false
	end
...
function MountData:CanHuanhuaUpgradeList()
	local list = {}
...
function MountData:CanShenCiHuanhuaUpgradeList()
	local list = {}
	if self.mount_info.grade == nil or self.mount_info.grade <= 0 then
		return list
	end
...
function MountData:CanShenCiHuanhuaUpgrade()
...
function MountData:GetBigTargetImageId()
	return JinJieRewardData.Instance:GetSingleRewardCfgParam0(JINJIE_TYPE.JINJIE_TYPE_MOUNT)
end
...
function MountData:CalEquipRemind(equip_index)
	...
	if self.mount_info.grade < level_limit or equip_level >= equip_level_toplimit then
		return 0
	end
...
function MountData:GetEquipLevelLimit()
...
function MountData:GetCurMountState()
	...
	if multi_appeid and multi_appeid > 0 then
		return MountState.multi_appe
	elseif fight_mount_appeid and fight_mount_appeid > 0 then
		return MountState.fight_mount_appe
	elseif mount_appeid and mount_appeid > 0 then
		return MountState.mount_appe
	else
		return MountState.mount_nil
	end
end
...
function MountData:GetCurGradeBaseFightPower()
	local power = 0
	local grade = self:GetGrade()
	local cur_grade = grade == 0 and 1 or grade
	local attr_cfg = self:GetMountGradeCfg(cur_grade)
...
function MountData:SuperPowerIsOpenByCfg()
	local other_cfg = self:GetOhterCfg()
...
--特殊星星是否显示
function MountData:GetStarIsShowSuperPower(huanhua_id)
	local is_show = false
...
--超级战力是否显示
function MountData:IsShowSuperPower(huanhua_id)
	local is_show = false
	local is_open = self:SuperPowerIsOpenByCfg()
	if not is_open then
		return is_show
...
--获取单个幻化形象特殊战力配置
function MountData:GetSingleHuanHuaSpecialCapAddList(huanhua_id)
	local list = {}
	if nil == huanhua_id or nil == self.huanhua_special_cap_add or nil == self.huanhua_special_cap_add[huanhua_id] then
		return list
	end
...
--获取激活超级形象的要求等级
function MountData:GetActiveSuperPowerNeedLevel(huanhua_id)
	local level = 0
...
--特殊战力面板显示数据
function MountData:GetSpecialHuanHuaShowData(huanhua_id)
...
function MountData:IsHaveZhiShengDanInGrade()
	if self.mount_info and next(self.mount_info) then
		...
		for k, v in pairs(zhishengdan_list) do
			...
			if item_cfg.use_type == 67 and item_cfg.param2 == self.mount_info.grade then
...
function MountData:IsShenCiHuanhuaIdByItemId(item_id)
...
function MountData:IsShenCiHuanhuaIdAndCanJumpByItemId(item_id)
	local vo = GameVoManager.Instance:GetMainRoleVo()
	local zhuanzhi_prof = math.floor(vo.prof / 10)
	local cfg = self:GetSpecialImagesCfg()
	for k, v in pairs(cfg) do
