...
function MapFindData:HasRareItemNotBuy()
	local has_rare_item = false
	local randact_cfg = self:GetMapData()
	randact_cfg = ActivityData.Instance:GetRandActivityConfig(randact_cfg, ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_MAP_HUNT)
	if self.route_info.city_list ~= nil and randact_cfg ~= nil then
		for i = 1,#(self.route_info.city_list) do
			if self.route_info.city_list[i] ~= 0 then
				local flag =  bit:d2b(self.route_info.city_fetch_flag)
				if randact_cfg[self.route_info.city_list[i]].is_broadcast == 1 and flag[33 - i] == 0 then
					has_rare_item = true
				end
				break
			end
...
function MapFindData:GetMapData()
	return ServerActivityData.Instance:GetCurrentRandActivityConfig().map_hunt_city or {}
end

function MapFindData:GetMapCampData()
	return ServerActivityData.Instance:GetCurrentRandActivityConfig().map_hunt_route or {}
end

function MapFindData:GetMapFlushData()
	return ServerActivityData.Instance:GetCurrentRandActivityConfig().map_hunt_server_reward or {}
end

function MapFindData:GetMapTotalFree()
	return ServerActivityData.Instance:GetCurrentRandActivityConfig().other[1].map_hunt_free_count or 0
end

function MapFindData:GetMapFlushSpend()
	return ServerActivityData.Instance:GetCurrentRandActivityConfig().other[1].map_hunt_flush_gold or 0
end

function MapFindData:GetMapFindSpend()
	return ServerActivityData.Instance:GetCurrentRandActivityConfig().other[1].map_hunt_xunbao_glod or 0
end
...
function MapFindData:GetRouteNumber()
	local data = self:GetMapCampData()
	return data[#data] ~= nil and data[#data].route_index or 0
end
...
function MapFindData:IsRareMap(index)
	local data = self:GetMapData()
	return data[index] ~= nil and data[index].is_broadcast == 1 or false
end

-- 全服刷新次数领取奖励红点
function MapFindData:SetRewardRed()
	local flush_times = self:GetFlushTimes()
	if nil == flush_times then
		return false
	end
