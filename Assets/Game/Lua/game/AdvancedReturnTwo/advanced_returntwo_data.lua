AdvancedReturnTwoData = AdvancedReturnTwoData or BaseClass()

function AdvancedReturnTwoData:__init()
	if AdvancedReturnTwoData.Instance then
		print_error("[AdvancedReturnTwoData] Attempt to create singleton twice!")
		return
	end
	AdvancedReturnTwoData.Instance = self
	...
	self.need_gold = 0
	self.max_grade = 0
	self.config = ServerActivityData.Instance:GetCurrentRandActivityConfig().jinjie_return2
	...
	RemindManager.Instance:Register(RemindName.FanHuanTwo, BindTool.Bind(self.GetFanHuanTwoRemind, self))
end

function AdvancedReturnTwoData:__delete()

	AdvancedReturnTwoData.Instance = nil
	self.is_first_open = true
	self.toggle_not_is_on = nil
end
--进阶返还
function AdvancedReturnTwoData:SetUpGradeReturnInfo(protocol)
...
function AdvancedReturnTwoData:SetUpgradeCardBuyInfo(protocol)
	if protocol.activity_id == RA_OPEN_SERVER_ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_UPLEVEL_RETURN_2 then
		self.upgrade_return_info.grade = protocol.grade 
		self.upgrade_return_info.sign = protocol.is_already_buy
	end
end

function AdvancedReturnTwoData:GetJiangLiCfg()
	local day = nil
	local open_day = TimeCtrl.Instance:GetCurOpenServerDay()
	for k,v in ipairs(self.shop_buy) do 
		if v.related_activity_s == RA_OPEN_SERVER_ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_UPLEVEL_RETURN_2 and (nil == day or v.open_game_day == day) and v.open_game_day >= open_day 
...
function AdvancedReturnTwoData:GetReturnShowItemCfg()
	return self.act_item, self.act_grade 
end
function AdvancedReturnTwoData:GetReturnNeedGoldCfg()
	return self.need_gold
end

function AdvancedReturnTwoData:GetUpGradeReturnInfo()
	return self.upgrade_return_info
end

function AdvancedReturnTwoData:GetUpGradeReturnActType()
	return self.upgrade_return_info.act_type or 0
end

function AdvancedReturnTwoData:GetUpGradeReturnList()
	...
	if cfg[self.upgrade_return_info.act_type] ~= nil then
		for i,v in ipairs(cfg[self.upgrade_return_info.act_type]) do
			...
			data.fetch_reward_flag = fetch_reward_flag or 0
...
function AdvancedReturnTwoData:GetImageResPath(image_type)
	local info = {}
	local cfg = {}
...
function AdvancedReturnTwoData:GetImageResID(data_list,grade)
...
function AdvancedReturnTwoData:GetFashionImageResID(data_list, grade, type)
...
function AdvancedReturnTwoData:ActivetyFuanHuanIsShow()
	local open_day = TimeCtrl.Instance:GetCurOpenServerDay()
	if ActivityData.Instance:GetActivityIsOpen(RA_OPEN_SERVER_ACTIVITY_TYPE.RAND_ACTIVITY_TYPE_UPLEVEL_RETURN_2) then
		if open_day > self.shop_other.limit_opengame_day_2 and self.show_time ~= 0 then
			return true
		end
	end
	return false
...
function AdvancedReturnTwoData:GetFanHuanTwoRemind()
...
function AdvancedReturnTwoData:MaxLevelGrade(info)
	self.max_grade = #info
end

function AdvancedReturnTwoData:GetMaxLevelGrade()
	return self.max_grade or 0
end

function AdvancedReturnTwoData:GetActivitytimes(sever_time)
