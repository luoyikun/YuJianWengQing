...
function Story:OpPlayCg(bundle_name, asset_name)
	local end_callback = function()
		-- 播完第一个cg, 强行设置场景初始摄像机角度
		if "w3_yw_xinshoucun01_Cg01_1" == asset_name and Scene.Instance then
			Scene.Instance:SetGuideFixedCamera(20, 275)
		end
...
function Story:OpCreateEffectObj(effect_id)
	...
	local effect_obj = self.effect_obj_map[effect_id] or AllocAsyncLoader(self, "effect_obj_loader_" .. effect_id)
