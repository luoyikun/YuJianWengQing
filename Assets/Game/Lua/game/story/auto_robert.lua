...
AutoRobert.can_next_pos_time = 0
...
function AutoRobert:Update(now_time, elapse_time)
	...
	if 0 ~= self.goto_next_pos_time and self.now_time >= self.goto_next_pos_time and self.now_time >= AutoRobert.can_next_pos_time then
		if self:IsJump() then
...
function AutoRobert:DoMoveOperate(x, y, range, arrive_func, is_guaji)
	...
	AutoRobert.can_next_pos_time = self.now_time + 0.1 -- 防止多个机器人在同一帧发起寻路
...
function AutoRobert:MoveOneTargetComplete()
	if self.is_talk ~= self.target_pos_list[1].is_talk then
		self.is_talk = self.target_pos_list[1].is_talk or false
		self.param = self.target_pos_list[1].param or 1
		self:SetTalkImage(self.is_talk, self.param)
	end

	-- 会去打怪
	if self.target_pos_list[1].is_monster_task then
		self.target_pos_list[1].is_monster_task = false
		local skill_id_list = PROF_SKILL_ID_LIST[self.vo.prof % 10] or ""
...
-- 机器人根据任务变身
function AutoRobert:SetTalkImage(is_talk, param)
	if is_talk then
		self.vo.task_appearn = CHANGE_MODE_TASK_TYPE.TALK_IMAGE
		self.vo.task_appearn_param_1 = param
		self.vo.move_speed = 500
		self:SetAttr("task_appearn", 3)
	else
		self.vo.task_appearn = 0
		self.vo.task_appearn_param_1 = 0
		self.vo.move_speed = 720
		self:SetAttr("prof", self.vo.prof)
	end
end