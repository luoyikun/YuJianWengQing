...
function RobertMgr:__init()
	...
	self.need_role_min_level = 10 						-- 开启需要人物最低等级
	...
	self.check_roundrobert_born_quency = 3 				-- 检查周围机器人出生频率

	self.wuqi_id_list = {8100, 8200, 8300, 8400} 		-- 机器人武器列表（1-4职业）
	self.mount_appeid = 1 								-- 机器人坐骑形象（主角骑着时才显示）
	self.mount_speed = 1000								-- 机器人坐骑速度
	self.role_speed = 720								-- 人的速度
...
function RobertMgr:Update(now_time, elapse_time)
	...
	if GameVoManager.Instance:GetMainRoleVo().level >= self.need_role_min_level and GameVoManager.Instance:GetMainRoleVo().level <= self.need_role_max_level then
		...
		if GameVoManager.Instance:GetMainRoleVo().level >= self.need_role_max_level then
...
function RobertMgr:BornRobert(born_x, born_y, target_pos_list)
	...
	role_vo.sex = ROLE_PROF_SEX[role_vo.prof]
...
function RobertMgr:CalcTaskRobertTargetPosList()
	...
	for i = 1, 10 do
		...
		if npc_cfg and npc_cfg.scene == cur_scene_id then
			local is_talk = false
			if cfg.condition == TASK_COMPLETE_CONDITION.HUG and cfg.c_param1 == CHANGE_MODE_TASK_TYPE.TALK_IMAGE then
				is_talk = true
			end
			table.insert(target_pos_list, {x = npc_cfg.x, y = npc_cfg.y, is_monster_task = false, is_talk = is_talk, param = cfg.c_param2})
		else 
			break
		end
		...
		if npc_cfg and npc_cfg.scene == cur_scene_id then
			table.insert(target_pos_list, {x = npc_cfg.x, y = npc_cfg.y, is_monster_task = false, is_talk = false})
		else
			break
		end
		...
		if npc_cfg and npc_cfg.scene == cur_scene_id then
			...
			table.insert(target_pos_list, {x = npc_cfg.x, y = npc_cfg.y, is_monster_task = is_monster_task, is_talk = false})
		end
