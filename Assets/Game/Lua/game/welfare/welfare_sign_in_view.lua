...
function SignInView:__init()
	...
	self.node_list["BtnAutoSign"].button:AddClickListener(BindTool.Bind(self.AutoSign, self))
...
function SignInView:CellRefreshDel(data_index, cell)
	...
	if sign_data then
		...
		item_cell:ShowHasGet(is_get)
		item_cell:ShowHighLight(false)--can_show_highlight
		item_cell:ShowRepairImage(can_repair)
		item_cell:ShowItemRewardEffect(show_get_effect)
	...
	for i = max_num, 1, -1 do
		if sign_change_flag_list[i] == 1 then
			...
			for _, v in pairs(self.cell_list) do
				...
				if item_cell and not item_cell:IsNil() then
					...
					if sign_data then
						...
						item_cell:ShowHasGet(true)
						item_cell:ShowRepairImage(false)
						item_cell:ShowItemRewardEffect(false)
						break
					end
				end
			end
...
function SignInView:Flush()
	...
	self.node_list["TxtMonth"].text.text = self.time_table.month --.. Language.Common.TimeList.m

	local total_sign_count = WelfareData.Instance:GetAccmulationSigninDays()
	self.node_list["TxtTotalSignDays"].text.text = string.format(Language.Welfare.SignDay, total_sign_count)
	...
	if process then
		self.node_list["TxtSliderValue"].slider.value = process
	end
...
function SignDayRewardGroupCell:ShowHasGet(i, enable)
	self.item_cell_list[i]:ShowHasGet(enable)
end

function SignDayRewardGroupCell:ShowItemRewardEffect(i, enable)
	self.item_cell_list[i]:ShowItemRewardEffect(enable)
end
