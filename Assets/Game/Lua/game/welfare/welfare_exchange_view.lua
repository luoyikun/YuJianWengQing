...
function WelfareExchangeView:__init()
	...
	self.node_list["BtnOnClickGetReward"].button:AddClickListener(BindTool.Bind(self.OnClickGetReward, self))
end
...
function WelfareExchangeView:OnClickGetReward()
	local card = self.node_list["Input"].input_field.text
	if card == "" then
...
function WelfareExchangeView:FetchGift(card)
	...
	local url = GLOBAL_CONFIG.param_list.gift_fetch_url or (GlobalUrl .. "/api/c2s/use_card.php")
	...
	print_log("[FetchGift] request fetch", req_str)
