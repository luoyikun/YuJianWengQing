...
function IncreaseSuperiorData:__init()
	...
	RemindManager.Instance:Register(RemindName.IncreaseSuperior, BindTool.Bind(self.ShowIncreaseSuperiorPoint, self))
end

function IncreaseSuperiorData:__delete()
	IncreaseSuperiorData.Instance = nil
	RemindManager.Instance:UnRegister(RemindName.IncreaseSuperior)
end
...
function IncreaseSuperiorData:ShowIncreaseSuperiorPoint()
	if RemindManager.Instance:RemindToday(RemindName.IncreaseSuperior) then
		return 0
	else
		return 1
	end
