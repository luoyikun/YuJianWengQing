...
function PetObj:PlayAppearEffect(time)
	...
	self.pet_appear_effect = AllocAsyncLoader(self, "appear_effect_loader")
	local bundle_name, asset_name = ResPath.GetMiscEffect("xianchongchuchang")
	local pet_obj = main_part:GetObj()
	self.pet_appear_effect:SetParent(pet_obj.transform)
	self.pet_appear_effect:Load(bundle_name, asset_name)
