...
function CityOwnerStatue:CreateCityOwnerObj()
	...
	if nil ~= self.city_owner_obj then
		...
		if nil ~= gameobj then
			...
			self.city_owner_obj.draw_obj:GetRoot().transform.localRotation = Quaternion.Euler(90, -90, -90)
		end
	end
end
...
function CityOwnerStatue:CreateCityOwnerLoverObj()
	...
	if nil ~= self.city_owner_lover_obj then
		self.city_owner_lover_obj.draw_obj:GetRoot().transform:SetParent(self.ownerlovergameobj.transform)
		self.city_owner_lover_obj.draw_obj:GetRoot().transform.localPosition = Vector3(0, 0, 0)
		self.city_owner_lover_obj.draw_obj:GetRoot().transform.localRotation = Quaternion.Euler(90, -90, -90)
	end
end

function CityOwnerStatue:CreateCenterObj()													-- 不知道为啥写这个东西
	-- local lover_role_info = CityCombatData.Instance:GetLoverRoleInfo()
	-- local owner_role_info = CityCombatData.Instance:GetCityOwnerRoleInfo()
...
function CityOwnerStatue:GetObjectVoByInfo(info)
	...
	vo.prof = (info.prof % 10) or GameEnum.ROLE_PROF_1
	if nil ~= info.shizhuang_part_list then
		...
		vo.appearance.fashion_wuqi_is_special = info.shizhuang_part_list[1].use_special_img > 0 and 1 or 0
		...
		for k, v in pairs(FashionData.Instance:GetShizhuangImgCfg()) do
			if v.item_id == other_cfg.cz_fashion_yifu_id then
				vo.appearance.fashion_body = v.image_id
			end
		end
	end
	return vo
...
function CityOwnerStatue:RemoveAllRoleStatueObject()
	if self.city_owner_obj then
		self.city_owner_obj = nil
	end
	if self.city_owner_lover_obj then
		self.city_owner_lover_obj = nil
	end
	if self.center_obj then
		self.center_obj = nil
	end
	Scene.Instance:DeleteObjsByType(SceneObjType.CityOwnerObj)
end