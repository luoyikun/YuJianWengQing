...
function LingChongObj:__init(vo)
	...
	self.draw_obj:SetIsDisableAllAttachEffects(not self:IsMainRoleLingChong())
...
function LingChongObj:SetAttr(key, value)
	...
	if key == "lingchong_used_imageid" then
		...
		local name = ""
		local image_cfg_info = LingChongData.Instance:GetCurUseImageCfgByImageId(value)
...
function LingChongObj:ChangeLingChongModel()
	...
	self:ChangeModel(SceneObjPart.Main, bundle, asset, function ()
		if self.lingqi_res_id > 0 then
			local part = self.draw_obj:GetPart(SceneObjPart.Main)
			part:SetLayer(LINGCHONG_ANIMATOR_PARAM.MOUNT_LAYER, 1)
		end
	end)
end
