...
function BaseSceneLogic:Enter(old_scene_type, new_scene_type)
	...
	if HIDE_MAINUI_ICON_SCENE_TYPE[new_scene_type] then
		GlobalEventSystem:Fire(MainUIEventType.CHNAGE_FIGHT_STATE_BTN, true)
		ViewManager.Instance:FlushView(ViewName.Main, "change_fight_enable", {false})
		MainUICtrl.Instance:ChangeFightStateEnable(false)
	else
		ViewManager.Instance:FlushView(ViewName.Main, "change_fight_enable", {true})
...
-- 进度条结束关掉面板的时候调用
function BaseSceneLogic:SceneLoadEnd(scene_id)
...
-- 不屏蔽怪物的列表 返回参数 100为怪物ID {id = 100}, 1
function BaseSceneLogic:NotShieldMonsterList()
	return {}, 0
end

-- 是否在头上的特殊图标右边显示数字
function BaseSceneLogic:GetIsShowSpecialImageRightNum(scene_obj)
	return false
end
...
function BaseSceneLogic:GetRoleNameBoardText(role_vo)
	...
	local is_fightback_obj = role_vo.is_fightback_obj or 0
	local t = {}
	local index = 1
	...
	if role_vo.role_id == GameVoManager.Instance:GetMainRoleVo().role_id then
		t[index].color = name_color == EvilColorList.NAME_COLOR_WHITE and TEXT_COLOR.GREEN or COLOR.RED
	else
		...
		t[index].color = (not FightData.Instance:GetIsFightbackObj(is_fightback_obj) and name_color == EvilColorList.NAME_COLOR_WHITE) and color or COLOR.RED
	end

	local role_name = role_vo.name or role_vo.role_name
	role_name = name_color == EvilColorList.NAME_COLOR_WHITE and role_name or role_name .. Language.Player.RedName
...
function BaseSceneLogic:IsEnemy(target_obj, main_role, ignore_table)
	...
	if main_role:IsBaiYe() then
		return false, Language.Fight.BaiYeNoAttack								-- 拜谒期间
	end
	...
	if target_obj:GetVo().is_shadow == 1 then
		if target_obj:GetVo().shadow_type == ROLE_SHADOW_TYPE.ROLE_SHADOW_TYPE_CLONE_ROLE and
			target_obj:GetVo().role_id == main_role:GetVo().role_id then
			return false
		end

		if MainUIData.IsInCampStatueoScene() then									-- 判断军团雕像是否可攻击
			if CampData.ShowCampStatueFollow() then
				return true
			else
				return false
			end
	...
	if target_obj:GetType() == SceneObjType.Role then
		...
		if Scene.Instance:GetSceneType() == SceneType.Common then
			local scene_id = Scene.Instance:GetSceneId()
			if not BossData.IsBossScene(scene_id) then
		...
		if target_obj:GetVo().is_yinshen and target_obj:GetVo().is_yinshen > 0 then		-- 目标是否隐身
			return false
		end

		if target_obj:IsQingGong() then
			return false, Language.QingGong.TargetQingGong
		end

		if target_obj:GetVo().is_in_xunyou == 1 or MarriageData.Instance:GetObjIdIsXunYouing(target_obj:GetVo().obj_id) then
			return false
		end
...
function BaseSceneLogic:IsRoleEnemy(target_obj, main_role)
	...
	--密藏boss不允许打人
	if BossData.Instance:IsSecretBossScene(Scene.Instance:GetSceneId()) then
	...
	elseif attack_mode == GameEnum.ATTACK_MODE_SREVER then
		return main_role:GetVo().merge_server_id ~= target_obj:GetVo().merge_server_id
	elseif attack_mode == GameEnum.ATTACK_MODE_HATRED then
		return ScoietyData.Instance:GetIsEnemy(target_obj:GetVo().merge_server_id, target_obj:GetVo().plat_role_id)
	end
...
function BaseSceneLogic:GetPickItemMaxDic(item_id)
	if EquipData.Instance:GetImpGuardActiveInfo() then
		return 0
	end
	return 2
end
...
function BaseSceneLogic:GetTargetScenePos(scene_id)
	...
	-- 如果在目标场景找不到传送阵,则去当前场景找传送阵
	if fly_x == nil or fly_y == nil then
		fly_x, fly_y = Scene.Instance:GetSceneDoorPos(Scene.Instance:GetSceneId(), scene_id)
	end
	return fly_x, fly_y
end
...
function BaseSceneLogic:GmAutoGather()
	self.auto_moveto_gather_state = 1
	self.next_req_gather_time = Status.NowTime
	self.next_check_auto_gather_time = Status.NowTime
end
...
--检查采集次数是否足够
function BaseSceneLogic:CheckHaveGatherTimes(scene_id)
	if scene_id == AncientRelicsData.SCENE_ID then
		--远古遗迹
...
function BaseSceneLogic:IsCanAutoGather()
	...
	for k,v in pairs(AUTO_GATHER_COMMON_SCENE) do
		if v == Scene.Instance:GetSceneId() and self:CheckHaveGatherTimes(v) then
			is_in_auto_gather_scene = true
			break
		end
	end

	if is_in_auto_gather_scene and GameVoManager.Instance:GetMainRoleVo().vip_level >= 1 then
		is_auto_gather = true
	end
