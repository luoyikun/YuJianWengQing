GLOBAL_CONFIG = {
	...
	param_list = {						-- 参数列表
		...
		chat_report_url = "",			-- 渠道聊天推送
		copyrighttext = "",				-- 版号信息
		auditldtext = "",				-- 审核版本的时候通讯文字
		...
		switch_list = {
			...
			language_tw = false,		-- 开关 - 是否加载繁体字体(台湾版本)
			create_role = true,			-- 开关 - 是否允许整个渠道注册
		},
	},
...
GlobalUrl = "http://120.77.87.236:8014"

local agent_id = ChannelAgent.GetChannelID()
local init_urls = nil
if UnityEngine.Debug.isDebugBuild then
	init_urls = {"http://120.77.87.236:8014/init-query.php"}
else
...
if UnityEngine.Application.platform == UnityEngine.RuntimePlatform.WindowsPlayer then
	local file = io.open(ResUtil.GetAgentAssetPath("win_agent.txt"))
	if nil ~= file then
...
-- init_urls = {GlobalUrl .. "/dev/query.php"}
-- init_urls = {GlobalUrl .. "/ayk/query.php"}
-- agent_id = "ayk"
-- GLOBAL_CONFIG.package_info.version = "3.0.0"
...
GLOBAL_CONFIG.assets_info.version = ResMgr:GetHashCode()
GLOBAL_CONFIG.assets_info.lua_version = ResMgr:GetLuaHashCode()
