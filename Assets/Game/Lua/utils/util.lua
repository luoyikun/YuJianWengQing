...
-- 获取位移后base64解码后的数据(配合php用的)
function Base64Decode(data, data_length)
	local length = string.len(data) - data_length
	local data_b64 = string.sub(data, length + 1) .. string.sub(data, 1, length)
	return mime.unb64(data_b64)
end

function ListToMap(list, ...)
	if nil == list then
		print_error("ListToMap list is nil")
		return nil
	end
	...
	if max_depth <= 0 then
		print_error("ListToMap max_depth is error")
		return nil
	end
...
function ListToMapList(list, ...)
	if nil == table then
		print_error("ListToMapList list is nil")
		return nil
	end
	...
	if max_depth <= 0 then
		print_error("ListToMapList max_depth is error")
		return nil
	end
