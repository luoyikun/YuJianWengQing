...
--获取角色模型配置数据
function ConfigManager:GetAutoPrefabConfig(sex, prof)
	local base_prof = PlayerData.Instance:GetRoleBaseProf(prof)
	return self:GetConfig("config/prefab_data/config/role/1".. sex .. "0".. base_prof .. "001_config")
end
...
function ConfigManager:GetPrefabDataAutoConfig(model_type, model_id)
	...
	if self.load_model_data[path] then
		...
		return self:GetConfig("config/prefab_data/config/base_config")
	end
end
